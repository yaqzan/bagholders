...
def copy_score_components_from_version(source_token, days=DEFAULT_LOOKBACK, *,
    ...
    for lo, hi in _yearly_chunks(start, end):
        where = ["src.`version_id`=%s", "src.`date` >= %s", "src.`date` <= %s"]
        where_params = [source.id, lo, hi]
        if symbol:
            where.append("src.`symbol`=%s")
            where_params.append(symbol)
        where.append("(st.`delisted_date` IS NULL OR src.`date` <= st.`delisted_date`)")
        where_sql = " AND ".join(where)

        count_sql = (
            "SELECT COUNT(*) FROM `scores` src "
            "JOIN `stocks` st ON st.`symbol` = src.`symbol` "
            f"WHERE {where_sql}"
        )
        ...
        sql = (
            ...
            f"SELECT src.`symbol`, src.`date`, %s, {src_cols} "
            "FROM `scores` src "
            "JOIN `stocks` st ON st.`symbol` = src.`symbol` "
            f"WHERE {where_sql}{conflict}"
        )
...
def should_run_db_tuning_preflight(days_back, components=None):
    """Return True when the recalc scope is large enough for DB tuning hints."""
    ...
    try:
        ...
        return True
    if days <= 30 and (not components or set(components).issubset(DAILY_COMPONENTS)):
        return False
    return True


def _effective_cutoff_end_for_stock(stock, cutoff_start, cutoff_end=None):
    eff_end = cutoff_end if cutoff_end is not None else date.today()
    if stock.delisted_date is not None and eff_end > stock.delisted_date:
        eff_end = stock.delisted_date
...
def recalculate(symbol, days=DEFAULT_LOOKBACK, components=None, use_batch=True,
    ...
    effective_cutoff_end = _effective_cutoff_end_for_stock(stock, cutoff_start, cutoff_end)
    if effective_cutoff_end is None:
        return 0, 0, 0

    weekly_u = weekly_e = 0
    if 'weekly' in components:
        weekly_u, weekly_e = stock.recalculate_weekly_scores(
            cutoff_start, show_progress=inner_progress, cutoff_end=effective_cutoff_end)

    dte_u = dte_s = dte_e = 0
    if 'dte' in components:
        from recalculate_dte import recalculate_dte_for_stock
        dte_u, dte_s, dte_e = recalculate_dte_for_stock(
            symbol, days=days, cutoff_end=effective_cutoff_end, force=force,
    ...
    u, s, e = (
        stock.recalculate_scores_full(
            cutoff_start, daily, cutoff_end=effective_cutoff_end, force=force,
        ...
        if use_batch
            ...
            cutoff_end=effective_cutoff_end)
    )
...
def run(symbol=None, days=DEFAULT_LOOKBACK, components=None,
    ...
    if symbol:
        ...
        # Delisted names are excluded by DEFAULT so the daily live recalc never spends
        # time on dead tickers. But a survivorship-honest historical rebuild needs them:
        # a company that died in 2008 must carry scores up to its delisting date or it
        # cannot appear in any backtest of 2008, which is the whole point of holding it.
        # Scoring them is inherently bounded -- core.py caps every effective end date at
        # delisted_date, so a dead name is scored only over dates it actually traded.
        if include_delisted:
            stocks = list(Stock.select().order_by(Stock.symbol))
        else:
            stocks = list(Stock.select().where(Stock.delisted_date.is_null())
