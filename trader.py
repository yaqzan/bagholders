...
# A bar within this many days of today has not yet been touched by a split or
# dividend, so its adjusted close still equals the price actually printed. Beyond
# it, only a vendor with a real as-traded series can supply close_unadj.
_UNADJ_FRESH_DAYS = 5


def _unadj_split_floors(symbols):
    """Latest recent split_date per symbol, for the close_unadj fresh-bar shortcut.

    The shortcut above breaks exactly when a split lands inside the fresh window:
    an adjusted feed then quotes the bars BEFORE the split in post-split scale, so
    writing them as close_unadj stamps a 2x-30x wrong "as-traded" price (measured on
    MNST's 2026-08-11 2:1: pre-split bars stored at half their real prints). Bars on
    or after the newest split are safe -- nothing has re-adjusted them yet.
    Returns {symbol: split_date} for symbols with a split recent enough to matter.
    """
    horizon = date.today() - timedelta(days=_UNADJ_FRESH_DAYS + 3)
    try:
        return dict(
            SplitEvent
            .select(SplitEvent.symbol, fn.MAX(SplitEvent.split_date))
            .where(SplitEvent.symbol.in_(list(symbols))
                   & (SplitEvent.split_date >= horizon))
            .group_by(SplitEvent.symbol)
            .tuples())
    except Exception:
        return {}   # table missing on a cold DB; the floor is a guard, not a gate


def _find_unadj_straggler_symbols(symbols, lookback_days=_UNADJ_FRESH_DAYS):
    """Symbols owning a recent bar that never received close_unadj.

    The daily row builder (_build_valid_price_history_row) stamps close_unadj on
    EVERY bar it writes within _UNADJ_FRESH_DAYS of today -- unless the bar
    predates the symbol's newest recent split (unadj_floor). So a fresh NULL has
    exactly one writer left: the premarket 1m pass, which cannot supply it. That
    is the fossilized-premarket-bar leak (diagnosed 2026-08-21): when yfinance
    fails a symbol's daily pulls all day, its open=close / volume=0 premarket row
    survives as that day's permanent bar, invisible to every later pull because
    determine_yf_period returns '1d' for an up-to-date symbol. Re-pulling the
    flagged symbols at '5d' lets PriceHistory.bulk_build's COALESCE upsert fill
    close_unadj and conflict-refresh the frozen OHLC.

    Today's own rows are deliberately EXCLUDED from the window: every non-
    premarket update rewrites them via the normal '1d' pull anyway, and including
    them would flag the whole universe on the first intraday run (all premarket
    rows are unadj-less until that pull lands). A symbol whose pulls keep failing
    today ages into the window tomorrow and heals then. Bars before a split floor
    are excluded -- their NULL is legitimate.

    Returns a sorted symbol list; empty when clean. Single indexed range scan on
    price_history_date_close_IDX (date, close_unadj).
    """
    ...
    try:
        rows = (
            ...
            .where(PriceHistory.symbol.in_(list(symbols))
                   ...
                   & PriceHistory.close_unadj.is_null())
            .tuples())
    except Exception:
        return []   # never let bookkeeping break an update
    if not rows:
        return []
    floors = _unadj_split_floors({sym for sym, _ in rows})
    stragglers = set()
    for sym, d in rows:
        floor = floors.get(sym)
        if floor is not None and d < floor:
            continue   # pre-split-floor bars legitimately carry no close_unadj
        stragglers.add(sym)
    return sorted(stragglers)


# ---- vendor symbol translation --------------------------------------------------
# Our canonical `stocks.symbol` is NOT always what the price vendor calls the same
# security. Renaming our rows to match would break every `scores` / portfolio row that
# references the canonical symbol, so the translation happens only on the way OUT to the
# vendor; everything we store stays canonical.
#
# Found 2026-07-29: BF.B, BRK.B and SATS were failing EVERY pull — bulk and per-symbol
# retry alike — and nothing surfaced it. Their history looked healthy only because the
# Sharadar rebuild had just backfilled it.
#
# Two distinct causes, and they need different handling:
#   * share-class suffix — vendor writes BRK-B, we write BRK.B
#   * vendor rename      — EchoStar trades as ECHO there, SATS here
#
# The class rule is deliberately narrow. A blanket '.'->'-' would corrupt exchange
# suffixes: ENA.V (TSX-V) and VNP.TO / PINV.TO / HPS-A.TO (Toronto) are already correct
# and resolve fine, so only a single trailing A/B/C class letter is rewritten. Verified
# against the vendor 2026-07-29: BRK-B/BF-B/ECHO/MOG-A/ENA.V/VNP.TO all return data;
# BRK.B/BF.B/SATS/MOG.A all return none.
_VENDOR_TICKER_OVERRIDES = {
    'SATS': 'ECHO',   # EchoStar; we keep SATS canonical (17,859 scores reference it)
    # NOTE 2026-08-22: the LC->HAPN and EQR->VMRK renames are deliberately NOT handled
    # here. A concurrent session owns them via the split-at-cutover approach instead
    # (old symbol stamped delisted at its last trading day, successor gets its own
    # stocks row -- LC/HAPN done, EQR/VMRK in flight). Don't add overrides for them.
}
_CLASS_SUFFIX_RE = re.compile(r'^([A-Z]{1,5})\.([A-C])$')


def _vendor_ticker(symbol):
    """Canonical symbol -> the ticker this price vendor answers to."""
    if symbol in _VENDOR_TICKER_OVERRIDES:
        return _VENDOR_TICKER_OVERRIDES[symbol]
    m = _CLASS_SUFFIX_RE.match(symbol)
    if m:
...
# ---- persistent pull-failure ledger ---------------------------------------------
# A symbol that fails its price pull is currently retried once and then forgotten, so a
# permanently-broken symbol (wrong vendor ticker, delisting, rename) goes stale silently
# and indefinitely -- exactly how BF.B / BRK.B / SATS went unnoticed. Every final failure
# is recorded here, every success clears the entry, and the ops heartbeat reads the file
# so a symbol failing day after day becomes visible instead of invisible.
_PULL_FAILURE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                  '.cache', 'pull_failures.json')


def _load_pull_failures():
    try:
        with open(_PULL_FAILURE_FILE) as f:
            return json.load(f)
    except Exception:
        return {}


def _save_pull_failures(data):
    try:
        os.makedirs(os.path.dirname(_PULL_FAILURE_FILE), exist_ok=True)
        tmp = _PULL_FAILURE_FILE + '.tmp'
        with open(tmp, 'w') as f:
            json.dump(data, f, indent=2)
        os.replace(tmp, _PULL_FAILURE_FILE)
    except Exception:
        pass   # never let bookkeeping break a pull


def _record_pull_failure(symbol, reason='no data'):
    today = date.today().isoformat()
    data = _load_pull_failures()
    rec = data.get(symbol) or {'first_seen': today, 'consecutive_days': 0}
    if rec.get('last_seen') != today:
        rec['consecutive_days'] = rec.get('consecutive_days', 0) + 1
    rec['last_seen'] = today
    rec['reason'] = reason
    rec['vendor_ticker'] = _vendor_ticker(symbol)
    data[symbol] = rec
    _save_pull_failures(data)


def _clear_pull_failure(symbol):
    data = _load_pull_failures()
    if symbol in data:
...
def _build_valid_price_history_row(symbol, index, row, *, nested_symbol=None,
    ...
    unadj_floor: newest recent split_date for this symbol (from _unadj_split_floors).
    Bars dated before it never get close_unadj -- their adjusted close is already in
    post-split scale, not the price that printed.
    """
    def get_value(field):
        try:
    ...
    # AS-TRADED close, for fresh bars only.
    #
    # yfinance has no as-traded feed: with auto_adjust=True `close` is split+dividend
    # adjusted, and even auto_adjust=False returns a split-adjusted Close. But the two
    # conventions COINCIDE at the moment a bar is first printed, because no split or
    # dividend has happened since. So a bar we are seeing today can be recorded as-traded
    # for free, and an as-traded price never needs revising afterwards.
    #
    # Only recent bars qualify. For an older date in a backfill window the adjusted close
    # has already drifted from what actually printed, and writing it as close_unadj would
    # be a quiet lie. Those dates are owned by the Sharadar load, which has the real
    # as-traded series. See database/models/technical.py PriceHistory.bulk_build.
    # A recent split re-breaks the equivalence for bars BEFORE it (unadj_floor).
    if ((date.today() - d).days <= _UNADJ_FRESH_DAYS
            and (unadj_floor is None or d >= unadj_floor)):
        row['close_unadj'] = close
    return row
...
def _run_update_command(client, args=None, *, with_options=False, command_name='update'):
    ...
    try:
        ...
        # A crash inside update_stocks (e.g. a yfinance history-fetch cascade →
        # the "(0,'')" error that killed the 6/26 close-update) must NOT skip the
        # portfolio sync below: that sync is the SOLE source of the live buy/sell
        # pushes, it reads scores from the DB (the last successful run's) and prices
        # options with its own model, so it can still fire timely alerts even when
        # THIS run's fetch died. Defer the re-raise to AFTER the sync so the exit
        # code stays non-zero for the scheduler/queue, but the alerts are no longer
        # hostage to a transient fetch crash (root of the 7pm-instead-of-15:30
        # pushes). Mirrors the phase-independence fix in post_market_daily.ps1.
        update_failed = None
        try:
        ...
        if update_failed is not None:
            # already alerted above; re-raise now (after the live-alert sync) so
            # Task Scheduler / the queue still see a non-zero exit.
            raise update_failed
...
class Trader():
    ...
    def pull_price_history(self, symbol, period='max', start=None, line=None, price_key='price', prefetched=False):
        ...
        if prefetched:
            ...
            return True
        attempts = 0
        # Ask the vendor by ITS name; everything we store stays canonical.
        vendor = _vendor_ticker(symbol)
        ...
        while attempts < 5:
            data = yf.download([vendor], **dl_kwargs)
            if len(data) > 0: break
            sym_errs = shared._ERRORS.get(vendor)
        ...
        if len(data) == 0:
            _record_pull_failure(symbol, 'no data returned')
            if line:
                line.error(price_key)
            else:
                print(Fore.RED + f'No data found for {symbol} (vendor ticker: {vendor})')
            return False

        rows = []
        unadj_floor = _unadj_split_floors([symbol]).get(symbol)
        for index, row in data.iterrows():
            # nested_symbol is the VENDOR column name in the response; the row is
            # still stored under our canonical `symbol`.
            cleaned = _build_valid_price_history_row(symbol, index, row, nested_symbol=vendor,
                                                     unadj_floor=unadj_floor)
    ...
    def pull_price_history_bulk(self, symbol_period_pairs, batch_size=100):
        ...
        for period, symbols in groups.items():
            ...
            for i in range(0, len(symbols), eff_batch):
                ...
                # Vendor names on the wire, canonical names everywhere else: the
                # response's top-level columns come back keyed by the VENDOR ticker.
                vendor_of = {sym: _vendor_ticker(sym) for sym in batch}
                ...
                for attempt in range(3):
                    try:
                        data = yf.download(list(vendor_of.values()), **dl_kwargs)
                    except Exception:
                        data = None
                ...
                unadj_floors = _unadj_split_floors(batch)
                for symbol in batch:
                    vendor = vendor_of[symbol]
                    if vendor not in top_level:
                        failed.add(symbol)
                        continue
                    try:
                        sym_df = data[vendor].dropna(subset=['Close'])
                    except KeyError:
                        failed.add(symbol)
                        continue
                    if sym_df.empty:
                    ...
                    for index, row in sym_df.iterrows():
                        cleaned = _build_valid_price_history_row(
                            symbol, index, row, unadj_floor=unadj_floors.get(symbol))
                        if cleaned is not None:
                            rows.append(cleaned)
                    if not rows:
                        failed.add(symbol)
    ...
    def _split_already_reflected(symbol, split_date, ratio):
        ...
        """
        # Magnitude in EITHER direction: a reverse split reports ratio < 1 (BYND's
        # 1-for-30 was 0.0333) and a bare `ratio < 1.15` waved every reverse split
        # through as "too small", leaving a raw 30x cliff in the adjusted series.
    ...
    def check_and_apply_splits(self, symbol, recent_only=False):
        ...
        for split_date, ratio in all_split_dates.items():
            _, created = SplitEvent.get_or_create(
                symbol=symbol,
                split_date=split_date,
                defaults={'ratio': ratio, 'processed_at': datetime.now()}
            )
            if created:
                newly_registered.append(split_date)

        if not needs_repull:
            # No re-pull, but a brand-new split may already have poisoned close_unadj:
            # any pull that ran after the split but before this registration (the bulk
            # prefetch does, every morning) stamped post-split ADJUSTED closes onto
            # pre-split fresh bars. NULL them -- consumers fall back to `close`, and
            # the vendor backfill supplies the true prints later.
            self._scrub_poisoned_unadj(symbol, newly_registered)
            return False

        # Re-pull needed: wipe and rebuild from scratch.
        #
        # The wipe also destroys close_unadj -- the vendor-loaded AS-TRADED series
        # that yfinance cannot restore (no as-traded feed; the re-pull re-derives only
        # fresh bars). Snapshot it and restore afterwards: a split never changes what
        # actually printed, so the stored prints stay true verbatim. The one exception
        # is the poison window just before a fresh split (see _unadj_poison_window) --
        # those bars may hold post-split adjusted values written minutes earlier and
        # are dropped rather than restored.
        unadj_snapshot = list(
            PriceHistory
            .select(PriceHistory.date, PriceHistory.close_unadj)
            .where((PriceHistory.symbol == symbol)
                   & PriceHistory.close_unadj.is_null(False))
            .tuples())
        poison = self._unadj_poison_window(list(all_split_dates))
        self._backup_unadj_snapshot(symbol, unadj_snapshot)
        ...
        restored = self._restore_unadj_after_repull(symbol, unadj_snapshot, poison)
        if restored:
            print(Fore.CYAN + f'  [split] {symbol}: restored {restored} as-traded closes '
                              f'(close_unadj) from pre-wipe snapshot')
    ...
    @staticmethod
    def _unadj_poison_window(split_dates):
        """Date range whose stored close_unadj may be a post-split ADJUSTED value.

        A pull that runs between a split and its registration stamps adjusted closes
        onto the pre-split bars inside its fresh window, so for a split recent enough
        to matter the suspect range is [split - _UNADJ_FRESH_DAYS, split). Older
        splits return None: nothing has re-stamped bars near them.
        Returns (start, end_exclusive) or None.
        """
        recent = [sd for sd in split_dates
                  if (date.today() - sd).days <= _UNADJ_FRESH_DAYS + 3]
        if not recent:
            return None
        floor = max(recent)
        return (floor - timedelta(days=_UNADJ_FRESH_DAYS), floor)

    def _scrub_poisoned_unadj(self, symbol, new_split_dates):
        """NULL close_unadj in the poison window of splits first seen this call."""
        win = self._unadj_poison_window(new_split_dates)
        if not win:
            return
        n = (PriceHistory.update(close_unadj=None)
             .where((PriceHistory.symbol == symbol)
                    & (PriceHistory.date >= win[0]) & (PriceHistory.date < win[1])
                    & PriceHistory.close_unadj.is_null(False))
             .execute())
        if n:
            print(Fore.YELLOW + f'  [split] {symbol}: cleared {n} close_unadj value(s) '
                                f'in pre-split poison window {win[0]}..{win[1]}')

    @staticmethod
    def _backup_unadj_snapshot(symbol, unadj_rows):
        """Best-effort on-disk copy of the as-traded series a wipe is about to delete."""
        if not unadj_rows:
            return
        try:
            bdir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                '.cache', 'unadj_snapshots')
            os.makedirs(bdir, exist_ok=True)
            path = os.path.join(bdir, f'{symbol}_{date.today().isoformat()}.json')
            with open(path, 'w') as f:
                json.dump({str(d): float(cu) for d, cu in unadj_rows}, f)
        except Exception:
            pass   # the in-memory snapshot still drives the restore

    @staticmethod
    def _restore_unadj_after_repull(symbol, unadj_rows, poison_window):
        """Write snapshotted as-traded closes back after a wipe + re-pull.

        Restores the ORIGINAL prints verbatim -- never re-derived from adjusted
        close. Values inside poison_window are dropped (see _unadj_poison_window).
        Before the newest recent split the snapshot wins outright (anything the
        re-pull wrote there would be an adjusted price); elsewhere only NULLs are
        filled, so the re-pull's fresh post-split prints stand.
        """
        if not unadj_rows:
            return 0
        floor = poison_window[1] if poison_window else None
        keep = [(d, float(cu)) for d, cu in unadj_rows
                if not (poison_window and poison_window[0] <= d < poison_window[1])]
        if not keep:
            return 0
        if floor is not None:
            guard = ' AND (`date` < %s OR `close_unadj` IS NULL)'
        else:
            guard = ' AND `close_unadj` IS NULL'
        restored = 0
        CH = 500
        for i in range(0, len(keep), CH):
            chunk = keep[i:i + CH]
            case = ' '.join(['WHEN %s THEN %s'] * len(chunk))
            marks = ','.join(['%s'] * len(chunk))
            params = [p for d, cu in chunk for p in (d, cu)]
            params += [symbol] + [d for d, _ in chunk]
            if floor is not None:
                params.append(floor)
            cur = DB.execute_sql(
                f'UPDATE price_history SET close_unadj = CASE `date` {case} '
                f'ELSE close_unadj END WHERE `symbol`=%s AND `date` IN ({marks})' + guard,
                tuple(params))
    ...
    def update_stocks(self, full=False, with_options=False, only_remainder=False,
        ...
        # Self-heal pass (chip 2026-08-22): force a '5d' re-pull for any symbol
        # whose recent bar never got close_unadj -- the fossilized-premarket-bar
        # signature. bulk_build's COALESCE upsert fills unadj and conflict-refresh
        # replaces the frozen open=close / volume=0 row; idempotent, since healed
        # rows no longer match. Typically 0-40 symbols; one extra '5d' batch.
        straggler_symbols = _find_unadj_straggler_symbols(
            [s.symbol for s in stock_list])
        if straggler_symbols:
            print(Fore.YELLOW + f"Self-heal: {len(straggler_symbols)} symbol(s) with fresh NULL close_unadj "
                                f"-> forcing '5d' re-pull: {', '.join(straggler_symbols[:15])}"
                                + (' ...' if len(straggler_symbols) > 15 else ''))
            for _sym in straggler_symbols:
                if period_by_symbol.get(_sym) == '1d':
                    period_by_symbol[_sym] = '5d'
        ...
        def process_stock(stock):
            ...
            fetch_ok = True
            # Detect and apply any unprocessed splits BEFORE today's pull; if applied,
            # force full recalc. recent_only=True: ignore splits older than 1 year.
            # Order matters for close_unadj: registering the split first keeps the
            # row builder's split floor current, so the pull below cannot stamp
            # post-split adjusted closes onto pre-split bars. (The bulk prefetch runs
            # before any check -- check_and_apply_splits scrubs/drops its damage.)
...
def _cmd_tp_stop(command, args):
    ...
    Values are printed from the LIVE loaded strategy_config at runtime — this
    docstring deliberately carries no numbers (every baked copy went stale; the
    2026-08-10 tpsl_refine ship is the current canon: tight-TP scalp-and-dead-hold
    for 30 DTE, base==stress). Per-DTE option configs split 2026-05-04; see
    strategy_config.py STRATEGY_30DTE / STRATEGY_15DTE for current values.
    """
...
def _cmd_alloc(args):
    ...
    EXITS:
        30 DTE (hard sell day 15) — tpsl_refine ship 2026-08-10 (scalp-and-dead-hold):
            Calls  TP=+10% / SL=-100%  (tight TP, disaster-stop SL; deep fires
                   reroute to dead-hold; Apex profile pins TP=+10%/SL=-60%)
            Puts   off (net-negative on honest v70+ scores)
            Hard sell  -40%
            (Live values printed in output reflect the actual loaded config.)
    ...
    SPREAD_TILT_ENABLED = getattr(_alloc_cfg, 'SPREAD_TILT_ENABLED', False)
    SPREAD_TILT_LO      = getattr(_alloc_cfg, 'SPREAD_TILT_LO', 0.0)
    SPREAD_TILT_HI      = getattr(_alloc_cfg, 'SPREAD_TILT_HI', 0.0)
    SPREAD_TILT_DEPTH   = getattr(_alloc_cfg, 'SPREAD_TILT_DEPTH', 0.0)
    # LIQUIDITY_FLOOR — option-volume admission filter (staged ship-candidate
    # 2026-08-07, Core-evidence-only; see mechanism_registry.py). 0.0 = OFF.
    LIQUIDITY_FLOOR    = getattr(_alloc_cfg, 'LIQUIDITY_FLOOR', 0.0)
    ...
    if SPREAD_TILT_ENABLED:
        print(f"    SPREAD_TILT component-spread: {Fore.YELLOW}lo {SPREAD_TILT_LO} hi {SPREAD_TILT_HI} depth {SPREAD_TILT_DEPTH} (floor {1.0-SPREAD_TILT_DEPTH:.2f}x){Style.RESET_ALL}   (haircut high-disagreement 75-79 calls; down-weight only; calls only; ship 2026-06-15)")
    if LIQUIDITY_FLOOR > 0:
        print(f"    Liquidity floor: {Fore.YELLOW}ON floor={int(LIQUIDITY_FLOOR)} c/d{Style.RESET_ALL}   (drop 75+ CALL signals below trailing-30d avg option_volume_30d; calls only; staged 2026-08-07)")
    else:
        print(f"    Liquidity floor: {Fore.RED}OFF (0 c/d){Style.RESET_ALL}   (Core-evidence-only staged ship-candidate; enable gated on P2.B live-fill confirmation; FF3_STAGEB_RESULTS.md)")
...
def _cmd_backtest(args):
    ...
    SPREAD_TILT_ENABLED   = bool(getattr(_scfg_f3f, 'SPREAD_TILT_ENABLED', False))
    SPREAD_TILT_LO        = float(getattr(_scfg_f3f, 'SPREAD_TILT_LO', 26.9))
    SPREAD_TILT_HI        = float(getattr(_scfg_f3f, 'SPREAD_TILT_HI', 31.3))
    SPREAD_TILT_DEPTH     = float(getattr(_scfg_f3f, 'SPREAD_TILT_DEPTH', 0.40))
    ...
    # LIQUIDITY_FLOOR — option-volume admission filter (staged ship-candidate
    # 2026-08-07, Core-evidence-only; see mechanism_registry.py). Reads from
    # _scfg_f3f like WEAK_WEEKLY_CALL_DROP above so it naturally resolves 0.0
    # (off) on both DTEs. 0.0 = OFF.
    LIQUIDITY_FLOOR = float(getattr(_scfg_f3f, 'LIQUIDITY_FLOOR', 0.0))
    ...
    # LIQUIDITY_FLOOR — option-volume admission filter (staged ship-candidate
    # 2026-08-07, default OFF => no-op). Reuses backtest_cascade's filter (same
    # (symbol,date)->option_volume_30d map, same drop-from-list shape) rather
    # than duplicating it, mirroring the CTSL import above.
    if LIQUIDITY_FLOOR > 0 and raw:
        from backtest_cascade import _apply_liquidity_floor_filter as _liq_apply
        raw = _liq_apply(raw, floor=LIQUIDITY_FLOOR)
    ...
    if SPREAD_TILT_ENABLED:
        try:
            ...
            print(Fore.YELLOW + f'  [SPREAD_TILT] map load failed ({_st_e}); SPREAD_TILT inactive')
            _spread_tilt_map = {}
    ...
    if LIQUIDITY_FLOOR > 0:
        print(f'  Liquidity floor: drop 75+ CALL signals below {int(LIQUIDITY_FLOOR)} c/d trailing-30d avg option_volume_30d (calls only; staged 2026-08-07)')
    else:
        print(f'  Liquidity floor: OFF (0 c/d) — Core-evidence-only staged ship-candidate, enable gated on P2.B live-fill confirmation')
...
Common commands:
  update [--score-versions v57,v58]      score today (scheduler entrypoint)
  close-update [--score-versions ...]    post-close scoring + options pull
  recalculate [SYM] [lookback] [--force] [--full] [--all] [--rebuild-parquets]
  assess [--force] [--dte 30|15|both] [--profile sentinel|core|apex] [lookback]
  backtest [--dte 15] [--from DATE] [--capital N]
  simulate [SYM...] [days] [--compare] [--assess] [--diff-assess]
  explain-scores [SYM...] [days]
  intraday-swings [days] [--fakeouts] [--min-swing N] [--version vNN]
  intraday-drill SYM [YYYY-MM-DD] [--version vNN]
  historic-update [window] | breadth-backfill [days] | regime-backfill [days]
  alloc <portfolio_value_cad> [--strategy 30dte|15dte] [--staging sector-etf]
  staging [list|migrate-legacy [--delete-legacy]]
  algorithm [active|list|snapshot-staging|snapshot-current|diff|promote-candidate]
  portfolio-snapshot [current|list|diff]
  revert <version|v14|git-hash>
  portfolio [sync|reset|status|pending|notify]
  temporal-refresh [--dte 30|15|both] [--profiles all|sentinel,core,apex]
  queue <submit|list|status|show|logs|wait|kill|cancel|priority|hold|release|...>
  mc-run [--dte 15] [--n N] [--windows 22-now,5y] [--smoke]
  pull SYM [SYM...] | pull-options | rebuild-parquets [--exp NAME] [--all]
  rebuild                                cold-start only

Not exhaustive — the full dispatcher lives in trader.py main();
see CLAUDE.md "Trader CLI Reference" for the documented surface.
"""
...
def main(command=None, args=None):
    ...
    if command == 'pull' and args:
        ...
        if sub not in ('sync', 'reset', 'status', 'pending', 'notify',
                       'sprint-status', 'sprint-clear', 'sprint-arm',
                       'pause', 'resume', 'record-fill'):
            print(Fore.YELLOW + 'Usage: trader portfolio [sync|reset|status|pending|notify|'
                                 'sprint-status|sprint-clear|sprint-arm|pause|resume|'
                                 'record-fill SYM PRICE QTY [--side buy|sell] [--ts ISO] [--pos-key KEY]]')
            return
        # record-fill takes positional args + flags beyond the bare verb --
        # every other subcommand is a single token, so this is the only case
        # that needs the FULL token list forwarded (not just [sub]).
        portfolio_engine._cli(tokens if sub == 'record-fill' else [sub])
        ...
        do_include_delisted = False  # --include-delisted: score dead tickers up to delisting
        ...
        if args:
            ...
            while i < len(arg_list):
                ...
                if stripped == 'all':
                    ...
                    do_scores_only = True
                elif stripped == 'include-delisted':
                    # Score dead tickers too, up to their delisting date. Off by default so
                    # the daily live recalc never pays for them; required for a
                    # survivorship-honest historical rebuild.
                    do_include_delisted = True
        ...
        if do_tail_only:
            ...
            _run_phase(
                ...
                lambda: recalculate_run(
                    ...
                    include_delisted=do_include_delisted,
                )
            )
            print(Fore.GREEN + "Done.")
        ...
        #   --metric tp26   → 2026-08-10 dual-anchor reanchor, 30 DTE only, explicit-only
        #                     (never bundled into 'both' — additive, doesn't touch wr/tp)
        ...
        if args:
            ...
            while i < len(tokens):
                ...
                if arg == '--compare':
                    ...
                    if i + 1 >= len(tokens):
                        print(Fore.RED + '--metric requires a value (wr, tp, tp26, or both).')
                        return
                    metric_explicit = tokens[i + 1]
                    if metric_explicit not in ('wr', 'tp', 'tp26', 'both'):
                        print(Fore.RED + f'--metric must be wr, tp, tp26, or both, got: {metric_explicit}')
                        return
                    i += 2
        ...
        if do_meta:
            ...
            import strategy_config as _strategy_cfg
            if metric_explicit == 'tp26':
                # tp26 (2026-08-10 dual-anchor reanchor, experiments/assess_reanchor_2026_08/)
                # is 30-DTE-only and explicit-only. Handled before the branches below on
                # purpose: assess_combos() only ever enumerates wr/tp pairs, so routing
                # tp26 through the --dte both path would filter down to an EMPTY combo
                # list and silently no-op (0 iterations, exit 0, nothing computed).
                _selected_dte = '30' if dte_explicit in (None, 'both') else dte_explicit
                _combos = [(_selected_dte, 'tp26')]
            elif dte_explicit == 'both':
        ...
        # Scripted callers (queue jobs, scheduler) must see a non-zero exit
        # here, never an interactive prompt.
        if command:
            print(Fore.RED + f'Unknown or incomplete command: {command}')
        else:
            print(Fore.RED + 'No command given.')
        print(CLI_USAGE)
        sys.exit(2)
