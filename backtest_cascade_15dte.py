...
SLIP_HARD           = _opt.SLIP_HARD   # canon: read from config (was hardcoded -0.005)
...
# Weak-weekly call filter — disabled for 15 DTE (not validated). Schema parity.
                ...
                for k in range(j + 1, len(ph_rows)):
                    ...
                    if bar_k.date >= deadline:
                        ...
                        return TradeOutcome(symbol, signal_date, score, tier,
                                            ...
                                            close_pnl + SLIP_ENTRY + SLIP_HARD, bars_k, stressed,  # forced day-7 close
                                            exit_price=close_k,
                                            tp_price=tp_price, sl_price=sl_price, deadline=deadline)
                last = ph_rows[-1]
...
def run_backtest(outcomes_by_date: dict,
    ...
    max_call_pos = cfg.get('max_positions_call', None)
    max_put_pos = cfg.get('max_positions_put', None)
    ...
    for today in trading_days:
        ...
        # 2b. Running DD snapshot for allocation modifiers.
        ...
        for outcome in outcomes_by_date.get(today, []):
            ...
            # SAW Put U-curve — scales put alloc by sector breadth U-shape (puts only).
        ...
        # Backfill portfolio_value for all trades logged today.
