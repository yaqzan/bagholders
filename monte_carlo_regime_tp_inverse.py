"""
Monte Carlo — Regime-Conditioned Take Profit Sweep (INVERSE)
============================================================
Complement to monte_carlo_regime_tp_sweep.py. That sweep held base TP=30%
and varied the STRESS TP. This inverse holds stress TP=30% and varies the
CALM (non-stress) TP — decoupling which side of the regime switch drives
the observed uplift.

Prior winning rule: brd_TP30/35 (calm=30%, stress=35%) beat fixed_TP30 by
+49%. Two possible explanations:

  (A) "Loosen when stress arrives" — stress regimes uniquely favor wider
      TPs because IV is elevated and σ-moves are larger. Calm at 30% is
      already optimal. If true, inverse rules that loosen calm (35%, 40%)
      will not help and will lose to fixed_TP30.

  (B) "Tighten when calm arrives" — calm regimes uniquely favor tighter
      TPs because capital velocity matters most when moves are smaller.
      Stress at 30% is already wide enough. If true, inverse rules that
      tighten calm (20%, 25%) will win.

  (C) Both effects stack — winner is brd_TP25/35 or similar (pair calm
      tighten + stress loosen). This sweep cannot test (C) directly but
      rules out the pure (A) vs (B) case.

Rule shape (13 total):
  stress TP = 30%  (locked — applied when regime signal ≥ threshold)
  base  TP ∈ {20%, 25%, 35%, 40%}  (non-stress / calm regime)

Three signal sources (same as forward sweep):
  composite ≥ 50     brd_inv ≥ 50     vix_score ≥ 70

Windows (starting at 2022 — worst-case bear-entry compounding test):
  2022→now   (primary, continuous)
  2022, 2023, 2024, 2025  (per-year context)

All other optimal-strategy parameters are locked:
...
Usage: python monte_carlo_regime_tp_inverse.py
"""
...
# Locked stress-regime TP + breadth-adaptive SL bands
TP_STRESS_FIXED = 0.30   # LOCKED stress TP — calm TP varies per rule
...
# ==============================================================================
# Rule definitions (INVERSE — stress TP locked, calm TP varies)
# ==============================================================================
# Each rule pairs a stress-detection signal with a CALM-regime TP %.
# Stressed regime (signal ≥ threshold) ALWAYS uses TP_STRESS_FIXED = 0.30.
# SL is always breadth-adaptive h35→40 (locked, not varied).
#
# Label format: <src>_TP<calm>/<stress>  → e.g. brd_TP25/30 = calm 25%, stress 30%.

RULES = [
    # ── Fixed baseline (no TP switching, always 30%) ──
    {'label': 'fixed_TP30',   'tp_source': None,        'tp_threshold': None,
     'tp_calm': 0.30, 'desc': 'Fixed TP=30% (baseline)'},

    # ── Composite-driven TP swaps (threshold @ 50; calm = below threshold) ──
    {'label': 'comp_TP20/30', 'tp_source': 'composite', 'tp_threshold': 50,
     'tp_calm': 0.20, 'desc': 'comp<50 → TP=20% (calm tighter)'},
    {'label': 'comp_TP25/30', 'tp_source': 'composite', 'tp_threshold': 50,
     'tp_calm': 0.25, 'desc': 'comp<50 → TP=25% (calm tighter)'},
    {'label': 'comp_TP35/30', 'tp_source': 'composite', 'tp_threshold': 50,
     'tp_calm': 0.35, 'desc': 'comp<50 → TP=35% (calm looser)'},
    {'label': 'comp_TP40/30', 'tp_source': 'composite', 'tp_threshold': 50,
     'tp_calm': 0.40, 'desc': 'comp<50 → TP=40% (calm looser)'},

    # ── Breadth-driven TP swaps (threshold @ 50, using brd_inv) ──
    {'label': 'brd_TP20/30',  'tp_source': 'brd_inv',   'tp_threshold': 50,
     'tp_calm': 0.20, 'desc': 'brd_inv<50 → TP=20% (calm tighter)'},
    {'label': 'brd_TP25/30',  'tp_source': 'brd_inv',   'tp_threshold': 50,
     'tp_calm': 0.25, 'desc': 'brd_inv<50 → TP=25% (calm tighter)'},
    {'label': 'brd_TP35/30',  'tp_source': 'brd_inv',   'tp_threshold': 50,
     'tp_calm': 0.35, 'desc': 'brd_inv<50 → TP=35% (calm looser)'},
    {'label': 'brd_TP40/30',  'tp_source': 'brd_inv',   'tp_threshold': 50,
     'tp_calm': 0.40, 'desc': 'brd_inv<50 → TP=40% (calm looser)'},

    # ── VIX-driven TP swaps (vix_score threshold @ 70) ──
    {'label': 'vix_TP20/30',  'tp_source': 'vix_score', 'tp_threshold': 70,
     'tp_calm': 0.20, 'desc': 'vix_score<70 → TP=20% (calm tighter)'},
    {'label': 'vix_TP25/30',  'tp_source': 'vix_score', 'tp_threshold': 70,
     'tp_calm': 0.25, 'desc': 'vix_score<70 → TP=25% (calm tighter)'},
    {'label': 'vix_TP35/30',  'tp_source': 'vix_score', 'tp_threshold': 70,
     'tp_calm': 0.35, 'desc': 'vix_score<70 → TP=35% (calm looser)'},
    {'label': 'vix_TP40/30',  'tp_source': 'vix_score', 'tp_threshold': 70,
     'tp_calm': 0.40, 'desc': 'vix_score<70 → TP=40% (calm looser)'},
]
...
def precompute_rule_outcomes(bar_data, signal_maps, rule):
    """
    INVERSE logic: stress regime locked at TP_STRESS_FIXED = 30%; calm regime
    uses rule['tp_calm']. Signal ≥ threshold = STRESS; below = CALM.
    SL always breadth-driven (brd_inv ≥ 50 → SL_STRESS else SL_BASE).
    """
    outcomes = {}
    tp_pcts  = []
    sl_pcts  = []
    ...
    tp_calm      = rule['tp_calm']

    for key, bd in bar_data.items():
        _, sig_date = key

        # ---- TP selection (inverse: stress=30% fixed, calm varies) --------
        if tp_map is None:
            tp_pct = tp_calm   # fixed baseline rule (no switching)
        else:
            sig_val = get_signal_for_date(tp_map, tp_dates, sig_date)
            if sig_val is not None and sig_val >= tp_threshold:
                tp_pct = TP_STRESS_FIXED
                tp_stress_count += 1
            else:
                tp_pct = tp_calm
...
def main():
    print('='*160)
    print("MONTE CARLO — Regime-Conditioned Take-Profit Sweep (INVERSE)")
    print('='*160)
    print(f"Strategy : stress TP={TP_STRESS_FIXED:.0%} LOCKED | calm TP varies per rule | "
    ...
    print(f"Rules    : {len(RULES)} total (1 fixed + 3 sources x 4 calm TP values)")
    print(f"Windows  : {len(WINDOWS)}  (2022→now primary + per-year context)")
    print(f"Question : Does tightening/loosening the CALM TP (with stress TP=30% fixed)")
    print(f"           beat fixed TP=30%? Isolates calm-vs-stress contribution.")
