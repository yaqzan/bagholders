"""Phase C (15 DTE) — TP/SL by breadth Bayesian sweep on v35 algorithm.

Sweeps breadth-step TP/SL for calls only on 15 DTE. Constraints:
  - PUT_SL locked at -0.20 (docs: never widen)
  - PREMIUM_MULT=1.29 means smaller σ-thresholds for same TP%
  - HOLD_DAYS=7 means even tighter SL more rewarded (faster theta)

Search space scaled for 15 DTE characteristics.

Usage:
  PYTHONIOENCODING=utf-8 BUDGET=18 N_ITER=100 python -u \\
    experiments/v15_optim/phase_c_tpsl.py 2>&1 | tee experiments/v15_optim/phase_c_tpsl.log
"""
...
# Same option-side grids as 30 DTE — directionally similar tradeoff space
PARAM_GRID = {
...
def is_valid(p):
    if not (p['tp_stress'] > p['tp_base']): return False
    if not (p['sl_stress'] < p['sl_base']): return False
    be_base   = abs(p['sl_base'])   / (p['tp_base']   + abs(p['sl_base']))
    be_stress = abs(p['sl_stress']) / (p['tp_stress'] + abs(p['sl_stress']))
    if be_base > 0.50 or be_stress > 0.50: return False
    return True
...
def apply_cfg(params, n_iter):
    ...
    mc15.TP_BASE   = params['tp_base']
    mc15.TP_STRESS = params['tp_stress']
    mc15.SL_BASE   = params['sl_base']
    mc15.SL_STRESS = params['sl_stress']
    mc15.BREADTH_THRESHOLD = params['breadth']
    mc15.NET_TP_BASE   = params['tp_base']
    mc15.NET_TP_STRESS = params['tp_stress']
    mc15.NET_SL_BASE   = params['sl_base']
    mc15.NET_SL_STRESS = params['sl_stress']
    mc15.TP_SIGMA_BASE   = params['tp_base']        * cfg15.PREMIUM_MULT / opt.DELTA
    mc15.TP_SIGMA_STRESS = params['tp_stress']      * cfg15.PREMIUM_MULT / opt.DELTA
    mc15.SL_SIGMA_BASE   = abs(params['sl_base'])   * cfg15.PREMIUM_MULT / opt.DELTA
    mc15.SL_SIGMA_STRESS = abs(params['sl_stress']) * cfg15.PREMIUM_MULT / opt.DELTA
    mc15.N_ITER = n_iter
...
def main():
    ...
    print(f"Phase C (15 DTE) TP/SL — budget={BUDGET}, batch={BATCH}, N_ITER={N_ITER}, "
    ...
    seeds = [
        # current shipped 15 DTE
        {'tp_base':0.35,'tp_stress':0.40,'sl_base':-0.30,'sl_stress':-0.35,'breadth':50},
        # apply 30 DTE v32_optim winner
        {'tp_base':0.33,'tp_stress':0.42,'sl_base':-0.27,'sl_stress':-0.40,'breadth':40},
        # tighter SL only
    ...
    print("\n" + "="*120)
    print(f"PHASE C (15 DTE) SUMMARY — {len(evaluated)} configs evaluated")
    print("="*120)
