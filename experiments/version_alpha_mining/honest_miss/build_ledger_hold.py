...
OPT = {'k_low':1.274,'m_low':10.0,'k_high':1.274,'m_high':10.0}  # HOLD: TP-reach, no early stop
...
def main():
    ...
    with open(os.path.join(outdir, "recs_hold_%s.json" % tag), "w") as f:
        json.dump(recs, f)
    with open(os.path.join(outdir, "supply_hold_%s.json" % tag), "w") as f:
        json.dump(supply, f)

    import polars as pl
    df = pl.DataFrame(recs, infer_schema_length=None).filter(pl.col("opt15").is_not_null())
    out = os.path.join(outdir, "ledger_hold_%s.parquet" % tag)
    ...
    with open(os.path.join(outdir, "ledger_hold_%s_summary.json" % tag), "w") as f:
