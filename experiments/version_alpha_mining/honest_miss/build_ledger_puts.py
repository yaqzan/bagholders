...
def main():
    ...
    with open(os.path.join(outdir, "recs_puts_%s.json" % tag), "w") as f:
        json.dump(recs, f)
    with open(os.path.join(outdir, "supply_puts_%s.json" % tag), "w") as f:
        json.dump(supply, f)

    import polars as pl
    df = pl.DataFrame(recs, infer_schema_length=None).filter(pl.col("opt15").is_not_null())
    out = os.path.join(outdir, "ledger_puts_%s.parquet" % tag)
    ...
    with open(os.path.join(outdir, "ledger_puts_%s_summary.json" % tag), "w") as f:
