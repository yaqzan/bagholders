...
OUT = ROOT / "experiments" / "dd_residual2_v70"
...
def add_bins(T):
    e = [
        ...
        # ---- FRESH axes for this run (NH/NL participation quality, %above-EMA, breadth velocity) ----
        # nhnl = new_highs_52w - new_lows_52w. Dist p5=-53 p25=+15 p50=+54 p75=+96 p95=+157.
        # Divergence cohort = LOW/negative nhnl (narrow leadership / distribution) despite index up.
        pl.when(pl.col("nhnl").is_null()).then(pl.lit("nhnl_na"))
          .when(pl.col("nhnl") < -15).then(pl.lit("nhnl1_distrib_un15"))
          .when(pl.col("nhnl") < 15).then(pl.lit("nhnl2_narrow_15_15"))
          .when(pl.col("nhnl") < 55).then(pl.lit("nhnl3_mild_15_55"))
          .when(pl.col("nhnl") < 100).then(pl.lit("nhnl4_healthy_55_100"))
          .otherwise(pl.lit("nhnl5_broad_100p")).alias("b_nhnl"),
        pl.when(pl.col("pe50").is_null()).then(pl.lit("pe50_na"))
          .when(pl.col("pe50") < 40).then(pl.lit("pe50_1_u40"))
          .when(pl.col("pe50") < 55).then(pl.lit("pe50_2_40_55"))
          .when(pl.col("pe50") < 70).then(pl.lit("pe50_3_55_70"))
          .otherwise(pl.lit("pe50_4_70p")).alias("b_pe50"),
        pl.when(pl.col("bvel5").is_null()).then(pl.lit("bv_na"))
          .when(pl.col("bvel5") < -5).then(pl.lit("bv1_falling_un5"))
          .when(pl.col("bvel5") < -1).then(pl.lit("bv2_dn_5_1"))
          .when(pl.col("bvel5") < 1).then(pl.lit("bv3_flat_1_1"))
          .when(pl.col("bvel5") < 5).then(pl.lit("bv4_up_1_5"))
          .otherwise(pl.lit("bv5_rising_5p")).alias("b_bvel"),
...
def main():
    ...
    for ax in ["b_nhnl", "b_pe50", "b_bvel", "b_vixvel", "b_trin", "b_concur", "b_dd", "b_adslope", "b_vix", "b_mcc", "b_brd", "b_tier"]:
        report["cohorts"][ax] = cohort(T, [ax], base, f"single: {ax}")

    # crash-artifact guards on the most-promising FRESH low-EV bands
    report["robustness"]["nhnl_distrib"]   = robustness(T, "b_nhnl", ["nhnl1_distrib_un15", "nhnl2_narrow_15_15"], wins)
    report["robustness"]["nhnl_lowmid"]    = robustness(T, "b_nhnl", ["nhnl3_mild_15_55"], wins)
    report["robustness"]["pe50_mid"]       = robustness(T, "b_pe50", ["pe50_2_40_55"], wins)
    report["robustness"]["bvel_falling"]   = robustness(T, "b_bvel", ["bv1_falling_un5", "bv2_dn_5_1"], wins)
    report["robustness"]["vixvel_rising"]  = robustness(T, "b_vixvel", ["vv4_up_1_3", "vv5_rising_3p"], wins)
    report["robustness"]["vixvel_falling"] = robustness(T, "b_vixvel", ["vv1_falling_un3", "vv2_dn_3_1"], wins)
    ...
    if O.height >= MIN_N:
        ...
        report["ortho"] = {}
        for ax in ["b_nhnl", "b_pe50", "b_bvel", "b_vixvel", "b_trin", "b_concur", "b_dd", "b_adslope"]:
