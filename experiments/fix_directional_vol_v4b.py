"""
Iteration 4b: hybrid — keep multiplicative amplification when vol direction
aligns with score side, use directional blend only on contra cases.
Preserves the old "boost away from 50" behavior for aligned conviction while
eliminating the inversion bug.
"""
import math
import numpy as np
...
def scoring_fn(
    ...
    **_ignored,
    ...
    if blend_w > 0:
        ...
        # CONVICTION-MACD contradiction gate (parity with original L152-154)
        ...
        if vol_sig in ('CONVICTION', 'REJECTION'):
            score_dev  = weighted_sum - 50
            vol_dir    = 1 if vol_mult > 1.0 else -1
            score_side = 1 if score_dev >= 0 else -1

            if vol_dir == score_side or abs(score_dev) < 5.0:
                # ALIGNED (or near-neutral): use original multiplicative
                # amplification — keeps the top-end push that generates 90+
                deviation = (vol_mult - 1.0) * vol_boost
                weighted_sum = 50 + (weighted_sum - 50) * (1.0 + deviation)
            else:
                # CONTRA: blend toward signal's target to prevent inversion
                target = 100.0 if vol_dir == 1 else 0.0
                bw = min(vol_mag * 0.55 * vol_boost, 0.45)
...
if __name__ == '__main__':
    ...
    syms = sys.argv[1:]
    print(f"[v4b: hybrid aligned-mult / contra-blend] {len(syms)} stocks")
