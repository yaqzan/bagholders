"""
Phase G — per-trade WR validation for the Phase F ship candidate.

Phase F portfolio gates cleared fl060_rc090 and fl060_rc092 on 2023+2025.
fl060_rc092 is the declared ship candidate (beats fl060_rc090 on 22-now
compound by +13%, wins 3/5 per-year, DD-C max 75.3%).

CLAUDE.md Priority #8 mandates per-trade validation BEFORE ship:
  CT-PUT  (overall<=25, trend>=70) WR15 >= 81%   (the PSKY case — headline win)
  AL-PUT  (overall<=25, trend<=30) WR15 within ±0.5pp of baseline
  AL-CALL (overall>=70, trend>=70) WR15 within ±0.5pp of baseline
  Total N within ±5%

This is the diagnostic that v22 passed per-trade but failed portfolio. Doing
the reverse check (portfolio first, then per-trade) on fl060_rc092 before ship.

Adapted from experiments/x_conf_counter_trend.py — swaps make_v22 for
make_ct_score('floored_regime', ...).

Output: experiments/ct_phase_g.out
"""
...
VARIANTS = [
    ('baseline_v19',     None),
    ('fl060_rc090',      make_ct_score('floored_regime', alpha=0.50, floor=0.60, regime_cutoff=0.90)),
    ('fl060_rc092',      make_ct_score('floored_regime', alpha=0.50, floor=0.60, regime_cutoff=0.92)),
]
