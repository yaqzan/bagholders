"""N=300 × 8-window validation for top WEAK_WEEKLY_CALL_DROP candidates.

Runs baseline + variants D (70-84 w<0 ∧ stoch>=35) and E (70-79 w<0) on the
canonical 8-window suite per the P1-P6 portfolio ship gate (CLAUDE.md
known-issues.md "Portfolio Change Ship Criteria").

Output: experiments/call_wadj_70_filter/validate_n300.jsonl + table.
"""
...
LOG = Path(__file__).parent / 'validate_n300.jsonl'

VARIANTS = [
    ('baseline', {}),
    ('D_70-84_w0_stoch35', {'WEAK_WEEKLY_CALL_DROP':'1', 'WEAK_WEEKLY_CALL_MIN_OV':'70',
                            'WEAK_WEEKLY_CALL_MAX_OV':'84', 'WEAK_WEEKLY_CALL_WADJ':'0',
                            'WEAK_WEEKLY_CALL_STOCH_GE':'35'}),
    ('E_70-79_w0',         {'WEAK_WEEKLY_CALL_DROP':'1', 'WEAK_WEEKLY_CALL_MIN_OV':'70',
                            'WEAK_WEEKLY_CALL_MAX_OV':'79', 'WEAK_WEEKLY_CALL_WADJ':'0'}),
]
...
def main():
    if LOG.exists():
        LOG.unlink()
    print(f'[validate] N_ITER={N_ITER} variants={len(VARIANTS)} windows={len(WINDOWS)} '
    ...
    print('\n' + '='*120)
    print('P1-P6 GATE EVALUATION (vs baseline, N=300 × 8 windows)')
    print('='*120)
    for vlabel in [v[0] for v in VARIANTS if v[0] != 'baseline']:
        ...
        if b5 and x5 and b22 and x22:
            p3 = (x5['med_ret'] >= b5['med_ret']) and (x22['med_ret'] >= b22['med_ret'])
        else:
            p3 = False
        ...
        print(f'  P6 (DD vs baseline): max DD = {p6_dd:.1f}%')
