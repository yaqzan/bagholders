"""
Put exhaustion gate: when weighted_sum is deeply bearish (< 35) but MACDh is
non-negative or turning up, the move is an oversold bounce setup — dampen the
score back toward 50 in proportion to how positive macdh is.

Evidence (v15 2y DB, drop>rise classifier on 7,424 LOW signals):
    overall<=15 + macdh<-0.05 -> WR 50.9%  (continuation down)
    overall<=15 + macdh>=0    -> WR 39.0%  (bounce)
  spread: 12pp
"""
import math
import numpy as np
...
def scoring_fn(
    ...
    **_ignored,
    ...
    # ── Put exhaustion gate ────────────────────────────────────────────
    # Deep bearish score + MACDh non-negative = oversold bounce setup.
    # Pull back toward 50 proportional to how positive macdh is.
    # macdh normalized through tanh / 0.05: +0.05 ≈ 0.76, +0.10 ≈ 0.96.
    if weighted_sum < 35 and macdh_raw is not None and macdh_raw >= 0:
        macdh_pos = float(np.tanh(macdh_raw / 0.05))  # [0, 1] since mh >= 0
        # exhaustion_strength scales up as weighted_sum goes deeper (more oversold
        # = more likely exhaustion). At ws=35, factor=0; at ws=10, factor=1.
        exhaustion = min(1.0, max(0.0, (35 - weighted_sum) / 25.0))
        damp_weight = 0.25 * macdh_pos * exhaustion  # max 0.25 (lighter)
...
if __name__ == '__main__':
    ...
    if argv and argv[0].isdigit():
        ...
        random.seed(42)
        syms = random.sample(all_syms, min(n_random, len(all_syms)))
    else:
        syms = argv
    print(f"[put_exhaustion_gate] {len(syms)} stocks")
