"""Phase E-C: validate top-3 from Phase E at full N=500 × 8 windows."""
...
def main():
    ...
    top_path = os.path.join(os.path.dirname(__file__), 'phase_e_top_candidates.json')
    with open(top_path) as f:
        top_k = json.load(f)
    top_n = int(os.environ.get('PHASE_E_TOP_N', '3'))
    candidates = top_k[:top_n]

    version = AlgorithmVersion.get_active_scores_version()
    print(f'\nv27 algorithm: {version.git_commit}')
    print(f'Validating Phase E top-{top_n} + production baseline at N=500 × {len(VALIDATION_WINDOWS)} windows × 3 modes')
    ...
    for i, cand in enumerate(candidates):
        cfg = dict(PHASE_E_BASE)
        cfg['N_ITER'] = 500
        for k, v in cand['params'].items():
            cfg[k] = v
        # PUT_TIER_ALLOC dotted keys: merge into nested dict
        if 'PUT_TIER_ALLOC.put_top' in cand['params']:
            cfg['PUT_TIER_ALLOC'] = {
                'put_top': cand['params'].get('PUT_TIER_ALLOC.put_top', 0.15),
                'put_mid': cand['params'].get('PUT_TIER_ALLOC.put_mid', 0.12),
                'put_low': cand['params'].get('PUT_TIER_ALLOC.put_low', 0.12),
            }
    ...
    # Comparison
    print('\n' + '=' * 110)
    print('PHASE E-C — RESULTS COMPARISON (vs baseline)')
    print('=' * 110)
