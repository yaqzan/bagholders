...
LO, HI = 60, 99   # wide band so reweight boundary-crossings (62->76) have outcome coverage
...
def main():
    ...
    import polars as pl
    outdir = os.path.join(_ROOT, ".cache", "component_reweight")
    os.makedirs(outdir, exist_ok=True)
    df = pl.DataFrame(recs, infer_schema_length=None)
    out = os.path.join(outdir, "ledger_v%d_5y.parquet" % VERSION_ID)
    df.write_parquet(out)
    with open(os.path.join(outdir, "supply_v%d.json" % VERSION_ID), "w") as f:
        json.dump(supply, f)
    ...
    with open(os.path.join(outdir, "ledger_v%d_meta.json" % VERSION_ID), "w") as f:
