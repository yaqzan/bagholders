"""
Softer directional volume blend. Replaces 83851db's (mag*0.55, cap 0.60)
with (mag*0.35, cap 0.40) to fix zero-cluster without bucket inflation.
"""
import numpy as np
...
def scoring_fn(
    ...
    **_ignored,
    ...
    if blend_w > 0:
        ...
        # SOFTER directional blend for CONVICTION/REJECTION
            ...
            if vol_sig == 'CONVICTION':
                if (vol_dir == 1 and macd < 40 and weighted_sum < 65) or \
                   (vol_dir == -1 and macd > 60 and weighted_sum > 35):
                    mag_eff *= 0.25
            bw = min(mag_eff * COEF * vol_boost, CAP)
...
if __name__ == '__main__':
    ...
    if argv and argv[0].isdigit():
        ...
        syms = argv
    print(f"[softer_directional COEF={COEF} CAP={CAP}] {len(syms)} stocks")
