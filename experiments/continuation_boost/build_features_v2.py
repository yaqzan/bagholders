...
PEAKS_PQ_V2     = OUT_DIR / 'peaks_v2.parquet'
PRIORS_PQ_V2    = OUT_DIR / 'priors_v2.parquet'

# Calls only (Q3: puts have no edge)
# components_v32_1825.parquet only has scores >=70 OR <=29 — so we can't admit 60-69 here.
# Sub-threshold filtering happens at boost-apply time (filter overall < 75 in sweep).
PEAK_CALL_LO   = 70   # admit 70+ (have data; sub-threshold = 70-74 vs already-qualifying = 75+)
PEAK_CALL_HI   = 100  # admit all the way up to 100 — needed for full destination tier accounting
