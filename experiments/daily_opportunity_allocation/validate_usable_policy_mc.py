"""Monte Carlo validation for mined daily policy cases.

Experiment-local only. This patches `monte_carlo.run_single_sim` in-process so
one named rule/action policy can scale allocation, cap puts, skip a side, or
adjust MaxPos at the actual fill point. It never writes scores or strategy
constants and disables MC DB persistence by default.
"""
...
from experiments.daily_opportunity_allocation.mine_usable_policy_cases import (
    PolicyCase,
    _baseline_case,
    _candidate_cases,
)
...
def candidate_cases() -> dict[str, PolicyCase]:
    cases = {"baseline": _baseline_case()}
    cases.update({case.name: case for case in _candidate_cases() if case.name != "baseline"})
    return cases
...
def _run_window_with_retry(mc, window_name: str, d_start: date, d_end: date, av: AlgorithmVersion, attempts: int = 3) -> dict[str, Any]:
...
def _max_positions(base: int, case: PolicyCase, active: bool) -> int:
    if not active or not case.action.max_pos_delta:
        return base
    return max(6, min(30, base + case.action.max_pos_delta))


def _side_scale(case: PolicyCase, active: bool, side: str) -> float:
    if not active:
        return 1.0
    return case.action.put_scale if side == "put" else case.action.call_scale


def make_run_single_sim(case: PolicyCase, daily: dict[date, dict[str, Any]]):
    ...
    def run_single_sim(
        ...
        for day_idx, today in enumerate(trading_days):
            ...
            row = daily.get(today, {})
            active = case.rule.predicate(row, dd)
            max_positions = _max_positions(int(mc.MAX_POSITIONS), case, active)
            ...
            def _try_fill_call(sym_id, score, key, ct, ern):
                ...
                if len(positions) >= max_positions or (side_capped and call_open >= cap_call):
                    return False
                policy_scale = _side_scale(case, active, "call")
                if policy_scale <= 0.0:
                    return False
                if ct == "ct_call":
            ...
            def _try_fill_put(sym_id, score, key, ct):
                ...
                if put_cap > 0 and put_open >= put_cap:
                    return False
                policy_scale = _side_scale(case, active, "put")
                if policy_scale <= 0.0:
                    return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--policies", default="baseline")
    ...
    status = out_dir / "usable_policy_mc_status.json"
    started_at = datetime.now(timezone.utc).isoformat()
    status.write_text(
        json.dumps({"phase": "usable_policy_mc", "state": "running", "started_at": started_at}, indent=2),
        encoding="utf-8",
    )
    ...
    missing = [p for p in wanted_policies if p not in cases]
    if missing:
    ...
    try:
        ...
        for policy_name in wanted_policies:
            case = cases[policy_name]
            mc.run_single_sim = make_run_single_sim(case, daily)
            ...
            for window_name in wanted_windows:
                ...
                row["rule"] = case.rule.name
                row["rule_description"] = case.rule.description
                row["action"] = json.dumps(asdict(case.action), sort_keys=True)
                rows.append(row)
                completed += 1
                _write_csv(out_dir / "usable_policy_mc.csv", rows)
                scored = _score_rows(rows)
                _write_csv(out_dir / "usable_policy_mc_ranked.csv", scored)
                status.write_text(
                    json.dumps(
                        {
                            "phase": "usable_policy_mc",
                            "state": "running",
    ...
    scored = _score_rows(rows)
    out_csv = out_dir / "usable_policy_mc.csv"
    ranked_csv = out_dir / "usable_policy_mc_ranked.csv"
    out_md = out_dir / "usable_policy_mc_summary.md"
