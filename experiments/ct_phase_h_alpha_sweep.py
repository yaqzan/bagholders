"""
Phase H — softer-alpha sweep on the fl060_rc092 ship candidate.

Phase G per-trade results on fl060_rc092 (alpha=0.50) were ambiguous:
  CT-PUT  WR15 = 80.5% on 22-now (gate 81%, -0.5pp miss)
  AL-PUT  WR15 = 70.7% (+0.3pp — within tolerance)
  AL-CALL WR15 = 84.9% (+0.1pp — within tolerance)
  Total N: -9.3% (gate +/-5%, outside tolerance but concentrated in AL shrinkage)

Hypothesis: softer alpha (gentler gate) preserves CT-PUT lift while letting
more AL peaks through, shrinking the N drop. If alpha is too soft we lose
the CT-PUT WR win entirely and regress to baseline.

Sweep alpha in {0.25, 0.35, 0.40, 0.45} keeping floor=0.60, regime_cutoff=0.92
(the V2 winner from Phase F).

Output: experiments/ct_phase_h_alpha_sweep.out
"""
...
VARIANTS = [
    ('baseline_v19',          None),
    ('fl060_rc092_a050_REF',  make_ct_score('floored_regime', alpha=0.50, floor=0.60, regime_cutoff=0.92)),
    ('fl060_rc092_a045',      make_ct_score('floored_regime', alpha=0.45, floor=0.60, regime_cutoff=0.92)),
    ('fl060_rc092_a040',      make_ct_score('floored_regime', alpha=0.40, floor=0.60, regime_cutoff=0.92)),
    ('fl060_rc092_a035',      make_ct_score('floored_regime', alpha=0.35, floor=0.60, regime_cutoff=0.92)),
    ('fl060_rc092_a025',      make_ct_score('floored_regime', alpha=0.25, floor=0.60, regime_cutoff=0.92)),
]
