...
SCORING = ScoringConfig(
    ...
    CONT_BOOST_ENABLED=False,   # v74 lean (Phase 3d): neutral-EV supply, DD-inducing via over-deployment
    ...
    # WVD-Wave
    WVD_WAVE_ENABLED=False,   # v74 lean (Phase 3d): harmful — dampened out the highest-EV signals
    WVD_WAVE_GATE_LO=70,
    WVD_WAVE_GATE_HI=85,
    ...
    DAILY_VOLUME_AUTHORITY_WAVE_ENABLED=False,   # v74 lean (Phase 3d): dilutive — added negative-EV supply
    ...
    PESS_TARGET=28,
    # EARN_BOOST
    EARN_BOOST_ENABLED=False,   # v74 lean (Phase 3d): neutral-EV (honest-recal "thin"; +7% apex-EV was N=77 noise)
...
OPT_30DTE = OptionStrategyConfig(
    ...
    # 2026-08-10 TP/SL joint retune (portfolio-only, NO version bump) — "scalp-and-
    # dead-hold": TP 0.30->0.10, SL -0.70->-1.00 (~3.64-sigma disaster stop; deep-SL
    # fires mostly reroute to dead-hold). Evidence: experiments/tpsl_refine_2026_08/
    # (prereg-locked blast->refine->N=500x12 confirm; Core 5y WorstDD 53.2->40.8,
    # DD better on all 12 windows, collapse 0; survivor-robust, parity-clean,
    # fill-probe 0.10 AND calibrated-reality 0.15+GAP_AWARE differential-positive).
    # The May-2026 sub-20%-TP realism veto was re-adjudicated by the owner 2026-08-10
    # after the measured fill-fidelity study (experiments/tp_fill_fidelity_30dte/:
    # economic never-fill 14.1% @TP15 vs 15.8% @TP30 — no tight-TP fill cliff).
    # CAVEAT (program-level, pre-existing): under calibrated-reality fill knobs
    # (TP_FILL_MISS_P=0.15 + TP_FILL_GAP_AWARE) ALL configs incl. the prior baseline
    # read absolutely negative — see tpsl_refine FINDINGS "MC-realism escalation".
    TP_BASE=0.10,
    TP_STRESS=0.10,
    SL_BASE=-1.00,
    SL_STRESS=-1.00,
    BREADTH_THRESHOLD=40,   # inert (base==stress both TP and SL); breadth-conditional
                            # TP/SL re-tested + REJECTED at N=500 2026-08-10 (tpsl_refine Phase C/D)
...
class DteStrategyConfig:
    ...
    # SPREAD_TILT — component-disagreement (ensemble-spread) CALL alloc haircut,
    # GATED STRICTLY to the 75-79 band, DOWN-WEIGHT ONLY (weatherization #6).
    # spread = sqrt(pop-variance of the 5 members [trend,bb,rsi,macd,stoch] / 5).
    # Kill-test (experiments/ensemble_spread/spread_kill_test.py) PASSED SHIP-TO-MC:
    # within 75-79, HIGH-spread (member disagreement) calls carry ~-1.76pp lower
    # apex option-EV than LOW-spread (Welch t +2.05/+2.17), the edge is orthogonal to
    # `overall` (corr -0.013), sign-stable 5/5 years (concentrated bear/recovery/choppy
    # 2021-2023, NOT a bull-2024 artifact). 80-84 INVERTS sign so the band gate is
    # load-bearing — must stay strictly 75<=overall<=79. Linear ramp: full alloc at/below
    # SPREAD_LO, contract toward (1-SPREAD_DEPTH) at/above SPREAD_HI. Never up-size
    # (down-weight-only keeps it collapse-safe per the Apex risk budget).
    # SPREAD_TILT_ENABLED=False → no-op (byte-identical). NOT yet MC-validated.
    SPREAD_TILT_ENABLED: bool
    SPREAD_TILT_LO: float       # spread <= LO -> no haircut (low-disagreement / agreement)
    SPREAD_TILT_HI: float       # spread >= HI -> full haircut (high disagreement)
    SPREAD_TILT_DEPTH: float    # max contraction at/above HI (0..1; alloc *= 1-DEPTH)

    # LIQUIDITY_FLOOR — option-volume admission filter (staged ship-candidate
    # 2026-08-07; see mechanism_registry.py + FF3_STAGEB_RESULTS.md). Drops a
    # 75+ CALL signal when its per-signal option_volume_30d (nearest-ATM 20-45
    # DTE call, trailing-30-cal-day avg contract volume; Stock.average_volume*
    # 0.005 fallback pre-coverage) is below this floor (contracts/day).
    # Core-evidence only: Stage B' N=500x12 beat its matched random-drop
    # control (T4 5y floor-vs-A1 +4.65pp better, collapse=0 every cell); Apex
    # FAILED T4 (5y DD +1.47pp worse) — this ship keeps Apex OFF regardless.
    # 0.0 = OFF (byte-identical); enable is gated on P2.B live-fill
    # confirmation, NOT this wiring pass. See FF3_STAGEB_RESULTS.md.
...
STRATEGY_30DTE = DteStrategyConfig(
    ...
    TIER_ALLOC={
        'ultra':    0.20,   # 95+
        'top':      0.15,   # 85-94
        # v73 retune 2026-06-12: mid 0.10 -> 0.08. The v73 dampener retirements
        # (WCF/ICH/CWCF/CSWC/SCW) restored signal across the WHOLE 75-94 band,
        # not just 75-79 (ReSim 85-89 N 132->220), so the 80-84 tier densified
        # too. Trimming mid alongside low (c04_low03_mid08) is the targeted
        # selectivity lever (leaves ultra/top full = preserves the bull-window
        # compounding engine) over the uniform alloc scale-down (velocity loss).
        'mid':      0.08,   # 80-84
        # v71 retune 2026-06-10: low 0.10 -> 0.05; v73 retune 2026-06-12:
        # 0.05 -> 0.03 (PRF-matched). v71 DOUBLED 75-79 supply (+83%); v73
        # raised it a further +77% (ReSim 5y 3,262 -> 5,769 75+ signals). At
        # 0.05 the doubled-then-redoubled tier again dominated correlated book
        # exposure (DD +5-13pp at unchanged params = the v71 signature). The
        # PRF (portfolio_response.py --derive v73, real supply) proposed low
        # 0.03; squeezing the over-supplied slug = same participation, better
        # diversification, faster recycling. Phase D N=500x10 incl COVID
        # (c04_low03_mid08): EVERY window's DD improves (5y 66.3->61.7, 22-now
        # 66.9->61.5, 2024 -7.4pp, dip -6.4pp), 5y compound +1% (Pareto),
        # collapse=0 everywhere. experiments/v73_portfolio_retune/FINDINGS.md.
        'low':      0.03,   # 75-79
    ...
    # SPREAD_TILT — 75-79-band-only high-disagreement CALL alloc haircut (weatherization
    # #6). SHIPPED 2026-06-15: Stage-3 N=500x9 paired (#195 baseline vs #196 arm) PASS
    # T1-T7 — 5y WorstDD 53.5%->49.4% (-4.1pp), DD down on 8/9 windows, return held-or-up,
    # collapse=0 on every window incl COVID. Calibration from the 75-79 spread distribution
    # (15,980 in-sample rows): p50=26.9 (LO — below = agreement, full alloc) / p75=31.3
    # (HI — the high-disagreement tercile boundary, full haircut). DEPTH 0.40 -> floor 0.60x.
    SPREAD_TILT_ENABLED=True,
    SPREAD_TILT_LO=26.9,
    SPREAD_TILT_HI=31.3,
    SPREAD_TILT_DEPTH=0.40,

    # LIQUIDITY_FLOOR — staged ship-candidate 2026-08-07 (wired_neutral). 0.0=OFF
    # pending P2.B live-fill confirmation. Core-evidence only; Apex failed T4;
    # see FF3_STAGEB_RESULTS.md.
    LIQUIDITY_FLOOR=0.0,
...
STRATEGY_15DTE = DteStrategyConfig(
    ...
    # SPREAD_TILT — disabled for 15 DTE (not wired; fields present for schema parity).
    SPREAD_TILT_ENABLED=False,
    SPREAD_TILT_LO=26.9,
    SPREAD_TILT_HI=31.3,
    SPREAD_TILT_DEPTH=0.40,
