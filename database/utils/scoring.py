...
WCF_LIFT_RAMP_TOP           = _envi('WCF_LIFT_RAMP_TOP',      _S.WCF_LIFT_RAMP_TOP)
...
def compute_pit_mcap_b(mcap_b_latest, close_t, anchor_close):
    """Point-in-time market-cap proxy in $B (F4 survivorship fix, 2026-06-09).

    Stock.market_cap is a CURRENT yfinance snapshot; applying it unchanged to
    historical scoring dates lets stocks that GREW into large caps escape
    historical MCD dampening while shrinkers get over-dampened (survivorship
    leak in the 8.2pp mcap<->TP gradient evidence).

    Proxy: mcap_t ~= mcap_latest * close_t / anchor_close, where anchor_close
    is the symbol's most recent close (the price state matching the snapshot).
    Approximation ignores share-count drift (buybacks/dilution) and is
    split-consistent with adjusted closes — far better than static.

    Falls back to the static value when inputs are missing/degenerate.
    """
    if mcap_b_latest is None:
        return None
    try:
        if close_t is None or anchor_close is None or float(anchor_close) <= 0:
            return mcap_b_latest
        return float(mcap_b_latest) * float(close_t) / float(anchor_close)
    except Exception:
        return mcap_b_latest


def build_pit_mcap_map(mcap_b_latest, ph_rows_asc, anchor_close):
    """Build {date: pit_mcap_b} for all dates in ph_rows_asc.

    Mirrors the build_ret10d_sigma_map pattern so batch/recalc/simulator paths
    share one implementation. Dates with a missing close fall back to the
    static mcap_b_latest (preserves pre-fix behavior on data gaps).
    """
    if mcap_b_latest is None or not ph_rows_asc:
        return {}
    out = {}
    for p in ph_rows_asc:
        out[p.date] = compute_pit_mcap_b(mcap_b_latest, p.close, anchor_close)
    return out
...
def _load_earn_boost_strength_map():
    ...
    # v70 honest recalibration (2026-06-01): rebuilt on look-ahead-free v69
    # pre_boost scores, holdout-locked (<=2026-05-15), both-barrier-gated
    # (generic AND option-aligned WR15 lift > 0). The honest edge is in the
    # pre7 run-up (4-7 cal days out), NOT pre1 (the v34 pre1-heavy cells were a
    # weekly-look-ahead artifact). Calls only; puts have no honest edge (1 cell).
    # v34 table preserved at experiments/v34_calibration/lift_table_v34.json;
    # calibration trail in experiments/earnboost_honest/FINDINGS.md.
    repo_root = Path(__file__).resolve().parents[2]
    table_path = repo_root / 'experiments' / 'earnboost_honest' / 'lift_table_v70.json'
...
# ════════════════════════════════════════════════════════════════════════════
# MOS (Mechanism / Output Separation) — two-layer scoring architecture
# ════════════════════════════════════════════════════════════════════════════
#
# `compute_overall_score` is factored into two pure layers + a registry so the
# look-ahead-leak class (a fitted table living *inside* the scorer, e.g. the old
# EARN_BOOST lift table) is structurally isolated, and every post-`pre_boost`
# mechanism is individually toggleable + independently auditable:
#
#   1. `_compute_core_score(...)`   — the dynamical core (former L948..L1334):
#        dynamic weighting -> asymmetric MACD gate -> asymmetric RSI gate ->
#        weighted_sum -> weekly transition-blend -> volume amplification ->
#        pre_regime -> regime + mis_stress -> capitulation / exhaustion /
#        ext-focal / WCF-lift / CWCF / CWWD / CSWC.  Returns
#        (pre_boost_overall, core_state).
#
#   2. `apply_bias_corrections(overall, core_state, ...)` — the post-pre_boost
#        bias/lift layer (former L1335..L1679), driven by MECHANISM_REGISTRY in
#        byte-identical order: PCD -> MCD -> ICH -> WVD -> PESS -> EARN_BOOST ->
#        SCW -> continuation-echo -> sector-breadth -> DVAW.  Returns
#        (overall, bias_state).
#
# `compute_overall_score` stays the public entrypoint (byte-identical signature
# + 3-tuple return); it calls the two layers then assembles weight_info per the
# contract below.
#
# ─────────────────────────────────────────────────────────────────────────────
# weight_info REASSEMBLY CONTRACT (BLOCKER 2)
# ─────────────────────────────────────────────────────────────────────────────
# weight_info is assembled in `compute_overall_score` AFTER both layers run. Dict
# insertion order is OBSERVABLE (the bit-exact test compares the full ordered
# dict), so the order + emission condition below MUST be reproduced exactly. For
# each key: which layer's state provides it, the emission condition, the value
# expression, in insertion order:
#
#   order  key                          source(state.key)        emit-if
#   -----  ---------------------------  -----------------------  ----------------------------------
#    1     trend  (= w_trend)           core.w_trend             always               round(_,1)
#    2     bb     (= w_bb)              core.w_bb                always               round(_,1)
#    3     rsi    (= w_rsi)             core.w_rsi               always               round(_,1)
#    4     macd   (= w_macd)            core.w_macd              always               round(_,1)
#    5     stoch  (= w_stoch)          core.w_stoch             always               round(_,1)
#    6     ta     (= w_ta)             core.w_ta                always               round(_,1)
#    7     td                          core.trend_dominance     always               round(_,3)
#    8     pre_regime                  core.pre_regime_overall  always               (int)
#   (then, conditionally, in this exact order:)
#    9     cap_dampened                core._cap_dampened       if _cap_dampened     -> True
#   10     exh_damp                    core._exh_damp           if _exh_damp > 0.01  round(_,3)
#   11     ext_damp                    core._ext_damp           if _ext_damp > 0.01  round(_,3)
#   12     wcf_lift                    core._wcf_lift           if _wcf_lift > 0.5   round(_,2)
#   13     cwcf_dampen                 core._cwcf_dampen        if _cwcf_dampen > 0.5  round(_,2)
#   14     cwwd_dampen                 core._cwwd_dampen        if _cwwd_dampen > 0.5  round(_,2)
#   15     pess_lift                   bias._pess_lift          if _pess_lift > 0.5  round(_,2)
#   16     cswc_dampen                 core._cswc_dampen        if _cswc_dampen > 0.5  round(_,2)
#   17     scw_dampen (+ block)        bias._scw_*              if _scw_dampen > 0.5  round(_,2)
#            within the scw block, in order: scw_dampen, scw_base_dampen,
#            scw_scalar, then scw_conf (if _scw_conf_ratio is not None),
#            scw_raw_stoch (if _scw_raw_stoch is not None), and
#            scw_ext_idx + scw_ext_taper (if _scw_ext_index is not None AND
#            SCW_EXT_TAPER_STRENGTH > 0).
#   18     pre_boost                   core._pre_boost_overall  always               (int)
#   19     cont_lift (+ block)         bias._cont_lift          if _cont_lift > 0
#            within: cont_lift, cont_raw_lift, then cont_sig (if prior_signal not None)
#   20     sector_breadth_wave         bias._sector_breadth_meta if truthy           (dict)
#   21     daily_volume_authority_wave bias._dvaw_meta          if truthy            (dict)
#   22     pcd_active (+ pcd_r10sigma) bias._pcd_active         if _pcd_active        pcd_active=1;
#            then pcd_r10sigma (if ret_10d_sigma is not None) round(_,3)
#   23     mcd_dampen (+ mcd_mcap_b)   bias._mcd_dampen         if _mcd_dampen > 0.5  round(_,2);
#            then mcd_mcap_b (if mcap_b is not None) round(_,2)
#   24     ich_call_dampen             bias._ich_call_d         if _ich_call_d > 0.5  round(_,2)
#   25     ich_put_lift                bias._ich_put_l          if _ich_put_l > 0.5   round(_,2)
#   26     kijun_pct                   (input)                  if (_ich_call_d>0.5 or _ich_put_l>0.5)
#                                                               AND kijun_pct not None  round(_,2)
#   27     wvd_lift                    bias._wvd_lift           if abs(_wvd_lift) > 0.5  round(_,2)
#   28     wvd_dampen                  bias._wvd_dampen         if abs(_wvd_dampen) > 0.5 round(_,2)
#   29     wv_force1                   (input)                  if (abs(_wvd_lift)>0.5 or
#                                                               abs(_wvd_dampen)>0.5) AND
#                                                               wv_force1 not None  round(_,4)
#   30     ern_boost (+ days_to_ern)   bias._ern_boost_strength if _ern_boost_strength > 0  round(_,3);
#            then days_to_ern (if days_to_earnings is not None) int(_)
#   31     put_regime_mult             (input)                  if put_regime_multiplier not None round(_,4)
#   32     mis_stress                  (input)                  if mis_stress > 0    round(_,3)
#   33     weekly_detail (.update)     core.weekly_detail       if weekly_detail (truthy) — LAST,
#            spreads all weekly_detail keys (w_adj, wadj_completed, wadj_partial,
#            weekly_transition_t, weekly_adj_gap, + base calculate_weekly_adjustment keys)
#
# vol_update = {'volume': vol_raw, 'volume_signal': vol_sig, 'volume_magnitude':
#               vol_mag} if vol_sig != 'NEUTRAL' else None.
# ════════════════════════════════════════════════════════════════════════════


class LiftTableMechanism:
    """Holdout-safety contract for any registry mechanism that loads a FITTED
    table from disk (the look-ahead-leak class the MOS split exists to prevent).

    A fitted table MUST ship with a sibling `<table>.meta.json` carrying a
    `holdout_cutoff` (ISO date) stamp recording the last date its training data
    may include. `assert_holdout_safe()` refuses (or loudly warns) if the table
    was fit on data past the project holdout cutoff. EARN_BOOST is the canonical
    case — it is config-DEAD in v74 (`compute_earnings_boost_strength`
    short-circuits before any table load), so this is forward-proofing: the
    contract is wired but not yet load-bearing.
    """

    def __init__(self, name, table_path, *, strict=False):
        self.name = name
        self.table_path = Path(table_path)
        self.strict = strict
        self.meta_path = self.table_path.with_suffix(self.table_path.suffix + '.meta.json')
        self._verified = False   # one-shot per process — keep the hot path quiet

    def assert_holdout_safe(self):
        """Verify a sibling .meta.json exists and its holdout_cutoff does not
        leak past the project CALIBRATION_CUTOFF_DATE. Returns the parsed meta
        dict, or {} when the table is absent. Raises (strict) / warns (loose)
        on a missing meta or a leaking cutoff. Verifies once per process (the
        check is config/table state, not per-row) so the scoring hot path stays
        quiet."""
        if self._verified:
            return self._meta_cache
        self._verified = True
        self._meta_cache = self._do_assert()
        return self._meta_cache
    ...
    def _do_assert(self):
        ...
        if not self.meta_path.exists():
            msg = (f"[LiftTableMechanism:{self.name}] fitted table {self.table_path.name} "
                   f"has no sibling {self.meta_path.name} (holdout-cutoff stamp). "
                   f"Fitted tables inside the scorer are the look-ahead-leak class — "
                   f"add {self.meta_path.name} with a 'holdout_cutoff' ISO date.")
            if self.strict:
        ...
        with open(self.meta_path) as f:
            meta = json.load(f)
        cutoff = meta.get('holdout_cutoff')
        try:
            from strategy_config import CALIBRATION_CUTOFF_DATE as _PROJ_CUTOFF
        except Exception:
            _PROJ_CUTOFF = None
        if cutoff and _PROJ_CUTOFF and str(cutoff) > str(_PROJ_CUTOFF):
            msg = (f"[LiftTableMechanism:{self.name}] table holdout_cutoff {cutoff} "
                   f"is PAST the project cutoff {_PROJ_CUTOFF} — possible look-ahead leak.")
            if self.strict:
                raise RuntimeError(msg)
            import sys as _sys
            print(msg, file=_sys.stderr)
...
# EARN_BOOST is the one current mechanism that (when enabled) loads a fitted
# table. Wire the contract even though it is config-DEAD in v74.
_EARN_BOOST_LIFT_TABLE = LiftTableMechanism(
    'EARN_BOOST',
    Path(__file__).resolve().parents[2] / 'experiments' / 'earnboost_honest' / 'lift_table_v70.json',
)


# ─────────────────────────────────────────────────────────────────────────────
# Layer 1 — the dynamical core (former L948..L1334, verbatim arithmetic).
# ─────────────────────────────────────────────────────────────────────────────
def _compute_core_score(
    trend, bb, rsi, macd, stoch, technical_alignment,
    *,
    bb_pct, pct_from_ema50, pct_from_ema200,
    ws_trend, ws_rsi, ws_macd,
    prev_ws_trend, prev_ws_rsi, prev_ws_macd,
    pit_ws_trend, pit_ws_rsi, pit_ws_macd,
    weekly_transition_t,
    vol_mult, vol_raw, vol_sig, vol_mag,
    blend_w, vol_target,
    macdh_raw,
    regime_multiplier, put_regime_multiplier, mis_stress,
):
    """Compute everything up to and including `_pre_boost_overall`.

    Returns (pre_boost_overall, core_state) where core_state carries every local
    the weight_info assembly reads from this layer (see the reassembly contract).
    """
    # ── Dynamic weighting ────────────────────────────────────────────
    ...
    # ── Weekly adjustment (point-in-time transition blend) ───────────
    # Two point-in-time weekly adjustments are computed and blended by how far
    # into the trading week the signal sits (weekly_transition_t):
    #
    #   comp_adj = calculate_weekly_adjustment(LAST-COMPLETED week W-1,
    #                                          TWO-WEEKS-AGO week W-2)
    #     -> state 1: exactly what the LIVE single-row scorer uses
    #        (Score.weekly_score property = date-(weekday()+7)).  Honest but
    #        ~1 week stale.  ws_* / prev_ws_* MUST be the W-1 / W-2 weekly rows.
    #
    #   pit_adj  = calculate_weekly_adjustment(PARTIAL current week W [Mon->date,
    #                                          reconstructed from daily bars],
    #                                          LAST-COMPLETED week W-1)
    #     -> state 2: freshest point-in-time reading (no future bars).
    #
    #   wadj_effective = (1-t)*comp_adj + t*pit_adj
    #     t = trading-bars-elapsed-this-week / 5  (Mon~=0.2 -> Fri=1.0).
    #
    # Early week (t->0) the partial is 1-2 noisy bars, so lean on the stable
    # completed week; late week (t->1) the partial is ~complete + fresh, so
    # trust it.  Both inputs are point-in-time, so this removes the recalc
    # look-ahead (which read the COMPLETE Mon-Fri current week for a mid-week
    # signal) AND smooths the Monday weekly cliff into a gradual t-ramp.
    #
    # weekly_detail['w_adj'] is overridden with the blended value so every
    # downstream weekly-gated dampener (WCF / CWCF / CWWD / CSWC / SCW) sees the
    # blended adjustment.  pit_* kwargs absent -> graceful fallback to comp_adj.
    weekly_detail = None
    if None not in (ws_rsi, ws_macd):
        comp_adj, weekly_detail = calculate_weekly_adjustment(
            ws_trend, ws_rsi, ws_macd,
            prev_ws_trend, prev_ws_rsi, prev_ws_macd,
        )
        adj = comp_adj
        pit_adj = None
        if None not in (pit_ws_rsi, pit_ws_macd):
            pit_adj, _pit_detail = calculate_weekly_adjustment(
                pit_ws_trend, pit_ws_rsi, pit_ws_macd,
                ws_trend, ws_rsi, ws_macd,
            )
            if pit_adj is not None:
                _t = max(0.0, min(1.0, float(weekly_transition_t)))
                adj = (1.0 - _t) * comp_adj + _t * pit_adj
        if weekly_detail is not None:
            weekly_detail['w_adj'] = round(adj, 1)
            weekly_detail['wadj_completed'] = round(comp_adj, 1)
            weekly_detail['wadj_partial'] = (round(pit_adj, 1)
                                             if pit_adj is not None else None)
            weekly_detail['weekly_transition_t'] = round(
                max(0.0, min(1.0, float(weekly_transition_t))), 3)
            weekly_detail['weekly_adj_gap'] = (
                round(abs(pit_adj - comp_adj), 1) if pit_adj is not None else None)
        weighted_sum += adj
    ...
    # Score-axis ramp (candidate 2026-06-11, experiments/wcf_score_ramp/):
    # the original binary integer gate `overall < 28` produced a 21-point
    # intraday cliff — a 1-pt post-regime wobble at 27/28 toggled wcf_lift
    # 21.85 <-> None (GIS/CBRE/GEHC/TEM fakeout family in score_intraday_logs,
    # 2026-06-10/11). Full lift is unchanged at overall <= GATE-1 (=27); the
    # lift now fades linearly to zero at RAMP_TOP (=33) instead of cutting off.
    # RAMP_TOP == GATE reproduces the legacy binary gate bit-exactly.
    # Outputs of the partial-lift zone (28-30) land in 31-49 — above every
    # put gate (<=25/27) and below every call gate (>=69) — so all
    # cascade-tradable buckets are byte-identical by construction.
    _wcf_lift = 0.0
    if overall < WCF_LIFT_RAMP_TOP and weekly_detail is not None:
        ...
        if _wadj_value > WCF_LIFT_WADJ_CUTOFF:
            ...
            _wcf_span = max(1, WCF_LIFT_RAMP_TOP - (WCF_LIFT_GATE - 1))
            _wcf_sgrad = max(0.0, min(1.0, (WCF_LIFT_RAMP_TOP - overall) / _wcf_span))
            _wcf_weakness *= _wcf_sgrad
            if _wcf_weakness > 0:
    ...
    core_state = {
        'w_trend': w_trend, 'w_bb': w_bb, 'w_rsi': w_rsi, 'w_macd': w_macd,
        'w_stoch': w_stoch, 'w_ta': w_ta,
        'trend_dominance': trend_dominance,
        'pre_regime_overall': pre_regime_overall,
        'weekly_detail': weekly_detail,
        '_cap_dampened': _cap_dampened,
        '_exh_damp': _exh_damp,
        '_ext_damp': _ext_damp,
        '_wcf_lift': _wcf_lift,
        '_cwcf_dampen': _cwcf_dampen,
        '_cwwd_dampen': _cwwd_dampen,
        '_cswc_dampen': _cswc_dampen,
        '_pre_boost_overall': _pre_boost_overall,
    }
    return _pre_boost_overall, core_state


# ─────────────────────────────────────────────────────────────────────────────
# Layer 2 — the post-pre_boost bias/lift layer, registry-driven.
#
# Each mechanism is `_mech_<name>(overall, ctx, st) -> overall`, where:
#   ctx  : a dict of the raw scoring inputs the bias layer reads
#   st   : the mutable bias_state dict, into which the mechanism writes the
#          traces weight_info later emits.
# MECHANISM_REGISTRY fixes the apply order (byte-identical to the former
# top-to-bottom sequence): PCD -> MCD -> ICH -> WVD -> PESS -> EARN_BOOST ->
# SCW -> continuation-echo -> sector-breadth -> DVAW.  Each spec's `enabled`
# is a 0-arg callable so a config/env flip is read live.
# ─────────────────────────────────────────────────────────────────────────────
def _mech_pcd(overall, ctx, st):
    ...
    ret_10d_sigma = ctx['ret_10d_sigma']
    if (overall <= PCD_GATE
...
def _mech_mcd(overall, ctx, st):
    ...
    mcap_b = ctx['mcap_b']
    if (MCD_GATE_LO <= overall <= MCD_GATE_HI
        ...
        if log_mcap < MCD_LOG_HI:
            ...
            if weakness > 0.0:
                st['_mcd_dampen'] = MCD_ALPHA * weakness * (overall - MCD_TARGET)
                overall = int(max(0, min(100, round(overall - st['_mcd_dampen']))))
    return overall


def _mech_ich(overall, ctx, st):
    ...
    kijun_pct = ctx['kijun_pct']
    if kijun_pct is not None and kijun_pct < 0.0:
        ...
        if (ICH_GATE_CALL_HI > ICH_GATE_CALL_LO
            ...
            if _k_eff_c > 0.0 and _ig_c > 0.0:
                st['_ich_call_d'] = _k_eff_c * _ig_c * (overall - ICH_TARGET_CALL)
                if st['_ich_call_d'] > 0:
                    overall = int(max(0, min(100, round(overall - st['_ich_call_d']))))
        ...
        if (ICH_GATE_PUT_HI > ICH_GATE_PUT_LO
            ...
            _w_p = _sg_p * _ig_p
            if _w_p > 0.0:
                st['_ich_put_l'] = ICH_K_PUT * _w_p * (ICH_TARGET_PUT - overall)
                if st['_ich_put_l'] > 0:
                    overall = int(max(0, min(100, round(overall + st['_ich_put_l']))))
    return overall


def _mech_wvd(overall, ctx, st):
    ...
    wv_force1 = ctx['wv_force1']
    if (wv_force1 is not None
        ...
        st['_wvd_lift']   = (WVD_WAVE_K_LIFT   * _wvd_score_w * _wvd_bell
                        * (WVD_WAVE_TARGET_LIFT - overall))
        st['_wvd_dampen'] = (WVD_WAVE_K_DAMPEN * _wvd_score_w * _wvd_dampen_grad
                        * (overall - WVD_WAVE_TARGET_DAMPEN))
        overall = int(max(0, min(100,
            round(overall + st['_wvd_lift'] - st['_wvd_dampen']))))
    return overall


def _mech_pess(overall, ctx, st):
    ...
    if (PESS_GATE_LO <= overall <= PESS_GATE_HI
        ...
        if _pess_weakness > 0.0:
            st['_pess_lift'] = PESS_K * _pess_weakness * (PESS_TARGET - overall)
            overall = int(max(0, min(100, round(overall + st['_pess_lift']))))
    return overall


def _mech_earn_boost(overall, ctx, st):
    ...
    #
    # LiftTableMechanism: this is the one current mechanism that (when enabled)
    # loads a fitted table from disk — the look-ahead-leak class the MOS split
    # isolates. The holdout-cutoff contract is verified here. It is config-DEAD
    # in v74 (compute_earnings_boost_strength short-circuits on
    # EARN_BOOST_ENABLED before loading the table), so the verify is a no-op
    # forward-proof until the mechanism is re-enabled.
    if EARN_BOOST_ENABLED:
        _EARN_BOOST_LIFT_TABLE.assert_holdout_safe()
    st['_ern_boost_strength'] = compute_earnings_boost_strength(overall, ctx['days_to_earnings'])
    if st['_ern_boost_strength'] > 0:
        boosted = 50 + (overall - 50) * (1.0 + st['_ern_boost_strength'])
        overall = int(max(0, min(100, round(boosted))))
    return overall


def _mech_scw(overall, ctx, st):
    ...
    weekly_detail = ctx['weekly_detail']
    stoch = ctx['stoch']
    macdh_raw = ctx['macdh_raw']
    bb_pct = ctx['bb_pct']
    pct_from_ema50 = ctx['pct_from_ema50']
    pct_from_ema200 = ctx['pct_from_ema200']
    _extra = ctx['_extra']
    if (overall >= SCW_GATE_CALL
        ...
        st['_scw_base_dampen'] = SCW_MAX_PENALTY * _scw_stoch_wave * _scw_weekly_gap * _scw_conviction_decay
        st['_scw_conf_ratio'] = _call_explain_confirmation_ratio(
        ...
        st['_scw_raw_stoch'] = _finite_float(_extra.get('raw_stoch'), 50.0)
        ...
        _confirm_relief = _sigmoid_width(st['_scw_conf_ratio'], SCW_CONFIRM_MID, 0.10)
        _raw_stoch_relief = _sigmoid_width(st['_scw_raw_stoch'], SCW_RAW_STOCH_MID, 8.0)
        st['_scw_ext_index'] = _call_overextension_index(
            raw_rsi=_extra.get('raw_rsi'),
            raw_stoch=_extra.get('raw_stoch'),
            bb_pct=bb_pct,
            pct_from_ema50=pct_from_ema50,
        ...
        st['_scw_ext_taper'] = _sigmoid_width(st['_scw_ext_index'], SCW_EXT_TAPER_MID, SCW_EXT_TAPER_WIDTH)
        _scw_relief_scale = 1.0 - _clamp_float(SCW_EXT_TAPER_STRENGTH, 0.0, 1.0) * st['_scw_ext_taper']
        ...
        st['_scw_scalar'] = _clamp_float(st['_scw_scalar'], 0.0, 2.0)
        st['_scw_dampen'] = _clamp_float(st['_scw_base_dampen'] * st['_scw_scalar'], 0.0, 12.0)
        if st['_scw_dampen'] > 0.0:
            overall = int(max(0, min(100, round(overall - st['_scw_dampen']))))
    return overall


def _mech_cont(overall, ctx, st):
    ...
    prior_signal = ctx['prior_signal']
    _pre_boost_overall = ctx['_pre_boost_overall']
    if (prior_signal is not None
        ...
        st['_cont_raw_lift'] = min(
            CONT_BOOST_MAX_LIFT,
            CONT_BOOST_ALPHA * float(prior_signal) * (CONT_BOOST_TARGET - float(overall)),
        )
        if st['_cont_raw_lift'] > 0.0:
            _cont_candidate = int(max(0, min(100, round(overall + st['_cont_raw_lift']))))
            if _cont_candidate >= CONT_BOOST_PROMOTE_TARGET:
                st['_cont_lift'] = float(_cont_candidate - overall)
                overall = _cont_candidate
    return overall


def _mech_sector_breadth(overall, ctx, st):
        ...
        if _sector_breadth_side is not None:
            ...
            if _sector_breadth_after != _sector_breadth_before:
                ...
                if abs(_sector_overlay_delta) >= 0.01:
                    st['_sector_breadth_meta']["overlay_delta"] = round(float(_sector_overlay_delta), 3)
                    st['_sector_breadth_meta']["overlay_scale"] = round(float(_sector_overlay_scale), 4)
    return overall


def _mech_dvaw(overall, ctx, st):
    ...
    overall, st['_dvaw_meta'] = _apply_daily_volume_authority_wave(
        overall,
        vol_sig=ctx['vol_sig'],
        vol_mag=ctx['vol_mag'],
        pct_from_ema50=ctx['pct_from_ema50'],
        wv_force1=ctx['wv_force1'],
        price_impulse_sigma=ctx['_extra'].get('price_impulse_sigma'),
        log_dv_expansion=ctx['_extra'].get('log_dv_expansion'),
    )
    return overall


# Ordered registry — name, enabled-flag source (live), apply callable.
# Order MUST stay byte-identical to the former top-to-bottom sequence.
MECHANISM_REGISTRY = [
    ('PCD',            lambda: PCD_ENABLED,                            _mech_pcd),
    ('MCD',            lambda: MCD_ENABLED,                            _mech_mcd),
    ('ICH',            lambda: ICH_ENABLED,                            _mech_ich),
    ('WVD',            lambda: WVD_WAVE_ENABLED,                       _mech_wvd),
    ('PESS',           lambda: True,                                   _mech_pess),
    ('EARN_BOOST',     lambda: True,                                   _mech_earn_boost),
    ('SCW',            lambda: SCW_ENABLED,                            _mech_scw),
    ('CONT_ECHO',      lambda: CONT_BOOST_ENABLED,                     _mech_cont),
    ('SECTOR_BREADTH', lambda: SECTOR_BREADTH_WAVE_ENABLED,            _mech_sector_breadth),
    ('DVAW',           lambda: True,                                   _mech_dvaw),
]
# Notes on enabled-flag placement (to preserve bit-exact control flow):
#   * PESS, EARN_BOOST, DVAW carry their enable test INSIDE the mechanism body
#     (PESS/DVAW were never flag-gated at the call site; EARN_BOOST is gated in
#     compute_earnings_boost_strength). Their registry `enabled` is therefore
#     always-True so the body runs and applies the identical internal gate.
#   * PCD/MCD/ICH/WVD/SCW/CONT_ECHO/SECTOR_BREADTH had their *_ENABLED (or, for
#     SECTOR_BREADTH, the `score_date is not None`-paired) flag at the call site;
#     the registry `enabled` reproduces that outer guard so a disabled mechanism
#     is skipped entirely, exactly as before.
...
def apply_bias_corrections(
    ...
    *,
    ...
    vol_sig, vol_mag,
    macdh_raw, stoch,
    days_to_earnings, prior_signal, ret_10d_sigma, mcap_b,
    kijun_pct, wv_force1, score_date,
    _extra,
):
    """Run the post-pre_boost bias/lift layer via MECHANISM_REGISTRY in fixed
    order.  Returns (overall, bias_state) carrying every local weight_info reads
    from this layer."""
    bias_state = {
        '_pcd_active': False,
        '_mcd_dampen': 0.0,
    ...
    ctx = {
        ...
        'weekly_detail': core_state['weekly_detail'],
        '_pre_boost_overall': core_state['_pre_boost_overall'],
        '_extra': _extra,
    }
    for _name, _enabled, _apply in MECHANISM_REGISTRY:
        if _enabled():
            overall = _apply(overall, ctx, bias_state)
    return overall, bias_state
...
def compute_overall_score(
    ...
    *,
    ...
    pit_ws_trend=None, pit_ws_rsi=None, pit_ws_macd=None,
    weekly_transition_t=1.0,
    ...
    **_extra,
    ...
    # ── Layer 1: dynamical core -> pre_boost_overall + core_state ────────────
    _pre_boost_overall, _core = _compute_core_score(
        trend, bb, rsi, macd, stoch, technical_alignment,
        bb_pct=bb_pct, pct_from_ema50=pct_from_ema50, pct_from_ema200=pct_from_ema200,
        ...
        pit_ws_trend=pit_ws_trend, pit_ws_rsi=pit_ws_rsi, pit_ws_macd=pit_ws_macd,
        weekly_transition_t=weekly_transition_t,
    ...
    # ── Layer 2: post-pre_boost bias/lift layer via MECHANISM_REGISTRY ───────
    overall, _bias = apply_bias_corrections(
        _pre_boost_overall,
        core_state=_core,
        bb_pct=bb_pct, pct_from_ema50=pct_from_ema50, pct_from_ema200=pct_from_ema200,
        vol_sig=vol_sig, vol_mag=vol_mag,
        macdh_raw=macdh_raw, stoch=stoch,
        days_to_earnings=days_to_earnings, prior_signal=prior_signal,
        ret_10d_sigma=ret_10d_sigma, mcap_b=mcap_b,
        kijun_pct=kijun_pct, wv_force1=wv_force1, score_date=score_date,
        _extra=_extra,
    )

    # ── weight_info reassembly (see contract at top of MOS section) ──────────
    # Locals aliased from the two state dicts so the assembly below reads
    # byte-identically to the pre-refactor inline block.
    w_trend = _core['w_trend']; w_bb = _core['w_bb']; w_rsi = _core['w_rsi']
    w_macd = _core['w_macd']; w_stoch = _core['w_stoch']; w_ta = _core['w_ta']
    trend_dominance = _core['trend_dominance']
    pre_regime_overall = _core['pre_regime_overall']
    weekly_detail = _core['weekly_detail']
    _cap_dampened = _core['_cap_dampened']
    _exh_damp = _core['_exh_damp']; _ext_damp = _core['_ext_damp']
    _wcf_lift = _core['_wcf_lift']; _cwcf_dampen = _core['_cwcf_dampen']
    _cwwd_dampen = _core['_cwwd_dampen']; _cswc_dampen = _core['_cswc_dampen']
    _pcd_active = _bias['_pcd_active']; _mcd_dampen = _bias['_mcd_dampen']
    _ich_call_d = _bias['_ich_call_d']; _ich_put_l = _bias['_ich_put_l']
    _wvd_lift = _bias['_wvd_lift']; _wvd_dampen = _bias['_wvd_dampen']
    _pess_lift = _bias['_pess_lift']; _ern_boost_strength = _bias['_ern_boost_strength']
    _scw_dampen = _bias['_scw_dampen']; _scw_base_dampen = _bias['_scw_base_dampen']
    _scw_scalar = _bias['_scw_scalar']; _scw_conf_ratio = _bias['_scw_conf_ratio']
    _scw_raw_stoch = _bias['_scw_raw_stoch']; _scw_ext_index = _bias['_scw_ext_index']
    _scw_ext_taper = _bias['_scw_ext_taper']
    _cont_lift = _bias['_cont_lift']; _cont_raw_lift = _bias['_cont_raw_lift']
    _sector_breadth_meta = _bias['_sector_breadth_meta']
    _dvaw_meta = _bias['_dvaw_meta']
