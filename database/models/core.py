...
from database.utils.scoring import (
    ...
    compute_cont_prior_signal, CONT_BOOST_ENABLED,
    build_ret10d_sigma_map,
)
...
class EarningsDate(BaseModel):
    ...
    # Trading day on which the price reaction appears. BMO/intraday: same as
    # `date`. AMC: next trading day. Precomputed at insert/update so the
    # earnings-proximity hot path (EARN_BOOST + V6 volume suppression) can
    # query a single indexed column instead of running compute_effective_date
    # in Python per row. Nullable for migration safety; backfilled by
    # ensure_schema() and refreshed by build().
    ...
    def build(cls, symbol, date, **kwargs):
        ...
        for k, v in kwargs.items():
            setattr(record, k, v)
        # Recompute effective_date whenever the row is touched, so it stays in
        # sync with call_time edits. Trading calendar omitted (weekday-based
        # fallback in compute_effective_date is correct ~99% of the time;
        # backfill jobs use the calendar for higher precision).
        from iv_crush_model import compute_effective_date
        record.effective_date = compute_effective_date(record.date, record.call_time)
        # Ghost guard: if THIS record has no reported_eps and a sibling within
        # ±30 days does, skip the insert/save. Yahoo/yfinance periodically
        # creates a fresh upcoming-quarter row a few days after a real
        # reaction; without this guard, EARN_BOOST and V6 fire on the stale
        # row. Apply only on first creation so manual edits to existing rows
        # still flush. See _filter_ghost_earnings for the lookup-time mirror.
        if created and record.reported_eps is None:
            from datetime import timedelta as _td
            window_lo = record.date - _td(days=30)
            window_hi = record.date + _td(days=30)
            sibling_real = (
                cls.select()
                .where(
                    (cls.symbol == symbol)
                    & (cls.date.between(window_lo, window_hi))
                    & (cls.id != record.id)
                    & (cls.reported_eps.is_null(False))
                )
                .exists()
            )
            if sibling_real:
                ...
                return None, False
        record.save()
        # Symmetric ghost cleanup: when yfinance later fills actual EPS for the
        # real report, delete any nearby placeholder sibling that was inserted
        # while the event was still ambiguous.
        if record.reported_eps is not None:
            from datetime import timedelta as _td
            window_lo = record.date - _td(days=30)
            window_hi = record.date + _td(days=30)
            stale = list(
                cls.select(cls.id, cls.effective_date)
                .where(
                    (cls.symbol == symbol)
                    & (cls.date.between(window_lo, window_hi))
                    & (cls.id != record.id)
                    & (cls.reported_eps.is_null(True))
                )
            )
            if stale:
                stale_ids = [r.id for r in stale]
                stale_eff = [r.effective_date for r in stale if r.effective_date is not None]
                if stale_eff:
                    EarningsJump.delete().where(
                        (EarningsJump.symbol == symbol)
                        & (EarningsJump.earnings_date.in_(stale_eff))
                    ).execute()
                cls.delete().where(cls.id.in_(stale_ids)).execute()
        # Refresh the per-event realized-move row in earnings_jumps. Idempotent
        # (upsert) and silent on failure (PH missing, future event, etc.) — the
        # full `backfill_earnings_jumps.py` script catches anything missed.
        record._refresh_earnings_jump()
        return record, created

    def _refresh_earnings_jump(self):
        """Compute the realized post-earnings move for this event and upsert
        into the `earnings_jumps` cache. No-ops cleanly when the event is in
        the future or when PriceHistory rows aren't available yet."""
        from datetime import date as _date
        if self.effective_date is None or self.effective_date > _date.today():
            return
        try:
            from database.models.technical import PriceHistory
            rows = list(
                PriceHistory.select(PriceHistory.date, PriceHistory.close)
                .where(PriceHistory.symbol == self.symbol_id)
                .where(PriceHistory.date <= self.effective_date)
                .order_by(PriceHistory.date.desc())
                .limit(2)
            )
            if len(rows) < 2 or rows[0].date != self.effective_date:
                return
            prev_close, eff_close = float(rows[1].close), float(rows[0].close)
            if prev_close <= 0 or eff_close <= 0:
                return
            jump_pct = abs(eff_close / prev_close - 1.0) * 100.0
            EarningsJump.insert(
                symbol=self.symbol_id,
                earnings_date=self.effective_date,
                jump_pct=jump_pct,
    ...
    @classmethod
    def prune_unreported_missing_from_source(cls, symbol, source_dates, window_days=30):
        """Delete unreported placeholder rows missing from the latest source pull.

        yfinance can move a pending event from one date to another without ever
        filling reported EPS on the abandoned placeholder. Once a successful
        pull gives us the current source date set, rows absent from that set
        but close to a current source date are stale duplicates.
        """
        if not source_dates:
            return 0
        from datetime import timedelta as _td
        ...
        if not source_dates:
            return 0
        window_lo = min(source_dates) - _td(days=window_days)
        window_hi = max(source_dates) + _td(days=window_days)
        stale = []
        for row in (
            cls.select(cls.id, cls.date, cls.effective_date)
            .where(
                (cls.symbol == symbol)
                & (cls.date.between(window_lo, window_hi))
                & (cls.reported_eps.is_null(True))
            ...
            if row.date in source_dates:
                continue
            if any(abs((row.date - src).days) <= window_days for src in source_dates):
                stale.append(row)
        ...
        return cls.delete().where(cls.id.in_(stale_ids)).execute()

    @classmethod
    def ensure_schema(cls):
        from database.trader_database import DB
        # Add call_time column (legacy migration)
        try:
            DB.execute_sql('ALTER TABLE earnings_dates ADD COLUMN call_time VARCHAR(255)')
        except Exception:
            pass
        # Add effective_date column (2026-04-29: precomputed reaction-day
        # for fast EARN_BOOST + V6 lookup; see model docstring).
        try:
            DB.execute_sql('ALTER TABLE earnings_dates ADD COLUMN effective_date DATE NULL')
        except Exception:
            pass
        # Index for the (symbol, effective_date) range scans the proximity
        # helpers will issue. CREATE INDEX is idempotent in MySQL only via
        # try/except — peewee's auto-migrate doesn't reliably ALTER indexes.
        try:
            DB.execute_sql(
                'CREATE INDEX earningsdate_symbol_effective '
                'ON earnings_dates (symbol, effective_date)'
            )
        except Exception:
            pass
        # Backfill any rows still missing effective_date. Safe to re-run; only
        # touches NULLs. AMC shift uses MySQL day-of-week arithmetic
        # (Mon..Fri = trading days; misses NYSE holidays — same approximation
        # as compute_effective_date's weekday fallback).
        try:
            DB.execute_sql("""
                UPDATE earnings_dates
                SET effective_date = CASE
                    WHEN call_time IS NOT NULL
                         AND TIME(call_time) >= '16:00:00'
                    THEN CASE DAYOFWEEK(date)
                        WHEN 6 THEN DATE_ADD(date, INTERVAL 3 DAY)  -- Fri AMC -> Mon
                        WHEN 7 THEN DATE_ADD(date, INTERVAL 2 DAY)  -- Sat AMC -> Mon
            ...
            pass
        # Companion cache table — populated by build()'s _refresh_earnings_jump
        # hook and the standalone backfill_earnings_jumps.py script.
        try:
...
class EarningsJump(BaseModel):
    """Cached per-event earnings jump magnitudes for variance-additive 15-DTE
    pricing.

    One row per (symbol, past earnings event). At runtime the 15-DTE premium
    formula aggregates a stock's recent rows (median of last N events,
    require N>=3) into a per-stock `j_i`. Falls back to the universe constant
    `EARN_JUMP_PCT` (9.3%) when insufficient history exists.

    `source` documents which method computed the row:
      - 'realized_move': |close_{eff_date} / close_{eff_date−1trd} − 1| × 100
                          (Tier B — always available, backward-looking).
      - 'iv_inversion':  j² = (DTE_cal/365) × (IV²_annual − σ²_realized) × 100²
                          using ATM 7-15 DTE option_prices.iv from D-1 of the
                          event (Tier A — forward-looking, Feb 2025+ only).

    Refreshed nightly via `trader update` for newly-completed earnings events.
    """
    symbol = DeferredForeignKey('Stock', column_name='symbol', field='symbol', backref='earnings_jumps')
    earnings_date = DateField()  # AMC-shifted effective_date
    jump_pct = FloatField()
    source = CharField(max_length=20)
    iv_used = FloatField(null=True)
    sigma_used = FloatField(null=True)
    computed_at = DateTimeField(default=datetime.now)

    class Meta:
        table_name = 'earnings_jumps'
        primary_key = CompositeKey('symbol', 'earnings_date')
...
class Score(BaseModel):
    ...
    def calculate_overall_score(
        ...
        regime_composite / put_regime_multiplier / mis_stress let same-day
        update paths replay the full score formula after regime is computed.
        """
        ...
        ph = ind.price_history() if ind else None
        ...
        _pct_from_ema200 = None
        if ind and ph and ph.close:
            if ind.ema_50:
                _pct_from_ema50 = (float(ph.close) - float(ind.ema_50)) / float(ind.ema_50) * 100
            if ind.ema_200:
                _pct_from_ema200 = (float(ph.close) - float(ind.ema_200)) / float(ind.ema_200) * 100

        # Weekly context — last-completed WeeklyScore (state 1, point-in-time).
        ws = self.weekly_score                 # date-(weekday()+7)  = W-1
        pws = self.prev_weekly_score if ws else None   # date-(weekday()+14) = W-2
        ...
        # Weekly transition blend — reconstruct the current PARTIAL week (state 2)
        # point-in-time (Mon->self.date, no future bars) and the bars-elapsed t.
        _pit_ws_trend = _pit_ws_rsi = _pit_ws_macd = None
        _weekly_t = 1.0
        if None not in (ws_rsi, ws_macd):
            try:
                from database.models.technical import PriceHistory as _PH_pit
                from database.utils.weekly_pit import (
                    reconstruct_partial_week_weekly as _recon_pit,
                    week_start as _wk_start_pit,
                    transition_t as _trans_t_pit,
                )
                # ~7y of daily bars: enough to warm weekly EMA200/MACD without
                # pulling the full (decades-long) history on every update call.
                _pit_lo = self.date - timedelta(days=2600)
                _pit_daily = list(
                    _PH_pit.select(_PH_pit.date, _PH_pit.open, _PH_pit.high,
                                   _PH_pit.low, _PH_pit.close, _PH_pit.volume)
                    .where((_PH_pit.symbol == self.symbol)
                           & (_PH_pit.date <= self.date)
                           & (_PH_pit.date >= _pit_lo))
                    .order_by(_PH_pit.date.asc())
                )
                _pit_tuples = [
                    (p.date,
                     float(p.open) if p.open is not None else float(p.close),
                     float(p.high) if p.high is not None else float(p.close),
                     float(p.low) if p.low is not None else float(p.close),
                     float(p.close), int(p.volume) if p.volume is not None else 0)
                    for p in _pit_daily if p.close is not None
                ]
                _cur_wk_pit = _wk_start_pit(self.date)
                _bars_pit = sum(1 for x in _pit_tuples
                                if _cur_wk_pit <= x[0] <= self.date)
                _weekly_t = _trans_t_pit(_bars_pit)
                _pit = _recon_pit(self, _pit_tuples, self.date)
                if _pit is not None:
                    _pit_ws_trend, _pit_ws_rsi, _pit_ws_macd = _pit
            except Exception:
                _pit_ws_trend = _pit_ws_rsi = _pit_ws_macd = None
                _weekly_t = 1.0
        ...
        if regime_multiplier is None:
            ...
            if regime_row and regime_row.regime_multiplier is not None:
                ...
                regime_composite = float(regime_row.regime_composite) if regime_row.regime_composite is not None else None
                self.regime_composite = regime_composite
        ...
        if put_regime_multiplier is None and regime_composite is not None:
            try:
                _spy_wk_map = _load_spy_wk_composite_map()
                _spy_wk = _spy_wk_last_completed(
                    sorted(_spy_wk_map.keys()),
                    _spy_wk_map,
                    self.date,
                )
                put_regime_multiplier = _compute_put_regime_mult(regime_composite, _spy_wk)
            except Exception:
                put_regime_multiplier = None

        if mis_stress is None:
            mis_stress = 0.0

        # Earnings boost: days to next earnings within boost window
        d_to_ern = _days_to_next_earnings_for(self.symbol, self.date)

        # PCD: pull last ~80 bars to compute ret_10d_sigma at this date.
        # 80 covers the 60-bar sigma window + 10 bars for the return + small buffer.
        from database.models.technical import PriceHistory as _PH
        from database.utils.scoring import compute_ret_10d_sigma
        _ph_recent = list(
            _PH.select(_PH.date, _PH.close)
            .where((_PH.symbol == self.symbol) & (_PH.date <= self.date))
            .order_by(_PH.date.desc())
            .limit(80)
        )
        _ph_recent.reverse()  # now ascending
        _r10d_sigma = None
        if len(_ph_recent) >= 61:
            _closes = [float(p.close) if p.close is not None else None for p in _ph_recent]
            # target_idx = last index = signal date
            _r10d_sigma = compute_ret_10d_sigma(_closes, len(_closes) - 1)

        # MCD: pull mcap_b from the Stock row (one query per scoring call),
        # then deflate to a point-in-time proxy for historical dates (F4
        # survivorship fix 2026-06-09): mcap_t ~= mcap_latest * close_t/close_latest.
        _mcap_b = None
        try:
            _stock_row = Stock.get_or_none(Stock.symbol == self.symbol)
            if _stock_row and _stock_row.market_cap is not None:
                _mcap_b = float(_stock_row.market_cap) / 1e9
            if _mcap_b is not None:
                from database.utils.scoring import compute_pit_mcap_b as _pit_mcap
                _close_t = (float(_ph_recent[-1].close)
                            if _ph_recent and _ph_recent[-1].close is not None else None)
                _anchor_row = (
                    _PH.select(_PH.close)
                    .where((_PH.symbol == self.symbol) & (_PH.close.is_null(False)))
                    .order_by(_PH.date.desc()).limit(1).first()
                )
                _anchor_close = float(_anchor_row.close) if _anchor_row else None
                _mcap_b = _pit_mcap(_mcap_b, _close_t, _anchor_close)
        except Exception:
            _mcap_b = None

        # ICH: compute kijun_pct from last completed weekly bar (lookup at
        # scoring_date - 7 days to avoid partial-week bias). Need 26 weekly
        # bars + 1-week lag = ~190 calendar days of weekly data.
        _kijun_pct = None
        _wv_force1 = None
        try:
            from database.models.technical import WeeklyPriceHistory as _WPH
            from database.utils.scoring import (
                build_kijun_pct_map as _build_kij,
                build_wv_force1_map as _build_wvf,
            )
            _wph_rows = list(
                _WPH.select(_WPH.date, _WPH.high, _WPH.low, _WPH.close, _WPH.volume)
                .where((_WPH.symbol == self.symbol) & (_WPH.date <= self.date))
                .order_by(_WPH.date.desc())
                .limit(60)   # 60 weekly bars covers 26-bar lookback comfortably
            )
            _wph_rows.reverse()  # ascending
            if len(_wph_rows) >= 26:
                _kij_map = _build_kij(_wph_rows, _ph_recent)
                _kijun_pct = _kij_map.get(self.date)
            if len(_wph_rows) >= 5:
                _wvf_map = _build_wvf(_wph_rows, _ph_recent)
                _wv_force1 = _wvf_map.get(self.date)
        except Exception:
            _kijun_pct = None
            _wv_force1 = None
        ...
        score_kwargs = {
            ...
            'pct_from_ema200': _pct_from_ema200,
            'ws_trend': ws_trend,
            'ws_rsi': ws_rsi,
            'ws_macd': ws_macd,
            ...
            'pit_ws_trend': _pit_ws_trend,
            'pit_ws_rsi': _pit_ws_rsi,
            'pit_ws_macd': _pit_ws_macd,
            'weekly_transition_t': _weekly_t,
            'vol_mult': vol_mult,
            'vol_raw': vol_raw,
            'vol_sig': vol_sig,
            'vol_mag': vol_mag,
            ...
            'raw_rsi': float(ind.rsi) if ind and ind.rsi is not None else None,
            'raw_stoch': float(ind.stoch) if ind and ind.stoch is not None else None,
            'macdh_raw': float(ind.macd_hist) if ind and ind.macd_hist is not None else None,
            'regime_multiplier': regime_multiplier,
            'put_regime_multiplier': put_regime_multiplier,
            'mis_stress': mis_stress or 0.0,
            'days_to_earnings': d_to_ern,
            'ret_10d_sigma': _r10d_sigma,
            'mcap_b': _mcap_b,
            'kijun_pct': _kijun_pct,
            'wv_force1': _wv_force1,
            'score_date': self.date,
        }
        self._last_overall_score_args = score_args
        self._last_overall_score_kwargs = dict(score_kwargs)
        overall, weight_info, vol_update = compute_overall_score(*score_args, **score_kwargs)
    ...
    def ensure_schema(cls):
        ...
        # Indexes added via raw CREATE INDEX (not in Meta.indexes because
        # those are managed by peewee on table creation; we want idempotent
        # additions for already-existing tables in production).
        # scores_version_date_IDX (2026-05-08): drops MAX(date) WHERE
        # version_id=N from 740ms → 1ms. Was the dominant slow-query during
        # API dashboard polls + recalculate concurrency. See trader_database.py
        # timeout history for context.
        for idx_sql in (
            ...
            try:
                ...
                # Ignore "Duplicate key name" (1061) — idempotent
                msg = str(e)
                if '1061' not in msg and 'Duplicate' not in msg:
                    pass  # silently swallow other errors too — schema migration is best-effort
...
class ScoreIntradayLog(BaseModel):
    """Append-only score-factor snapshot captured during `trader update`."""

    id = AutoField()
    symbol = DeferredForeignKey('Stock', column_name='symbol', field='symbol', backref='score_intraday_logs')
    date = DateField()
    version = DeferredForeignKey('AlgorithmVersion', backref='score_intraday_logs', on_delete='CASCADE')
    logged_at = DateTimeField(default=datetime.now)
    source = CharField(max_length=48, default='trader_update')
    run_key = CharField(max_length=80, null=True)
    score_updated_at = DateTimeField(null=True)

    overall = IntegerField(null=True)
    pre_regime = IntegerField(null=True)
    pre_boost = IntegerField(null=True)
    trend = IntegerField(null=True)
    bb = IntegerField(null=True)
    rsi = IntegerField(null=True)
    macd = IntegerField(null=True)
    stoch = IntegerField(null=True)
    technical_alignment = IntegerField(null=True)
    volume = IntegerField(null=True)

    price = FloatField(null=True)
    daily_change = FloatField(null=True)
    pct_from_ema50 = FloatField(null=True)
    pct_from_ema200 = FloatField(null=True)
    bb_position = FloatField(null=True)
    score_velocity_7d = IntegerField(null=True)
    volume_signal = CharField(max_length=32, null=True)
    volume_magnitude = FloatField(null=True)
    regime_composite = FloatField(null=True)
    regime_multiplier = FloatField(null=True)

    weight_info_json = TextField(null=True)
    factor_json = TextField(null=True)
    inputs_json = TextField(null=True)

    _schema_checked = False

    class Meta:
        table_name = 'score_intraday_logs'
        indexes = (
            (('date', 'version', 'logged_at'), False),
            (('symbol', 'date', 'version', 'logged_at'), False),
            (('run_key', 'logged_at'), False),
        )
    ...
    def ensure_schema(cls):
        ...
        DB.create_tables([cls], safe=True)
        for col in ('weight_info_json', 'factor_json', 'inputs_json'):
            try:
                DB.execute_sql(f'ALTER TABLE score_intraday_logs MODIFY {col} LONGTEXT NULL')
            except Exception:
                pass
        for idx_sql in (
            'CREATE INDEX score_intraday_logs_date_version_logged '
            'ON score_intraday_logs (date, version_id, logged_at)',
            'CREATE INDEX score_intraday_logs_symbol_date_version_logged '
            'ON score_intraday_logs (symbol, date, version_id, logged_at)',
            'CREATE INDEX score_intraday_logs_run_logged '
            'ON score_intraday_logs (run_key, logged_at)',
        ):
            try:
                DB.execute_sql(idx_sql)
            except Exception:
    ...
    @classmethod
    def _json_dumps(cls, payload):
        return json.dumps(cls._json_ready(payload), sort_keys=True)

    @classmethod
    def _parse_weight_info(cls, score):
        raw = getattr(score, 'weight_info', None)
        if not raw:
            return {}
    ...
    @classmethod
    def _input_snapshot(cls, score):
        symbol = getattr(score, 'symbol_id', None) or getattr(score, 'symbol', None)
        score_date = getattr(score, 'date', None)
        snapshot = {}
        try:
            from database.models.technical import Indicator, PriceHistory, WeeklyPriceHistory

            ind = Indicator.get_or_none((Indicator.symbol == symbol) & (Indicator.date == score_date))
            if ind:
                snapshot['indicator'] = {
                    'rsi': ind.rsi,
                    'rsi_ma': ind.rsi_ma,
                    ...
                    'stoch_rsi_signal': ind.stoch_rsi_signal,
                    'stoch': ind.stoch,
                    'stoch_signal': ind.stoch_signal,
                    'macd': ind.macd,
            ...
            ph = PriceHistory.get_or_none((PriceHistory.symbol == symbol) & (PriceHistory.date == score_date))
            if ph:
            ...
            week_start = score_date - timedelta(days=score_date.weekday()) if score_date else None
            if week_start:
                ws = WeeklyScore.get_or_none((WeeklyScore.symbol == symbol) & (WeeklyScore.date == week_start))
                pws = WeeklyScore.get_or_none(
                    (WeeklyScore.symbol == symbol) & (WeeklyScore.date == week_start - timedelta(weeks=1))
                )
                if ws:
                    snapshot['weekly_score'] = {
                        'date': ws.date,
                        'trend': ws.trend,
                        'rsi': ws.rsi,
                        'macd': ws.macd,
                ...
                if pws:
                    snapshot['previous_weekly_score'] = {
                        'date': pws.date,
                        'trend': pws.trend,
                        'rsi': pws.rsi,
                        'macd': pws.macd,
                        'composite': pws.composite,
                    }
                wph = WeeklyPriceHistory.get_or_none(
                    (WeeklyPriceHistory.symbol == symbol) & (WeeklyPriceHistory.date == week_start)
                )
                if wph:
                    snapshot['weekly_price_history'] = {
                        'date': wph.date,
                        'open': wph.open,
                        'high': wph.high,
                        'low': wph.low,
    ...
    @classmethod
    def record_from_score(cls, score, *, source='trader_update', run_key=None):
        if score is None:
            return False
        try:
            cls.ensure_schema_once()
            weight_info = cls._parse_weight_info(score)
            factor_snapshot = {
                'components': {
                    'trend': score.trend,
                    'bb': score.bb,
                ...
                'score': {
                    ...
                    'pre_regime': weight_info.get('pre_regime'),
                    'pre_boost': weight_info.get('pre_boost'),
                    'score_velocity_7d': score.score_velocity_7d,
                    'updated_at': score.updated_at,
                },
                'market': {
                    'price': score.price,
                    'daily_change': score.daily_change,
                    'pct_from_ema50': score.pct_from_ema50,
                    'pct_from_ema200': score.pct_from_ema200,
                    'bb_position': score.bb_position,
                },
                'volume': {
                    'signal': score.volume_signal,
                    'magnitude': score.volume_magnitude,
                },
                'regime': {
                    'composite': score.regime_composite,
                    'multiplier': score.regime_multiplier,
                    'put_regime_multiplier': weight_info.get('put_regime_mult'),
                    'mis_stress': weight_info.get('mis_stress'),
                },
                'weights': weight_info,
            }
            cls.create(
                symbol=getattr(score, 'symbol_id', None),
                date=score.date,
                version=getattr(score, 'version_id', None),
                source=source,
                run_key=run_key,
                score_updated_at=score.updated_at,
                overall=score.overall,
                pre_regime=cls._as_int(weight_info.get('pre_regime')),
                pre_boost=cls._as_int(weight_info.get('pre_boost')),
                trend=score.trend,
                bb=score.bb,
                rsi=score.rsi,
                macd=score.macd,
                ...
                technical_alignment=score.technical_alignment,
                volume=score.volume,
                price=cls._as_float(score.price),
                daily_change=cls._as_float(score.daily_change),
                pct_from_ema50=cls._as_float(score.pct_from_ema50),
                pct_from_ema200=cls._as_float(score.pct_from_ema200),
                bb_position=cls._as_float(score.bb_position),
                score_velocity_7d=score.score_velocity_7d,
                volume_signal=score.volume_signal,
                volume_magnitude=cls._as_float(score.volume_magnitude),
                regime_composite=cls._as_float(score.regime_composite),
                regime_multiplier=cls._as_float(score.regime_multiplier),
                weight_info_json=cls._json_dumps(weight_info),
                factor_json=cls._json_dumps(factor_snapshot),
                inputs_json=cls._json_dumps(cls._input_snapshot(score)),
            )
            return True
        except Exception:
            return False
    ...
    @classmethod
    def record_latest(cls, symbol, *, target_date=None, source='trader_update', run_key=None):
        if target_date is None:
            target_date = date.today()
        try:
            version = AlgorithmVersion.get_or_create_current()
            score = Score.get_or_none(
                (Score.symbol == symbol)
                & (Score.date == target_date)
                & (Score.version == version)
            )
            return cls.record_from_score(score, source=source, run_key=run_key)
        except Exception:
            return False
...
class AlgorithmVersion(BaseModel):
    ...
    display_description = CharField(max_length=160, null=True)
    cadence_enabled = BooleanField(default=False, index=True)
    selector_rank = IntegerField(null=True)
    created_at = DateTimeField(default=datetime.now)
    _current = None

    # Honest era only (2026-06-12 pre-v69 purge): every pre-v69 version carried
    # the weekly look-ahead (or was overwritten by hybrid honest re-recalcs),
    # and their score rows were deleted from the scores table. Cadence policy is
    # v69+ — CADENCE_MIN_VERSION_ID auto-enables every newer shipped version in
    # seed_selector_defaults so future ships stay comparable without a dict edit.
    CADENCE_MIN_VERSION_ID = 69

    DEFAULT_SELECTOR_VERSIONS = {
        72: (1, 'WCF score-gate ramp'),
        71: (2, 'Integrity-audit honest fixes'),
        70: (3, 'Honest v70 baseline (EARN_BOOST recal)'),
        69: (4, 'First honest weekly transition blend'),
    }
    ...
    def ensure_schema(cls):
        ...
        for column, definition in (
            ('display_description', 'VARCHAR(160) NULL'),
            ('cadence_enabled', 'TINYINT(1) NOT NULL DEFAULT 0'),
            ('selector_rank', 'INT NULL'),
        ):
            try:
                cur = DB.execute_sql(f"SHOW COLUMNS FROM algorithm_versions LIKE '{column}'")
                if cur.fetchone() is None:
                    DB.execute_sql(f'ALTER TABLE algorithm_versions ADD COLUMN {column} {definition}')
            except Exception:
                pass
        try:
            DB.execute_sql('CREATE INDEX algorithm_versions_cadence_IDX ON algorithm_versions (cadence_enabled, selector_rank)')
        except Exception:
            pass

    @classmethod
    def seed_selector_defaults(cls, *, force: bool = False) -> int:
        cls.ensure_schema()
        updated = 0
        for version_id, (rank, description) in cls.DEFAULT_SELECTOR_VERSIONS.items():
            version = cls.get_or_none(cls.id == version_id)
            if version is None or cls.is_legacy_staging_commit(version.git_commit):
                continue
            changed_fields = []
            if force or not version.cadence_enabled:
                version.cadence_enabled = True
                changed_fields.append(cls.cadence_enabled)
            if force or version.selector_rank is None:
                version.selector_rank = rank
                changed_fields.append(cls.selector_rank)
            if force or not version.display_description:
                version.display_description = description
                changed_fields.append(cls.display_description)
            if changed_fields:
                version.save(only=changed_fields)
                updated += 1
        # Honest-era floor: any version at/after CADENCE_MIN_VERSION_ID joins
        # the cadence automatically (new ships stay comparable without a dict
        # edit). Enable-only, like the dict seeding above.
        floor_rows = cls.select().where(
            (cls.id >= cls.CADENCE_MIN_VERSION_ID)
            & (cls.cadence_enabled == False)  # noqa: E712
        )
        for version in floor_rows:
            if cls.is_legacy_staging_commit(version.git_commit):
                continue
            version.cadence_enabled = True
            version.save(only=[cls.cadence_enabled])
            updated += 1
        return updated

    @classmethod
    def cadence_versions(cls):
        cls.ensure_schema()
        cls.seed_selector_defaults()
        return list(
            cls.select()
            .where(
                (cls.cadence_enabled == True)  # noqa: E712
                & (~(cls.git_commit.startswith('shadow/')))
            )
            .order_by(fn.COALESCE(cls.selector_rank, 9999), cls.id.desc())
        )

    @classmethod
    def cadence_score_version_tokens(cls, *, include_current: bool = False) -> list[str]:
        cls.ensure_schema()
        current = cls.get_or_create_current()
        tokens = []
        for version in cls.cadence_versions():
            if not include_current and int(version.id) == int(current.id):
                continue
            tokens.append(version.production_label)
        return tokens

    @staticmethod
    def is_legacy_staging_commit(commit: str | None) -> bool:
        return bool(commit and commit.startswith('shadow/'))

    @classmethod
    def production_versions(cls):
        return list(
            cls.select()
            .where(~(cls.git_commit.startswith('shadow/')))
            .order_by(cls.id)
        )

    @classmethod
    def production_label_for_id(cls, version_id: int) -> int | None:
        version = cls.get_or_none(cls.id == version_id)
        if version is None or cls.is_legacy_staging_commit(version.git_commit):
            return None
        return version_id
    ...
    @classmethod
    def get_by_production_label(cls, label_id: int):
        version = cls.get_or_none(cls.id == label_id)
        if version is None or cls.is_legacy_staging_commit(version.git_commit):
            return None
        return version
    ...
    def production_label(self) -> str:
        label_id = self.production_label_for_id(self.id)
        return f"v{label_id}" if label_id is not None else f"db:{self.id}"
...
class StagingVersion(BaseModel):
    """Inactive score variant metadata that must not consume production vN ids."""

    variant = CharField(max_length=64)
    base_version = ForeignKeyField(AlgorithmVersion, backref='staging_versions', on_delete='CASCADE')
    base_commit = CharField(max_length=40)
    label = CharField(max_length=120, null=True)
    status = CharField(max_length=24, default='active')  # active | promoted | archived
    promoted_version = ForeignKeyField(AlgorithmVersion, backref='promoted_staging_versions',
                                       null=True, on_delete='SET NULL')
    legacy_algorithm_version = ForeignKeyField(AlgorithmVersion, backref='migrated_staging_versions',
                                               null=True, on_delete='SET NULL')
    legacy_algorithm_version_id_raw = IntegerField(null=True)
    notes = TextField(null=True)
    created_at = DateTimeField(default=datetime.now)
    updated_at = DateTimeField(default=datetime.now)
    ...
    def ensure_schema(cls):
        from database.trader_database import DB
        DB.create_tables([cls, StagingScore], safe=True)
        try:
            DB.execute_sql('ALTER TABLE staging_versions ADD COLUMN legacy_algorithm_version_id_raw INT NULL')
        except Exception:
            pass

    @classmethod
    def get_active(cls, variant: str | None = None, base_version=None):
        query = cls.select().where(
            (cls.status == 'active')
            & cls.promoted_version.is_null(True)
        )
        if variant:
            query = query.where(cls.variant == variant)
        if base_version is not None:
            query = query.where(cls.base_version == base_version)
        return query.order_by(cls.updated_at.desc(), cls.id.desc()).first()

    @classmethod
    def get_or_create_active(cls, variant: str, base_version, label: str | None = None,
                             notes: str | None = None):
        row = cls.get_active(variant=variant, base_version=base_version)
        if row is None:
            row = cls.create(
                variant=variant,
                base_version=base_version,
                base_commit=base_version.git_commit,
                label=label,
                notes=notes,
            )
        else:
            row.base_commit = base_version.git_commit
    ...
    @classmethod
    def migrate_legacy_algorithm_rows(cls, delete_legacy: bool = False) -> dict:
        """Move legacy inactive score rows out of algorithm_versions.

        The old sector ETF sidecar stored rows in ``scores`` under
        ``algorithm_versions.git_commit = 'shadow/...'``. This copies those
        rows into staging tables. When ``delete_legacy`` is true it removes the
        old score rows and metadata rows after the copy succeeds.
        """
        from database.trader_database import DB
        ...
        for legacy in legacy_rows:
            ...
            base_version = None
            if legacy.notes:
                m = re.search(r'base_version_id=(\d+)', legacy.notes)
                if m:
                    base_version = AlgorithmVersion.get_or_none(AlgorithmVersion.id == int(m.group(1)))
            if base_version is None:
                m = re.search(r'/v(\d+)$', legacy.git_commit)
                if m:
                    base_version = AlgorithmVersion.get_or_none(AlgorithmVersion.id == int(m.group(1)))
            if base_version is None:
                continue

            staging = cls.get_or_create_active(
                variant=variant,
                base_version=base_version,
                label='Sector ETF staging',
                notes=f'Legacy inactive score version migrated from algorithm_versions.v{legacy.id}',
            )
            if staging.legacy_algorithm_version_id is None:
                staging.legacy_algorithm_version = legacy
            staging.legacy_algorithm_version_id_raw = legacy.id
            staging.save()

            cols = StagingScore.score_column_names()
            src_cols = ', '.join(f'`{c}`' for c in cols)
            dst_cols = ', '.join(f'`{c}`' for c in ['symbol', 'date', 'staging_version_id', *cols])
            updates = ', '.join(f'`{c}`=VALUES(`{c}`)' for c in cols)
            DB.execute_sql(
                f"INSERT INTO `staging_scores` ({dst_cols}) "
                f"SELECT `symbol`, `date`, %s, {src_cols} "
                f"FROM `scores` WHERE `version_id`=%s "
                f"ON DUPLICATE KEY UPDATE {updates}",
                (staging.id, legacy.id),
            )
            score_count = DB.execute_sql(
                "SELECT COUNT(*) FROM `staging_scores` WHERE `staging_version_id`=%s",
                (staging.id,),
            ).fetchone()[0]
            copied.append({'legacy_version_id': legacy.id, 'staging_version_id': staging.id,
                           'base_version_id': base_version.id, 'score_rows': int(score_count)})

            if delete_legacy:
                DB.execute_sql("DELETE FROM `scores` WHERE `version_id`=%s", (legacy.id,))
                legacy.delete_instance()

        return {'migrated': copied, 'deleted_legacy': bool(delete_legacy)}

    class Meta:
        table_name = 'staging_versions'
        indexes = (
            (('variant', 'base_version', 'status'), False),
            (('status', 'updated_at'), False),
        )


class StagingScore(BaseModel):
    """Score rows for inactive staging variants."""

    symbol = DeferredForeignKey('Stock', column_name='symbol', field='symbol', backref='staging_scores')
    date = DateField()
    staging_version = ForeignKeyField(StagingVersion, backref='score_rows', on_delete='CASCADE')
    bb = IntegerField()
    trend = IntegerField()
    volume = IntegerField()
    rsi = IntegerField()
    ...
    @classmethod
    def score_column_names(cls) -> list[str]:
        return [
            'bb', 'trend', 'volume', 'rsi', 'macd', 'stoch',
            'ma20', 'high_30', 'high_60', 'high_90',
            'high_30_days', 'high_60_days', 'high_90_days',
            'high_30_biggest_drop', 'high_60_biggest_drop', 'high_90_biggest_drop',
            'technical_alignment', 'overall', 'updated_at',
            'daily_change', 'price', 'price_target_growth', 'next_earnings',
            'price_target', 'growth_score', 'volume_signal', 'volume_magnitude',
            'pct_from_ema50', 'pct_from_ema200', 'bb_position',
            'score_velocity_7d', 'regime_composite', 'regime_multiplier',
            'weight_info',
        ]

    @classmethod
    def promote_to_algorithm_version(cls, staging_version, algorithm_version):
        """Copy a staging score set into production scores for a promoted commit."""
        from database.trader_database import DB

        cols = cls.score_column_names()
        src_cols = ', '.join(f'`{c}`' for c in cols)
        dst_cols = ', '.join(f'`{c}`' for c in ['symbol', 'date', 'version_id', *cols])
        updates = ', '.join(f'`{c}`=VALUES(`{c}`)' for c in cols)
        DB.execute_sql(
            f"INSERT INTO `scores` ({dst_cols}) "
            f"SELECT `symbol`, `date`, %s, {src_cols} "
            f"FROM `staging_scores` WHERE `staging_version_id`=%s "
            f"ON DUPLICATE KEY UPDATE {updates}",
            (algorithm_version.id, staging_version.id),
        )
        staging_version.status = 'promoted'
        staging_version.promoted_version = algorithm_version
        staging_version.updated_at = datetime.now()
        staging_version.save()

    class Meta:
        table_name = 'staging_scores'
        primary_key = CompositeKey('symbol', 'date', 'staging_version')
        indexes = (
            (('date',), False),
            (('date', 'overall'), False),
            (('overall',), False),
            (('symbol', 'date', 'overall'), False),
            (('staging_version', 'date'), False),
        )
...
class MarketBreadth(BaseModel):
    ...
    # Sector ETF breadth overlay, persisted by the daily breadth/regime cadence.
    sector_etf_asof_date = DateField(null=True)
    sector_etf_pct_above_ema50 = DecimalField(max_digits=5, decimal_places=2, null=True)
    sector_etf_pct_above_ema200 = DecimalField(max_digits=5, decimal_places=2, null=True)
    sector_etf_avg_rsi = DecimalField(max_digits=5, decimal_places=2, null=True)
    sector_etf_breadth_1d_change = DecimalField(max_digits=6, decimal_places=2, null=True)
    sector_etf_breadth_5d_change = DecimalField(max_digits=6, decimal_places=2, null=True)
    sector_etf_breadth_10d_change = DecimalField(max_digits=6, decimal_places=2, null=True)
    sector_etf_breadth_15d_change = DecimalField(max_digits=6, decimal_places=2, null=True)
    sector_etf_breadth_avg5 = DecimalField(max_digits=5, decimal_places=2, null=True)
    sector_etf_breadth_10d_position = DecimalField(max_digits=6, decimal_places=2, null=True)
    sector_etf_breadth_30d_position = DecimalField(max_digits=6, decimal_places=2, null=True)
    sector_etf_market_wave_score = DecimalField(max_digits=5, decimal_places=2, null=True)
    sector_etf_market_wave_signed = DecimalField(max_digits=6, decimal_places=2, null=True)
    sector_etf_market_wave_state = CharField(max_length=24, null=True)
    sector_etf_crash_echo = DecimalField(max_digits=6, decimal_places=4, null=True)
    sector_etf_bull_wave = DecimalField(max_digits=6, decimal_places=4, null=True)
    sector_etf_effective_crash_echo = DecimalField(max_digits=6, decimal_places=4, null=True)
    sector_etf_source = CharField(max_length=32, null=True)
    sector_etf_issues = IntegerField(null=True)
    ...
    def ensure_schema(cls):
        ...
        for col, definition in [
            ...
            ('sector_etf_asof_date', 'DATE NULL'),
            ('sector_etf_pct_above_ema50', 'DECIMAL(5,2) NULL'),
            ('sector_etf_pct_above_ema200', 'DECIMAL(5,2) NULL'),
            ('sector_etf_avg_rsi', 'DECIMAL(5,2) NULL'),
            ('sector_etf_breadth_1d_change', 'DECIMAL(6,2) NULL'),
            ('sector_etf_breadth_5d_change', 'DECIMAL(6,2) NULL'),
            ('sector_etf_breadth_10d_change', 'DECIMAL(6,2) NULL'),
            ('sector_etf_breadth_15d_change', 'DECIMAL(6,2) NULL'),
            ('sector_etf_breadth_avg5', 'DECIMAL(5,2) NULL'),
            ('sector_etf_breadth_10d_position', 'DECIMAL(6,2) NULL'),
            ('sector_etf_breadth_30d_position', 'DECIMAL(6,2) NULL'),
            ('sector_etf_market_wave_score', 'DECIMAL(5,2) NULL'),
            ('sector_etf_market_wave_signed', 'DECIMAL(6,2) NULL'),
            ('sector_etf_market_wave_state', 'VARCHAR(24) NULL'),
            ('sector_etf_crash_echo', 'DECIMAL(6,4) NULL'),
            ('sector_etf_bull_wave', 'DECIMAL(6,4) NULL'),
            ('sector_etf_effective_crash_echo', 'DECIMAL(6,4) NULL'),
            ('sector_etf_source', 'VARCHAR(32) NULL'),
            ('sector_etf_issues', 'INT NULL'),
        ]:
            try:
...
class ScoreAssessmentRun(BaseModel):
    ...
    def ensure_schema(cls):
        ...
        try:
            ...
            pass
        for col in ['correlation_3d', 'correlation_5d']:
            try:
                DB.execute_sql(f'ALTER TABLE score_assessment_runs ADD COLUMN {col} FLOAT NULL')
            except Exception:
                pass
...
class ScoreAssessmentResult(BaseModel):
    ...
    def ensure_schema(cls):
        ...
        for table in ['score_assessment_runs', 'score_assessment_meta']:
            for col in ['correlation_3d', 'correlation_5d', 'correlation_150d']:
                try:
                    DB.execute_sql(f'ALTER TABLE {table} ADD COLUMN {col} FLOAT NULL')
                except Exception:
                    pass
        DB.create_tables([ScoreAssessmentBandIC, ScoreAssessmentOptionResult, ScoreAssessmentTierResult], safe=True)
        for col in ['ic_3d', 'ic_5d']:
            try:
                DB.execute_sql(f'ALTER TABLE score_assessment_band_ic ADD COLUMN {col} FLOAT NULL')
            except Exception:
                pass
...
_ASSESSMENT_SHORT_PERIODS = ('3d', '5d')
_ASSESSMENT_PERIOD_FLOAT_PREFIXES = (
    'avg_return', 'win_rate', 'median_return', 'avg_peak', 'median_peak',
    'avg_mae', 'median_mae', 'avg_mae_winner', 'avg_mae_loser',
    'avg_mae_winner_sigma', 'avg_mae_loser_sigma',
    'avg_mfe', 'median_mfe', 'mfe_p25', 'mfe_p75', 'mfe_p90',
    'avg_mfe_sigma', 'median_mfe_sigma', 'mfe_sigma_p25', 'mfe_sigma_p75',
    'capture_ratio', 'swing_p_stop', 'swing_p_expire',
    'swing_avg_win_pnl', 'swing_avg_stop_pnl', 'win_rate_unscaled',
)


def _assessment_period_float_cols(periods=_ASSESSMENT_SHORT_PERIODS):
    return [
        f'{prefix}_{period}'
        for period in periods
        for prefix in _ASSESSMENT_PERIOD_FLOAT_PREFIXES
    ]


def _add_assessment_float_fields(model, names):
    """Attach additive assessment metric fields without rewriting the legacy field block."""
    for name in names:
...
_add_assessment_float_fields(ScoreAssessmentRun, ['correlation_3d', 'correlation_5d'])
_add_assessment_float_fields(ScoreAssessmentResult, _assessment_period_float_cols())
_add_assessment_float_fields(
    ScoreAssessmentMeta,
    _assessment_period_float_cols() + ['correlation_3d', 'correlation_5d'],
)
_add_assessment_float_fields(ScoreAssessmentBandIC, ['ic_3d', 'ic_5d'])
...
class BacktestTemporalStats(BaseModel):
    ...
    dte_strategy, portfolio profile). Rebuilt by `trader assess --force` after
    the 1y window completes."""
    ...
    # Each (version, dte_strategy, portfolio_profile) is a unique row.
    dte_strategy     = CharField(max_length=8, default='30')
    portfolio_profile = CharField(max_length=32, default='sentinel')

    class Meta:
        table_name = 'backtest_temporal_stats'
        indexes = ((('version', 'dte_strategy', 'portfolio_profile'), True),)
    ...
    def ensure_schema(cls):
        ...
        try:
            ...
            pass
        try:
            DB.execute_sql("ALTER TABLE backtest_temporal_stats ADD COLUMN portfolio_profile VARCHAR(32) NOT NULL DEFAULT 'sentinel'")
        except Exception:
            pass
        try:
            DB.execute_sql("UPDATE backtest_temporal_stats SET portfolio_profile='sentinel' WHERE portfolio_profile IS NULL OR portfolio_profile=''")
        except Exception:
            pass
        # Drop legacy unique indexes, then add profile-aware uniqueness.
        for idx_name in ('version_id', 'backtest_temporal_stats_version_id',
                         'backtesttemporalstats_version_id',
                         'uq_bts_version_dte',
                         'backtest_temporal_stats_version_id_dte_strategy',
                         'backtesttemporalstats_version_id_dte_strategy'):
            try:
        ...
        try:
            DB.execute_sql(
                'ALTER TABLE backtest_temporal_stats ADD UNIQUE INDEX uq_bts_version_dte_profile '
                '(version_id, dte_strategy, portfolio_profile)'
            )
        except Exception:
            pass
...
class BacktestRun(BaseModel):
    """One persisted execution of /api/backtest/run.

    Backtest is fully deterministic per (algorithm_version, dte_strategy, params)
    — bounded-midpoint TP/SL fills mean same inputs always produce the same
    equity curve. params_hash is therefore a unique cache key: auto-save on
    each /api/backtest/run is an upsert (only run_at refreshes on a re-run).
    Browse-friendly summary cols + JSON blobs for full detail on click.
    """
    version           = ForeignKeyField(AlgorithmVersion, backref='backtest_runs',
                                        on_delete='CASCADE')
    dte_strategy      = CharField(max_length=8, default='30')
    params_hash       = CharField(max_length=32)         # md5 hex of canonical params
    params_json       = TextField()                       # full params dict as JSON
    run_at            = DateTimeField(default=datetime.now)

    # Date range for browse / filter
    start_date        = DateField(null=True)
    end_date          = DateField(null=True)

    # Summary metrics (also denormalized into params/blobs but indexed for sort)
    initial_capital   = FloatField()
    final_equity      = FloatField()
    total_return_pct  = FloatField()                      # (final/initial - 1) * 100
    max_dd_pct        = FloatField()                      # max drawdown * 100
    n_trades          = IntegerField(default=0)
    n_tp              = IntegerField(default=0)
    n_sl              = IntegerField(default=0)
    n_hard            = IntegerField(default=0)
    n_open            = IntegerField(default=0)

    # Optional user metadata
    label             = CharField(max_length=120, null=True)

    # Full response payload (the entire dict returned by /api/backtest/run).
    # Single blob keeps schema simple and avoids drift when new view fields
    # are added (tier_stats, yearly, monthly, etc.). Browse-list speed comes
    # from the denormalized summary columns above.
    response_json     = TextField(null=True)

    class Meta:
        table_name = 'backtest_run'
        indexes = (
            # Cache key: re-running with identical params upserts the same row.
            (('version', 'dte_strategy', 'params_hash'), True),
            # Browse/sort: most recent first per version.
            (('version', 'run_at'), False),
        )
    ...
    def ensure_schema(cls):
        ...
        try:
            ...
            pass
        # MySQL TEXT caps at 64KB — a 5y backtest with thousands of trades
        # plus a 500-point equity curve easily exceeds that. Promote the
        # blob columns to LONGTEXT (4GB ceiling). Idempotent — re-running
        # is a no-op once the columns are already LONGTEXT.
        for col in ('params_json', 'response_json'):
            try:
                DB.execute_sql(f'ALTER TABLE backtest_run MODIFY {col} LONGTEXT')
            except Exception:
                pass
...
class MonteCarloRun(BaseModel):
    """One persisted Monte Carlo run per (algorithm_version, dte_strategy,
    window_label, params_hash). Written by `monte_carlo.py` / `monte_carlo_15dte.py`
    at the end of each window's iteration loop so the dashboard can surface
    canonical N=500 x 8-window results without re-running.

    Why params_hash in the unique key: two runs at different N (e.g. N=300
    smoke vs N=500 ship gate) or with different env-var strategy overrides
    produce different rows. A re-run of the *same* config (same N, same
    params, same engine) upserts.

    Engine generation tracked in `engine_version` so we don't mix
    apples-and-oranges across MC engine rewrites (e.g. pre-2026-04-29 static
    3-mode vs current bounded-fill).
    """
    version           = ForeignKeyField(AlgorithmVersion, backref='mc_runs',
                                        on_delete='CASCADE')
    dte_strategy      = CharField(max_length=8, default='30')          # '30' or '15'
    window_label      = CharField(max_length=16)                       # 2021..2025, dip, 22-now, 5y
    window_start      = DateField(null=True)
    window_end        = DateField(null=True)

    # Engine identity - bumps when monte_carlo.py architecture changes.
    # Current generation: 'bounded-fill' (post-2026-04-29). Legacy: 'static-3-mode'.
    engine            = CharField(max_length=24, default='seeded')     # mode label
    engine_version    = CharField(max_length=24, default='bounded-fill')

    # Iteration + parameter identity
    n_iter            = IntegerField()
    params_hash       = CharField(max_length=32)                       # md5 of canonical params
    params_json       = TextField(null=True)                           # full param snapshot
    git_commit_at_run = CharField(max_length=40, null=True)            # HEAD at run time

    # Headline metrics - return distribution across iterations
    mean_ret_pct      = FloatField(null=True)                          # arithmetic mean (skewed by tails on lognormal)
    med_ret_pct       = FloatField(null=True)                          # median (P50) - primary
    p25_ret_pct       = FloatField(null=True)                          # P25 (worst quartile)
    p75_ret_pct       = FloatField(null=True)                          # P75 (best quartile)
    mean_final_usd    = FloatField(null=True)                          # mean final equity ($)

    # Drawdown distribution - DD survives translation to live execution
    worst_dd_pct      = FloatField(null=True)                          # max DD across iterations (P3 ship gate)
    mean_dd_pct       = FloatField(null=True)
    med_dd_pct        = FloatField(null=True)

    # Collapse + per-trade quality
    collapse_rate_pct = FloatField(null=True)                          # P5 ship gate: must be 0%
    call_tp_rate_pct  = FloatField(null=True)
    put_tp_rate_pct   = FloatField(null=True)
    n_call_trades     = FloatField(null=True)                          # mean across iters
    n_put_trades      = FloatField(null=True)

    starting_cash     = FloatField(default=50000.0)
    runtime_sec       = FloatField(null=True)
    notes             = TextField(null=True)

    run_at            = DateTimeField(default=datetime.now)

    class Meta:
        table_name = 'monte_carlo_run'
        indexes = (
            # Same logical config replaces the prior row (upsert pattern).
            (('version', 'dte_strategy', 'window_label', 'params_hash'), True),
            (('version', 'dte_strategy'), False),     # browse by version+dte
            (('run_at',),                  False),    # recent-runs query
        )
    ...
    def ensure_schema(cls):
        ...
        try:
            ...
            pass
        # Backfill columns added later (idempotent - fails silently if exists).
        for col_def in [
            ('engine_version',    "VARCHAR(24) NOT NULL DEFAULT 'bounded-fill'"),
            ('engine',            "VARCHAR(24) NOT NULL DEFAULT 'seeded'"),
            ('git_commit_at_run', 'VARCHAR(40) NULL'),
            ('p25_ret_pct',       'DOUBLE NULL'),
            ('p75_ret_pct',       'DOUBLE NULL'),
            ('med_dd_pct',        'DOUBLE NULL'),
            ('runtime_sec',       'DOUBLE NULL'),
            ('notes',             'LONGTEXT NULL'),
        ]:
            try:
                DB.execute_sql(f'ALTER TABLE monte_carlo_run ADD COLUMN {col_def[0]} {col_def[1]}')
            except Exception:
                pass
    ...
    def to_dict(self):
        return {
            ...
            'dte_strategy':      self.dte_strategy,
            ...
            'git_commit_at_run': self.git_commit_at_run,
            'mean_ret_pct':      self.mean_ret_pct,
            'med_ret_pct':       self.med_ret_pct,
            'p25_ret_pct':       self.p25_ret_pct,
            'p75_ret_pct':       self.p75_ret_pct,
            'mean_final_usd':    self.mean_final_usd,
            'worst_dd_pct':      self.worst_dd_pct,
            'mean_dd_pct':       self.mean_dd_pct,
            'med_dd_pct':        self.med_dd_pct,
            'collapse_rate_pct': self.collapse_rate_pct,
            'call_tp_rate_pct':  self.call_tp_rate_pct,
            'put_tp_rate_pct':   self.put_tp_rate_pct,
            'n_call_trades':     self.n_call_trades,
            'n_put_trades':      self.n_put_trades,
...
class HistoricPeak(BaseModel):
    ...
    # Algorithm version that produced peak_score. Cache is rebuilt whole on
    # `trader historic-update`; if a rebuild was skipped after a version bump,
    # rows written by an older version persist. Readers MUST filter on this
    # column so a stale row never serves a different version's score (the
    # COHR=84 ghost on 2026-05-06 was this exact failure mode).
    algorithm_version = ForeignKeyField(AlgorithmVersion, backref='historic_peaks',
                                        null=True, on_delete='SET NULL')
    ...
    def ensure_schema(cls):
        ...
        try:
            ...
            pass
        # Migrate: add algorithm_version_id FK so readers can filter on the
        # active version (prevents stale-version rows from serving the wrong
        # peak score when a rebuild is skipped after a version bump).
        try:
            DB.execute_sql(
                'ALTER TABLE historic_peaks ADD COLUMN algorithm_version_id INT NULL,'
                ' ADD CONSTRAINT fk_historic_peaks_version FOREIGN KEY (algorithm_version_id)'
                ' REFERENCES algorithm_versions(id) ON DELETE SET NULL'
            )
        except Exception:
            pass
...
#
# RETIRED 2026-06-10 (v71 integrity audit, F2): the original validation read
# the SPY weekly composite with current-week look-ahead. On the leak-fixed
# (last-completed-week) substrate the blend is a wash at <=25 and mildly
# NEGATIVE at <=30 (ON-admits 39.6% vs shared 41.0% optWR15, z=-2.48; 5y full
# universe), and puts are OFF portfolio-wide. Weight 0.0 short-circuits
# _compute_put_regime_mult to None -> every caller falls back to the standard
# regime_multiplier (exact pre-JA4 behavior). Restore 0.25 to re-enable.

_JA4_SPY_WK_WEIGHT = 0.0   # fraction of composite replaced by SPY_wk (0 = retired)
...
def _spy_wk_last_completed(sorted_dates, date_map, target):
    """Last-COMPLETED-week SPY_wk lookup (look-ahead-free).

    WeeklyScore rows are Monday-keyed and, in recalc/backfill, each row holds
    the COMPLETE Mon-Fri week. A plain on_or_before(target) therefore selects
    the *current* week's row for a Mon-Thu target — bars AFTER the signal date
    leak in (the v69 weekly look-ahead class; fixed here 2026-06-09).

    Looking up at target - 7d lands in the prior calendar week, returning the
    last completed week's composite — the same convention build_kijun_pct_map /
    build_wv_force1_map use in database/utils/scoring.py. Live update and
    recalc become consistent (both read the completed week).
    """
    return _spy_wk_on_or_before(sorted_dates, date_map, target - timedelta(days=7))


def _compute_put_regime_mult(composite, spy_wk):
    """JA4: blended put multiplier = apply_bands(75%*composite + 25%*SPY_wk).

    Returns None if either input is unavailable OR the mechanism is retired
    (weight 0) — caller falls back to standard regime_multiplier unchanged.
    """
    if _JA4_SPY_WK_WEIGHT <= 0.0:
        return None
...
# NOTE: the LIVE softener strength is strategy_config.SCORING.MIS_STRESS_CALL_DAMPEN
# (read via database.utils.scoring) — RETIRED to 0.0 in v71 (2026-06-10).
# This module-level copy is unreferenced; kept aligned to avoid confusion.
MIS_STRESS_CALL_DAMPEN = 0.0    # softener strength on call regime compression
...
def _days_to_next_earnings(by_sym, symbol, signal_date, window=None):
    """Smallest positive calendar-day delta to next earnings within window.

    Returns delta in [1, window] for strictly-future earnings only. A signal
    on the reaction day itself (effective_date == signal_date) returns None
    because the gap is already in that day's prices — boost/suppression
    would double-count post-reaction price action as "upcoming."

    `eds` are EFFECTIVE dates (BMO same-day, AMC shifted to next trading
    day) so the strict-future check covers both:
      - BMO Apr 29, signal Apr 29: eff=Apr 29 -> bisect_right skips -> None
      - AMC Apr 28, signal Apr 29: eff=Apr 29 -> bisect_right skips -> None
      - AMC Apr 28, signal Apr 28: eff=Apr 29 -> delta=1 -> 'pre1'
    """
    if window is None:
    ...
    if not eds:
        return None
    # bisect_right ensures equal dates are skipped (strictly-future semantics)
    idx = bisect.bisect_right(eds, signal_date)
    if idx >= len(eds):
        return None
...
_GHOST_FILTER_WINDOW_DAYS = 30


def _filter_ghost_earnings(rows):
    """Drop earnings rows that look like ghost duplicates of an already-reported
    sibling. A ghost is any row with `reported_eps IS NULL` whose effective_date
    is within ±30 days of another row for the same symbol that HAS reported_eps.

    Source data (Yahoo / yfinance) periodically inserts a fresh upcoming-earnings
    row a few days after the real reaction — same fiscal quarter, no actual yet,
    different date. Without filtering, _days_to_earn_from_sorted picks up the
    ghost and triggers a phantom EARN_BOOST (and V6 volume suppression) for ~5
    days after a stock has already reported.

    Inputs: list of (date, call_time, effective_date, reported_eps).
    Outputs: filtered list, same shape.
    """
    ...
    for r in rows:
        ...
        if rep is not None or eff is None:
            out.append(r)
            continue
        if any(abs((eff - re).days) <= _GHOST_FILTER_WINDOW_DAYS for re in real_eff):
            continue  # ghost — sibling already reported
        out.append(r)
    return out
...
def _load_effective_earnings_dates(symbol):
    ...
    BMO/AMC announcements identically.

    Reads the precomputed `effective_date` column (populated by
    ensure_schema backfill + EarningsDate.build save hook). Falls back to
    runtime compute_effective_date if any rows are still NULL — covers the
    window between schema migration and first daily refresh, and any rows
    inserted by external tools that bypass build().

    Filters ghost duplicates (same fiscal quarter, no actual reported yet,
    sibling within 30 days HAS reported actual) so EARN_BOOST does not fire
    on stale rows that linger after a real earnings reaction.
    """
    rows = list(
        EarningsDate.select(
            EarningsDate.date, EarningsDate.call_time,
    ...
    if not rows:
        return []
    rows = _filter_ghost_earnings(rows)
    if not rows:
        return []
    if all(eff is not None for _, _, eff, _ in rows):
        return sorted(eff for _, _, eff, _ in rows)
    # Fallback path — at least one row missing precomputed effective_date.
    from iv_crush_model import compute_effective_date
    return sorted(
        eff if eff is not None else compute_effective_date(d, ct)
        for d, ct, eff, _ in rows
    )


def _days_to_earn_from_sorted(sorted_dates, signal_date, window=None):
    """Same as _days_to_next_earnings but operates on a pre-sorted list
    (per-stock). Strictly-future semantics — see _days_to_next_earnings
    docstring for the BMO same-day rationale.
    """
...
def _load_mis_stress_map(regime_map=None):
    ...
    for r in rows:
        ...
        if comp is None or r.spy_close is None or r.spy_ema200 is None or r.vix_close is None:
            continue
        s_wk = _spy_wk_last_completed(spy_wk_dates, spy_wk_raw, r.date)
        if s_wk is None:
            continue
...
_CONT_CACHE_COVERAGE_WARNED = False


def _load_cont_barrier_wins(symbol, lo_date, hi_date, cache_db=None):
    """Load 30dte_generic 'low' barrier wins for the continuation echo.

    Returns {date: {w_days: result}} for W in (7, 15, 30, 60),
    filtered to [lo_date, hi_date]. Empty dict on cache miss.

    Coverage guard (F3, 2026-06-09): the barrier cache has been observed
    truncated (nightly refresh_recent re-seeding only ~160 recent days). A
    recalc against a truncated cache silently zeroes cont_lift for every date
    before the cache's min date, writing era-inconsistent history. Warn loudly
    (once per process) instead of failing silently.

    cache_db: optional explicit path (tests); default = repo .cache location.
    """
    import sqlite3
    from pathlib import Path as _Path
    global _CONT_CACHE_COVERAGE_WARNED
    _CACHE_DB = (_Path(cache_db) if cache_db is not None
                 else _Path(__file__).resolve().parents[2] / '.cache' / 'barrier_outcomes.db')
    if not _CACHE_DB.exists():
        if not _CONT_CACHE_COVERAGE_WARNED:
            _CONT_CACHE_COVERAGE_WARNED = True
            import sys
            print("WARNING: barrier_outcomes.db MISSING - continuation echo "
                  "priors unavailable; cont_lift will be 0 for ALL dates. "
                  "Rebuild via: python -m database.barrier_cache backfill 3650",
                  file=sys.stderr)
        return {}
    wins = {}
    try:
        with sqlite3.connect(str(_CACHE_DB)) as _con:
            if not _CONT_CACHE_COVERAGE_WARNED:
                _min_row = _con.execute(
                    "SELECT MIN(date) FROM barrier_outcomes "
                    "WHERE barrier_set='30dte_generic'").fetchone()
                _min_d = _min_row[0] if _min_row else None
                if _min_d is not None and lo_date.isoformat() < _min_d:
                    _CONT_CACHE_COVERAGE_WARNED = True
                    import sys
                    print(f"WARNING: continuation echo priors missing before "
                          f"{_min_d} - barrier cache truncated? Requested "
                          f"lo_date={lo_date.isoformat()}. cont_lift will be 0 "
                          f"for earlier dates. Rebuild via: "
                          f"python -m database.barrier_cache backfill 3650",
                          file=sys.stderr)
            _rows = _con.execute(
                """SELECT date, w_days, result FROM barrier_outcomes
                   WHERE symbol=? AND side='low' AND barrier_set='30dte_generic'
                     AND w_days IN (7,15,30,60)
                     AND date>=? AND date<=?""",
                (symbol, lo_date.isoformat(), hi_date.isoformat())
            ).fetchall()
        for d_str, w_days, result in _rows:
            d = date.fromisoformat(d_str)
            if d not in wins:
                wins[d] = {}
            wins[d][w_days] = result
    except Exception:
        pass
    return wins


def _cont_preboost_or_overall(score_row):
    overall = getattr(score_row, 'overall', None)
    weight_info = getattr(score_row, 'weight_info', None)
    if weight_info:
        try:
            parsed = json.loads(weight_info) if isinstance(weight_info, str) else weight_info
            raw = parsed.get('pre_boost', overall) if isinstance(parsed, dict) else overall
            if raw is not None:
...
def reapply_regime_today(regime_multiplier, regime_composite=None, target_date=None,
                         audit_run_key=None):
    """Re-apply a freshly-computed regime state to all of today's Score rows.

    Called by `trader update` immediately after `compute_regime()` creates a new
    MarketRegime row for today. Replays the full overall-score path so late
    score modifiers such as WCF, PCD, ICH, PESS, earnings, and sector waves
    remain reflected after the same-day regime state is known.
    ...
    if regime_composite is not None:
        ...
        if _spy_wk_map:
            ...
            _spy_wk_today = _spy_wk_last_completed(_spy_wk_sorted, _spy_wk_map, target_date)
    ...
    if pending:
        ...
        for s in pending:
            try:
                ScoreIntradayLog.record_from_score(
                    s,
                    source='regime_reapply_today',
...
class Stock(BaseModel):
    ...
    delisted_date = DateField(null=True)

    @classmethod
    def ensure_schema(cls):
        from database.trader_database import DB as _db
        _db.connect(reuse_if_open=True)
        cur = _db.execute_sql("SHOW COLUMNS FROM stocks LIKE 'delisted_date'")
        if cur.fetchone() is None:
            _db.execute_sql("ALTER TABLE stocks ADD COLUMN delisted_date DATE NULL")
    ...
    def pull_stock_data(self, ticker, full=True):
        if "Earnings Date" in ticker.calendar and ticker.calendar["Earnings Date"]: 
            ...
            if earnings_dates is not None and not earnings_dates.empty:
                ...
                EarningsDate.prune_unreported_missing_from_source(
                    self.symbol, source_earnings_dates
                )
    ...
    def calculate_scores(self, cutoff_date=None, full=False, weekly=False, silent=False,
                          ...
                          extra_score_engines=None):
        from database.models.technical import Indicator, WeeklyIndicator

        IndicatorModel = WeeklyIndicator if weekly else Indicator
        ScoreModel = WeeklyScore if weekly else Score
        extra_score_engines = list(extra_score_engines or [])
        version_summary = {"written": [], "skipped": [], "errors": []}

        def _merge_version_summary(summary):
            if not summary:
                return
            for key in version_summary:
                version_summary[key].extend(summary.get(key, []))
        
        # For incremental updates (full=False) we only need the most recent 1-2
        # rows: one to score today and one to trigger the not-created break.
        # Without this limit, a stock with no existing scores (created=True for
        # every row) would iterate its entire history as if full=True were set.
        _q = (IndicatorModel
              .select(IndicatorModel.date)
              .where(IndicatorModel.symbol == self.symbol)
        ...
        for indicator in _q:
            ...
            if weekly:
                if None in (score.trend, score.rsi, score.macd):
                    if created:
                        score.delete_instance()  # don't leave an empty shell row
                    if not full:
                        break   # incremental: latest date uncomputable, stop here
                    continue    # full rebuild: skip this date, try earlier ones
                ...
                try:
                    ...
                    pass
                score.save()
                if extra_score_engines:
                    try:
                        from algorithm_versions.runtime import score_primary_for_engines
                        _merge_version_summary(
                            score_primary_for_engines(
                                score,
                                extra_score_engines,
                                score_args=getattr(score, '_last_overall_score_args', None),
                                score_kwargs=getattr(score, '_last_overall_score_kwargs', None),
                            )
                        )
                    except Exception as exc:
                        version_summary["errors"].append({
                            "key": "inline-version-service",
    ...
    def calculate_scores_batched(self, silent=False, regime_multiplier=None, regime_composite=None):
        ...
        # PCD: ret_10d_sigma map for post-crash put dampener (one-time per symbol)
        _ret10d_sigma_map = build_ret10d_sigma_map(ph_rows_asc)

        # MCD: mcap_b for mcap dampener — point-in-time proxy per date (F4
        # survivorship fix 2026-06-09). Anchor = symbol's true latest close
        # (matches the yfinance market_cap snapshot; loaded history may be
        # bounded so the last loaded row is not a safe anchor).
        _mcap_b = float(self.market_cap) / 1e9 if self.market_cap is not None else None
        _pit_mcap_map = {}
        if _mcap_b is not None:
            from database.utils.scoring import build_pit_mcap_map as _build_pit_mcap
            try:
                _anchor_row = (
                    PriceHistory.select(PriceHistory.close)
                    .where((PriceHistory.symbol == self.symbol)
                           & (PriceHistory.close.is_null(False)))
                    .order_by(PriceHistory.date.desc()).limit(1).first()
                )
                _mcap_anchor = float(_anchor_row.close) if _anchor_row else None
            except Exception:
                _mcap_anchor = None
            _pit_mcap_map = _build_pit_mcap(_mcap_b, ph_rows_asc, _mcap_anchor)

        # ICH: kijun_pct map for Ichimoku Kijun-sen state dampener
        # WVD-Wave: wv_force1 map for weekly-volume inverted-U modulator
        # (both one-time per symbol; weekly indicators evolve slowly)
        from database.models.technical import WeeklyPriceHistory as _WPH_ich
        from database.utils.scoring import (
            build_kijun_pct_map as _build_kij_map,
            build_wv_force1_map as _build_wvf_map,
        )
        _wph_rows_asc = list(
            _WPH_ich.select(_WPH_ich.date, _WPH_ich.high, _WPH_ich.low,
                            _WPH_ich.close, _WPH_ich.volume)
            .where(_WPH_ich.symbol == self.symbol)
            .order_by(_WPH_ich.date.asc())
        )
        _kijun_pct_map = _build_kij_map(_wph_rows_asc, ph_rows_asc) if _wph_rows_asc else {}
        _wv_force1_map = _build_wvf_map(_wph_rows_asc, ph_rows_asc) if _wph_rows_asc else {}

        # Weekly transition blend — point-in-time PARTIAL current-week weekly
        # component scores per date (state 2).  {date -> (trend,rsi,macd,t)}.
        from database.utils.weekly_pit import build_pit_weekly_map as _build_pit_map
        _pit_weekly_map = _build_pit_map(self, ph_rows_asc)
        ...
        # Continuation echo: pre-load barrier wins + recent active-version priors.
        # Use weight_info['pre_boost'] when present so the echo does not cascade
        # through its own prior lifts.
        _cb_barrier_wins_bs = {}
        _cb_prior_db_pairs = []  # [(date, pre_boost_or_overall)] sorted asc
        if CONT_BOOST_ENABLED:
            _today = date.today()
            _cb_bs_lo = _today - timedelta(days=150)  # 90d look-back plus outcome buffer
            _cb_barrier_wins_bs = _load_cont_barrier_wins(self.symbol, _cb_bs_lo, _today + timedelta(days=60))
            _active_version = AlgorithmVersion.get_active_scores_version()
            _cb_db_rows = list(
                Score.select(Score.date, Score.overall, Score.weight_info)
                .where(
                    (Score.symbol == self.symbol) &
                    (Score.date >= _cb_bs_lo) &
                    (Score.version == _active_version) &
                    (Score.overall.is_null(False))
                )
                .order_by(Score.date.asc())
            )
            _cb_prior_db_pairs = [(r.date, _cont_preboost_or_overall(r)) for r in _cb_db_rows]

        def _cb_prior_signal_for(d):
            if not CONT_BOOST_ENABLED or not _cb_prior_db_pairs:
                return None
            _lo_d = d - timedelta(days=90)
            _hi_d = d - timedelta(days=1)
            _pairs = [(pd_, pov) for (pd_, pov) in _cb_prior_db_pairs
                      if _lo_d <= pd_ <= _hi_d]
            return compute_cont_prior_signal(d, _pairs, _cb_barrier_wins_bs) if _pairs else 0.0
        ...
        for ind in ind_rows:
            ...
            try:
                ...
                if ind.ema_50 and ph and ph.close:
                    _pct_from_ema50 = (float(ph.close) - float(ind.ema_50)) / float(ind.ema_50) * 100
                if ind.ema_200 and ph and ph.close:
                    _pct_from_ema200 = (float(ph.close) - float(ind.ema_200)) / float(ind.ema_200) * 100

                # ── Weekly context (state 1: last-completed week, point-in-time)
                # Was state 3 (current-complete week) = look-ahead on mid-week
                # signals.  Now W-1 / W-2 (matching the live single-row scorer),
                # blended with the point-in-time PARTIAL current week below.
                ws  = ws_map.get(_week_start(d) - timedelta(weeks=1))
                pws = ws_map.get(_week_start(d) - timedelta(weeks=2))
                ws_rsi  = ws.rsi  if ws and ws.rsi and ws.macd else None
                ws_macd = ws.macd if ws and ws.rsi and ws.macd else None
                _pit_wk = _pit_weekly_map.get(d)
                _pit_ws_trend = _pit_wk[0] if _pit_wk else None
                _pit_ws_rsi   = _pit_wk[1] if _pit_wk else None
                _pit_ws_macd  = _pit_wk[2] if _pit_wk else None
                _weekly_t     = _pit_wk[3] if _pit_wk else 1.0
                ...
                _spy_wk = _spy_wk_last_completed(_spy_wk_sorted, _spy_wk_map, d)
                ...
                overall, weight_info, vol_update = compute_overall_score(
                    ...
                    prev_ws_rsi=pws.rsi   if pws else None,
                    prev_ws_macd=pws.macd if pws else None,
                    pit_ws_trend=_pit_ws_trend, pit_ws_rsi=_pit_ws_rsi,
                    pit_ws_macd=_pit_ws_macd, weekly_transition_t=_weekly_t,
                    ...
                    prior_signal=_cb_prior_signal_for(d),
                    ret_10d_sigma=_ret10d_sigma_map.get(d),
                    mcap_b=_pit_mcap_map.get(d, _mcap_b),
                    kijun_pct=_kijun_pct_map.get(d),
                    wv_force1=_wv_force1_map.get(d),
                    score_date=d,
                )
    ...
    def recalculate_scores(self, cutoff, components, persist_lo=None, persist_hi_exclusive=None,
        ...
        if self.delisted_date is not None:
            eff_end = min(eff_end or date.today(), self.delisted_date)
            if eff_end < cutoff:
                return 0, 0, 0
        score_q = (
            Score.select()
        ...
        if eff_end is not None:
            score_q = score_q.where(Score.date <= eff_end)
    ...
    def backfill_scores(self, cutoff=None, fill_lo=None, fill_hi_exclusive=None,
                        cutoff_end=None, show_progress=True):
        ...
        if self.delisted_date is not None:
            eff_end = min(eff_end or date.today(), self.delisted_date)
            if cutoff is not None and eff_end < cutoff:
                return 0, 0
        ...
        if eff_end is not None:
            ph_q = ph_q.where(PriceHistory.date <= eff_end)
            ind_q = ind_q.where(Indicator.date <= eff_end)
        ...
        if eff_end is not None:
            wph_q = wph_q.where(WeeklyPriceHistory.date <= eff_end)
            wind_q = wind_q.where(WeeklyIndicator.date <= eff_end)
        ...
        if eff_end is not None:
            ind_query = ind_query.where(Indicator.date <= eff_end)
        ...
        if eff_end is not None:
            score_query = score_query.where(Score.date <= eff_end)
    ...
    def recalculate_scores_batched(self, cutoff, components, cutoff_end=None, force=False,
        ...
        extra_score_engines = list(extra_score_engines or [])
        if self.delisted_date is not None:
            cutoff_end = min(cutoff_end or date.today(), self.delisted_date)
        ...
        # MCD: mcap_b for mcap dampener — point-in-time proxy per date (F4
        # survivorship fix 2026-06-09). Anchor = symbol's true latest close
        # (recalc slices bound ph_rows_asc by cutoff_end, so the last loaded
        # row is NOT a safe anchor for historical slices).
        _mcap_b = float(self.market_cap) / 1e9 if self.market_cap is not None else None
        _pit_mcap_map = {}
        if _mcap_b is not None:
            from database.utils.scoring import build_pit_mcap_map as _build_pit_mcap
        ...
        # ICH: kijun_pct map for Ichimoku Kijun-sen state dampener
        # WVD-Wave: wv_force1 map for weekly-volume inverted-U modulator
        # (both one-time per symbol; weekly indicators evolve slowly so a single
        # bulk computation covers the full scoring window)
        from database.models.technical import WeeklyPriceHistory as _WPH_ich
        from database.utils.scoring import (
            build_kijun_pct_map as _build_kij_map,
            build_wv_force1_map as _build_wvf_map,
        )
        _wph_q_ich = _WPH_ich.select(_WPH_ich.date, _WPH_ich.high, _WPH_ich.low,
                                       _WPH_ich.close, _WPH_ich.volume).where(
            (_WPH_ich.symbol == self.symbol) & (_WPH_ich.date >= _ind_lo)
        )
        if cutoff_end is not None:
            _wph_q_ich = _wph_q_ich.where(_WPH_ich.date <= cutoff_end)
        _wph_rows_asc = list(_wph_q_ich.order_by(_WPH_ich.date.asc()))
        _kijun_pct_map = _build_kij_map(_wph_rows_asc, ph_rows_asc) if _wph_rows_asc else {}
        _wv_force1_map = _build_wvf_map(_wph_rows_asc, ph_rows_asc) if _wph_rows_asc else {}

        # Weekly transition blend — point-in-time PARTIAL current-week weekly
        # component scores per date (state 2).  Needs a DEEP daily series (weekly
        # EMA200 warmup ~6y), so pull the full daily history rather than the
        # volume-amplifier-clamped `ph_rows_asc`.  {date -> (trend,rsi,macd,t)}.
        from database.utils.weekly_pit import build_pit_weekly_map as _build_pit_map
        _pit_ph_q = PriceHistory.select(
            PriceHistory.date, PriceHistory.open, PriceHistory.high,
            PriceHistory.low, PriceHistory.close, PriceHistory.volume
        ).where(PriceHistory.symbol == self.symbol)
        if cutoff_end is not None:
            _pit_ph_q = _pit_ph_q.where(PriceHistory.date <= cutoff_end)
        _pit_ph_rows = list(_pit_ph_q.order_by(PriceHistory.date.asc()))
        _pit_targets = [p.date for p in ph_rows_asc]
        _pit_weekly_map = _build_pit_map(self, _pit_ph_rows, target_dates=_pit_targets)
        ...
        # Continuation echo: load barrier wins and build prior context during
        # the ascending score walk. New-version full recalcs start with blank
        # same-version overalls, so a precomputed prior map would miss every
        # newly calculated prior; appending each completed pre_boost keeps the
        # recalc path equivalent to the DB-backed daily update path.
        _cb_barrier_wins = {}
        _cb_static_prior_pairs = []
        _cb_static_prior_dates = []
        _cb_recalc_prior_pairs = []
        _cb_recalc_prior_dates = []
        _cb_extra_states = {}
        if CONT_BOOST_ENABLED:
            _cb_lo = cutoff - timedelta(days=150)
            _cb_hi = (cutoff_end or date.today()) + timedelta(days=60)
            _cb_barrier_wins = _load_cont_barrier_wins(self.symbol, _cb_lo, _cb_hi)
            _cb_pre_rows = list(
                Score.select(Score.date, Score.overall, Score.weight_info)
                .where(
                    (Score.symbol == self.symbol) &
                    (Score.version == version) &
                    (Score.date >= cutoff - timedelta(days=90)) &
                    (Score.date < cutoff) &
                    (Score.overall.is_null(False))
                )
                .order_by(Score.date.asc())
            )
            _cb_static_prior_pairs = [(r.date, _cont_preboost_or_overall(r)) for r in _cb_pre_rows]
            _cb_static_prior_dates = [d for d, _ in _cb_static_prior_pairs]
            for engine in extra_score_engines:
                _rows = list(
                    Score.select(Score.date, Score.overall, Score.weight_info)
                    .where(
                        (Score.symbol == self.symbol) &
                        (Score.version == engine.version_id) &
                        (Score.date >= cutoff - timedelta(days=90)) &
                        (Score.date < cutoff) &
                        (Score.overall.is_null(False))
                    )
                ...
                _pairs = [(r.date, _cont_preboost_or_overall(r)) for r in _rows]
                _cb_extra_states[engine.version_id] = {
                    'static_pairs': _pairs,
                    'static_dates': [d for d, _ in _pairs],
                    'recalc_pairs': [],
                    'recalc_dates': [],
                }

        def _cb_prior_signal_for_recalc(d):
            if not CONT_BOOST_ENABLED:
                return None
            _lo_d = d - timedelta(days=90)
            _hi_d = d - timedelta(days=1)
            _pairs = []
            if _cb_static_prior_pairs:
                _i_lo = bisect.bisect_left(_cb_static_prior_dates, _lo_d)
                _i_hi = bisect.bisect_right(_cb_static_prior_dates, _hi_d)
                _pairs.extend(_cb_static_prior_pairs[_i_lo:_i_hi])
            if _cb_recalc_prior_pairs:
                _i_lo = bisect.bisect_left(_cb_recalc_prior_dates, _lo_d)
                _i_hi = bisect.bisect_right(_cb_recalc_prior_dates, _hi_d)
                _pairs.extend(_cb_recalc_prior_pairs[_i_lo:_i_hi])
            return compute_cont_prior_signal(d, _pairs, _cb_barrier_wins) if _pairs else 0.0

        def _cb_prior_signal_for_engine(version_id, d):
            if not CONT_BOOST_ENABLED:
                return None
            state = _cb_extra_states.get(version_id)
            if not state:
                return 0.0
            _lo_d = d - timedelta(days=90)
            _hi_d = d - timedelta(days=1)
            _pairs = []
            static_dates = state['static_dates']
            if static_dates:
                _i_lo = bisect.bisect_left(static_dates, _lo_d)
                _i_hi = bisect.bisect_right(static_dates, _hi_d)
                _pairs.extend(state['static_pairs'][_i_lo:_i_hi])
            recalc_dates = state['recalc_dates']
            if recalc_dates:
                _i_lo = bisect.bisect_left(recalc_dates, _lo_d)
                _i_hi = bisect.bisect_right(recalc_dates, _hi_d)
                _pairs.extend(state['recalc_pairs'][_i_lo:_i_hi])
            return compute_cont_prior_signal(d, _pairs, _cb_barrier_wins) if _pairs else 0.0
        ...
        def _in_persist_window(score_date):
            if save_only_dates is not None:
                return score_date in save_only_dates
            lo_ok = True if persist_lo_eff is None else score_date >= persist_lo_eff
            hi_ok = True if persist_hi_exclusive is None else score_date < persist_hi_exclusive
            return lo_ok and hi_ok

        # Score uses a CompositeKey so Peewee's bulk_update is not available.
        # Use INSERT ... ON DUPLICATE KEY UPDATE batches for current and
        # historical sidecar version rows.
        _upd_cols = [
            'overall', 'weight_info',
        ...
        def _clone_sidecar_score(score, engine, overall, weight_info, vol_update):
            alt = Score()
            alt.symbol_id = score.symbol_id
            alt.date = score.date
            alt.version_id = engine.version_id
            for col in _upd_cols:
                setattr(alt, col, getattr(score, col, None))
            alt.overall = overall
            alt.weight_info = json.dumps(weight_info)
            alt.regime_composite = score.regime_composite
            alt.regime_multiplier = score.regime_multiplier
            alt.updated_at = datetime.now()
            if vol_update and (vol_update['volume_signal'] != 'NEUTRAL'
                               or not alt.volume_signal
                               or alt.volume_signal == 'NEUTRAL'):
                alt.volume = vol_update['volume']
                alt.volume_signal = vol_update['volume_signal']
                alt.volume_magnitude = vol_update['volume_magnitude']
            return alt

        pending_saves = []  # collected current-version rows modified in this pass
        pending_sidecar_saves = []
        ...
        for score in it:
            try:
                ...
                if 'overall' in components or changed:
                    ...
                    if ind and ind.ema_200 and ph and ph.close:
                        _pct_from_ema200_r = (float(ph.close) - float(ind.ema_200)) / float(ind.ema_200) * 100

                    # Weekly context
                    # Weekly context (state 1: last-completed week, point-in-time).
                    # Was state 3 (current-complete week) = look-ahead.  Blended
                    # with the point-in-time PARTIAL current week below.
                    ws  = ws_map.get(_week_start(score.date) - timedelta(weeks=1))
                    pws = ws_map.get(_week_start(score.date) - timedelta(weeks=2))
                    ws_trend = ws.trend if ws else None
                    ws_rsi   = ws.rsi   if ws and ws.rsi and ws.macd else None
                    ws_macd  = ws.macd  if ws and ws.rsi and ws.macd else None
                    _pit_wk = _pit_weekly_map.get(score.date)
                    _pit_ws_trend = _pit_wk[0] if _pit_wk else None
                    _pit_ws_rsi   = _pit_wk[1] if _pit_wk else None
                    _pit_ws_macd  = _pit_wk[2] if _pit_wk else None
                    _weekly_t     = _pit_wk[3] if _pit_wk else 1.0
                    ...
                    # JA4: blended put multiplier
                    _spy_wk_r = _spy_wk_last_completed(_spy_wk_sorted_r, _spy_wk_map_r, score.date)
                    ...
                    _score_kwargs = {
                        ...
                        'pct_from_ema200': _pct_from_ema200_r,
                        'ws_trend': ws_trend,
                        'ws_rsi': ws_rsi,
                        'ws_macd': ws_macd,
                        'prev_ws_trend': pws.trend if pws else None,
                        ...
                        'prior_signal': _cb_prior_signal_for_recalc(score.date),
                        'ret_10d_sigma': _ret10d_sigma_map.get(score.date),
                        'mcap_b': _pit_mcap_map.get(score.date, _mcap_b),
                        'kijun_pct': _kijun_pct_map.get(score.date),
                        'wv_force1': _wv_force1_map.get(score.date),
                    ...
                    score.weight_info = json.dumps(weight_info)
                    if CONT_BOOST_ENABLED:
                        _cb_recalc_prior_dates.append(score.date)
                        _cb_recalc_prior_pairs.append((
                            score.date,
                            int(round(float(weight_info.get('pre_boost', overall)))),
                        ))
                    if vol_update and (vol_update['volume_signal'] != 'NEUTRAL'
                                       or not score.volume_signal
                                       or score.volume_signal == 'NEUTRAL'):
                    ...
                    if extra_score_engines and _in_persist_window(score.date):
                        for engine in extra_score_engines:
                            try:
                                engine_kwargs = dict(_score_kwargs)
                                engine_kwargs['prior_signal'] = _cb_prior_signal_for_engine(
                                    engine.version_id, score.date)
                                alt_overall, alt_weight_info, alt_vol_update = engine.compute_overall_score(
                                    *_score_args,
                                    **engine_kwargs,
                                )
                                if alt_overall is None:
                                    continue
                                alt = _clone_sidecar_score(
                                    score, engine, alt_overall, alt_weight_info, alt_vol_update)
                                pending_sidecar_saves.append(alt)
                                if CONT_BOOST_ENABLED:
                                    state = _cb_extra_states.get(engine.version_id)
                                    if state is not None:
                                        state['recalc_dates'].append(score.date)
                                        state['recalc_pairs'].append((
                                            score.date,
                                            int(round(float(alt_weight_info.get('pre_boost', alt_overall)))),
                                        ))
                            except Exception as sidecar_exc:
                                errors += 1
                                key = getattr(engine, 'key', f"v{getattr(engine, 'version_id', '?')}")
                                sidecar_error_counts[key] = sidecar_error_counts.get(key, 0) + 1
                                if sidecar_error_counts[key] <= 3:
                                    print(
                                        Fore.YELLOW
                                        + f"  {self.symbol} {score.date} {key}: "
                                        + f"sidecar score failed ({type(sidecar_exc).__name__}: {sidecar_exc})"
                                    )
                    changed = True

                in_win = _in_persist_window(score.date)
                if changed and in_win:
                    score.updated_at = datetime.now()
        ...
        all_pending_saves = pending_saves + pending_sidecar_saves
        if all_pending_saves:
            ...
            _BATCH = 500
            try:
                for i in range(0, len(all_pending_saves), _BATCH):
                    _batch = all_pending_saves[i:i + _BATCH]
    ...
    def recalculate_scores_full(self, cutoff_start, components, cutoff_end=None,
        ...
        if self.delisted_date is not None:
            eff_end = min(eff_end, self.delisted_date)
            if eff_end < cutoff_start:
                return 0, 0, 0
        ...
        if use_batch:
            ...
            return self.recalculate_scores_batched(
                ...
                cutoff_end=eff_end, force=force, score_version=version,
