...
const MacdScoreChart = ({ scores, onHoverIndex, hoveredIndex }) => {
  if (!scores || scores.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 text-gray-400">
        No MACD score data available
      </div>
    );
  }
  ...
  const watermarkPlugin = {
    ...
    beforeDatasetsDraw: (chart) => {
      ...
      ctx.fillText('MACD SCORE', centerX, centerY);
      ctx.restore();
    }
  };
    ...
    datasets: [
      {
        ...
        segment: {
          borderColor: (ctx) => {
            if (ctx.p0DataIndex !== undefined && ctx.p1DataIndex !== undefined) {
              const score1 = reversedScores[ctx.p0DataIndex]?.macd;
              const score2 = reversedScores[ctx.p1DataIndex]?.macd;
        ...
        pointBackgroundColor: (ctx) => {
          const score = reversedScores[ctx.dataIndex]?.macd;
  ...
  const highlightedData = {
    ...
    datasets: data.datasets.map((dataset) => ({
      ...
      pointBackgroundColor: dataset.data.map((_, index) => {
        ...
        const score = reversedScores[index]?.macd;
        return getScoreColor(score);
      }),
