...
class HistoricPeak(BaseModel):
    """Cached table of the most extreme score (≥80 or ≤20) per stock over the last 90 days.
    One row per symbol. Rebuilt nightly via historic_peaks.update_historic_peaks()."""
    symbol = DeferredForeignKey('Stock', column_name='symbol', field='symbol', backref='historic_peak', unique=True)
    peak_date = DateField()
    ...
    # HIGH (call): (max_high − price_at_peak) / price_at_peak × 100
    # LOW  (put):  (price_at_peak − min_low)  / price_at_peak × 100
    ...
    def ensure_schema(cls):
        ...
        DB.create_tables([cls], safe=True)
        for col, typedef in [
            ('price_peak_pct',        'FLOAT NULL'),
            ('price_peak_days_after', 'INT NULL'),
        ]:
            try:
                DB.execute_sql(f'ALTER TABLE historic_peaks ADD COLUMN {col} {typedef}')
            except Exception:
                pass
