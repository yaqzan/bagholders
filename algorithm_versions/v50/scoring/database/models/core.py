...
def _load_cont_barrier_wins(symbol, lo_date, hi_date):
    """Load 30dte_generic 'low' barrier wins for the continuation boost.
...
class Stock(BaseModel):
    ...
    def calculate_scores_batched(self, silent=False, regime_multiplier=None, regime_composite=None):
        ...
        # Continuation boost: pre-load barrier wins + recent prior scores from DB.
        # Uses existing DB overalls (any version) for prior_signal — these are the
        # raw pre-boost values needed for the no-cascade architecture.
        _cb_barrier_wins_bs = {}
        _cb_prior_db_pairs = []  # [(date, overall)] sorted asc, for bisect lookup
        if CONT_BOOST_ENABLED:
            _today = date.today()
            _cb_bs_lo = _today - timedelta(days=120)  # cover 60d look-back for any date scored
            ...
            _cb_prior_db_pairs = [(r.date, r.overall) for r in _cb_db_rows]

        def _cb_prior_signal_for(d):
            if not CONT_BOOST_ENABLED or not _cb_prior_db_pairs:
                return None
            _lo_d = d - timedelta(days=60)
            _hi_d = d - timedelta(days=7)
    ...
    def recalculate_scores_batched(self, cutoff, components, cutoff_end=None, force=False,
        ...
        # Continuation boost: load barrier wins + pre-compute prior_signal_map.
        # Uses v32 overalls (pre-recalculate values in `scores`) as no-cascade priors.
        # Signal map built once before the scoring loop; passed per-date to compute_overall_score.
        _cb_barrier_wins = {}
        _cb_prior_signal_map = {}
        if CONT_BOOST_ENABLED:
            _cb_lo = cutoff - timedelta(days=60)
            ...
            for _i, _s in enumerate(scores):
                _d = _s.date
                _lo_d = _d - timedelta(days=60)
                _hi_d = _d - timedelta(days=7)
                _i_lo = bisect.bisect_left(_score_dates, _lo_d)
                _i_hi = bisect.bisect_right(_score_dates, _hi_d)
                _prior_pairs = [(scores[_j].date, scores[_j].overall)
