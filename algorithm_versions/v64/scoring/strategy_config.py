...
class ScoringConfig:
    ...
    # ── BB Location Taper (v63 candidate) ───────────────────────────────
    # Late post-score admission taper from the v60 miss ledger. It removes
    # weaker CALL admissions when the BB component is middling and lifts
    # oversold PUT admissions when the BB component is very low.
    BB_LOCATION_TAPER_ENABLED: bool
    BB_LOCATION_TAPER_CALL_GATE: int
    BB_LOCATION_TAPER_CALL_MID_LO: float
    BB_LOCATION_TAPER_CALL_MID_HI: float
    BB_LOCATION_TAPER_CALL_DAMPEN: float
    BB_LOCATION_TAPER_PUT_GATE: int
    BB_LOCATION_TAPER_PUT_BB_LO: float
    BB_LOCATION_TAPER_PUT_LIFT: float

    # ── Call Boundary Freshness (v64 candidate) ───────────────────────
    # Late CALL-side 70-84 controller. Promotes fresh 70-74 boundary calls
    # and dampens risky 75-84 calls using weekly freshness, SCW/continuation,
    # volatility, and component-agreement context.
    CALL_BOUNDARY_FRESHNESS_ENABLED: bool
    CALL_BOUNDARY_FRESHNESS_PARAMS: Mapping[str, float]
...
SCORING = ScoringConfig(
    ...
    SECTOR_BREADTH_WAVE_ENABLED=True,
    ...
    # BB Location Taper
    BB_LOCATION_TAPER_ENABLED=True,
    BB_LOCATION_TAPER_CALL_GATE=75,
    BB_LOCATION_TAPER_CALL_MID_LO=35.0,
    BB_LOCATION_TAPER_CALL_MID_HI=65.0,
    BB_LOCATION_TAPER_CALL_DAMPEN=3.0,
    BB_LOCATION_TAPER_PUT_GATE=25,
    BB_LOCATION_TAPER_PUT_BB_LO=35.0,
    BB_LOCATION_TAPER_PUT_LIFT=3.0,
    # Call Boundary Freshness
    CALL_BOUNDARY_FRESHNESS_ENABLED=True,
    CALL_BOUNDARY_FRESHNESS_PARAMS={
        'promote_max': 2.75,
        'damp_max': 0.8,
        'mom_mid': 1.7472197255054263,
        'mom_width': 2.3258830798657355,
        'comp_mid': 69.80982351744979,
        'comp_width': 6.2851248864358675,
        'bias_mid': 1.625528689023521,
        'bias_width': 8.994565973938322,
        'score_damp_width': 5.613854416441694,
        'score_damp_power': 1.5192401267522992,
        'fresh_power': 1.4571831994748121,
        'risk_power': 2.200368604352241,
        'cont_w': 1.9850176086859672,
        'scw_w': 1.5611613132083748,
        'volume_w': 1.124889907058259,
        'regime_w': 0.5657739250452438,
        'agreement_w': 0.6495131420472546,
        'extension_w': 0.9395830256136597,
        'sigma_w': 1.4910393105390027,
    },
    # PESS
    PESS_GATE_LO=16,
    PESS_GATE_HI=20,
...
STRATEGY_30DTE = DteStrategyConfig(
    ...
    MAX_POSITIONS=14,
    # v60 r054 Stage 3 overlay: keep the shared 14-slot pool, but cap calls at
    # 12 so put hedges can still occupy residual slots during crowded call runs.
    MAX_POSITIONS_CALL=12,
    MAX_POSITIONS_PUT=8,
    # v62 Stage 3 practical exposure saturation. Promotion candidate:
    # g80_c65_p25_ref16_4_pow05_floor55_25m. Portfolio-only; preserves
    # Score.overall and therefore does not require an ALGORITHM_VERSION bump.
    PRACTICAL_EXPOSURE_ENABLED=True,
    PRACTICAL_CAPITAL_CEILING=25_000_000.0,
