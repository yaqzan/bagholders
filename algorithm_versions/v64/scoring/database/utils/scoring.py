...
# BB Location Taper
BB_LOCATION_TAPER_ENABLED     = _envb('BB_LOCATION_TAPER_ENABLED',      _S.BB_LOCATION_TAPER_ENABLED)
BB_LOCATION_TAPER_CALL_GATE   = _envi('BB_LOCATION_TAPER_CALL_GATE',    _S.BB_LOCATION_TAPER_CALL_GATE)
BB_LOCATION_TAPER_CALL_MID_LO = _envf('BB_LOCATION_TAPER_CALL_MID_LO',  _S.BB_LOCATION_TAPER_CALL_MID_LO)
BB_LOCATION_TAPER_CALL_MID_HI = _envf('BB_LOCATION_TAPER_CALL_MID_HI',  _S.BB_LOCATION_TAPER_CALL_MID_HI)
BB_LOCATION_TAPER_CALL_DAMPEN = _envf('BB_LOCATION_TAPER_CALL_DAMPEN',  _S.BB_LOCATION_TAPER_CALL_DAMPEN)
BB_LOCATION_TAPER_PUT_GATE    = _envi('BB_LOCATION_TAPER_PUT_GATE',     _S.BB_LOCATION_TAPER_PUT_GATE)
BB_LOCATION_TAPER_PUT_BB_LO   = _envf('BB_LOCATION_TAPER_PUT_BB_LO',    _S.BB_LOCATION_TAPER_PUT_BB_LO)
BB_LOCATION_TAPER_PUT_LIFT    = _envf('BB_LOCATION_TAPER_PUT_LIFT',     _S.BB_LOCATION_TAPER_PUT_LIFT)

# Call Boundary Freshness
CALL_BOUNDARY_FRESHNESS_ENABLED = _envb(
    'CALL_BOUNDARY_FRESHNESS_ENABLED', _S.CALL_BOUNDARY_FRESHNESS_ENABLED
)
CALL_BOUNDARY_FRESHNESS_PARAMS = {
    key: _env_param(f"CALL_BOUNDARY_FRESHNESS_{key.upper()}", value)
    for key, value in _S.CALL_BOUNDARY_FRESHNESS_PARAMS.items()
}
...
def compute_signal_sigma_pct(closes_asc, target_idx):
    """Compute signal-date 60-day realized daily volatility in percent."""
    if target_idx < 60:
        return None
    daily_rets = []
    ...
    if len(daily_rets) < 60:
        return None
    sigma_d = float(np.std(daily_rets, ddof=1))
    return sigma_d * 100.0 if sigma_d > 0 else None


def _signal_sigma_pct_by_index(closes_asc):
    """Vectorized 60-return realized-vol series matching compute_signal_sigma_pct."""
    n = len(closes_asc)
    sigmas = [None] * n
    if n < 61:
        return sigmas
    ...
    closes = np.array(
        [float(c) if c is not None else np.nan for c in closes_asc],
        dtype=float,
    )
    prev = closes[:-1]
    cur = closes[1:]
    ...
    rets[valid] = (cur[valid] - prev[valid]) / prev[valid]

    counts = np.concatenate(([0], np.cumsum(valid.astype(np.int64))))
    sums = np.concatenate(([0.0], np.cumsum(rets)))
    sums_sq = np.concatenate(([0.0], np.cumsum(rets * rets)))
    window = 60
    denom = window - 1

    for idx in range(window, n):
        lo = idx - window
        count = int(counts[idx] - counts[lo])
        if count < window:
            continue
        total = float(sums[idx] - sums[lo])
        total_sq = float(sums_sq[idx] - sums_sq[lo])
        var_num = total_sq - (total * total / window)
        if var_num <= 0:
            continue
        sigma_d = math.sqrt(max(var_num / denom, 0.0))
        if sigma_d > 0:
            sigmas[idx] = sigma_d * 100.0
    return sigmas
...
def compute_ret_10d_sigma(closes_asc, target_idx):
    ...
    sigma_pct = compute_signal_sigma_pct(closes_asc, target_idx)
...
def build_ret10d_sigma_map(ph_rows_asc):
    ...
    sigmas = _signal_sigma_pct_by_index(closes)
    out = {}
    for idx in range(60, len(ph_rows_asc)):
        if PCD_ENABLED is False:
            out[ph_rows_asc[idx].date] = None
            continue
        try:
            close_today = float(closes[idx])
            close_10ago = float(closes[idx - 10])
        except (IndexError, TypeError, ValueError):
            out[ph_rows_asc[idx].date] = None
            continue
        sigma_pct = sigmas[idx]
        if close_10ago <= 0 or close_today <= 0 or sigma_pct is None or sigma_pct <= 0:
            out[ph_rows_asc[idx].date] = None
            continue
        ret_10d = (close_today - close_10ago) / close_10ago
        out[ph_rows_asc[idx].date] = ret_10d / ((sigma_pct / 100.0) * math.sqrt(10))
    return out


def build_signal_sigma_pct_map(ph_rows_asc):
    """Build {date: 60-day realized daily volatility %} at each signal date."""
    if not ph_rows_asc or len(ph_rows_asc) < 61:
        return {}
    closes = [float(p.close) if p.close is not None else None for p in ph_rows_asc]
    sigmas = _signal_sigma_pct_by_index(closes)
    return {ph_rows_asc[idx].date: sigmas[idx] for idx in range(60, len(ph_rows_asc))}
...
def _call_boundary_freshness_params() -> dict:
    return {
        "enabled": CALL_BOUNDARY_FRESHNESS_ENABLED,
        **CALL_BOUNDARY_FRESHNESS_PARAMS,
    }
...
def _sigmoid_width_nullable(value, mid: float, width: float) -> float:
    if value is None:
        return 0.0
    try:
        return _sigmoid_width(float(value), mid, width)
    except (TypeError, ValueError):
        return 0.0


def _gaussian_width_nullable(value, mid: float, width: float) -> float:
    if value is None:
        return 0.0
    try:
        width = max(float(width), 1e-6)
        return math.exp(-((float(value) - mid) / width) ** 2)
    except (TypeError, ValueError):
        return 0.0
...
def _apply_bb_location_taper(overall: int, bb: float):
    if not BB_LOCATION_TAPER_ENABLED:
        return overall, None
    try:
        bb_score = float(bb)
    except (TypeError, ValueError):
    ...
    side = None
    delta = 0.0
    if (overall >= BB_LOCATION_TAPER_CALL_GATE
            and BB_LOCATION_TAPER_CALL_MID_LO <= bb_score < BB_LOCATION_TAPER_CALL_MID_HI):
        side = "CALL"
        delta = -float(BB_LOCATION_TAPER_CALL_DAMPEN)
    elif overall <= BB_LOCATION_TAPER_PUT_GATE and bb_score < BB_LOCATION_TAPER_PUT_BB_LO:
        side = "PUT"
        delta = float(BB_LOCATION_TAPER_PUT_LIFT)

    if abs(delta) < 1e-9:
        return overall, None
    new_overall = int(_clamp_float(round(float(overall) + delta), 0, 100))
    if new_overall == overall:
        return overall, None
    return new_overall, {
        "side": side,
...
def _apply_call_boundary_freshness(
    overall: int,
    *,
    bb_location_taper_meta=None,
    weekly_detail=None,
    stoch=None,
    technical_alignment=None,
    macd=None,
    vol_sig='NEUTRAL',
    vol_mag=0.0,
    pct_from_ema50=None,
    regime_multiplier=None,
    signal_sigma_pct=None,
    cont_lift=0.0,
    scw_dampen=0.0,
    scw_ext_index=None,
    dvaw_meta=None,
    wv_force1=None,
    wv_force1_active=False,
):
    params = _call_boundary_freshness_params()
    if not params.get('enabled') or overall < 70 or overall > 84:
        return overall, None

    w_comp = weekly_detail.get('w_comp') if weekly_detail else None
    w_bias = weekly_detail.get('w_bias') if weekly_detail else None
    w_mom = weekly_detail.get('w_mom') if weekly_detail else None
    wv_context = wv_force1 if wv_force1_active else None

    dvaw_delta = 0.0
    if isinstance(dvaw_meta, dict):
        dvaw_delta = abs(_finite_float(dvaw_meta.get('delta'), 0.0))

    cont = _sigmoid_width_nullable(cont_lift, 0.5, 1.2)
    scw = max(
        _sigmoid_width_nullable(scw_dampen, 0.5, 2.2),
        1.0 - _sigmoid_width_nullable(stoch, 42.0, 8.0),
        _sigmoid_width_nullable(scw_ext_index, 1.0, 0.35),
    )
    volume_base = max(
        _sigmoid_width_nullable(vol_mag, 0.45, 0.15),
        _sigmoid_width_nullable(dvaw_delta, 0.5, 0.8),
    )
    volume = volume_base if str(vol_sig or '').upper() in {'CONVICTION', 'CLIMAX', 'REJECTION'} else 0.35 * volume_base
    regime = 0.65 if regime_multiplier is None else 1.0 - _sigmoid_width_nullable(regime_multiplier, 0.90, 0.05)
    agreement = max(
        1.0 - _sigmoid_width_nullable(technical_alignment, 58.0, 8.0),
        1.0 - _sigmoid_width_nullable(macd, 55.0, 10.0),
    )
    extension = max(
        _sigmoid_width_nullable(pct_from_ema50, 14.0, 7.0),
        _sigmoid_width_nullable(w_comp, 70.0, 7.0),
        _sigmoid_width_nullable(w_bias, 8.0, 2.0),
        _sigmoid_width_nullable(wv_context, 0.12, 0.05),
    )
    sigma = _sigmoid_width_nullable(signal_sigma_pct, 3.0, 1.0)

    weights = {
        'cont': float(params.get('cont_w', 0.0)),
        'scw': float(params.get('scw_w', 0.0)),
        'volume': float(params.get('volume_w', 0.0)),
        'regime': float(params.get('regime_w', 0.0)),
        'agreement': float(params.get('agreement_w', 0.0)),
        'extension': float(params.get('extension_w', 0.0)),
        'sigma': float(params.get('sigma_w', 0.0)),
    }
    risk_parts = {
        'cont': cont,
        'scw': scw,
    ...
    raw_risk = sum(risk_parts[k] * weights[k] for k in risk_parts) / denom if denom > 0 else 0.0
    risk = _clamp_float(raw_risk, 0.0, 1.0) ** max(0.1, float(params.get('risk_power', 1.0)))

    mom = _sigmoid_width_nullable(w_mom, float(params.get('mom_mid', 2.0)), float(params.get('mom_width', 1.2)))
    comp = _gaussian_width_nullable(w_comp, float(params.get('comp_mid', 65.0)), float(params.get('comp_width', 6.0)))
    bias = _gaussian_width_nullable(w_bias, float(params.get('bias_mid', 4.0)), float(params.get('bias_width', 5.5)))
    ta_fresh = _sigmoid_width_nullable(technical_alignment, 56.0, 9.0)
    macd_fresh = _sigmoid_width_nullable(macd, 55.0, 10.0)
    stoch_fresh = _sigmoid_width_nullable(stoch, 48.0, 10.0)
    regime_fresh = 0.5 if regime_multiplier is None else _sigmoid_width_nullable(regime_multiplier, 0.90, 0.05)
    no_exhaust = 1.0 - max(
        _sigmoid_width_nullable(pct_from_ema50, 18.0, 6.0),
        _sigmoid_width_nullable(wv_context, 0.16, 0.05),
    )
    fresh = (
        mom
        * (0.55 + 0.45 * comp)
        * (0.65 + 0.35 * bias)
        * (0.50 + 0.20 * ta_fresh + 0.15 * macd_fresh + 0.15 * stoch_fresh)
        * (0.65 + 0.35 * regime_fresh)
        * (0.45 + 0.55 * _clamp_float(no_exhaust, 0.0, 1.0))
    )
    fresh = _clamp_float(fresh, 0.0, 1.0) ** max(0.1, float(params.get('fresh_power', 1.0)))

    delta = 0.0
    reason = 'none'
    if overall < 75:
        if isinstance(bb_location_taper_meta, dict) and bb_location_taper_meta.get('side') == 'CALL':
            reason = 'blocked_bblt'
        else:
            need = max(1.0, 75.0 - float(overall))
            delta = float(params.get('promote_max', 0.0)) * fresh * (1.0 - 0.55 * risk)
            reason = 'fresh_promote' if delta >= need else 'fresh_insufficient'
    else:
        score_dist = max(0.0, float(overall) - 75.0)
        score_width = max(0.5, float(params.get('score_damp_width', 1.0)))
        score_power = max(0.25, float(params.get('score_damp_power', 1.0)))
        score_decay = math.exp(-((score_dist / score_width) ** score_power))
        delta = -float(params.get('damp_max', 0.0)) * risk * score_decay * (1.0 - 0.35 * fresh)
        reason = 'risk_dampen' if abs(delta) >= 0.25 else 'risk_insufficient'
...
def compute_overall_score(
    ...
    **_extra,
    ...
    overall, _bb_location_taper_meta = _apply_bb_location_taper(overall, bb)
    overall, _call_boundary_freshness_meta = _apply_call_boundary_freshness(
        overall,
        bb_location_taper_meta=_bb_location_taper_meta,
        ...
        signal_sigma_pct=signal_sigma_pct,
        cont_lift=_cont_lift,
        scw_dampen=_scw_dampen,
        scw_ext_index=_scw_ext_index,
        dvaw_meta=_dvaw_meta,
        wv_force1=wv_force1,
        wv_force1_active=(abs(_wvd_lift) > 0.5 or abs(_wvd_dampen) > 0.5),
    )
    ...
    if _bb_location_taper_meta:
        weight_info['bb_location_taper'] = _bb_location_taper_meta
    if _call_boundary_freshness_meta:
        weight_info['call_boundary_freshness'] = _call_boundary_freshness_meta
    if _pcd_active:
