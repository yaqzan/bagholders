...
class StockContext:
    ...
    def __init__(self, stock, ind_rows, ph_rows_asc, ws_rows, earnings_dates,
        ...
        self.signal_sigma_pct_map = build_signal_sigma_pct_map(ph_rows_asc)
...
class ScoreSimulator:
            ...
            for ind_row in ctx.ind_rows:
                ...
                if self.scoring_fn is not None:
                    ...
                    overall, _, _ = compute_overall_score(
                        ...
                        signal_sigma_pct=ctx.signal_sigma_pct_map.get(d),
                        score_date=d,
                    )
    ...
    def simulate_full(self, since: Optional[date] = None):
        ...
        for sym, ctx in self._contexts.items():
            ...
            for ind_row in ctx.ind_rows:
                ...
                overall, weight_info, _ = compute_overall_score(
                    trend, bb, rsi, macd, stoch, ta,
                    bb_pct=bb_pct,
                    pct_from_ema50=_pct_ema50,
                    pct_from_ema200=_pct_ema200,
    ...
    def compare(self, sim_scores: SimScores) -> None:
        ...
        stored = {
            (getattr(s, 'symbol_id', s.symbol), s.date): s.overall
