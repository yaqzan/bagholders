...
# F1 guard (2026-06-09): the v57 Market Wave ran INERT for an unknown span
# because the source CSV silently vanished (_load_rows returned [] -> wave
# defaulted to 50.0 -> zero adjustment, no weight_info key, invisible).
# Warn loudly, once per process per condition. Never hard-raise: recalc and
# tests must still run — the failure mode to kill is *silent*.
_GUARD_WARNED: set[str] = set()


def _guard_warn(key: str, msg: str) -> None:
    if key in _GUARD_WARNED:
        return
    _GUARD_WARNED.add(key)
    import sys
    print(f"WARNING [sector_breadth_wave]: {msg}", file=sys.stderr)
...
def adjusted_score(
    ...
    # F1 staleness guard: a signal_date far past the last source row means the
    # wave forward-fills a stale value for live scoring (nothing refreshes the
    # CSV automatically). Historical signals inside coverage are fine.
    if series.dates and (signal_date - series.dates[-1]).days > 7:
        _guard_warn(
            f"stale:{series.dates[-1]}",
            f"wave source last row {series.dates[-1]} is >7 days older than "
            f"signal_date {signal_date} -> forward-filling STALE wave values. "
            f"Regenerate the source CSV.",
        )
    if params.get("mode") == "direct_market_wave":
...
def get_series(params: dict) -> SectorBreadthWaveSeries:
    ...
    # get_series is only reached when the wave is enabled (adjusted_score
    # gates on params['enabled'] first) — an empty series means the mechanism
    # is silently inert. F1 guard: shout.
    if not series.dates:
        _guard_warn(
            f"missing:{path}",
            f"wave ENABLED but source file missing/empty -> mechanism is "
            f"INERT (all scores pass through unadjusted). path={path}",
        )
    return series
