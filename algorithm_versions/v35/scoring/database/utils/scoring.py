...
def compute_overall_score(
    ...
    **_extra,
    ...
    # this covers the weakly-positive wadj=[1,12) residue when stoch also contradicts.
    #
    # Pattern: CALL 75+ with stoch<35 & wadj∈[0,13] misses at +3.7pp above baseline
    # (z=+4.5, N=3,379, 46% of 75+ calls in 5y v32 ledger).
    #
    # Calibrated: 175-variant gradient Bayesian sweep (stoch_wadj_bayes.py v2, 2026-05-04):
    #   stoch_neutral=35 (COMP_BINS lo/mid anchor), wadj_gate=12, K=0.30.
    #   56/175 variants pass H1+H3+H4+H5. Best semantic: sn=35 wg=12 K=0.30 (rank 2 by
    #   utility, rank 1 by semantic cleanliness). Per-tier 5y: 85+ +1.80pp, 80+ +1.17pp,
    #   75+ +0.93pp. N drops -12 to -14% (within H3 ±15%). Puts unchanged.
    #   Sign-consistent 1y/3y/5y for all N>=50 tiers (H5 pass).
    _cswc_dampen = 0.0
    if overall >= 75 and weekly_detail is not None:
        _wadj_sw = float(weekly_detail.get('w_adj', 0.0))
        if 1.0 <= _wadj_sw < 12.0:
            # Gradient: 0 when stoch=35 (neutral), 1 when stoch=0 (maximally bearish)
            _stoch_w = max(0.0, (35.0 - stoch) / 35.0)
            # Gradient: 0 when wadj=12, 1 when wadj=1 (CWCF handles <1)
            _wadj_sw_w = max(0.0, min(1.0, (12.0 - _wadj_sw) / 11.0))
            _cswc_weakness = _stoch_w * _wadj_sw_w
            if _cswc_weakness > 0.0:
                _cswc_dampen = 0.30 * _cswc_weakness * (overall - 55)
