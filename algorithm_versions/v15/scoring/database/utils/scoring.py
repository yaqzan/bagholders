...
def compute_overall_score(
    ...
    *,
    ...
    # ── Momentum confirmation gradient ───────────────────────────────────
    # When raw MACD histogram direction contradicts the trend (e.g. rising
    # MACDh while score is bearish), trend_dominance is dampened so RSI/MACD
    # weights recover toward sideways levels — giving their near-neutral values
    # more say without a hard gate. Max dampening: 40% (floor d *= 0.60).
    # Only active when trend is meaningful (trend_strength > 0.15) to avoid
    # noise in sideways markets where d is already near zero.
    ...
    if macdh_raw is not None and trend_strength > 0.15:
        ...
        alignment = macdh_dir * float(np.sign(trend_bias))  # +1 confirms, −1 contradicts
        momentum_confirmation = 0.5 + 0.5 * alignment   # [0, 1]
    ...
    if blend_w > 0:
        ...
        # CONVICTION / REJECTION now blend toward a direction-aware target
        # (0 or 100) instead of scaling distance from 50. The prior
        # multiplicative formula 50 + (score-50)*(1+deviation) was
        # asymmetric: bullish vol on a below-50 score amplified bearishness,
        # clustering scores at 0.  Blend gives symmetric behavior across
        # all four quadrants (bullish/bearish vol × above/below 50).
        # THIN_AIR keeps original dampening (vol_mult<1 → pull toward 50).
            ...
            if vol_sig == 'CONVICTION':
                if (vol_dir == 1 and macd < 40 and weighted_sum < 65) or \
                   (vol_dir == -1 and macd > 60 and weighted_sum > 35):
                    mag_eff *= 0.25
            bw = min(mag_eff * 0.55 * vol_boost, 0.60)
    ...
    weight_info = {
        ...
        'mc':    round(momentum_confirmation, 3),
