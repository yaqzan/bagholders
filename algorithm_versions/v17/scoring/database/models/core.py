...
def reapply_regime_today(regime_multiplier, regime_composite=None, target_date=None):
    ...
    for s in rows:
        ...
        if regime_multiplier is not None and regime_multiplier != 1.0:
            if pre >= 50:
                adjusted = 50 + (pre - 50) * regime_multiplier
            else:
                adjusted = 50 + (pre - 50) * (2.0 - regime_multiplier)
...
class Stock(BaseModel):
    ...
    def recalculate_scores_batched(self, cutoff, components, cutoff_end=None, force=False,
        ...
        if pending_saves:
            # Score has a composite PK (symbol, date, version), so Peewee's
            # bulk_update doesn't apply. Instead, wrap all per-row saves in a
            # single transaction — this eliminates per-row autocommit overhead
            # (the dominant cost when there are 1000+ UPDATEs per stock).
            from database.trader_database import DB
            try:
                with DB.atomic():
                ...
                # Transaction rolled back — retry per-row outside the txn so
                # one bad row doesn't lose the whole stock's work.
                print(Fore.YELLOW + f"  {self.symbol}: txn save fell back to per-row ({e})")
                for s in pending_saves:
                    try:
                        s.save()
