...
class BacktestTemporalStats(BaseModel):
    ...
    Rebuilt by `trader assess --force` after the 1y window completes."""
...
class Stock(BaseModel):
    ...
    def calculate_indicators(self, full=False, weekly=False, show_progress=False, silent=False):
        ...
        rsi_values = talib.RSI(close_arr, timeperiod=14)
        fastk, fastd = talib.STOCHRSI(close_arr, timeperiod=14, fastk_period=3, fastd_period=3, fastd_matype=0)
        ...
        upperband, middleband, lowerband = talib.BBANDS(close_arr, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)
        obv_values = talib.OBV(close_arr, np.array(volumes))
        rsi_ma = talib.SMA(rsi_values, timeperiod=9)
        rsi_ema = talib.EMA(rsi_values, timeperiod=9)
        ma_9 = talib.SMA(close_arr, timeperiod=9)
        ema_9 = talib.EMA(close_arr, timeperiod=9)
        ma_21 = talib.SMA(close_arr, timeperiod=21)
        ema_21 = talib.EMA(close_arr, timeperiod=21)
        ma_50 = talib.SMA(close_arr, timeperiod=50)
        ema_50 = talib.EMA(close_arr, timeperiod=50)
        ma_200 = talib.SMA(close_arr, timeperiod=200)
        ema_200 = talib.EMA(close_arr, timeperiod=200)

        # Determine which indices to write. full=True writes everything;
        # full=False walks descending and stops at the first date already in DB
        # (inclusive) — matches the old per-row "break on first existing" semantic.
        n = len(close_prices)
        if full:
            ...
            existing_dates = set(
                row.date for row in
                IndicatorModel.select(IndicatorModel.date).where(IndicatorModel.symbol == self.symbol)
            )
            indices = []
        ...
        if rows:
            IndicatorModel.bulk_build(self.symbol, rows)
    ...
    def calculate_scores_batched(self, silent=False, regime_multiplier=None, regime_composite=None):
        ...
        # Earnings suppression set — call_time-aware (replaces legacy ±1-cal-day).
        # See volume_amplifier.compute_earnings_suppressed_dates for semantics.
        from volume_amplifier import compute_earnings_suppressed_dates
        _earnings_rows = list(
            EarningsDate.select(EarningsDate.date, EarningsDate.call_time)
            .where(EarningsDate.symbol == self.symbol)
            .tuples()
        )
        earnings_set = compute_earnings_suppressed_dates(_earnings_rows, ph_dates_asc)
    ...
    def recalculate_scores_batched(self, cutoff, components, cutoff_end=None, force=False,
                                   ...
                                   regime_map=None, spy_wk_map=None):
        """
        ...
        _ph_dates_asc_for_earnings = [p.date for p in ph_rows_asc]
        earnings_set = compute_earnings_suppressed_dates(_earnings_rows, _ph_dates_asc_for_earnings)
