...
class StockContext:
    ...
    def __init__(self, stock, ind_rows, ph_rows_asc, ws_rows, earnings_rows):
        """earnings_rows: iterable of (date, call_time_str_or_None) tuples."""
        ...
        self.earnings_set = compute_earnings_suppressed_dates(earnings_rows, ph_dates_asc)
...
class ScoreSimulator:
    ...
    def _load(self, symbols: Optional[List[str]]):
        ...
        for r in all_ed:    ed_by_sym[r.symbol].append((r.date, r.call_time))
        ...
        for sym in sym_list:
            ...
            ctx = StockContext(
                ...
                earnings_rows = ed_by_sym[sym],         # [(date, call_time), ...]
            )
...
def run(symbols=None, days=365, do_assess=False, do_compare=False, do_diff_assess=False,
    ...
    if do_diff_assess:
        ...
        sim.diff_assess(scores, symbol=sym)
    if do_assess:
