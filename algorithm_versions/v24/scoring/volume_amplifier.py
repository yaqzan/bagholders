...
# US market hours in minutes-since-midnight (ET):
#   open  = 09:30 ET  →  570
#   close = 16:00 ET  →  960
_MARKET_OPEN_MIN  = 9 * 60 + 30
_MARKET_CLOSE_MIN = 16 * 60


def _classify_call_time(call_time_str):
    """Classify EarningsDate.call_time (HH:MM:SS ET) as BMO/AMC/INTRADAY/UNKNOWN.

    BMO      : call_time strictly before 09:30 ET (earnings released pre-market;
               opening session reflects the news → suppress earnings_date itself)
    AMC      : call_time at/after 16:00 ET (earnings released after close;
               next trading session reflects the news → suppress next trading day)
    INTRADAY : 09:30-16:00 ET (rare; treat like BMO since today's volume is affected)
    UNKNOWN  : null/unparseable → conservative {earnings_date, next trading day}
    """
    if call_time_str is None:
        return 'UNKNOWN'
    s = str(call_time_str).strip()
    if not s:
        return 'UNKNOWN'
    try:
        parts = s.split(':')
        ...
        return 'UNKNOWN'
    if total < _MARKET_OPEN_MIN:
        return 'BMO'
    if total >= _MARKET_CLOSE_MIN:
        return 'AMC'
    return 'INTRADAY'


def compute_earnings_suppressed_dates(earnings_rows, trading_dates):
    """Expand earnings events into trading-day-aware volume suppression set.

    Replaces the legacy ±1-calendar-day window which (a) wrongly suppressed the
    day BEFORE earnings (a clean trading session), and (b) didn't carry AMC
    earnings to the next trading day across weekends/holidays.

    Args:
        earnings_rows: iterable of (date, call_time_str_or_None) tuples or rows
                       with .date and .call_time attributes.
        trading_dates: sorted list of trading dates (e.g. from PriceHistory).
                       Required to expand AMC events to the next trading session.

    Returns:
        Set of trading dates whose volume signal should be neutralized.
    """
    if not earnings_rows or not trading_dates:
        return set()
    ...
    for row in earnings_rows:
        if isinstance(row, tuple):
            ed_date, call_time = row[0], row[1] if len(row) > 1 else None
        else:
            ed_date = getattr(row, 'date', None)
            call_time = getattr(row, 'call_time', None)
        if ed_date is None:
            continue

        timing = _classify_call_time(call_time)

        if timing == 'BMO' or timing == 'INTRADAY':
            # Suppress only if earnings_date is itself a trading day.
            if ed_date in trading_set:
                suppressed.add(ed_date)
            else:
                # Earnings on weekend/holiday with BMO time tag (rare):
                # suppress the next trading session.
                idx = bisect.bisect_right(trading_dates, ed_date)
                if idx < len(trading_dates):
                    suppressed.add(trading_dates[idx])

        elif timing == 'AMC':
            # Suppress the FIRST trading day strictly after earnings_date.
            idx = bisect.bisect_right(trading_dates, ed_date)
            if idx < len(trading_dates):
                suppressed.add(trading_dates[idx])

        else:  # UNKNOWN
            # Conservative: suppress earnings_date (if trading day) AND the
            # next trading day after it.
            if ed_date in trading_set:
                suppressed.add(ed_date)
            idx = bisect.bisect_right(trading_dates, ed_date)
            if idx < len(trading_dates):
...
def _analyze(symbol, today, pulled_at):
    ...
    # Earnings suppression — call_time-aware (replaces legacy ±1 calendar-day window).
    # BMO/intraday on `today` → suppress today.
    # AMC on the most recent prior trading day → suppress today (carryover).
    # UNKNOWN call_time → conservative: suppress both cases above.
    # Earnings on a non-trading gap day (weekend/holiday) BMO/UNKNOWN → suppress
    # the next trading day, which is `today` if it's the first session past the gap.
    try:
        from database.models.core import EarningsDate
        from database.models.technical import PriceHistory

        # Today's earnings (any classification suppresses today, except AMC
        # which only affects the NEXT trading day).
        today_e = (EarningsDate
                   .select(EarningsDate.call_time)
                   .where((EarningsDate.symbol == symbol) & (EarningsDate.date == today))
                   .first())
        if today_e is not None:
            t = _classify_call_time(today_e.call_time)
            if t in ('BMO', 'INTRADAY', 'UNKNOWN'):
                return _neutral(earnings_suppressed=True)
            # AMC on today — earnings happen after close; today's volume is unaffected.

        # Previous trading day from PriceHistory; check earnings on that day
        # (AMC/UNKNOWN carry over) AND any earnings on calendar gap days
        # between it and today (weekend/holiday).
        prev_pd = (PriceHistory
        ...
        if prev_pd is not None:
            prev_e = (EarningsDate
                      .select(EarningsDate.call_time)
                      .where((EarningsDate.symbol == symbol) & (EarningsDate.date == prev_pd.date))
                      .first())
            if prev_e is not None:
                t = _classify_call_time(prev_e.call_time)
                if t in ('AMC', 'UNKNOWN'):
                    return _neutral(earnings_suppressed=True)
                # BMO/INTRADAY on the prior trading day affects only that day.

            # Earnings on calendar days strictly between prev_trading and today
            # (weekend/holiday). Any such event suppresses today (the first
            # trading day after the gap) regardless of call_time, because:
            #   - BMO on Saturday (rare): pre-market on Monday reflects it
            #   - AMC on Friday is handled above (via prev_pd path)
            #   - AMC on Saturday: Monday open reflects it
            gap_e_exists = (EarningsDate
                            .select(EarningsDate.call_time)
                            .where((EarningsDate.symbol == symbol) &
                                   (EarningsDate.date > prev_pd.date) &
                                   (EarningsDate.date < today))
                            .exists())
            if gap_e_exists:
                return _neutral(earnings_suppressed=True)
    except Exception:
...
def _analyze_from_cache(today, ph_list_desc, prior_vol_signals, earnings_suppressed_set):
    """
    ...
    earnings_suppressed_set:
        Set of trading dates whose volume signal should be neutralized due to
        earnings.  Built by compute_earnings_suppressed_dates() with full
        call_time semantics — see that helper for the BMO/AMC/INTRADAY/UNKNOWN
        handling.  Replaces the legacy ±1 calendar-day window which incorrectly
        suppressed the day BEFORE earnings (a clean trading session) and missed
        AMC carryover across weekends/holidays.
    """
    if not ph_list_desc:
        return _neutral()
    ...
    # Earnings suppression — set is precomputed with call_time semantics by
    # compute_earnings_suppressed_dates() at caller load time.
    if today in earnings_suppressed_set:
        return _neutral(earnings_suppressed=True)
...
def get_volume_multiplier_from_cache(today, ph_list_desc, prior_vol_signals,
                                     earnings_suppressed_set, pre_volume_overall=None):
    """
    ...
    earnings_suppressed_set: set of trading dates whose volume should be neutralized
        for earnings.  Build via compute_earnings_suppressed_dates() — handles
        BMO/AMC/INTRADAY/UNKNOWN call_time semantics with trading-day awareness.
    """
    try:
        sig = _analyze_from_cache(today, ph_list_desc, prior_vol_signals, earnings_suppressed_set)
