...
def get_all_stocks_with_scores():
    ...
    try:
        ...
        # Bulk-compute realized 60d vol per symbol so the StockTable can
        # de-normalize sigma-scaled MAE stops without per-row price fetches.
        realized_vol_by_sym = {}
        try:
            from assess_scores import _realized_vol_pct, SWING_VOL_LOOKBACK
        ...
        for score in scores_list:
            ...
            result.append({
                ...
                'next_earnings': (
                    (next_earnings_by_sym[score.symbol.symbol] - today).days
                    if score.symbol.symbol in next_earnings_by_sym
                ...
                'realized_vol_60d': realized_vol_by_sym.get(score.symbol.symbol),
            })
...
def get_assessment():
    ...
    try:
        ...
        BUCKET_ORDER = ['95+', '90+', '85+', '80+', '75+', '70+', '<25', '<15', '<5']
...
def simulate_trader():
    ...
    try:
        ...
        # Merged 85+ tier allocation (CLAUDE.md alloc_sweep_85plus validated)
        TIER_ALLOC = {
            '85+':   0.15,
            '80-84': 0.12,
            '75-79': 0.12,
        ...
        # Deterministic tiebreak: score desc, then symbol asc
        for d in outcomes_by_date:
            outcomes_by_date[d].sort(key=lambda o: (-o['score'], o['symbol']))
