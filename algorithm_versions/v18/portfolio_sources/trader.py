...
def _cmd_tp_stop(command, args):
    """
    trader tp   <price>   — ideal take-profit price  (entry × 1.3)
    trader stop <price>   — ideal stop-loss price     (entry × 0.75)

    Price is the entry/current stock price (not option premium).
    Multipliers are from the MC-validated optimal strategy:
        TP   = entry × 1.30  (option gains ~+30% when underlying hits +1.092σ)
        Stop = entry × 0.75  (option loses ~-25% when underlying drops -0.910σ)
    """
    ...
    if not args:
        if command == 'tp':
            print("Usage: trader tp <price>")
            print("Example: trader tp 145.50")
        else:
            print("Usage: trader stop <price>")
            print("Example: trader stop 12.5")
        return

    try:
        price = float(args.strip().replace(',', ''))
    except ValueError:
        print(f"{Fore.RED}Invalid price: {args!r}{Style.RESET_ALL}")
        return
    ...
    if command == 'tp':
        ...
        t32 = price * 1.32
        t35 = price * 1.35
        print(f"{Fore.GREEN}${t30:.2f} (+30%)\n${t32:.2f} (+32%)\n${t35:.2f} (+35%){Style.RESET_ALL}")
    else:
        t20 = price * 0.80
        t25 = price * 0.75
        print(f"{Fore.RED}${t20:.2f} (-20%){Style.RESET_ALL}")
        print(f"{Fore.RED}${t25:.2f} (-25%){Style.RESET_ALL}")
...
def _cmd_alloc(args):
    ...
    Prints today's call signals (score >= 75) with cascade allocation amounts in USD,
    using the optimal MC-validated strategy:
        85+      → 15% of portfolio  (merged tier — 85-89 EV exceeds 90-94)
        80-84    → 12%
        75-79    → 12%
        70-74    →  5% (overflow — fills idle slots after 75+ signals)
    Max 10 concurrent positions. Amounts are in USD (CAD converted at live rate).
    """
    from colorama import Fore, Style, init as colorama_init
    colorama_init()
    ...
    signals = list(scores)
    if not signals:
        print(f"No signals >= 75 for {latest_date}.")
        return

    # --- cascade allocation ---
    TIERS = [
        (85, float('inf'), 0.15, '85+'),    # merged — 85-89 EV exceeds 90-94
        (80, 85,           0.12, '80-84'),
        (75, 80,           0.12, '75-79'),
        (70, 75,           0.05, '70-74'),  # overflow tier: fills idle slots after 75+
    ]
    MAX_POSITIONS = 10

    def get_tier(score):
        for lo, hi, pct, label in TIERS:
            if lo <= score < hi:
                return pct, label
        return 0.12, '75-79'

    # fetch 70-74 overflow signals too
    overflow_signals = list(
        Score.select()
        .where(Score.version == version,
               Score.date == latest_date,
               Score.overall >= 70,
    ...
    all_signals = signals + overflow_signals  # 75+ first, then 70-74
    ...
    print(f"  Signals date: {latest_date}  |  30 DTE ATM calls, TP=+30%, SL=-25%, hard sell day 15")
    print()
    print("  Allocation guideline:")
    for lo, hi, pct, label in TIERS:
        amt = portfolio_usd * pct
        suffix = "  (overflow — fills idle slots)" if label == '70-74' else ""
        print(f"    {label + ':':<8}  ${amt:>8,.0f} USD{suffix}")
    print()

    # signal table
    print(f"  {'SYMBOL':<8}  {'SCORE':>5}  {'USD':>10}")
    print(f"  {'-'*28}")
    ...
    for s in all_signals:
        ...
        pct, label = get_tier(score)
        cost_usd = portfolio_usd * pct
        if cost_usd > cash_remaining:
            continue
        ...
        sym = s.symbol_id if hasattr(s, 'symbol_id') and isinstance(s.symbol_id, str) else str(s.symbol)
        print(f"  {color}{sym:<8}  {score:>5.1f}  {f'${cost_usd:,.0f}':>10}{Style.RESET_ALL}")
    ...
    print(f"  {'Deployed':<8}       {f'${deployed:,.0f}':>10}")
    print(f"  {'Remaining':<8}       {f'${cash_remaining:,.0f}':>10}")
    n75 = len(signals)
    n70 = len(overflow_signals)
    print(f"\n  {Fore.CYAN}{positions} position(s) from {n75} signal(s) >= 75"
          f"  +  {n70} overflow signal(s) 70-74{Style.RESET_ALL}")
    print()
...
def _cmd_backtest(args):
    ...
    Portfolio options:
      ...
      --capital   N            starting capital, < 500 treated as thousands
                               e.g. 25 → $25,000  (default 25)
    ...
    def _outcome(symbol, signal_date, score, ph_rows):
    ...
    def _put_outcome(symbol, signal_date, score, ph_rows):
    ...
    n_skipped = 0
    for sig in raw:
        o = _outcome(sig.symbol, sig.date, float(sig.overall), ph.get(sig.symbol, []))
        if o is None:
            n_skipped += 1
        else:
            outcomes_by_date[sig.date].append(o)
    for sig in put_raw:
        o = _put_outcome(sig.symbol, sig.date, float(sig.overall), ph.get(sig.symbol, []))
        if o is None:
            n_skipped += 1
        else:
            outcomes_by_date[sig.date].append(o)
    # Sort per date: calls first (score desc), then puts (score asc — most negative first)
    for d in outcomes_by_date:
        outcomes_by_date[d].sort(
            key=lambda o: (0 if o.get('side', 'call') == 'call' else 1,
                           -o['score'] if o.get('side', 'call') == 'call' else o['score'],
                           o['symbol'])
        )
    ...
    for d in all_dates:
        ...
        for o in outcomes_by_date.get(d, []):
            ...
            premium = TIER_ALLOC[o['tier']] * equity
    ...
    print(f'  Capital ${capital:,.0f}  ·  Signals ≥ {min_score_arg:.0f}  ·  Max {_marker(MAX_POSITIONS, 14)} positions')
    print(f'  TP {tp_mode}  '
          f'SL {sl_mode}  '
    ...
    print(f'  Tiers: 85+→15%  80-84→12%  75-79→12%  70-74→5%'
    ...
    print(f'  Current equity:    {eq_color}${final_equity:>12,.2f}  '
    ...
    print(f'  Call cascade 85+→15%  80-84→12%  75-79→12%  70-74→5%  ·  '
    ...
    if len(equity_curve) >= 2:
        ...
        if hi > lo:
            ...
            print(f'  {" " * 13}{start_lbl}{" " * gap}{end_lbl}')
    print()
...
def main(command=None, args=None):
    ...
    if command == 'pull' and args:
        ...
        if days_explicit is not None:
            ...
            window_label = f"{num_years}yr ({start_date} → {today})"
        ...
        if do_force:
            ...
            _ASSESS_WINDOWS = [('1y', 365), ('2y', 730), ('3y', 1095), ('5y', 1825)]
        ...
        # All windows up to 5y, run automatically when no explicit lookback given
        _ASSESS_WINDOWS = [('1y', 365), ('2y', 730), ('3y', 1095), ('5y', 1825)]
