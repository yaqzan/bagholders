...
# Cascade allocation per score tier (CLAUDE.md — 85+ merged, 80-84/75-79 at 12%)
TIER_ALLOC = {
    '85+':   0.15,   # 85-89 and 90+ merged (85-89 EV > 90-94; v17 validated)
    '80-84': 0.12,
    '75-79': 0.12,
    '70-74': 0.05,
...
def score_to_tier(score: float) -> str:
    if score >= 85:  return '85+'    # 85-89 and 90+ merged
...
def compute_outcome(symbol: str, signal_date: date, score: float,
                    ph_rows: list, stressed: bool) -> 'TradeOutcome | None':
...
def compute_put_outcome(symbol: str, signal_date: date, score: float,
                        ph_rows: list) -> 'TradeOutcome | None':
...
def run_backtest(outcomes_by_date: dict,
    ...
    for today in trading_days:
        ...
        for outcome in outcomes_by_date.get(today, []):
            ...
            premium = TIER_ALLOC[outcome.tier] * equity
...
def main():
    ...
    for sig in put_raw:
        rows    = ph.get(sig.symbol, [])
        outcome = compute_put_outcome(sig.symbol, sig.date, float(sig.overall), rows)
        if outcome is None:
            n_skipped += 1
            continue
        n_put_outcomes += 1
    ...
    #   Calls first (by score desc), then puts (by score asc), then symbol asc.
    for d in outcomes_by_date:
        outcomes_by_date[d].sort(
            key=lambda o: (0 if o.side == 'call' else 1,
                           -o.score if o.side == 'call' else o.score,
                           o.symbol)
        )
