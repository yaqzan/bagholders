...
ETF_SYMBOLS = {
    ...
    'LABD', 'QQQ', 'SLV', 'SMH', 'SOXL', 'SPY', 'SVIX', 'TLT',
    'TNA', 'TQQQ', 'UFO', 'URA', 'XLC', 'XLE', 'XLF', 'XLP',
    'XLU',
}
...
def _cmd_tp_stop(command, args):
    ...
    _tp_opt = _scfg_tp.STRATEGY_30DTE.option
...
def _cmd_alloc(args):
    ...
    at runtime, so these auto-update when the dataclass is edited:
        30 DTE (Phase 7 monotonic, shipped 2026-05-01):
            Calls  95+:20%  85-94:15%  80-84:12%  75-79:10%  70-74:0% (disabled)
            Puts   <=15:10%  16-20:12%  21-25:12%
        15 DTE C1 (Phase 15B, asymmetric — separate optimization path):
            Calls  95+:18%  85-94:12%  80-84:15%  75-79:15%  70-74:0%
            Puts   <=15:10%  16-20:12%  21-25:12%
    ...
    DD_CIRCUIT_BREAKER = _alloc_cfg.DD_CIRCUIT_BREAKER
        ...
        while i < len(tokens):
            ...
            if tok == '--from' and i + 1 < len(tokens):
                ...
                if _dte_strategy_local == '15':
                    # Switch defaults to 15 DTE C1. TP/SL on premium are aliased
                    # via SHARED_OPTION (same TP=0.35/SL=-0.30 on both DTEs).
