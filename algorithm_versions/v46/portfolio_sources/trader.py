...
def _cmd_alloc(args):
    ...
    # SAW Put U-curve display (shipped 2026-05-08, portfolio-stage)
    if getattr(_alloc_cfg, 'SAW_PUT_UCURVE_ENABLED', False):
        ...
        print(f"    SAW Put U-curve: {Fore.GREEN}ENABLED{Style.RESET_ALL}   "
              f"({_alloc_cfg.SAW_PUT_UCURVE_SHAPE} mid={_saw_mid:.0f} hw={_saw_hw:.0f} "
              f"floor={_saw_fl:.2f} ceil={_saw_ce:.2f} power={_saw_pk:.1f}; "
              f"sector-breadth dampener on put alloc, contracts in 60-90 bad zone, amplifies at extremes)")
    else:
