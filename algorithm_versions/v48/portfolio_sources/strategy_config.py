...
class ScoringConfig:
    ...
    SCW_MAX_PENALTY: float        # 15.0
    SCW_STOCH_POWER: float        # 1.0
    SCW_DECAY_POWER: float        # 4.0
    SCW_WEEKLY_HI: float          # 18.0

    # ── Continuation boost (v33 ship 2026-05-03) ─────────────────────────
    CONT_BOOST_ENABLED: bool      # True
    CONT_BOOST_SIG_MIN: float     # 0.20  — gate on prior_signal
    CONT_BOOST_TAU: float         # 40.0  — exp decay on gap days
    CONT_BOOST_MAG_EXP: float     # 0.70  — magnitude exponent
    CONT_BOOST_SIG_NORM: float    # 3.0   — final tanh divisor
    CONT_BOOST_GATE_LO: int       # 70
    CONT_BOOST_GATE_HI: int       # 74
    CONT_BOOST_PROMOTE_TARGET: int  # 75    — lift to exactly this
...
SCORING = ScoringConfig(
    ...
    SCW_ENABLED=True,
    SCW_GATE_CALL=70,
    SCW_MAX_PENALTY=15.0,
    SCW_STOCH_POWER=1.0,
    SCW_DECAY_POWER=4.0,
    SCW_WEEKLY_HI=18.0,
    # Continuation boost
    CONT_BOOST_ENABLED=True,
    CONT_BOOST_SIG_MIN=0.20,
    CONT_BOOST_TAU=40.0,
    CONT_BOOST_MAG_EXP=0.70,
    CONT_BOOST_SIG_NORM=3.0,
    CONT_BOOST_GATE_LO=70,
    CONT_BOOST_GATE_HI=74,
