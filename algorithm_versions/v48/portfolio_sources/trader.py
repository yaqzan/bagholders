...
class Trader():
    ...
    def update_stocks(self, full=False, with_options=False, only_remainder=False,
        ...
        # Order by whether the stock already has a complete score for today under
        # the active algorithm version.  Stocks with no valid today's score sort
        # first (COALESCE → epoch); stocks with a score sort by when it was last
        # written, oldest first.  This guarantees that each partial run (killed
        # before completion) makes forward progress — newly-unscored stocks always
        # rise to the top rather than being pushed to the end by a fresh pulled_at.
        from database.models.core import AlgorithmVersion as _AV
        _active_ver = _AV.get_active_scores_version()
        _score_today = (
            Score
            .select(Score.symbol, Score.updated_at)
            .where(
                Score.version == _active_ver,
        ...
        stocks = (
            Stock
            .select(Stock.symbol)
            .join(_score_today, JOIN.LEFT_OUTER, on=(Stock.symbol == _score_today.c.symbol))
            .order_by(fn.COALESCE(_score_today.c.updated_at, datetime(1970, 1, 1)))
        )
        if only_delisted:
        ...
        # Experimental sidecar: write today-only sector ETF shadow scores after
        # production scores and any same-day regime reapply are finalized.
        try:
            from sector_etf_shadow import write_today_sector_etf_shadow_scores
            write_today_sector_etf_shadow_scores(target_date=date.today(), verbose=True)
        except Exception as e:
            print(Fore.YELLOW + f"Sector ETF shadow scores skipped: {e}")
...
def _cmd_alloc(args):
    """
    trader alloc <portfolio_cad> [--strategy 30dte|15dte] [--shadow sector-etf]
    ...
    # --- parse args: portfolio value [--strategy 30dte|15dte] [--shadow sector-etf] ---
    if not args:
        print("Usage: trader alloc <portfolio_value_cad> [--strategy 30dte|15dte] [--shadow sector-etf]")
        print("Example: trader alloc 50000")
        print("Example: trader alloc 50000 --strategy 15dte")
        print("Example: trader alloc 50000 --shadow sector-etf")
        return
    ...
    while i < len(tokens):
        ...
        if tok == '--strategy':
            ...
            if i + 1 >= len(tokens):
                print(f"{Fore.RED}--shadow requires a value (sector-etf){Style.RESET_ALL}")
                return
            shadow_variant = tokens[i + 1].lower()
            if shadow_variant != 'sector-etf':
                print(f"{Fore.RED}--shadow must be sector-etf, got: {shadow_variant}{Style.RESET_ALL}")
                return
            i += 2
        else:
            portfolio_arg = tok
    ...
    if shadow_variant == 'sector-etf':
        from sector_etf_shadow import get_sector_etf_shadow_version
        shadow_version = get_sector_etf_shadow_version(active_version, create=False)
        if shadow_version is None:
            print(Fore.RED + "No sector-etf shadow score version found. Run 'trader update' first.")
            return
    ...
    if shadow_variant == 'sector-etf':
        ...
        if active_latest and latest_date != active_latest:
            print(Fore.YELLOW + f"Warning: sector-etf shadow date {latest_date} differs from active date {active_latest}")
        active_count = (Score.select()
                        .where(Score.version == active_version, Score.date == latest_date)
                        .count())
        shadow_count = (Score.select()
                        .where(Score.version == version, Score.date == latest_date)
                        .count())
        if shadow_count < active_count:
            print(Fore.YELLOW + f"Warning: sector-etf shadow coverage {shadow_count}/{active_count} for {latest_date}")
    ...
    if shadow_variant == 'sector-etf':
        print(f"  {Fore.MAGENTA}SCORING: sector-etf SHADOW v{version.id} based on active v{active_version.id}; dashboard remains active scoring{Style.RESET_ALL}")
    if is_15dte:
    ...
    if getattr(_alloc_cfg, 'SAW_PUT_UCURVE_ENABLED', False):
        ...
        if _saw_ce > 1.0:
            _saw_eff = (f"sector-breadth dampener on put alloc; contracts to {_saw_fl:.2f}× "
                        f"in mid-zone bad zone (~{_saw_mid:.0f}±{_saw_hw:.0f}), "
                        f"amplifies to {_saw_ce:.2f}× at extremes (mean-reversion alpha)")
        else:
            _saw_eff = (f"sector-breadth dampener on put alloc; PURE CONTRACTION "
                        f"to {_saw_fl:.2f}× in mid-zone (~{_saw_mid:.0f}±{_saw_hw:.0f}), "
    ...
    if _alloc_cfg.DEAD_HOLD_ENABLED:
        ...
        # the OPTION must reach for popout to fire. e.g. -25% pnl = 0.75× entry.
        ...
        print(f"                  Stop-limit sell at {_dh_p_mult:.2f}× ENTRY premium "
