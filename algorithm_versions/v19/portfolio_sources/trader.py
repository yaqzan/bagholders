...
def _cmd_alloc(args):
    ...
    using the optimal MC-validated strategy. Calls fill slots first; puts fill
    remaining slots.
    ...
    Max 14 concurrent positions (shared pool). Amounts in USD (CAD converted at live rate).
    """
    from colorama import Fore, Style, init as colorama_init
    colorama_init()
    ...
    print(f"  Signals date: {latest_date}  |  30 DTE  "
          f"Calls TP=+30% SL=-35%  |  Puts TP=+30% SL=-20%  |  hard sell day 15")
    print()

    # allocation tiers summary
    print(f"  {'CALL tiers:':<22}  {'PUT tiers:'}")
    call_lines = [(f"{label}:", portfolio_usd * pct,
                   "  (ultra)" if label == '95+' else
                   "  (disabled)" if label == '70-74' else "")
                  for _, _, pct, label in CALL_TIERS]
    put_lines  = [(f"{label}:", portfolio_usd * pct, "")
