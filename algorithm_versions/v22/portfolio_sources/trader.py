...
def _cmd_alloc(args):
    ...
    CASCADE (% of portfolio equity, pre-regime):
    ...
    REGIME SCALING (applied here to $ amounts):
        Calls  slope_up=0.0  slope_down=1.0   → no BULL boost, full STRESS cut
        Puts   slope_up=-0.5 slope_down=0.0   → mild BULL cut, unchanged in STRESS
        scale = clamp(1.0 + slope*(regime_mult-1), 0.25, 1.75)
    ...
    # --- regime-aware scaling (asymmetric CUT_ONLY + put BULL cut, Phase 13+15) ---
    SLOPE_CALL_UP, SLOPE_CALL_DOWN = 0.0, 1.0    # calls: no BULL boost, full STRESS cut
    SLOPE_PUT_UP,  SLOPE_PUT_DOWN  = -0.5, 0.0   # puts:  mild BULL cut, STRESS unchanged
    SCALE_FLOOR, SCALE_CEIL = 0.25, 1.75

    # today's breadth + regime
    ...
    def _scale(slope_up, slope_down):
        slope = slope_up if regime_mult >= 1.0 else slope_down
        return max(SCALE_FLOOR, min(SCALE_CEIL, 1.0 + slope * (regime_mult - 1.0)))
    call_scale = _scale(SLOPE_CALL_UP, SLOPE_CALL_DOWN)
    put_scale  = _scale(SLOPE_PUT_UP,  SLOPE_PUT_DOWN)
    ...
    print(f"  {Fore.WHITE}{Style.BRIGHT}GUIDELINE (v18){Style.RESET_ALL}")
    print(f"    Breadth:      {brd_col}{brd_txt}{Style.RESET_ALL}"
          f"    Regime mult:  {reg_col}{regime_mult:.3f} ({reg_lbl}){Style.RESET_ALL}")
    ...
    print(f"    Regime scale: calls x{call_scale:.2f}   puts x{put_scale:.2f}"
          f"   (calls cut in STRESS, puts cut in BULL)")
    print(f"    Fill order:   calls first by |score-50|, then puts; same-symbol block across sides")
    print()

    # allocation tiers summary (post-regime scaling, as actually deployed below)
    print(f"  {'CALL tiers (x' + f'{call_scale:.2f}' + '):':<22}  {'PUT tiers (x' + f'{put_scale:.2f}' + '):'}")
...
def _cmd_backtest(args):
    ...
    regime_map = {}  # date → regime_multiplier
    try:
        rrows = list(
