...
def compute_overall_score(
    ...
    *,
    ...
    # ── v22: X-confidence gate on d (X_CONF_ALPHA=0.50, 5-component inputs) ──
    # When base components disagree (e.g. TREND extreme bearish but BB/RSI/MACD/
    # Stoch bullish — the PSKY 3/20 case), trend dominance should not get its
    # full structural weight. X_conf collapses to 0 when components cancel and
    # to 1 when they agree strongly; raising d to power alpha gates trend's
    # weight contribution without touching any other component.
    # Validated 5y diff-assess vs v21 (experiments/x_conf_counter_trend.py):
    # CT-PUT bucket (overall<=25, trend>=70) N 541→934 (+73%) WR15 79.5→81.9%
    # on 22-now; same direction in every annual window. Naturally absorbs the
    # counter-trend amplifier hypothesis from CLAUDE.md item #11.
    X_CONF_ALPHA = 0.50
    _xc_comps = (trend, bb, rsi, macd, stoch)
    _xc_bull = sum(1 for _s in _xc_comps if _s >= 50)
    _xc_n    = len(_xc_comps)
    _xc_agreement = max(_xc_bull, _xc_n - _xc_bull) / _xc_n
    _xc_avg_dist = sum(_s - 50 for _s in _xc_comps) / _xc_n
    _xc_sig_str  = min(abs(_xc_avg_dist) / 50.0, 1.0)
    x_conf = _xc_sig_str * (_xc_agreement ** 2)   # ∈ [0, 1]
    d_eff  = d * (x_conf ** X_CONF_ALPHA)

    # Weights use d_eff (gated trend dominance) instead of raw d.
    w_trend = 18 + 10 * d_eff
    w_bb    = 18
    w_rsi   = 25 -  9 * d_eff
    w_macd  = 25 -  6 * d_eff
