...
def _cmd_alloc(args):
    ...
    def _sector_breadth_on_or_before(d):
        """Sector ETF breadth used by the SAW put allocation scaler."""
        from pathlib import Path
        ...
        path = Path(__file__).resolve().parent / '.cache' / 'sector_etf_screen' / 'sector_breadth_daily_2020plus.csv'
        if not path.exists():
            return None, None
        ...
        idx = _bisect.bisect_right(dates, d) - 1
        if idx < 0:
            return 50.0, None
    ...
    def _saw_put_ucurve_scale(d):
        ...
        if not getattr(_alloc_cfg, 'SAW_PUT_UCURVE_ENABLED', False):
            return 1.0, None, None

        sec_brd, sec_brd_date = _sector_breadth_on_or_before(d)
        if sec_brd is None:
            return 1.0, None, None

        floor = _alloc_cfg.SAW_PUT_UCURVE_FLOOR
        ceil = _alloc_cfg.SAW_PUT_UCURVE_CEIL
        if _alloc_cfg.SAW_PUT_UCURVE_SHAPE == 'sigmoid':
            lo_thresh = _alloc_cfg.SAW_PUT_UCURVE_MIDPOINT - _alloc_cfg.SAW_PUT_UCURVE_HALFWIDTH
            hi_thresh = _alloc_cfg.SAW_PUT_UCURVE_MIDPOINT + _alloc_cfg.SAW_PUT_UCURVE_HALFWIDTH
            import math as _math
            try:
                sig_lo = 1.0 / (1.0 + _math.exp(-(lo_thresh - sec_brd) / max(0.5, _alloc_cfg.SAW_PUT_UCURVE_K)))
            except OverflowError:
                sig_lo = 0.0 if (lo_thresh - sec_brd) < 0 else 1.0
            try:
                sig_hi = 1.0 / (1.0 + _math.exp(-(sec_brd - hi_thresh) / max(0.5, _alloc_cfg.SAW_PUT_UCURVE_K)))
            except OverflowError:
                sig_hi = 0.0 if (sec_brd - hi_thresh) < 0 else 1.0
            activation = min(1.0, sig_lo + sig_hi)
            return floor + activation * (ceil - floor), sec_brd, sec_brd_date

        d_norm = abs(sec_brd - _alloc_cfg.SAW_PUT_UCURVE_MIDPOINT) / max(1.0, _alloc_cfg.SAW_PUT_UCURVE_HALFWIDTH)
        if d_norm > 1.0:
            d_norm = 1.0
        return floor + (d_norm ** _alloc_cfg.SAW_PUT_UCURVE_POWER) * (ceil - floor), sec_brd, sec_brd_date
    ...
    print(f"    Cost basis:   calls=tier % x portfolio x F3f; puts=tier % x portfolio x F3f x SAW")
    ...
    put_scale_label += "):"
    print(f"  {'CALL tiers (F3f x' + f'{call_scale:.2f}' + '):':<22}  {put_scale_label}")
    ...
    put_lines  = [(f"{label}:", portfolio_usd * pct * effective_put_scale, "")
    ...
    if put_signals:
        ...
        for s in put_signals:
            ...
            cost_usd = portfolio_usd * pct * effective_put_scale
            if cost_usd <= 0 or cost_usd > cash_remaining:
                continue
