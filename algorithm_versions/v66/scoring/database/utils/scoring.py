...
WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS = _envi('WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS', _S.WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS)
...
def compute_overall_score(
    ...
    *,
    ...
    bars_this_week=None,
    weekly_volume_fraction=None,
    avg_bars_per_week=None,
    ...
    **_extra,
    ...
    if None not in (ws_rsi, ws_macd):
        _derived_bars, _derived_avg_bars = weekly_momentum_evidence_from_date(score_date)
        _bars_this_week = bars_this_week if bars_this_week is not None else _derived_bars
        _avg_bars_per_week = avg_bars_per_week if avg_bars_per_week is not None else _derived_avg_bars
        _weekly_volume_fraction = weekly_volume_fraction
        if _bars_this_week >= _avg_bars_per_week:
            _weekly_volume_fraction = None
        adj, weekly_detail = calculate_weekly_adjustment(
            ws_trend, ws_rsi, ws_macd,
            prev_ws_trend, prev_ws_rsi, prev_ws_macd,
            bars_this_week=_bars_this_week,
            volume_fraction=_weekly_volume_fraction,
            avg_bars_per_week=_avg_bars_per_week,
        )
        weighted_sum += adj
...
def weekly_momentum_confidence(
    bars_this_week: int,
    volume_fraction: float = None,
    avg_bars_per_week: int = WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS,
) -> float:
    """
    Cosine evidence ramp for week-over-week momentum. Base weekly bias is not
    scaled by this confidence.
    """
    avg_bars = max(1.0, float(avg_bars_per_week or WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS))
    bar_progress = min(1.0, max(0.0, float(bars_this_week or 0)) / avg_bars)

    if volume_fraction is not None:
        vol_progress = min(1.0, max(0.0, float(volume_fraction)))
        progress = math.sqrt(bar_progress * vol_progress)
    else:
...
def effective_w_adj(
    ...
    """
    Evidence-weighted weekly adjustment before existing downstream weekly waves.
    """
    confidence = weekly_momentum_confidence(
        bars_this_week,
        volume_fraction=volume_fraction,
        avg_bars_per_week=avg_bars_per_week,
    )
    return float(w_base_bias) + (float(w_mom) * confidence)


def weekly_momentum_evidence_from_date(score_date):
    """
    Return (bars_elapsed_this_week, expected_bars_this_week) using the repo's
    NYSE holiday calendar. If no date is supplied, preserve legacy full weight.
    """
    if score_date is None:
        full_week = int(WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS)
        return full_week, full_week

    week_start = score_date - timedelta(days=score_date.weekday())
    try:
    ...
    week_days = [
        ...
        for i in range(int(WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS))
    ]
    expected_bars = sum(1 for d in week_days if is_trading_day(d))
    bars_elapsed = sum(1 for d in week_days if d <= score_date and is_trading_day(d))
    expected_bars = max(1, expected_bars)
    bars_elapsed = max(0, min(expected_bars, bars_elapsed))
    return bars_elapsed, expected_bars
...
def calculate_weekly_adjustment(
    ...
    *,
    ...
    if bars_this_week is None:
        bars_this_week = int(WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS)
    if avg_bars_per_week is None:
        avg_bars_per_week = int(WEEKLY_MOMENTUM_MAX_CONFIDENCE_BARS)
    momentum_confidence = weekly_momentum_confidence(
        bars_this_week,
        volume_fraction=volume_fraction,
        avg_bars_per_week=avg_bars_per_week,
    )
    expressed_momentum_bias = momentum_bias * momentum_confidence
    raw_total = base_bias + momentum_bias
    total = effective_w_adj(
        base_bias,
        momentum_bias,
        bars_this_week,
        volume_fraction=volume_fraction,
    ...
    detail = {
        ...
        'wbase_bias': round(base_bias, 1),
        'w_mom': round(expressed_momentum_bias, 1),
        'wmom_raw': round(momentum_bias, 1),
        'wmom_confidence': round(momentum_confidence, 6),
        'wmom_bars': int(bars_this_week),
        'wmom_avg_bars': int(avg_bars_per_week),
        'w_adj': round(total, 1)
    }
    if volume_fraction is not None:
        detail['wmom_volume_fraction'] = round(float(volume_fraction), 6)
    if raw_total != envelope_total:
        detail['w_adj_pre_envelope'] = round(raw_total, 1)
        detail['w_adj_enveloped'] = round(envelope_total, 1)
    if put_wave_scale is not None:
        detail['w_adj_raw'] = round(envelope_total, 1)
