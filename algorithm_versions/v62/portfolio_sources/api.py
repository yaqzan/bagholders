...
# ---------------------------------------------------------------------------
TEMPORAL_PORTFOLIO_WINDOWS = [
    {
        'name': 'covid_crash_2020',
        'label': 'Mar 2020 crash',
        'start': '2020-01-01',
        'end': '2020-04-30',
        'primary_metric': 'max_drawdown',
        'description': 'Crash-window drawdown capture for the March 2020 regime.',
    },
    {
        ...
        'description': 'Crash year plus following bull year.',
    },
    {
        'name': '2020_now',
        'label': '2020-now',
        ...
        'end': None,
        'primary_metric': 'compound_and_drawdown',
        'description': 'Full path from the COVID crash setup to current temporal coverage.',
    },
    {
        'name': '22_now',
        'label': '22-now',
        ...
        'end': None,
        'primary_metric': 'compound_and_drawdown',
        'description': 'Existing production-equivalent compounding/DD window.',
    },
]
...
def _load_stress_window_pack(version, dte_strategy):
    """Return optional post-recalc pack stress-window rows keyed by name."""
    try:
        root = Path(os.getcwd())
        path = root / '.cache' / 'algorithm_versions' / f'v{int(version.id)}' / 'research_pack' / 'stress_windows.json'
        if not path.exists():
            return {}, None
...
def _research_pack_compare_row(version, active_version, dte_strategy='30'):
    ...
    stress = _read_research_pack_json(pack_dir / 'stress_windows.json') or {}
    ...
    if str(stress.get('dte_strategy') or dte_strategy) != str(dte_strategy):
        stress = {}

    windows = {}
    for row in stress.get('windows') or []:
...
def _active_portfolio_overlay_payload(active_version, dte_strategy='30'):
    ...
    try:
        ...
        run_dir = Path(os.getcwd()) / '.codex' / 'runs' / 'v62_g80_postwire_20260521_044902'
...
def _temporal_portfolio_windows(version, summary, yearly, monthly, dte_strategy):
    pack_rows, pack_path = _load_stress_window_pack(version, dte_strategy)
    windows = []
    ...
    return {
        'source': 'temporal_monthly_with_optional_research_pack',
        'pack_path': pack_path,
        'windows': windows,
    }
...
def compare_algorithm_versions():
    ...
    try:
        ...
        rows = [
            _research_pack_compare_row(version, active_version, dte_strategy=dte_strategy)
            for version in versions
        ]
        rows = [
            row for row in rows
            if row['generated_at'] or row['windows'] or row['utility_5y_wr15'].get('utility_wr') is not None
        ]
...
def backtest_temporal():
    ...
    try:
        ...
        return jsonify({
            ...
            'summary':     summary,
            'portfolio_windows': _temporal_portfolio_windows(ver, summary, yearly, monthly, dte_strategy),
...
def run_backtest_endpoint():
    ...
    Query params (advanced — all optional, defaults = optimal strategy):
      ...
      practical_exposure_enabled bool use v62 practical exposure layer (default true for 30 DTE)
