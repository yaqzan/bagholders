...
# MACD put wave controls. The default candidate keeps the old cliff's broad
# protection but restores partial MACD weight only for deep bearish alignment.
MACD_PUT_GATE_MODE          = _envs('MACD_PUT_GATE_MODE',     _S.MACD_PUT_GATE_MODE)
MACD_PUT_WAVE_CENTER        = _envf('MACD_PUT_WAVE_CENTER',   _S.MACD_PUT_WAVE_CENTER)
MACD_PUT_WAVE_WIDTH         = _envf('MACD_PUT_WAVE_WIDTH',    _S.MACD_PUT_WAVE_WIDTH)
MACD_PUT_WAVE_SETUP_POWER   = _envf('MACD_PUT_WAVE_SETUP_POWER', _S.MACD_PUT_WAVE_SETUP_POWER)
MACD_PUT_WAVE_MACD_WIDTH    = _envf('MACD_PUT_WAVE_MACD_WIDTH', _S.MACD_PUT_WAVE_MACD_WIDTH)
MACD_PUT_WAVE_SUPPRESS      = _envf('MACD_PUT_WAVE_SUPPRESS', _S.MACD_PUT_WAVE_SUPPRESS)
MACD_PUT_WAVE_BOOST         = _envf('MACD_PUT_WAVE_BOOST',    _S.MACD_PUT_WAVE_BOOST)
MACD_PUT_WAVE_MIN_FACTOR    = _envf('MACD_PUT_WAVE_MIN_FACTOR', _S.MACD_PUT_WAVE_MIN_FACTOR)
MACD_PUT_WAVE_MAX_FACTOR    = _envf('MACD_PUT_WAVE_MAX_FACTOR', _S.MACD_PUT_WAVE_MAX_FACTOR)
...
def _macd_put_weight_factor(pre_no_macd: float, macd: float) -> tuple[float, dict | None]:
    """Return MACD weight factor for bearish-side setups.

    factor=0 reproduces the shipped full suppression; factor=1 preserves the
    original dynamic MACD weight; factor>1 lets aligned bearish MACD reclaim
    budget from the non-MACD components.
    """
    mode = (MACD_PUT_GATE_MODE or "cliff").strip().lower()
    if mode in ("cliff", "binary", ""):
        return (0.0 if pre_no_macd < PUT_MACD_GATE else 1.0), None

    center = float(MACD_PUT_WAVE_CENTER)
    width = max(float(MACD_PUT_WAVE_WIDTH), 1e-6)
    setup = _sigmoid_width(center - float(pre_no_macd), 0.0, width)
    power = max(float(MACD_PUT_WAVE_SETUP_POWER), 1e-6)
    if power != 1.0:
        setup = setup ** power

    if mode in ("smooth", "smooth_cliff", "logistic"):
        suppress = _clamp_float(MACD_PUT_WAVE_SUPPRESS, 0.0, 2.0) * setup
        boost = 0.0
    elif mode in ("signed", "signed_wave", "salvage"):
        macd_width = max(float(MACD_PUT_WAVE_MACD_WIDTH), 1e-6)
        bullish_macd = _sigmoid_width(float(macd) - 50.0, 0.0, macd_width)
        bearish_macd = _sigmoid_width(50.0 - float(macd), 0.0, macd_width)
        suppress = _clamp_float(MACD_PUT_WAVE_SUPPRESS, 0.0, 2.0) * setup * bullish_macd
        boost = _clamp_float(MACD_PUT_WAVE_BOOST, 0.0, 2.0) * setup * bearish_macd
    elif mode in ("cliff_salvage", "deep_salvage", "reservation"):
        if float(pre_no_macd) >= PUT_MACD_GATE:
            return 1.0, {
                "mode": mode,
                "pre_no_macd": round(float(pre_no_macd), 2),
        ...
        macd_width = max(float(MACD_PUT_WAVE_MACD_WIDTH), 1e-6)
        bearish_macd = _sigmoid_width(50.0 - float(macd), 0.0, macd_width)
        raw_factor = _clamp_float(MACD_PUT_WAVE_BOOST, 0.0, 2.0) * setup * bearish_macd
        factor = _clamp_float(raw_factor, MACD_PUT_WAVE_MIN_FACTOR, MACD_PUT_WAVE_MAX_FACTOR)
        return factor, {
            "mode": mode,
            "pre_no_macd": round(float(pre_no_macd), 2),
            "macd": round(float(macd), 2),
            "setup": round(setup, 4),
            "bearish_macd": round(bearish_macd, 4),
            "raw_factor": round(raw_factor, 4),
            "factor": round(factor, 4),
        }
    else:
        raise ValueError(f"unsupported MACD_PUT_GATE_MODE={MACD_PUT_GATE_MODE!r}")

    raw_factor = 1.0 - suppress + boost
    factor = _clamp_float(raw_factor, MACD_PUT_WAVE_MIN_FACTOR, MACD_PUT_WAVE_MAX_FACTOR)
    return factor, {
        "mode": mode,
        "pre_no_macd": round(float(pre_no_macd), 2),
        "macd": round(float(macd), 2),
        "setup": round(float(setup), 4),
        "suppress": round(float(suppress), 4),
...
def compute_overall_score(
    ...
    **_extra,
    ...
    # ── Asymmetric MACD: zero or smoothly reshape MACD on bearish setups ─────
    ...
    _macd_put_meta = None
    _macd_put_factor, _macd_put_meta = _macd_put_weight_factor(_pre_no_macd, macd)
    if abs(_macd_put_factor - 1.0) > 1e-9:
        _w_macd_new = _clamp_float(w_macd * _macd_put_factor, 0.0, 99.0)
        _scale_non_macd = (100.0 - _w_macd_new) / max(1e-6, 100.0 - w_macd)
        w_trend *= _scale_non_macd
        w_bb    *= _scale_non_macd
        w_rsi   *= _scale_non_macd
        w_stoch *= _scale_non_macd
        w_ta    *= _scale_non_macd
        w_macd   = _w_macd_new
    ...
    if _macd_put_meta:
        weight_info['macd_put_wave'] = _macd_put_meta
    if _pcd_active:
