...
# SVD — Score Velocity Dampener (v40 ship 2026-05-06).
#
# Score velocity over 5 trading days carries information beyond the score level
# itself. Per-trade evidence (5y v39 30dte_opt barrier @ w15):
#   * 75+ decel heavy (vel<=-10): WR15=44.7% (-18pp vs baseline, z=-2.33 **)
#     — BELOW call BE of 45%, EV-negative cohort
#   * 75+ decel mild (-10<vel<=-5): WR15=51.1% (-12pp, z=-1.69 *)
#   * 75+ accel heavy (vel>+10): WR15=64.0% (+5pp, z=+3.35 ***)
# The decelerating sub-cohort that just qualified at 75 is structurally weaker
# than peers at the same score level. Score-stage filter via gradient drift
# toward target=65 (out of cascade qualifying range).
#
# Mechanism (gate `overall >= 75 ∧ velocity_5d < 0`):
#     weakness = clip(-(velocity_5d - 0) / SVD_K, 0, 1)
#     overall -= ALPHA * weakness * (overall - SVD_TARGET)
#
# Calibration:
#   Stage 1 — coarse grid (experiments/score_velocity/sweep.py, 108 variants on v39 5y):
#     11 candidates passed affected-tier gate; top g0_t65_a95_k3.
#   Stage 2 — Bayesian-style dense local refinement on v39 10y baseline
#     (experiments/score_velocity/bayes_refine.py, 700 variants):
#     winner gate_vel=0, target=72, alpha=0.80, k=1.
#     Refined winner improves composite 5y+10y by +0.06pp over coarse grid winner.
#
# Multi-window 75+ TP%:  1y +0.36 / 3y +0.51 / 5y +0.37 / 10y +0.36pp.
# Sign-consistent across all four windows. N75 drop -3.3%, spillover ≤ 0.03pp.
# k=1 means weakness saturates for ANY decel (vel<0) — empirically the entire
# decelerating cohort underperforms regardless of magnitude. Refinement table
# at gate=0, target=72: k=1 beats k=2 by 0.10pp and k=3 by 0.13pp on composite.
#
# Mechanism (saturated, since k=1 → weakness=1.0 for any vel<0):
#   overall_new = (1 - alpha) × overall + alpha × target
# At alpha=0.80, target=72: every decel signal drifts to 0.20×overall + 57.6.
# So overall=75 → 72.6 (out), overall=85 → 74.6 (out), overall=95 → 76.6
# (drops to 75-79 lowest tier; not filtered out).
#
# 95+/90+ tiers untouched in practice (high-conviction signals don't decelerate;
# gate fires <0.1% of times in those tiers). 85+ spillover -0.03pp (4 of 600
# signals removed over 5y).
#
# Apply order: AFTER PESS, BEFORE earnings boost — so boost can't re-amplify
# a velocity-displaced peak. EARN_BOOST cohort lookup uses the post-SVD score
# (matches lift table calibration).
SVD_ENABLED         = os.environ.get('SVD_ENABLED', '1') == '1'
SVD_GATE_SCORE      = int(os.environ.get('SVD_GATE_SCORE', '75'))
SVD_GATE_VEL        = int(os.environ.get('SVD_GATE_VEL', '0'))
SVD_TARGET          = int(os.environ.get('SVD_TARGET', '72'))
SVD_ALPHA           = float(os.environ.get('SVD_ALPHA', '0.80'))
SVD_K               = int(os.environ.get('SVD_K', '1'))
SVD_LOOKBACK_BARS   = 5  # trading days for velocity computation


def build_velocity_5d_map(score_pairs_asc):
    """Build {date: velocity_5d} where velocity_5d = overall[date] - overall[date_5_trading_back].

    score_pairs_asc: ascending list of (date, overall) tuples.
    Returns dict; dates with insufficient lookback (<5 trading bars prior) get value None.

    Uses positional 5-bar shift, matching how score_velocity profile was built
    (5 trading days = 5 prior pair entries, regardless of calendar gaps).
    """
    if not score_pairs_asc or len(score_pairs_asc) < SVD_LOOKBACK_BARS + 1:
        return {}
    out = {}
    for idx in range(SVD_LOOKBACK_BARS, len(score_pairs_asc)):
        d_today, o_today = score_pairs_asc[idx]
        _, o_prior = score_pairs_asc[idx - SVD_LOOKBACK_BARS]
        if o_today is None or o_prior is None:
            continue
        out[d_today] = int(o_today) - int(o_prior)
    return out
...
def compute_overall_score(
    ...
    **_extra,
    ...
    # SVD — Score Velocity Dampener (v40 ship 2026-05-06).
    # Drift overall down toward SVD_TARGET when score >= SVD_GATE_SCORE AND
    # velocity_5d < SVD_GATE_VEL (signal is decelerating). Applied BEFORE
    # earnings boost so the boost can't re-amplify a velocity-displaced peak.
    # See module-level SVD constants for derivation + ship gate evidence.
    _svd_dampen = 0.0
    if (SVD_ENABLED
            and overall >= SVD_GATE_SCORE
            and velocity_5d is not None
            and velocity_5d < SVD_GATE_VEL):
        _svd_weakness = max(0.0, min(1.0, -(velocity_5d - SVD_GATE_VEL) / SVD_K))
        if _svd_weakness > 0.0:
            _svd_dampen = SVD_ALPHA * _svd_weakness * (overall - SVD_TARGET)
            overall = int(max(0, round(overall - _svd_dampen)))
    ...
    if _svd_dampen > 0.5:
        weight_info['svd_dampen'] = round(_svd_dampen, 2)
        if velocity_5d is not None:
            weight_info['svd_vel5d'] = int(velocity_5d)
    if _ern_boost_strength > 0:
