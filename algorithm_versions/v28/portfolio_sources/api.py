...
def backtest_temporal():
    ...
    to surface a historical version's stats. Populated by `trader assess --force`."""
    try:
        ...
        try:
            row = BacktestTemporalStats.get(BacktestTemporalStats.version == ver)
        except BacktestTemporalStats.DoesNotExist:
            return jsonify({
                'yearly': [], 'monthly': [], 'summary': {},
                'error': f'No temporal stats for version v{ver.id}. Run `trader assess --force` to generate.',
            }), 404
