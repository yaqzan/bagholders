...
def _days_to_next_earnings_for(symbol, signal_date):
    ...
    Hits DB once per call. The batched paths use _load_earnings_by_symbol +
    _days_to_next_earnings to avoid round-trips.
    """
    ...
    row = (
        ...
        .where(
            ...
            & (EarningsDate.date <= signal_date + _td(days=EARN_BOOST_WINDOW))
        )
    ...
    if not row:
        return None
    delta = (row.date - signal_date).days
    return delta if 0 <= delta <= EARN_BOOST_WINDOW else None
