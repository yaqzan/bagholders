...
# Intraday TYPE-stability gate (2026-06-03): hold a NEUTRAL volume TYPE until intraday
# confidence >= this. Early in the session the projected full-day volume (hence ratio_50)
# is noisy and oscillates across the CONVICTION/THIN_AIR cliffs -> the ATI/AMSC ~18pt
# intraday "fakeout". EOD/backtest/recalc run at confidence=1.0 (is_historical branch),
# so this is BIT-IDENTICAL there (no version bump, WR-invariant) — it only smooths the
# live partial-day evolution. Set env INTRADAY_TYPE_CONF_GATE=0 to disable.
INTRADAY_TYPE_CONF_GATE    = float(os.environ.get('INTRADAY_TYPE_CONF_GATE', '0.5'))
...
# ── Earnings volume suppression (V6 gradient, shipped 2026-04-28) ──
# Replaces the prior binary ±1-day null with a pre-earnings log-gradient blend.
# Mirrors v28's earnings-boost proximity formula: log(W+1-d)/log(W+1).
# Pre-only (post-earnings carries informational signal — confirmed by V7 collapse).
# Validated 5y per-trade: 95+ WR15 +1.4pp at +21% N, 90+ +1.1pp, calls/<25 puts neutral.
EARN_SUPP_WINDOW_DAYS  = 2     # W: pre-earnings window in calendar days
EARN_SUPP_MAX_STRENGTH = 1.0   # peak strength at d=0 (full blend toward NEUTRAL)
...
def _effective_type_confidence(observed_vol, avg_50, time_conf):
    """Confidence used to gate the volume TYPE (and scale its magnitude).

    BANKED-VOLUME LOCK: the OBSERVED (already-traded) volume is a lower bound on the
    final ratio_50 — the rest of the session can only ADD volume. So if observed
    alone already crosses the elevated cliff, the volume tier is LOCKED (cannot
    reverse by close) and the TYPE is confirmed *now*, regardless of how little of
    the session has elapsed. This fixes the early genuine-spike miss: an abnormal
    10am surge already far above the full-day average fires CONVICTION immediately
    instead of being suppressed to NEUTRAL by the time gate.

    When observed is below the elevated cliff (the low-volume / projection-driven
    case — e.g. the ATI low-volume rally whose projected ratio_50 oscillates), we
    fall back to time confidence so that fakeout stays gated early. Historical/EOD
    callers pass time_conf=1.0, so this is bit-identical there.
    """
    if avg_50 and observed_vol / avg_50 >= ELEVATED_RATIO_THRESHOLD:
        return 1.0
    return time_conf
...
def _classify_signal(ratio_50, body_ratio, upper_wick, lower_wick, body,
                     ...
                     prev_close=None, confidence=1.0):
    """Returns (signal_type, direction). First match wins.

    Intraday TYPE-stability gate: when intraday volume confidence is low, hold
    NEUTRAL rather than committing to a volume-cliff-dependent TYPE (the projected
    ratio_50 is noisy early in the session and flips CONVICTION<->THIN_AIR -> the
    ATI/AMSC ~18pt fakeout). Historical/EOD/backtest pass confidence=1.0, so this
    branch is dead there (bit-identical, no version bump); it only smooths live
    partial-day scoring. INTRADAY_TYPE_CONF_GATE=0 disables it.
    """
    if confidence < INTRADAY_TYPE_CONF_GATE:
        return 'NEUTRAL', 0
...
# ── Earnings volume suppression helpers (V6 gradient) ─────────────

def _earnings_supp_strength(nearest_d):
    """Pre-earnings volume suppression strength.

    Args:
        nearest_d: calendar-day distance to nearest STRICTLY-FUTURE earnings
                   reaction (effective_date > today), or None if no event
                   within EARN_SUPP_WINDOW_DAYS forward. Same-day reaction
                   (nearest_d == 0) and post-reaction (negative) both return
                   0 — today's volume already contains the gap, so
                   suppressing it would muffle the post-event price action
                   we want to read.

    Returns:
        Strength in [0, 1]. Blend the volume amplifier output toward
        NEUTRAL by this strength.
    """
    if nearest_d is None:
        return 0.0
    if nearest_d < 1 or nearest_d > EARN_SUPP_WINDOW_DAYS:
        return 0.0
    W = EARN_SUPP_WINDOW_DAYS
    return EARN_SUPP_MAX_STRENGTH * math.log(W + 1 - nearest_d) / math.log(W + 1)


def _apply_earn_supp(result, strength):
    """Blend a volume amplifier result toward NEUTRAL_RESULT by `strength`.

    `result` is the 6-tuple (mult, raw, sig_type, mag, blend_w, vol_target)
    returned by get_volume_multiplier*.  Categorical fields (raw, sig_type,
    vol_target) are preserved; numeric fields (mult, mag, blend_w) are
    blended toward NEUTRAL_RESULT's values (1.0, 0.0, 0.0) respectively.
    """
    if strength <= 0.0:
        return result
    s = min(1.0, strength)
    mult, raw, sig_type, mag, blend_w, vol_target = result
    return (
        mult * (1.0 - s) + 1.0 * s,
        raw,
        sig_type,
...
def _query_earn_nearest_d_db(symbol, today):
    """Live-DB path: query EarningsDate for nearest STRICTLY-FUTURE earnings
    reaction.

    Reads precomputed `effective_date` (BMO same-day; AMC shifted to next
    trading day) so a same-day-reaction signal correctly returns None
    (today's prices already contain the gap; volume signal should not be
    suppressed).

    Returns calendar-day distance in [1, EARN_SUPP_WINDOW_DAYS], or None.
    """
    try:
        from database.models.core import EarningsDate
        rows = (EarningsDate.select(EarningsDate.effective_date)
                .where((EarningsDate.symbol == symbol) &
                       (EarningsDate.effective_date > today) &
                       (EarningsDate.effective_date <= today + timedelta(days=EARN_SUPP_WINDOW_DAYS)))
                .order_by(EarningsDate.effective_date.asc())
                .limit(1))
        for r in rows:
            return (r.effective_date - today).days
    except Exception:
        pass
    return None


def _earn_nearest_d_from_set(today, earnings_dates_set):
    """Cache path: scan a pre-loaded set of EFFECTIVE earnings dates for
    nearest STRICTLY-FUTURE reaction.

    `earnings_dates_set` must contain effective_date values (per
    `_load_effective_earnings_dates` in core.py). Same-day reaction
    (today in set) returns None — see _earnings_supp_strength docstring.

    Returns calendar-day distance in [1, EARN_SUPP_WINDOW_DAYS], or None.
    """
    if not earnings_dates_set:
        return None
    for delta in range(1, EARN_SUPP_WINDOW_DAYS + 1):
...
def _analyze(symbol, today, pulled_at):
    ...
    # ── Step 3: Stock fetch ──
    # Earnings volume suppression moved to get_volume_multiplier outer wrapper
    # (V6 gradient — see _earnings_supp_strength / _apply_earn_supp).
    try:
    ...
    if no_candle or is_premarket:
        ...
        eff_conf = _effective_type_confidence(t_vol, avg_50, confidence)
        sig_type, direction = _classify_signal(
            r50, body_r, u_wick, l_wick, body, c_dir, recent_closes,
            today_open=t_open, prev_close=prev_close, confidence=eff_conf)
        ...
        if sig_type == 'NEUTRAL':
            ...
            adj_mag = raw_mag * w_agree * reg_disc * eff_conf
...
def _analyze_from_cache(today, ph_list_desc, prior_vol_signals, earnings_dates_set):
    ...
    # Today's row is preferred but may be absent/zero-volume on premarket or
    # bad-data rows. Match _analyze(): skip candle analysis but still let prior
    # volume signals decay forward.
    ...
    if today_ph is None:
        for ph in ph_list_desc:
            if ph.date == today:
                today_ph = ph
                break
    no_candle = today_ph is None or int(today_ph.volume) == 0

    # Earnings volume suppression moved to get_volume_multiplier_from_cache
    # outer wrapper (V6 gradient — see _earnings_supp_strength / _apply_earn_supp).
    ...
    if not no_candle:
        ...
        is_historical = today < date.today()
        is_premarket = not is_historical
    ...
    if no_candle or is_premarket:
        ...
        sig_type, direction = _classify_signal(
            r50, body_r, u_wick, l_wick, body, c_dir, recent_closes,
            today_open=t_open, prev_close=prev_close_val, confidence=eff_conf)
...
def get_volume_multiplier_from_cache(today, ph_list_desc, prior_vol_signals,
    ...
    try:
        ...
        # Compute the un-suppressed result first
        if sig.is_premarket:
            g = sig.premarket_gap_pct
            if g > GAP_SIGNAL_THRESHOLD:
                result = (GAP_MULTIPLIER_BULLISH, 50, 'NEUTRAL', 0.0, 0.0, 50.0)
            elif g < -GAP_SIGNAL_THRESHOLD:
                result = (GAP_MULTIPLIER_BEARISH, 50, 'NEUTRAL', 0.0, 0.0, 50.0)
            else:
            ...
            if sig.signal_type in _REVERSAL_SIGNALS and sig.adjusted_magnitude > 0:
                ...
                result = (1.0, raw, sig.signal_type, sig.adjusted_magnitude, blend_w, vol_target)
            else:
                ...
                result = (mult, raw, sig.signal_type, sig.adjusted_magnitude, 0.0, 50.0)

        # V6 earnings volume suppression gradient (pre-only log W=2 M=1.0)
        nearest_d = _earn_nearest_d_from_set(today, earnings_dates_set)
        return _apply_earn_supp(result, _earnings_supp_strength(nearest_d))
...
def get_volume_multiplier(symbol, today, pulled_at, pre_volume_overall=None):
    ...
    try:
        ...
        # V6 earnings volume suppression gradient (pre-only log W=2 M=1.0)
        nearest_d = _query_earn_nearest_d_db(symbol, today)
        return _apply_earn_supp(result, _earnings_supp_strength(nearest_d))
...
def explain_volume_signal(symbol, target_date, pre_volume_overall=None):
    ...
    earn_d = _query_earn_nearest_d_db(symbol, target_date)
    earn_s = _earnings_supp_strength(earn_d)
    if earn_s > 0:
        print(f"\n  {Fore.YELLOW}EARNINGS V6 GRADIENT — d=+{earn_d}, "
              f"strength={earn_s:.3f} (blend {earn_s*100:.1f}% toward NEUTRAL){Style.RESET_ALL}")
