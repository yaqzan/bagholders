...
def get_api_score_version():
    """Highest-id algorithm version that has a non-empty git_message; else highest id overall."""
    from database.models.core import AlgorithmVersion
    max_id = (
        AlgorithmVersion.select(fn.MAX(AlgorithmVersion.id))
        .where(
            (AlgorithmVersion.git_message.is_null(False))
            & (fn.TRIM(AlgorithmVersion.git_message) != '')
        )
        .scalar()
    )
    if max_id is not None:
        return AlgorithmVersion.get_by_id(max_id)
    v = AlgorithmVersion.select().order_by(AlgorithmVersion.id.desc()).first()
    return v if v is not None else AlgorithmVersion.get_or_create_current()
...
def get_stock(symbol):
    ...
    try:
        ...
        latest_score = Score.get_or_none(
            (Score.symbol == stock.symbol)
        ...
        ctx = get_signal_context(symbol.upper())
        if ctx:
            stock_data['dte_recommendation'] = recommend_dte(**ctx)
        else:
            stock_data['dte_recommendation'] = None
    ...
    try:
        ...
        for score in scores:
            ...
            if score.overall >= 75 or score.overall <= 25:
                ctx = get_signal_context(symbol.upper(), score.date)
                if ctx:
                    rec = recommend_dte(**ctx)
                    row['dte_recommendation'] = {k: v for k, v in rec.items() if k != 'symbol'}
            result.append(row)
...
def get_all_stocks_with_scores():
    ...
    try:
        ...
        scores_query = (Score
                       ...
                       .where((Score.date == target_date) & (Score.version == version) & (Score.overall.is_null(False)))
                       .limit(limit))
        ...
        for score in scores_query:
            ...
            if score.pct_from_ema50 is not None and score.pct_from_ema200 is not None:
                try:
                    rec = recommend_dte(
        ...
        for d in all_dates:
            ...
            if tr:
                ...
                score_dist = {
                    ...
                    # 50 = neutral; ≥75 = overbought / mean-reversion short territory)
                    ...
                    # overbought_75/80: stocks with score ≥75/80 = confirmed mean-reversion
                    # short setups in our system
                    'overbought_75': ob75,
                    'overbought_80': ob80,
                    'overbought_75_pct': pct(ob75, total),
                    'overbought_80_pct': pct(ob80, total),

                    # oversold_25/20: stocks with score ≤25/20 = confirmed long setups
                    'oversold_25': os25,
                    'oversold_20': os20,
                    'oversold_25_pct': pct(os25, total),
                    'oversold_20_pct': pct(os20, total),

                    # skew: positive = more overbought issues than oversold (market stretched high)
                    # negative = oversold dominance (broad weakness / capitulation)
