...
class Trader():
    ...
    def _detect_option_gaps(self, option_ids, today):
        """Flag options with collection gaps. Runs after pull_options per symbol."""
        if not option_ids:
            return
        try:
            ...
            for oid in option_ids:
                last_before = (
                    OptionPrice.select(OptionPrice.date)
                    .where((OptionPrice.option == oid) & (OptionPrice.date < today))
                ...
                if last_before is None:
                    continue
                gap = trading_days_between(last_before, today)
                if gap > 1:
                    opt = Option.get_by_id(oid)
                    logging.warning(
                        f"{opt.symbol} option {oid} (${opt.strike_price} {opt.expiration_date}): "
                        f"{gap}-day gap (last: {last_before}, today: {today})"
                    )
                    opt.last_gap_days = gap
                    opt.last_gap_detected_at = datetime.now()
                    opt.save()
        except Exception:
            pass
...
def main(command=None, args=None):
    ...
    if command == 'pull' and args:
        ...
        if args:
            ...
            while i < len(arg_list):
                ...
                if arg == '--all':
                    ...
                    use_batch = False
                elif arg == '--workers' and i + 1 < len(arg_list):
                    i += 1
                    try:
        ...
        recalculate_run(symbol, days, components, workers=workers, use_batch=use_batch)
    elif command == 'backfill':
        def _parse_backfill_timeframe(arg):
            s = arg.lower().strip()
            if len(s) < 2 or s[-1] not in 'dy':
                return None
            try:
                n = int(s[:-1])
            except ValueError:
        ...
        DEFAULT_BACKFILL = ['1y', '2y', '3y', '5y', '10y', '25y']
        ...
        if args:
            for arg in args.split(' '):
                ...
                if low == 'all':
                    continue
                parsed = _parse_backfill_timeframe(arg)
                if parsed:
                    timeframes.append(parsed)
                else:
                    symbol = arg.upper()
        if not timeframes:
            timeframes = [_parse_backfill_timeframe(s) for s in DEFAULT_BACKFILL]
        ...
        for wlabel, wdays in timeframes:
            ...
            for stock in tqdm(stocks, desc=f"Backfill {wlabel} stocks"):
