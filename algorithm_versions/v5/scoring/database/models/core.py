...
class Stock(BaseModel):
    ...
    def backfill_scores(self, cutoff=None):
        ...
        if ph_dates_in_range - ind_dates_in_range:
            self.calculate_indicators(full=True, show_progress=True)
        ...
        if wph_dates_in_range - wind_dates_in_range:
            self.calculate_indicators(full=True, weekly=True, show_progress=True)
        ...
        for i, p in enumerate(prices):
            prev_close[p.date] = prices[i - 1].close if i > 0 else p.close
        ...
        for d in tqdm(missing, desc=f"{self.symbol} backfill", leave=False):
            try:
                ...
                score.daily_change = round(((cur - prev) / prev) * 100, 2) if cur and prev and prev != 0 else 0
                score.price = cur or 0
                score.name = self.name
                score.next_earnings = score.calculate_next_earnings_days()
                score.price_target = score.calculate_price_target()
    ...
    def recalculate_scores_batched(self, cutoff, components):
        """
        ...
        # ── 2. Forward-walk: compute volume signals chronologically ────────
        # Decay requires prior computed signals, so we must process asc order.
        prior_vol_signals = {}   # date → (signal_type, magnitude, volume_raw)
        ...
        for score in tqdm(scores, desc=f"{self.symbol} {label} [batch]", leave=False):
            try:
                changed = False
