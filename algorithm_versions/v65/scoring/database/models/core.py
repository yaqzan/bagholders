...
class Score(BaseModel):
    ...
    @property
    def completed_weekly_score(self):
        week_start = self.date - timedelta(days=self.date.weekday() + 7)
        return WeeklyScore.get_or_none(
            (WeeklyScore.symbol == self.symbol) &
            (WeeklyScore.date == week_start)
    ...
    @property
    def prev_completed_weekly_score(self):
    ...
    def calculate_overall_score(
        ...
        # Weekly context — calendar WeeklyScore is the only source.
        ...
        completed_ws = self.completed_weekly_score
        completed_pws = self.prev_completed_weekly_score if completed_ws else None
        ...
        score_kwargs = {
            ...
            'completed_ws_trend': completed_ws.trend if completed_ws else None,
            'completed_ws_rsi': completed_ws.rsi if completed_ws and completed_ws.rsi and completed_ws.macd else None,
            'completed_ws_macd': completed_ws.macd if completed_ws and completed_ws.rsi and completed_ws.macd else None,
            'completed_prev_ws_trend': completed_pws.trend if completed_pws else None,
            'completed_prev_ws_rsi': completed_pws.rsi if completed_pws else None,
            'completed_prev_ws_macd': completed_pws.macd if completed_pws else None,
            'vol_mult': vol_mult,
            'vol_raw': vol_raw,
            'vol_sig': vol_sig,
            'vol_mag': vol_mag,
    ...
    def calculate_scores_batched(self, silent=False, regime_multiplier=None, regime_composite=None):
        ...
        for ind in ind_rows:
            ...
            try:
                ...
                completed_ws = pws
                completed_pws = ws_map.get(_week_start(d) - timedelta(weeks=2))
                ws_rsi  = ws.rsi  if ws and ws.rsi and ws.macd else None
                ws_macd = ws.macd if ws and ws.rsi and ws.macd else None
        ...
        for score in it:
            try:
                ...
                if 'overall' in components or changed:
                    ...
                    completed_ws = pws
                    completed_pws = ws_map.get(_week_start(score.date) - timedelta(weeks=2))
                    ws_trend = ws.trend if ws else None
                    ws_rsi   = ws.rsi   if ws and ws.rsi and ws.macd else None
                    ws_macd  = ws.macd  if ws and ws.rsi and ws.macd else None
