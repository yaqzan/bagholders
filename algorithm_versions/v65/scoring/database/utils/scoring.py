...
def compute_overall_score(
    ...
    *,
    ...
    completed_ws_trend=None, completed_ws_rsi=None, completed_ws_macd=None,
    completed_prev_ws_trend=None, completed_prev_ws_rsi=None, completed_prev_ws_macd=None,
    ...
    **_extra,
    ...
    if None not in (ws_rsi, ws_macd):
        ...
        weighted_sum += adj
        weekly_detail = dict(weekly_detail or {})
        weekly_detail['wadj_partial'] = round(float(adj), 1)
        if None not in (completed_ws_rsi, completed_ws_macd):
            completed_adj, completed_detail = calculate_weekly_adjustment(
                completed_ws_trend, completed_ws_rsi, completed_ws_macd,
                completed_prev_ws_trend, completed_prev_ws_rsi, completed_prev_ws_macd,
            )
            weekly_detail['wadj_completed'] = round(float(completed_adj), 1)
            weekly_detail['weekly_adj_gap'] = round(abs(float(adj) - float(completed_adj)), 1)
            weekly_detail['weekly_gap_flag'] = weekly_detail['weekly_adj_gap'] > 10.0
            if completed_detail:
                weekly_detail['w_comp_completed'] = completed_detail.get('w_comp')
                weekly_detail['w_bias_completed'] = completed_detail.get('w_bias')
                weekly_detail['w_mom_completed'] = completed_detail.get('w_mom')
        else:
            weekly_detail['wadj_completed'] = None
            weekly_detail['weekly_adj_gap'] = None
            weekly_detail['weekly_gap_flag'] = False
