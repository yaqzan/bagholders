...
def resolve(outcome, rng):
    ...
    resolve above SL — matching real limit/stop order semantics):
      ...
      kind='sl', call : fill ~ Uniform(low, sl_level)          stop-or-worse
      kind='sl', put  : fill ~ Uniform(sl_level, high)         stop-or-worse
    ...
    if kind == 'tp':
        ...
        if side == 'call':
            u_lo, u_hi = lo, sl_lvl    # at or below SL barrier (stop-or-worse)
        else:                          # put
            u_lo, u_hi = sl_lvl, hi    # at or above SL barrier
        slip = SLIP_SL
