...
class Trader():
    ...
    def update_stocks(self, full=False, with_options=False, only_remainder=False):
        ...
        # Most cached rows are stable forever once price_history extends past
        # the W=150 cal-day window. The trailing 160 days may have NULL
        # outcomes (insufficient forward data) that flip to definitive results
        # as new bars accumulate. Idempotent INSERT OR REPLACE.
        try:
...
def _cmd_alloc(args):
    """
    trader alloc <portfolio_cad> [--strategy 30dte|15dte]
    ...
    CASCADE (% of portfolio equity, pre-scale) — same for both DTEs (H5/C1):
        Calls  95+:18%  85-94:12%  80-84:15%  75-79:15%  70-74:0% (disabled)
        Puts   <=15:10%  16-20:12%  21-25:12%

    EXITS:
        30 DTE (hard sell day 15) — Phase H5:
            Calls  TP=+35% / SL=-30%   (adaptive: TP=+40% / SL=-35% when breadth<=50)
            Puts   TP=+35% / SL=-20%
            Hard sell  -40%
        15 DTE C1 (hard sell day 7) — Phase 15B:
            Same TP/SL/PUT params (anchored at H5 per-trade-EV winners)
    ...
    # --- parse args: portfolio value [--strategy 30dte|15dte] ---
    if not args:
        print("Usage: trader alloc <portfolio_value_cad> [--strategy 30dte|15dte]")
    ...
    # --- tier definitions — Updated 2026-04-28 (Phase H4): smaller cascade for DD ---
    CALL_TIERS = [
        (95, float('inf'), 0.18, '95+'),    # was 0.25 — smaller ultra alloc
        (85, 95,           0.12, '85-94'),  # was 0.15
        (80, 85,           0.15, '80-84'),
        (75, 80,           0.15, '75-79'),
        (70, 75,           0.00, '70-74'),  # disabled
    ]
    PUT_TIERS = [
        (None, 15, 0.10, '<=15'),  # was 0.15 — smaller put concentration
        (16,   20, 0.12, '16-20'),
        (21,   25, 0.12, '21-25'),
    ]
    # --- DTE-strategy specific overrides (Phase 16, 2026-04-28) ---
    # Default 30 DTE production (B68 ship 2026-04-29):
    #   MaxPos=14, F3F floors=0.50, hard sell -40% @ day 15, DD breaker @ 68%
    # 15 DTE C1: MaxPos=8, F3F floors=0.40, hard sell -45% @ day 7, DD breaker @ 60%
    if is_15dte:
        MAX_POSITIONS = 8
        F3F_CALL_FLOOR = 0.40
        F3F_PUT_FLOOR  = 0.40
    ...
    stressed = breadth_score is not None and breadth_score <= 50
    # Updated 2026-04-28 (Phase H5): TP widened (30→35), SL tightened (-35→-30)
    call_tp, call_sl = (40, 35) if stressed else (35, 30)
    ...
    strategy_label = "Phase 15B C1 (15 DTE)" if is_15dte else "Phase H5 (30 DTE)"
    ...
    print(f"    Call exits:   {Fore.GREEN}TP=+{call_tp}%  SL=-{call_sl}%{Style.RESET_ALL}"
          f"   (adaptive: +40/-35 when breadth<=50)")
    print(f"    Put exits:    {Fore.RED}TP=+35%  SL=-20%{Style.RESET_ALL}   (PUT_SL fixed; hold=0)")
    ...
    print(f"    F3f scale:    calls x{call_scale:.2f}   puts x{put_scale:.2f}"
          f"   (calls 1.0->0.50 as brd 50->20; puts 1.0->0.50 as brd 75->95)")
...
def _cmd_backtest(args):
    ...
    Strategy options (option-level gross %):
      --tp        N            base take-profit on option premium, gross % (default 35)
                               widens to 40% when breadth_score ≤ 50 (adaptive)
      --fixed-tp               disable breadth-adaptive TP, use fixed --tp value
      --sl        N            base stop-loss on option premium, gross % (default 30)
    ...
    min_score_arg = 70.0
    version_token = None
    # Updated 2026-04-28 (Phase H5_HOLD15_H40 winner): wider TP captures more
    # MFE, tighter SL is MAE-anchored, tighter HARD_SELL caps tail losses.
    tp_pct        = 35.0    # was 30 — option gross TP % (base; may widen per breadth)
    tp_stress_pct = 40.0    # was 35 — option gross TP % when breadth weak
    sl_pct        = 30.0    # was 35 — option gross SL % (base; MAE-anchored)
    sl_stress_pct = 35.0    # was 40 — option gross SL % when breadth weak
    hard_day      = 15      # hard-sell calendar day from entry (unchanged)
    max_pos       = 14      # max concurrent positions (unchanged)
    ...
    _dte_strategy_local = '30'   # Phase 16: --dte 15 switches to 15 DTE C1 strategy
        ...
        while i < len(tokens):
            ...
            if tok == '--from' and i + 1 < len(tokens):
                ...
                if _dte_strategy_local == '15':
                    # Override defaults to 15 DTE C1 (Phase 15B winner).
                    # TP/SL pct map identically to monte_carlo_15dte.py (TP=35/SL=30).
                    hard_day = 7
                    max_pos  = 8
                i += 2
            else:
    ...
    # Updated 2026-04-28 (Phase H4): F3f floors tightened to 0.50.
    ...
    # Updated 2026-04-28 (Phase H4): smaller cascade reduces DD.
    TIER_ALLOC = {
        '95+':   0.18,   # was 0.25 — smaller ultra
        '85-94': 0.12,   # was 0.15 — slight reduction
        '80-84': 0.15,
        '75-79': 0.15,
        '70-74': 0.00,
        # Put-side tiers
        'p<=15': 0.10,   # was 0.15 — smaller put concentration
        'p16-20':0.12,
        'p21-25':0.12,
    }

    # DD circuit breaker (shipped 2026-04-29 with V6 re-ship — see CLAUDE.md
    # "V6 + DD circuit breaker re-ship"). Pauses new entries when running
    # portfolio DD exceeds threshold. Mirrors monte_carlo.py / backtest_cascade.py.
    # Brought every Conservative DD-C window below 80% floor at N=500 under
    # IV-stochastic MC with +489% 5y compound vs v28 baseline.
    DD_CIRCUIT_BREAKER = 0.68

    # Put-side fixed TP/SL — Updated 2026-04-28 (Phase H5): PUT_TP widened to 0.35
    PUT_TP_SIGMA  = 1.274    # was 1.092 — wider TP barrier
    PUT_SL_SIGMA  = 0.728
    PUT_NET_TP    = +0.340   # was +0.290 (+35% − 1% entry)
    PUT_NET_SL    = -0.223
    PUT_THRESHOLD = 25.0
    # Put SL hard-hold — Updated 2026-04-28 (Phase H1/H5): hold=0 ships.
    # Hold mechanic created gap-through window in bug-fixed MC.
    ...
    # (signal_date, signal_date + 5 trading days]. N=1000 canonical 3-mode
    # MC: 5y compound +44.7% Realistic; all DD-C under 80%; 0% collapse.
    # Per-trade: 16-20 puts in earnings window WR15 -3.1pp at d=3 (N=735).
    EARN_SUPP_PUT          = True
    ...
    if _dte_strategy_local == '15':
        F3F_CALL_FLOOR = 0.40
        F3F_PUT_FLOOR  = 0.40
        # 15 DTE option-aligned sigma barriers (PREMIUM_MULT=1.29):
        # Call TP=35% -> 0.903σ; Call SL=30% -> 0.774σ
        # Put  TP=35% -> 0.903σ; Put  SL=20% -> 0.516σ
        PUT_TP_SIGMA = 0.903   # was 1.274 for 30 DTE
        PUT_SL_SIGMA = 0.516   # was 0.728 for 30 DTE
    ...
    if not fixed_tp and n_breadth_days:
        ...
        print(f'  TP {_marker(tp_pct, 35.0, ".0f")}% gross (+{NET_TP_BASE*100:.1f}% net, {TP_SIGMA_BASE:.3f}σ)')
    if not fixed_sl and n_breadth_days:
        ...
        print(f'  SL {_marker(sl_pct, 30.0, ".0f")}% gross (−{abs(NET_SL_BASE)*100:.1f}% net, {SL_SIGMA_BASE:.3f}σ)')
    print(f'  Tiers: 95+→18%  85-94→12%  80-84→15%  75-79→15%  70-74→0%'
          + (f'  {Fore.YELLOW + Style.BRIGHT}[custom params highlighted in yellow]{Style.RESET_ALL + Fore.WHITE + Style.BRIGHT}'
             if any([tp_pct != 35, sl_pct != 30, hard_day != 15, max_pos != 14]) else ''))
    ...
    print(f'  Call cascade 95+→18%  85-94→12%  80-84→15%  75-79→15%  70-74→0%  ·  '
          f'Put cascade ≤15→10%  16-20→12%  21-25→12%  ·  '
...
def main(command=None, args=None):
    ...
    if command == 'pull' and args:
        ...
        if do_force:
            ...
            # Phase 17 (2026-04-29): assess pipeline now produces THREE runs per window:
            #   (dte=30, metric=wr)  — directional WR (DTE-agnostic; API serves this for any DTE)
            #   (dte=30, metric=tp)  — 30 DTE option Take Profit %
            #   (dte=15, metric=tp)  — 15 DTE option Take Profit %
            # The (dte=15, metric=wr) combo is skipped — it would be byte-identical to (30, wr).
            _ASSESS_COMBOS = [('30', 'wr'), ('30', 'tp'), ('15', 'tp')]
        ...
        if do_meta:
            ...
            # Build the (dte, metric) combo list, skipping (15, wr) which would be
            # byte-identical to (30, wr) under design A (WR is DTE-agnostic).
            _combos = []
            _seen_wr = False
            for _ds in dte_strategies_to_run:
                for _m in metrics_to_run:
                    if _m == 'wr':
                        if _seen_wr:
                            continue   # WR computed once, served for any DTE
                        _seen_wr = True
                        _combos.append(('30', 'wr'))   # canonical WR row keyed at dte=30
                    else:
                        _combos.append((_ds, _m))

            _temporal_done = set()  # avoid running backtest-temporal twice per DTE
            ...
            for _dte_s, _metric in _combos:
                ...
                # backtest-temporal is DTE-specific but metric-agnostic — once per DTE.
                # Run only for DTEs the user actually selected (for `(30, wr)` canonical
                # row, this still produces a 30 DTE temporal — fine since 30 DTE is
                # always in the original `dte_strategies_to_run` when WR was requested).
                if (do_force and days is None and not do_regime_adjust
                        and _dte_s not in _temporal_done
                        and _dte_s in dte_strategies_to_run):
                    if _dte_s == '15':
                        print(Fore.CYAN + '\n── backtest-temporal (15 DTE C1) ──' + Style.RESET_ALL)
                        try:
                            from backtest_cascade_15dte import compute_and_store_temporal as _temp15
