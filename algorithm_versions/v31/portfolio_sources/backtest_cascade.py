...
# this threshold. Mirrors monte_carlo.py.
# 0.0 = OFF (legacy); 0.68 = V6/v29 ship (2026-04-29).
# Phase OP1 attempted tightening to 0.60 (4-window N=150 screening looked
# promising) but Phase OP1b full N=300 × 8-window validation reversed the
# finding (C2 was 22-now-biased; 5y compound -5.4%, 5y DD +2.5pp). Reverted.
import os as _os
DD_CIRCUIT_BREAKER  = float(_os.environ.get('DD_CIRCUIT_BREAKER', '0.68'))
...
# SL −1.3%, hard −0.5%) — updated 2026-04-28 (Phase H5_HOLD15_H40 winner)
NET_TP_BASE     = +0.340   # +35% gross − 1.0% entry  (was +30/+0.290)
NET_TP_STRESS   = +0.390   # +40% gross − 1.0% entry  (was +35/+0.340)
NET_SL_BASE     = -0.323   # −30% gross − 1.0% entry − 1.3% SL exit  (was -35/-0.373)
NET_SL_STRESS   = -0.373   # −35% gross − 1.0% entry − 1.3% SL exit  (was -40/-0.423)
NET_HARD        = -0.415   # −40% gross − 1.0% entry − 0.5% hard exit  (was -50/-0.515)

# Cascade allocation per score tier — updated 2026-04-28 (Phase H4 winner)
TIER_ALLOC = {
    '95+':   0.18,   # was 0.25 — smaller alloc reduces DD with minimal return cost on rare 95+
    '85-94': 0.12,   # was 0.15 — slight reduction for DD safety
    '80-84': 0.15,   # unchanged
    '75-79': 0.15,   # unchanged
    '70-74': 0.00,   # disabled
    # Put-side tiers
    'p<=15': 0.10,   # was 0.15 — smaller put concentration, less correlated DD
    'p16-20':0.12,   # unchanged
    'p21-25':0.12,   # unchanged
}

# Put-side fixed TP/SL (no breadth switch)
# Updated 2026-04-28: PUT_TP widened to 0.35 (Phase H5)
PUT_TP            =  0.35   # was 0.30 — capture more put MFE
PUT_SL            = -0.20
PUT_NET_TP        = +0.340   # +35% − 1.0% entry  (was +0.290)
...
# Updated 2026-04-28 (Phase H1/H5 winner): hold=0 ships. The hold mechanic
# created a gap-through window in bug-fixed MC; removing it lets SL fire at
# barrier (-20%) as soon as breached.
...
# N=1000 canonical 3-mode MC validation (experiments/esp_d5_validate_n1000.py):
# 5y compound +44.7% Realistic vs production baseline; all annual windows
# within ±7%; all DD-C under 80%; 0% collapse on every cell.
# Portfolio-stage only — no scoring change.
EARN_SUPP_PUT          = True
...
# Option pricing (delta + theta + vega) — shipped 2026-04-29.
# When OPTION_PRICING_AWARE=1 (default), TP/SL fire P&L is computed via
# `option_pricing.option_pnl_pct` at the trigger bar's mid-fill (open+close)/2,
# accounting for theta drag on bars_held and (deterministic) vega ratio when
# the trade spans earnings. This bypasses the static NET_TP/NET_SL constants
# at fire time. The JIT path is legacy static-pricing — set
# OPTION_PRICING_AWARE=0 to revert.
OPTION_PRICING_AWARE = _os.environ.get('OPTION_PRICING_AWARE', '1') == '1'

# Constants required for option-pricing path (mirror monte_carlo.py)
PREMIUM_MULT  = 1.82
DELTA         = 0.5
SLIP_ENTRY    = -0.010
SLIP_TP       =  0.000
...
def compute_outcome(symbol: str, signal_date: date, score: float,
    ...
    def _option_aware_pnl(kind_code, bars_held, fire_idx):
        """Compute realized net P&L using option_pricing model at mid-bar
        fill (deterministic). Replaces static NET_TP/NET_SL constants."""
        ...
        u_fill = (u_high + u_low) / 2.0   # deterministic mid of intraday range
        gross = option_pnl_pct('call', u_fill, entry_price, bars_held,
                               premium_pct=premium_pct,
                               total_dte=DEFAULT_TOTAL_DTE,
                               vega_ratio=1.0,  # deterministic: no IV crush sample
                               delta=DELTA)
    ...
    for j in range(sig_i + 1, len(ph_rows)):
        ...
        bars_held = j - sig_i
        if high >= tp_price:
            tp_pnl = _option_aware_pnl(1, bars_held, j) if OPTION_PRICING_AWARE else net_tp
        ...
        if low <= sl_price:
            sl_pnl = _option_aware_pnl(2, bars_held, j) if OPTION_PRICING_AWARE else net_sl
            return TradeOutcome(symbol, signal_date, score, tier,
        ...
        if bar_date >= deadline:
            hard_pnl = _option_aware_pnl(0, bars_held, j) if OPTION_PRICING_AWARE else net_hard
            return TradeOutcome(symbol, signal_date, score, tier,
...
def compute_put_outcome(symbol: str, signal_date: date, score: float,
    ...
    def _put_option_aware_pnl(kind_code, bars_held, fire_idx):
        from option_pricing import option_pnl_pct
        bar = ph_rows[fire_idx]
        u_high = float(bar.high) if hasattr(bar, 'high') else float(bar.close)
        u_low  = float(bar.low)  if hasattr(bar, 'low')  else float(bar.close)
    ...
    for j in range(sig_i + 1, len(ph_rows)):
        ...
        if low <= tp_price:
            tp_pnl = _put_option_aware_pnl(1, bars_held, j) if OPTION_PRICING_AWARE else put_net_tp
            return TradeOutcome(symbol, signal_date, score, tier,
        ...
        if high >= sl_price and bars_held > sl_hold:
            sl_pnl = _put_option_aware_pnl(2, bars_held, j) if OPTION_PRICING_AWARE else put_net_sl
            return TradeOutcome(symbol, signal_date, score, tier,
        ...
        if bar.date >= deadline:
            hard_pnl = _put_option_aware_pnl(0, bars_held, j) if OPTION_PRICING_AWARE else net_hard
            return TradeOutcome(symbol, signal_date, score, tier,
