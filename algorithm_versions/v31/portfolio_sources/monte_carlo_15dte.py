...
Phase 15B C1 SHIPPED config (DD-reduction levers):
  DD_CIRCUIT_BREAKER : 0.60   (pause new entries when running DD > 60%)
...
VOL_LOOKBACK       = 60
HOLD_DAYS          = 7           # 15 DTE option, hard sell at half-DTE
PREMIUM_MULT       = 1.29        # ATM 15-DTE premium ~ 1.29 * sigma_daily
DELTA              = 0.5
...
HARD_SELL_LOSS     = -0.45       # 15 DTE day-7 ~ -46% empirical (theta scaling). H5 spirit: slight tighten from empirical.
...
MAX_POSITIONS      = 8           # 15 DTE C1 — was 14; cap concurrent exposure for DD safety
...
F3F_CALL_FLOOR        = 0.40   # 15 DTE C1 — was 0.50; stronger weakness contraction
...
# DD circuit breaker (15 DTE Phase 15B — SHIPPED).
# When realized portfolio DD (peak-to-trough) exceeds this threshold, pause
# ALL new entries until DD recovers below the threshold. Existing positions
# continue to be processed (TP/SL/Hard exits fire normally).
# 0.0 = disabled. C1 winner: 0.60 (Phase 15B, 2026-04-28).
DD_CIRCUIT_BREAKER = float(os.environ.get('DD_CIRCUIT_BREAKER', '0.60'))
...
def compute_trade_outcome(sym_bars, signal_date, stressed):
    ...
    for i in range(base_idx + 1, end_idx):
        ...
        if tp_hit and sl_hit:
            ...
            if gap_already:
                ...
                realized_sl_pnl = max(DELTA * close_adv / premium_pct, -1.0)
            else:
        ...
        if sl_hit:
            ...
            gap_already = prev_close <= sl_level
            if gap_already:
                # Gap-down overnight: exit at current close (worse than barrier)
                close_adv = (closes[i] - entry_price) / entry_price
                realized_sl_pnl = max(DELTA * close_adv / premium_pct, -1.0)
            else:
    ...
    if realized_sl_pnl is not None:
        net_sl = realized_sl_pnl + SLIP_ENTRY + SLIP_SL
...
def compute_put_outcome(sym_bars, signal_date, put_stressed=False):
    ...
    premium_pct = PREMIUM_MULT * vol / 100
    kind = 'hard'; exit_bar = HOLD_DAYS
    fired_sl_pct = sl_pct  # default (used if no sl fires)
    for i in range(base_idx + 1, end_idx):
        ...
        if tp_hit and sl_hit:
            ...
            # SL leg realized loss: was the barrier already breached BEFORE
            # this bar (during hold or by prior bar's close)? If yes,
            # gap-through case — exit at close. Else stop fires intraday at
            # barrier (realized ≈ sl_pct).
            prev_close = closes[i-1] if i > base_idx else entry_price
            gap_already = prev_close >= sl_level_t
            if gap_already:
                close_adv = (closes[i] - entry_price) / entry_price
                realized = -DELTA * close_adv / premium_pct
        ...
        if sl_hit:
            ...
            #  - prev_close < sl_level: stop placed at barrier. Fires intraday
            #    at sl_level. Realized = sl_pct (with slippage).
            #  - prev_close >= sl_level: barrier was already breached at start
            #    of this bar (gap up overnight or end-of-hold gap-through).
    ...
    net_sl = fired_sl_pct + SLIP_ENTRY + SLIP_SL
