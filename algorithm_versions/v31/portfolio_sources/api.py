...
def get_bucket_winrates(symbol):
    try:
        from historic_peaks import (
            ...
            _SWING_VOL_LOOKBACK, MIN_EVENT_GAP,
        )
        ...
        # Cluster consecutive qualifying days within MIN_EVENT_GAP into one event.
        # Split events on side change (call vs put) so a regime flip starts a new event.
        events = []
        cur = []
        for r in rows:
            r_side = 'call' if r.overall >= 50 else 'put'
            if not cur:
                cur = [r]
                continue
            prev = cur[-1]
            prev_side = 'call' if prev.overall >= 50 else 'put'
            if r_side != prev_side or (r.date - prev.date).days > MIN_EVENT_GAP:
                events.append(cur)
                cur = [r]
            else:
                cur.append(r)
        ...
        # Pick most extreme score per event (max distance from 50).
        peaks = [max(ev, key=lambda r: abs(r.overall - 50)) for ev in events]
    ...
    try:
        ...
        if _query_dte == '30' and metric == 'wr':
            ...
            if not dte_runs:
                ...
                return jsonify({
                    'error': f'No assessment data for version v{version.id} (dte={_query_dte}, metric={metric}). '
            ...
            for bucket, (res, run) in by_bucket.items():
                ...
                for p_label in ['1d', '7d', '15d', '30d', '60d', '90d', '150d']:
...
def simulate_trader():
    ...
    try:
        ...
        # Updated 2026-04-28 (Phase H5_HOLD15_H40 winner): wider TP, tighter SL
        tp_pct    = float(request.args.get('tp',   35))   # was 30 — option gross TP %
        sl_pct    = float(request.args.get('sl',   30))   # was 35 — option gross SL %
        ...
        # Cascade — Updated 2026-04-28 (Phase H4): smaller cascade reduces DD
        TIER_ALLOC = {
            '95+':   0.18,   # was 0.25
            '85-94': 0.12,   # was 0.15
            '80-84': 0.15,
        ...
        # this fraction. 0.68 = V6/v29 ship (Phase EVS-IV-DD V3, N=500 validated).
        # Phase OP1 retune attempted 0.60 but OP1b full N=300 × 8-window
        # validation reverted (5y compound -5.4%, 5y DD +2.5pp).
        DD_CIRCUIT_BREAKER = float(request.args.get('dd_breaker', 0.68))
        ...
        # Floors tightened 2026-04-28 (Phase H4): 0.70/0.75 → 0.50/0.50.
        ...
        for today_d in all_dates:
            ...
            # 2.5 DD circuit breaker — pause new entries when DD > threshold.
            ...
            # Recovery safety valve: with no open positions, no recovery
            # possible — reset peak so the strategy can resume after deep DD.
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                ...
                'error': f'No temporal stats for version v{ver.id} (dte={dte_strategy}). Run `{cmd}` to generate.',
            }), 404
...
def run_backtest_endpoint():
    ...
    try:
        ...
        # Updated 2026-04-28 (Phase H5_HOLD15_H40 winner): wider TP, tighter SL.
        tp_pct        = _float('tp_pct',        35.0)   # was 30
        tp_stress_pct = _float('tp_stress_pct', 40.0)   # was 35
        sl_pct        = _float('sl_pct',        30.0)   # was 35
        sl_stress_pct = _float('sl_stress_pct', 35.0)   # was 40
        put_tp_pct    = _float('put_tp_pct',    35.0)   # was 30
        put_sl_pct        = _float('put_sl_pct',        20.0)
        put_sl_hold_default = _int('put_sl_hold_default', PUT_SL_HOLD_BARS_DEFAULT)
        put_sl_hold_monday  = _int('put_sl_hold_monday',  PUT_SL_HOLD_BARS_MONDAY)
        breadth_adap  = _bool('breadth_adaptive', True)
        hard_sell_day = _int('hard_sell_day',   HOLD_CALENDAR_DAYS)
        max_positions = _int('max_positions',   MAX_POSITIONS)

        # Updated 2026-04-28 (Phase H4): smaller cascade reduces DD.
        alloc_95plus  = _float('alloc_95plus',  18.0) / 100.0   # was 25
        alloc_85_94   = _float('alloc_85_94',   12.0) / 100.0   # was 15
        alloc_80_84   = _float('alloc_80_84',   15.0) / 100.0
        alloc_75_79   = _float('alloc_75_79',   15.0) / 100.0
        alloc_70_74   = _float('alloc_70_74',    0.0) / 100.0
        alloc_p15     = _float('alloc_p15',     10.0) / 100.0   # was 15
        alloc_p16_20  = _float('alloc_p16_20',  12.0) / 100.0
        alloc_p21_25  = _float('alloc_p21_25',  12.0) / 100.0

        # F3f breadth-driven allocation knob — Updated 2026-04-28: floors → 0.50
        breadth_alloc_enabled = _bool('breadth_alloc_enabled', True)
        f3f_call_thresh = _float('f3f_call_thresh', 50.0)
        f3f_call_floor  = _float('f3f_call_floor',  0.50)   # was 0.70
        f3f_call_low    = _float('f3f_call_low',    20.0)
        f3f_put_thresh  = _float('f3f_put_thresh',  75.0)
        f3f_put_floor   = _float('f3f_put_floor',   0.50)   # was 0.75
        f3f_put_high    = _float('f3f_put_high',    95.0)

        # Convert TP/SL % to sigma units: sigma = pct/100 * PREMIUM_MULT / DELTA
        PREMIUM_MULT = 1.82
        DELTA        = 0.5
        def _to_sigma(pct): return (pct / 100.0) * PREMIUM_MULT / DELTA
        def _net_tp(pct): return pct / 100.0 - 0.01          # gross − entry slip
        def _net_sl(pct): return -(pct / 100.0) - 0.01 - 0.013  # gross − entry − SL slip
