...
def _analyze(symbol, today, pulled_at):
    ...
    if not today_ph or today_ph.volume == 0:
        return _neutral()
    ...
    # ── Step 1: Pre-market → skip unreliable candle analysis, fall through
    #    to decay so prior day's signal carries forward until market opens ──
    premarket_gap = 0.0
    if is_premarket:
