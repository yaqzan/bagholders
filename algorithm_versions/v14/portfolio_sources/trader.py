...
def main(command=None, args=None):
    ...
    if command == 'pull' and args:
        ...
        # Single oldest-to-newest pass anchored at Jan 1 (N years ago).
        # Volume decay flows forward in time — one continuous chain is correct by design.
        _DEFAULT_YEARS = 5   # 5yr history by default
        ...
        workers   = None            # None → auto (min(cpu_count, 8)); set by --workers N
        use_batch = True
        do_force  = False
        do_full   = False
        ...
        if days_explicit is not None:
            ...
            # Single pass: Jan 1 (N years ago) → today, oldest-first, force=True.
            # Jan 1 is a natural cold-start (holiday gap exceeds all signal half-lives).
            ...
            print(Fore.CYAN + f"Recalculating {num_years}yr ({start_date} → {today}) …")
        ...
        # ── Assessment (all standard windows) ───────────────────────────
