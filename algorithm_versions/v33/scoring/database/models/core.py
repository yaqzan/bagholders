...
def _load_effective_earnings_dates(symbol):
    ...
    """
    rows = list(
        EarningsDate.select(EarningsDate.date, EarningsDate.call_time, EarningsDate.effective_date)
        .where(EarningsDate.symbol == symbol)
        .tuples()
    )
    if not rows:
        return []
    if all(eff is not None for _, _, eff in rows):
        return sorted(eff for _, _, eff in rows)
    # Fallback path — at least one row missing precomputed effective_date.
    from iv_crush_model import compute_effective_date
    return sorted(eff if eff is not None else compute_effective_date(d, ct) for d, ct, eff in rows)
