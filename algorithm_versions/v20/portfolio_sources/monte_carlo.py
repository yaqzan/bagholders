...
def load_regime_map(d_start, d_end):
    """Return (sorted_dates, {date: regime_multiplier}) — most-recent-on-or-before lookup."""
    rows = list(
...
def alloc_scale_for(regime_mult, is_put=False):
    """Return [floor..ceil] clamped scale factor for the given regime_mult.

    Asymmetric variant: if REGIME_SLOPE_UP/DOWN (or _PUT_UP/_DOWN) are set,
    use them for BULL (delta>=0) vs STRESS (delta<0) respectively.
    Falls back to symmetric REGIME_SLOPE[_PUT] when the up/down pair is None.
    """
        ...
        if day_calls:
            ...
            for sym_id, score, key in primary + overflow:
                ...
                if side_capped and call_open >= cap_call:
                    break
                alloc_frac   = TIER_ALLOC[score_to_tier(score)] * reg_scale_c
            ...
            if day_puts:
                ...
                for sym_id, score, key in pe:
                    ...
                    if side_capped and put_open >= cap_put:
                        break
                    alloc_frac   = PUT_TIER_ALLOC[put_score_to_tier(score)] * reg_scale_p
                    premium_cost = portfolio_value * alloc_frac
                    if premium_cost > cash or premium_cost <= 0:
                        continue
                    o = put_outcomes[key]
