...
# Weekly Mature Call Guard: late CALL-side guard for mature weekly setups.
WEEKLY_MATURE_CALL_GUARD_ENABLED = _envb(
    'WEEKLY_MATURE_CALL_GUARD_ENABLED',
    _S.WEEKLY_MATURE_CALL_GUARD_ENABLED,
)
WEEKLY_MATURE_CALL_GUARD_PARAMS = {
    key: _env_param(f"WEEKLY_MATURE_CALL_GUARD_{key.upper()}", value)
    for key, value in _S.WEEKLY_MATURE_CALL_GUARD_PARAMS.items()
}
...
def build_weekly_ret4_map(weekly_rows_asc, ph_rows_asc):
    """Build {daily_date: completed-week 4-week return} for scoring dates.

    Uses the same last-completed weekly bar convention as build_wv_force1_map:
    each daily score looks up the latest weekly row with date <= score_date - 7d.
    """
    import bisect
    from datetime import timedelta as _timedelta
    ...
    for ph in ph_rows_asc:
        ...
        if idx < 4:
            ...
            continue
        c_curr = weekly_closes[idx]
        c_prev4 = weekly_closes[idx - 4]
        if c_curr is None or c_prev4 is None or c_curr <= 0 or c_prev4 <= 0:
            out[dd] = None
            continue
        out[dd] = float((c_curr - c_prev4) / c_prev4)
    return out
...
def _weekly_mature_call_guard_params() -> dict:
    return {
        "enabled": WEEKLY_MATURE_CALL_GUARD_ENABLED,
        **WEEKLY_MATURE_CALL_GUARD_PARAMS,
    }
...
def _apply_weekly_mature_call_guard(
    overall: int,
    *,
    weekly_detail,
    wv_force1=None,
    ...
    """Late CALL-side guard for mature weekly setups with weak fresh momentum."""
    params = _weekly_mature_call_guard_params()
    if (not params.get("enabled")
            or weekly_detail is None
            or overall < 50):
        return overall, None
    ...
    band = (
        _sigmoid_width(
            score,
            float(params.get("score_lo", 73.0)),
            float(params.get("score_lo_width", 3.0)),
        )
        * (
            1.0
            - _sigmoid_width(
                score,
                float(params.get("score_hi", 84.0)),
                float(params.get("score_hi_width", 1.0)),
            )
        )
    )
    if band <= 1e-6:
    ...
    w_bias = _finite_float(weekly_detail.get("w_bias"), 0.0)
    w_comp = _finite_float(weekly_detail.get("w_comp"), 50.0)
    w_mom = _finite_float(weekly_detail.get("w_mom"), 0.0)

    mature = (
        float(params.get("bias_weight", 0.0))
        * _sigmoid_width(w_bias, float(params.get("bias_mid", 5.0)), float(params.get("bias_width", 4.0)))
        + float(params.get("comp_weight", 0.0))
        * _sigmoid_width(w_comp, float(params.get("comp_mid", 71.0)), float(params.get("comp_width", 4.0)))
    )
    if wv_force1 is not None:
        mature += (
            float(params.get("wv_weight", 0.0))
            * _sigmoid_width(
                _finite_float(wv_force1, 0.0),
                float(params.get("wv_mid", 0.055)),
                float(params.get("wv_width", 0.015)),
            )
        )
    if wk_ret4 is not None:
        mature += (
            float(params.get("wk4_weight", 0.0))
            * _sigmoid_width(
                _finite_float(wk_ret4, 0.0),
                float(params.get("wk4_mid", 0.09)),
                float(params.get("wk4_width", 0.05)),
            )
        )
    mature = _clamp_float(mature, 0.0, 1.0)
    if mature <= 1e-6:
        return overall, None

    mom_relief = _sigmoid_width(
        w_mom,
        float(params.get("mom_mid", 5.0)),
        float(params.get("mom_width", 1.7)),
    )
    relief = 1.0 - float(params.get("mom_relief", 0.0)) * mom_relief
    risk = _clamp_float(band * mature * relief, 0.0, 1.0)
    target = float(params.get("target", 66.0))
    if risk <= 1e-6 or score <= target:
        return overall, None

    dampen = float(params.get("k", 0.0)) * risk * (score - target)
    if dampen <= 1e-9:
        return overall, None

    new_overall = int(_clamp_float(round(score - dampen), 0, 100))
    if new_overall == overall:
        return overall, None
    ...
    return new_overall, {
        ...
        "after": new_overall,
        "delta": new_overall - int(overall),
        "dampen": round(float(dampen), 3),
        "risk": round(float(risk), 4),
        "mature": round(float(mature), 4),
        "band": round(float(band), 4),
        "w_bias": round(float(w_bias), 2),
        "w_comp": round(float(w_comp), 2),
        "w_mom": round(float(w_mom), 2),
        "wv_force1": None if wv_force1 is None else round(float(wv_force1), 4),
        "wk_ret4": None if wk_ret4 is None else round(float(wk_ret4), 4),
    }
...
def compute_overall_score(
    ...
    **_extra,
    ...
    overall, _wmcg_meta = _apply_weekly_mature_call_guard(
        overall,
        weekly_detail=weekly_detail,
        wv_force1=wv_force1,
        wk_ret4=_extra.get('wk_ret4'),
    )
    ...
    if _wmcg_meta:
        weight_info['weekly_mature_call_guard'] = _wmcg_meta
    if _pcd_active:
