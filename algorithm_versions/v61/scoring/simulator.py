...
class StockContext:
    ...
    __slots__ = (
        'stock', 'symbol',
        'ind_rows', 'ind_dates',
        'ph_map', 'ph_rows_asc', 'ph_rows_desc', 'ph_dates',
        'ws_map', 'earnings_set',
        'ret10d_sigma_map',
        'mcap_b',
        'kijun_pct_map',
    ...
    def __init__(self, stock, ind_rows, ph_rows_asc, ws_rows, earnings_dates,
        ...
        self.ph_dates    = [p.date for p in ph_rows_asc]
        ...
        self.wv_force1_map = (build_wv_force1_map(wph_rows_asc, ph_rows_asc)
                              if wph_rows_asc else {})
        self.wk_ret4_map = (build_weekly_ret4_map(wph_rows_asc, ph_rows_asc)
                            if wph_rows_asc else {})
    ...
    def ph_slice_desc_for(self, target_date) -> list:
        ...
        idx = _bisect_desc_upper(self.ph_dates, target_date)
...
class ScoreSimulator:
    ...
    def simulate_full(self, since: Optional[date] = None):
        ...
        for ctx_index, (sym, ctx) in enumerate(self._contexts.items(), start=1):
            stock = ctx.stock
            ...
            for ind_row in ctx.ind_rows:
                ...
                if vol_update:
                    vol_sig = vol_update.get('volume_signal', vol_sig)
                    vol_mag = vol_update.get('volume_magnitude', vol_mag)
                    vol_raw = vol_update.get('volume', vol_raw)
            ...
            if ctx_index % 50 == 0 or ctx_index == len(self._contexts):
                print(
                    Fore.CYAN
                    + f"[simulator] simulate_full progress {ctx_index}/{len(self._contexts)} "
                    + f"contexts, {len(rich)} rows"
                    + Style.RESET_ALL,
                    flush=True,
                )
