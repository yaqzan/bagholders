...
def main(command=None, args=None):
    ...
    if command == 'pull' and args:
        ...
        if unknown:
            print(Fore.RED + f"Unknown update arg(s): {', '.join(unknown)}")
            print(Fore.YELLOW + "Usage: trader update [--score-versions v57,v58] [--score-version v56]")
            return
        try:
            scoring_service = ScoreCalculationService(parsed_score_args.raw_versions)
        except Exception as e:
            print(Fore.RED + f"Invalid score version list: {e}")
            return
        if scoring_service.has_algorithm_versions:
            print(Fore.CYAN + "Extra algorithm score versions: " + ', '.join(scoring_service.algorithm_versions))
        try:
        ...
        from recalculate_scores import ALL_MODES, run as recalculate_run, print_db_tuning_hint
        ...
        if run_tail:
            ...
            try:
                ...
                _run_phase("backtest-temporal-30", compute_and_store_temporal)
            ...
            try:
                ...
                _run_phase("backtest-temporal-15", lambda: _temp15(dte_strategy='15'))
