...
def _cmd_alloc(args):
    ...
    CASCADE (% of portfolio equity, pre-scale) — Updated 2026-04-28 (Phase H4):
        Calls  95+:18%  85-94:12%  80-84:15%  75-79:15%  70-74:0% (disabled)
        Puts   <=15:10%  16-20:12%  21-25:12%

    EXITS (30 DTE, hard sell day 15) — Updated 2026-04-28 (Phase H5):
        Calls  TP=+35% / SL=-30%   (breadth-adaptive: TP=+40% / SL=-35% when breadth<=50)
        Puts   TP=+35% / SL=-20%   (PUT_SL fixed; widening becomes catastrophic post bug-fix)
        Hard sell  -40% (was -50% — tighter mid-window tail cap)
        PUT_SL_HOLD_BARS = 0 (was 3; hold mechanic created gap-through window)

    F3f BREADTH-DRIVEN ALLOC SCALING — Updated 2026-04-28 (Phase H4): floors → 0.50:
        Calls  scale = 1.0 if breadth >= 50, linear down to 0.50 at breadth = 20
        Puts   scale = 1.0 if breadth <= 75, linear down to 0.50 at breadth = 95
    ...
    # Updated 2026-04-28 (Phase H4): floors tightened to 0.50 for stronger
    # contraction in stressed tape.
    F3F_CALL_THRESH = 50.0   # breadth >= this -> no call cut
    F3F_CALL_FLOOR  = 0.50   # was 0.70 — Phase H4: stronger call cut at low breadth
    F3F_CALL_LOW    = 20.0   # breadth at which call floor is reached
    F3F_PUT_THRESH  = 75.0   # breadth <= this -> no put cut
    F3F_PUT_FLOOR   = 0.50   # was 0.75 — Phase H4: stronger put cut at high breadth
    ...
    print(f"  {Fore.WHITE}{Style.BRIGHT}GUIDELINE (v27 scoring + Phase H5 strategy + F3f alloc){Style.RESET_ALL}")
    ...
    print(f"    Hard sell:    {Fore.YELLOW}-40%{Style.RESET_ALL}   day 15  (was -50%; tighter mid-window cap)")
...
def _cmd_backtest(args):
    ...
    NET_HARD           = -(40.0    / 100 + 0.015)   # was -50, now -40% (Phase H5)
    ...
    print(f'  30 DTE ATM  ·  calls ≥ {min_score_arg:.0f}  puts ≤ {PUT_THRESHOLD:.0f}  ·  '
          ...
          f'hard day-{_cv(HOLD_CALENDAR_DAYS, 15)} −40%')
