...
def compute_vix_score(vix_close, vix_10d_change_pct):
    """Returns vix_score (0-100) or None on bad data."""
    if vix_close is None or vix_10d_change_pct is None:
        return None
    level_score = _band_lookup(_VIX_LEVEL_BANDS, vix_close)
    trend_score = _band_lookup(_VIX_TREND_BANDS, vix_10d_change_pct)
...
def compute_regime_composite(vix_score, market_trend, market_breadth_score=None):
    ...
    if market_breadth_score is not None:
        parts['market_breadth'] = market_breadth_score
    if vix_score is not None:
        parts['vix'] = vix_score
    if market_trend is not None:
        parts['market_trend'] = market_trend

    if not parts:
        return None

    total_weight = sum(SIGNAL_WEIGHTS[k] for k in parts)
    if total_weight == 0:
        return None
    composite = sum(parts[k] * SIGNAL_WEIGHTS[k] for k in parts) / total_weight
...
def compute_and_apply_regime(pull_date=None, vix_df=None, spy_df=None):
    ...
    composite = compute_regime_composite(vix_sc, market_trend, market_breadth_score)
...
def backfill_regime(days=365):
    ...
    for d in tqdm(scored_dates, desc="Backfilling regime"):
        ...
        composite = compute_regime_composite(vix_sc, mkt, mb_score)
