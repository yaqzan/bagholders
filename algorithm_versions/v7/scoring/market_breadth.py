...
def _get_daily_breadth(target_date: date):
    ...
    hist_rows = list(
        PriceHistory.select(
            PriceHistory.symbol,
            PriceHistory.high.alias('high'),
            PriceHistory.low.alias('low'),
        )
        .where(
            PriceHistory.symbol.in_(symbols_list),
    ...
    for row in hist_rows:
        ...
        if sym not in highs_52w or h > highs_52w[sym]:
            highs_52w[sym] = h
        if sym not in lows_52w or l < lows_52w[sym]:
            lows_52w[sym] = l
...
def compute_and_store_breadth(target_date: date = None) -> 'MarketBreadth | None':
    """
...
def backfill_breadth(days: int = 365):
    ...
    for d in tqdm(dates, desc="Breadth backfill"):
        row = compute_and_store_breadth(d)
        if row:
            ok += 1
