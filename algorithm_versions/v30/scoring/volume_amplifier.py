...
def _earnings_supp_strength(nearest_d):
    ...
    Args:
        nearest_d: calendar-day distance to nearest UPCOMING earnings event,
                   or None if no event within EARN_SUPP_WINDOW_DAYS forward.
                   Pre-only: post-earnings (negative d) returns 0.
    ...
    if nearest_d is None:
        return 0.0
    if nearest_d < 0 or nearest_d > EARN_SUPP_WINDOW_DAYS:
        return 0.0
...
def _query_earn_nearest_d_db(symbol, today):
    """Live-DB path: query EarningsDate for nearest forward earnings.

    Returns calendar-day distance in [0, EARN_SUPP_WINDOW_DAYS], or None.
    """
    try:
        from database.models.core import EarningsDate
        rows = (EarningsDate.select(EarningsDate.date)
                .where((EarningsDate.symbol == symbol) &
                       (EarningsDate.date >= today) &
                       (EarningsDate.date <= today + timedelta(days=EARN_SUPP_WINDOW_DAYS)))
                .order_by(EarningsDate.date.asc())
                .limit(1))
        for r in rows:
            return (r.date - today).days
    except Exception:
        pass
    return None
...
def _earn_nearest_d_from_set(today, earnings_dates_set):
    """Cache path: scan a pre-loaded set for nearest forward earnings.

    Returns calendar-day distance in [0, EARN_SUPP_WINDOW_DAYS], or None.
    """
    if not earnings_dates_set:
        return None
    for delta in range(0, EARN_SUPP_WINDOW_DAYS + 1):
