...
class Trader():
    ...
    def update_stocks(self, full=False, with_options=False, only_remainder=False):
        ...
        # Refresh rolling weekly indicators for any stocks that received fresh
        # daily price data. Independent of WEEKLY_MODE — we keep the table
        # populated either way so a flag flip doesn't require a full backfill.
        # Cheap: ~470 stocks × 5ms = ~3 sec per cron cycle.
        try:
            from database.utils.rolling_weekly import incremental_update
            from database.models.technical import RollingWeeklyIndicator
            RollingWeeklyIndicator.ensure_schema()
            today = date.today()
            updated_rolling = 0
            for symbol in (line.symbol if hasattr(line, 'symbol') else None
                            for line in []):
                pass  # noop placeholder; real iteration uses processed list below

            # Populate rolling indicator for today's date for each successfully
            # processed stock. We use the same set of stocks the daily scorer just
            # touched (best-effort — failures are logged but non-fatal).
            try:
                processed_stocks = list(_processed_for_rolling)  # populated above if available
            except NameError:
                processed_stocks = []

            # Fallback: refresh ALL stocks if we don't have a per-cycle list.
            if not processed_stocks:
                from database.models.core import Stock as _Stock
                processed_stocks = [s.symbol for s in _Stock.select(_Stock.symbol)]

            for sym in processed_stocks:
                try:
                    if incremental_update(sym, today):
                        updated_rolling += 1
                except Exception:
                    pass
            print(Fore.CYAN + f"Rolling weekly indicators: {updated_rolling}/{len(processed_stocks)} stocks updated for {today}")
        except Exception as e:
            print(Fore.YELLOW + f"Rolling weekly update skipped (non-fatal): {e}")
...
def _cmd_backtest(args):
    ...
    WEAK_WEEKLY_CALL_DROP     = _alloc_cfg.WEAK_WEEKLY_CALL_DROP
    WEAK_WEEKLY_CALL_MIN_OV   = _alloc_cfg.WEAK_WEEKLY_CALL_MIN_OV
    WEAK_WEEKLY_CALL_MAX_OV   = _alloc_cfg.WEAK_WEEKLY_CALL_MAX_OV
    WEAK_WEEKLY_CALL_WADJ_LT  = _alloc_cfg.WEAK_WEEKLY_CALL_WADJ_LT
    WEAK_WEEKLY_CALL_STOCH_GE = _alloc_cfg.WEAK_WEEKLY_CALL_STOCH_GE
