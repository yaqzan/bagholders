...
# ──────────────────────────────────────────────────────────────────────
# Weekly indicator mode (rolling vs calendar) — P1-P5 ROLLING WEEKLY SHIP
#
# 'calendar' (default): legacy WeeklyScore lookup keyed on Monday week_start.
#                       Calendar-week aggregated RSI/MACD/trend.
# 'rolling':            RollingWeeklyIndicator lookup keyed on daily date.
#                       Each scoring date uses a 14-bar non-overlapping 5-day
#                       ladder ending at date-1. Eliminates partial-current-
#                       week instability (the COHR-class Monday whiplash) and
#                       the calendar-week transition snap.
#
# The flag governs Score.calculate_overall_score's weekly inputs only; legacy
# WeeklyScore writes/reads continue (display, historic peaks, API endpoints).
# Switching modes requires only a flag change + recalc; no schema rollback.
#
# See experiments/rolling_weekly/FINDINGS.md for validation:
#   * Per-trade WR (faithful score functions on 10% sample, P0):
#       - 80+: rolling +4.51pp over calendar
#       - 85+: rolling +3.14pp over calendar
#       - 75+/70+/puts: rolling within ±1pp of calendar
#   * Stability: Mon |Δoverall| 6.245 → 4.384 (-30% under raw substitution;
#                                              -15% under proper scoring)
#   * COHR-class intra-day swing: eliminated by construction (rolling ladder
#     ends at T-1, so today's recompute can't change weekly inputs)
WEEKLY_MODE = os.environ.get('WEEKLY_MODE', 'rolling')   # P5 SHIP 2026-05-07
if WEEKLY_MODE not in ('calendar', 'rolling'):
    WEEKLY_MODE = 'rolling'
