...
@app.route('/api/stocks/<symbol>/rolling-weekly-indicators', methods=['GET'])
def get_rolling_weekly_indicators(symbol):
    """Get daily-keyed rolling weekly indicators for a stock.

    These are the indicators consumed by Score.calculate_overall_score when
    WEEKLY_MODE='rolling' (in strategy_config.py). Each row represents:
      * w_rsi/w_macd/w_macd_h: indicators on a 14-bar 5-day non-overlapping
        ladder ending at date - 1 trading day
      * prev_w_*: indicators on the same ladder shifted 5 days back

    See experiments/rolling_weekly/FINDINGS.md for design rationale.
    """
    try:
        from database.models.technical import RollingWeeklyIndicator
        limit = request.args.get('limit', type=int, default=100)
        rows = (RollingWeeklyIndicator
                .select()
                .where(RollingWeeklyIndicator.symbol == symbol.upper())
                .order_by(RollingWeeklyIndicator.date.desc())
                .limit(limit))
        result = []
        for r in rows:
            result.append({
                'date': r.date.isoformat(),
                'w_rsi':    float(r.w_rsi)    if r.w_rsi    is not None else None,
                'w_macd':   float(r.w_macd)   if r.w_macd   is not None else None,
                'w_macd_h': float(r.w_macd_h) if r.w_macd_h is not None else None,
                'prev_w_rsi':    float(r.prev_w_rsi)    if r.prev_w_rsi    is not None else None,
                'prev_w_macd':   float(r.prev_w_macd)   if r.prev_w_macd   is not None else None,
                'prev_w_macd_h': float(r.prev_w_macd_h) if r.prev_w_macd_h is not None else None,
        ...
        return jsonify({
            ...
            'indicators': result,
            'count': len(result),
            'weekly_mode_active': WEEKLY_MODE,
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def simulate_trader():
    ...
    try:
        ...
        # DD circuit breaker — query param override defaults to strategy config.
        DD_CIRCUIT_BREAKER = float(request.args.get('dd_breaker', _cfg.DD_CIRCUIT_BREAKER))
...
def list_backtest_runs():
    ...
    try:
        ...
        if ver_arg:
            try:
                ver = AlgorithmVersion.get(AlgorithmVersion.id == int(ver_arg))
            except Exception:
        ...
        # Pre-load version metadata for any version_ids we'll surface (most
        # rows share the active version, but historical runs may be on older
        # versions). Display label is `v<id>` (e.g. v34) — the auto-increment
        # row id is the canonical version number elsewhere in the system
        # (CLAUDE.md, version-history.md, /revert). git_commit is kept for
        ...
        runs = [
            {
                ...
                'version':          ver_meta.get(r.version_id, {'id': r.version_id, 'label': f'v{r.version_id}', 'git_commit': None}),
