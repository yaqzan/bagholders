...
class StockContext:
    ...
    def __init__(self, stock, ind_rows, ph_rows_asc, ws_rows, earnings_dates, score_pairs_asc=None):
        from database.utils.scoring import build_ret10d_sigma_map, build_velocity_5d_map
        self.stock       = stock
        ...
        # WEEKLY_MODE: pre-load rolling history once per stock when in rolling mode.
        # Sliced per scoring date inside the loop. Default 'calendar' = unchanged.
        try:
            from strategy_config import WEEKLY_MODE as _WM
        except Exception:
            _WM = 'calendar'
        self.weekly_mode = _WM
        self.rolling_history_full = None
        if _WM == 'rolling':
            from database.utils.rolling_weekly import load_rolling_history_map
            self.rolling_history_full = load_rolling_history_map(stock.symbol)
        ...
        # SVD: velocity_5d map for score velocity dampener (built from existing scores)
        self.vel5d_map = build_velocity_5d_map(score_pairs_asc) if score_pairs_asc else {}
...
class ScoreSimulator:
    ...
    def _load(self, symbols: Optional[List[str]]):
        from database.models.core import Stock, WeeklyScore, EarningsDate, MarketRegime, Score
        ...
        # SVD: bulk-load existing stored scores (any version) for velocity_5d.
        # Multiple versions per (symbol, date) — keep latest version_id per date.
        all_scores_for_vel_raw = list(
            Score.select(Score.symbol, Score.date, Score.overall, Score.version_id)
            .where(Score.symbol.in_(sym_list), Score.overall.is_null(False))
            .order_by(Score.symbol, Score.date, Score.version_id.desc())
            .namedtuples()
        )
        # Dedup: first row per (symbol, date) is highest version_id
        _seen_sym_date = set()
        all_scores_for_vel = []
        for r in all_scores_for_vel_raw:
            key = (r.symbol, r.date)
            if key not in _seen_sym_date:
                all_scores_for_vel.append(r)
                _seen_sym_date.add(key)
        ...
        for r in all_scores_for_vel: scores_by_sym[r.symbol].append((r.date, r.overall))
        ...
        for sym in sym_list:
            ...
            ctx = StockContext(
                ...
                score_pairs_asc = scores_by_sym.get(sym),
            )
    ...
    def simulate(self, since: Optional[date] = None) -> SimScores:
        ...
        for sym, ctx in self._contexts.items():
            ...
            for ind_row in ctx.ind_rows:
                ...
                if ctx.weekly_mode == 'rolling':
                    from database.utils.rolling_weekly import get_weekly_inputs_batch
                    _ws_trend = ws.trend if ws else None
                    (_, ws_rsi, ws_macd,
                     _, _prev_rsi, _prev_macd) = \
                        get_weekly_inputs_batch(ctx.symbol, d,
                                                  trend_score=_ws_trend,
                                                  full_history_desc=ctx.rolling_history_full)
                    class _PWS:
                        __slots__ = ('trend', 'rsi', 'macd')
                        def __init__(s, t, r, m):
                            s.trend = t; s.rsi = r; s.macd = m
                    pws = _PWS(_ws_trend, _prev_rsi, _prev_macd)
                else:
                ...
                if self.scoring_fn is not None:
                    ...
                    overall, _, _ = compute_overall_score(
                        ...
                        velocity_5d=ctx.vel5d_map.get(d),
                    )
