...
class Score(BaseModel):
    ...
    def calculate_overall_score(self, regime_multiplier=None):
        ...
        # Weekly context
        # WEEKLY_MODE: 'calendar' (default, legacy) or 'rolling' (P1-P5 ship).
        # Calendar reads last week's complete WeeklyScore. Rolling reads a
        # daily-keyed RollingWeeklyIndicator computed on a non-overlapping
        # 5-day ladder ending at scoring_date - 1, then runs raw values
        # through the same calculate_rsi_score / calculate_macd_score pipeline.
        # Both branches return the SAME tuple shape so downstream code is
        # unchanged. See strategy_config.WEEKLY_MODE / experiments/rolling_weekly/.
        try:
            from strategy_config import WEEKLY_MODE
        except Exception:
            WEEKLY_MODE = 'calendar'

        ws = self.weekly_score
        pws = self.prev_weekly_score if ws else None
        if WEEKLY_MODE == 'rolling':
            from database.utils.rolling_weekly import get_weekly_inputs_for_scoring
            # Trend stays calendar — rolling weekly only swaps RSI/MACD.
            cal_trend = ws.trend if ws else None
            (ws_trend, ws_rsi, ws_macd,
             prev_trend_for_pws, prev_rsi_for_pws, prev_macd_for_pws) = \
                get_weekly_inputs_for_scoring(self.symbol, self.date,
                                                trend_score=cal_trend)
            # Synthesize a "pws" tuple for downstream compatibility (the
            # compute_overall_score caller reads pws.trend / pws.rsi / pws.macd).
            class _PWS:
                __slots__ = ('trend', 'rsi', 'macd')
                def __init__(s, t, r, m):
                    s.trend = t
                    s.rsi = r
                    s.macd = m
            pws = _PWS(prev_trend_for_pws, prev_rsi_for_pws, prev_macd_for_pws)
        else:
        ...
        # SVD: velocity_5d from stored scores. For incremental single-row scoring
        # we don't yet have today's overall — use yesterday's stored score as a
        # proxy for today's pre-SVD value. velocity ≈ stored[D-1] - stored[D-5]
        # (i.e. the velocity of the prior bar; today's velocity will be
        # consistent on full recalc which uses exact same-date stored values).
        # For batched recalc this proxy is irrelevant — that path uses
        # build_velocity_5d_map() with exact (stored_at_D - stored_at_D-5).
        _vel5d = None
        try:
            _prior_score_rows = list(
                Score.select(Score.overall)
                .where((Score.symbol == self.symbol) & (Score.date < self.date) & (Score.overall.is_null(False)))
                .order_by(Score.date.desc())
                .limit(5)
            )
            # Need 5 prior bars: bars[0]=yesterday ... bars[4]=5d ago
            if len(_prior_score_rows) == 5:
                _yesterday = _prior_score_rows[0].overall
                _5d_ago = _prior_score_rows[4].overall
                if _yesterday is not None and _5d_ago is not None:
                    _vel5d = int(_yesterday) - int(_5d_ago)
        except Exception:
            _vel5d = None
...
class HistoricPeak(BaseModel):
    ...
    def ensure_schema(cls):
        ...
        try:
            ...
            pass
        # Migrate: add algorithm_version_id for version-aware cache invalidation.
        # Existing rows get NULL — rebuild via `trader historic-update` to backfill.
        try:
            DB.execute_sql('ALTER TABLE historic_peaks ADD COLUMN algorithm_version_id INT NULL')
        except Exception:
            pass
...
class Stock(BaseModel):
    ...
    def calculate_scores_batched(self, silent=False, regime_multiplier=None, regime_composite=None):
        ...
        # SVD: velocity_5d map for score velocity dampener (v40 ship 2026-05-06).
        # Built from EXISTING stored Score.overall values for the symbol.
        # Multiple versions can exist per (symbol, date); we pick the latest
        # version's overall as the canonical prior score for velocity.
        from database.utils.scoring import build_velocity_5d_map
        _existing_score_rows = list(
            Score.select(Score.date, Score.overall, Score.version_id)
            .where((Score.symbol == self.symbol) & (Score.overall.is_null(False)))
            .order_by(Score.date.asc(), Score.version_id.desc())
            .namedtuples()
        )
        # Dedup by date, keeping highest version_id (first encountered after the desc sort)
        _seen_dates = set()
        _existing_score_pairs = []
        for _r in _existing_score_rows:
            if _r.date not in _seen_dates:
                _existing_score_pairs.append((_r.date, _r.overall))
                _seen_dates.add(_r.date)
        _existing_score_pairs.sort(key=lambda x: x[0])
        _vel5d_map = build_velocity_5d_map(_existing_score_pairs)
        ...
        # WEEKLY_MODE: pre-load rolling history once per symbol when in
        # rolling mode. The history cache is shared across all scoring dates
        # for this symbol — sliced per scoring date inside the loop below.
        try:
            from strategy_config import WEEKLY_MODE as _WEEKLY_MODE
        except Exception:
            _WEEKLY_MODE = 'calendar'
        _rolling_history_full = None
        if _WEEKLY_MODE == 'rolling':
            from database.utils.rolling_weekly import load_rolling_history_map
            _rolling_history_full = load_rolling_history_map(self.symbol)
        ...
        for ind in ind_rows:
            ...
            try:
                ...
                if _WEEKLY_MODE == 'rolling':
                    from database.utils.rolling_weekly import get_weekly_inputs_batch
                    _cal_trend = ws.trend if ws else None
                    (_, ws_rsi, ws_macd,
                     _, _prev_rsi_for_pws, _prev_macd_for_pws) = \
                        get_weekly_inputs_batch(self.symbol, d,
                                                  trend_score=_cal_trend,
                                                  full_history_desc=_rolling_history_full)
                    # Synthesize a pws shadow for the compute_overall_score caller
                    class _PWS:
                        __slots__ = ('trend', 'rsi', 'macd')
                        def __init__(s, t, r, m):
                            s.trend = t
                            s.rsi = r
                            s.macd = m
                    pws = _PWS(_cal_trend, _prev_rsi_for_pws, _prev_macd_for_pws)
                else:
                    ws_rsi  = ws.rsi  if ws and ws.rsi and ws.macd else None
                    ws_macd = ws.macd if ws and ws.rsi and ws.macd else None
    ...
    def recalculate_scores_batched(self, cutoff, components, cutoff_end=None, force=False,
        ...
        # SVD: velocity_5d map for score velocity dampener (v40 ship 2026-05-06).
        # Reads existing stored Score.overall values for the symbol (any version).
        # Multiple versions can exist per (symbol, date); we pick the latest
        # version's overall as the canonical prior score for velocity.
        from database.utils.scoring import build_velocity_5d_map
        _existing_score_rows_r = list(
            Score.select(Score.date, Score.overall, Score.version_id)
            .where((Score.symbol == self.symbol) & (Score.overall.is_null(False)))
            .order_by(Score.date.asc(), Score.version_id.desc())
            .namedtuples()
        ...
        _seen_dates_r = set()
        _existing_score_pairs_r = []
        for _r in _existing_score_rows_r:
            if _r.date not in _seen_dates_r:
                _existing_score_pairs_r.append((_r.date, _r.overall))
                _seen_dates_r.add(_r.date)
        _existing_score_pairs_r.sort(key=lambda x: x[0])
        _vel5d_map = build_velocity_5d_map(_existing_score_pairs_r)
        ...
        # WEEKLY_MODE: pre-load rolling history for this symbol once if in
        # rolling mode (mirrors calculate_scores_batched above).
        try:
            from strategy_config import WEEKLY_MODE as _WEEKLY_MODE
        except Exception:
            _WEEKLY_MODE = 'calendar'
            ...
            try:
                ...
                if 'overall' in components or changed:
                    ...
                    if _WEEKLY_MODE == 'rolling':
                        ...
                        (_, ws_rsi, ws_macd,
                         _, _prev_rsi_for_pws, _prev_macd_for_pws) = \
                            get_weekly_inputs_batch(self.symbol, score.date,
                                                      trend_score=ws_trend,
                                                      full_history_desc=_rolling_history_full)
                        class _PWS:
                            __slots__ = ('trend', 'rsi', 'macd')
                            def __init__(s, t, r, m):
                                ...
                                s.rsi = r
                                s.macd = m
                        pws = _PWS(ws_trend, _prev_rsi_for_pws, _prev_macd_for_pws)
                    else:
                        ws_rsi   = ws.rsi   if ws and ws.rsi and ws.macd else None
                        ws_macd  = ws.macd  if ws and ws.rsi and ws.macd else None
                    ...
                    overall, weight_info, vol_update = compute_overall_score(
                        ...
                        velocity_5d=_vel5d_map.get(score.date),
                    )
