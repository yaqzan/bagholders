...
NET_TP_BASE   = _opt.NET_TP_BASE
NET_TP_STRESS = _opt.NET_TP_STRESS
NET_SL_BASE   = _opt.NET_SL_BASE
NET_SL_STRESS = _opt.NET_SL_STRESS
NET_HARD_SELL = _cfg.NET_HARD_SELL

# Derived: underlying σ thresholds for TP/SL barrier-touch detection.
# Note: 15 DTE PREMIUM_MULT=1.29 (vs 30 DTE 1.82) makes σ smaller for the
# same option TP/SL — captured by @property scaling.
TP_SIGMA_BASE   = _cfg.TP_SIGMA_BASE
TP_SIGMA_STRESS = _cfg.TP_SIGMA_STRESS
SL_SIGMA_BASE   = _cfg.SL_SIGMA_BASE
SL_SIGMA_STRESS = _cfg.SL_SIGMA_STRESS
...
# DD circuit breaker — 15 DTE C1 uses 0.60 (tighter than 30 DTE's 0.68)
# to clear the 80% Conservative DD-C floor every window.
...
def resolve(outcome, rng):
    ...
    if (DEAD_HOLD_ENABLED and kind == 'sl'
        ...
        return dh_kind, dh_pnl + SLIP_ENTRY
