...
from database import Stock, PriceHistory, Indicator, Score, Option, OptionPrice, Position, Trend
...
class Trader():
    ...
    def pull_price_history(self, symbol, period='max'):
        attempts = 0
        while attempts < 5:
            data = yf.download([symbol], period=period, interval='1d',rounding=True, auto_adjust=False)
            if len(data) > 0: break
            if 'YFPricesMissingError' in shared._ERRORS[symbol]:
                return
            attempts += 1
    ...
    def pull_premarket_data(self, symbol):
    ...
    def update_stocks(self, full=False, with_options=False, only_remainder=False):
        ...
        stocks = Stock.select(Stock.symbol, Stock.forward_pe)
        if only_remainder:
        ...
        for stock in stocks:
            ...
            if premarket:
                if period != '1d': 
                    self.pull_price_history(stock.symbol, period)
                self.pull_premarket_data(stock.symbol)
            else:
                if with_options:
                    threading.Thread(target=self.pull_options, args=(stock.symbol,)).start()
                self.pull_price_history(stock.symbol, period)
...
def main(command=None, args=None):
    ...
    :param str command: what to do with trader. Can be 'pull', 'calculate-indicators, indices'
    ...
    if command == 'pull' and args:
        ...
        symbol, days, components = None, DEFAULT_LOOKBACK, set()
        days_from_numeric = False
        if args:
            for arg in args.split(' '):
                stripped = arg.lstrip('-')
                if arg == '--all':
                    ...
                    try:
                        days = int(arg)
                        days_from_numeric = True
                    except ValueError:
                        symbol = arg.upper()
        if not components:
            components = ALL_MODES.copy() if (days_from_numeric or symbol) else {'overall'}
        recalculate_run(symbol, days, components)
    elif command == 'backfill':
        BACKFILL_WINDOWS = [('1y', 365), ('2y', 730), ('3y', 1095), ('5y', 1825), ('10y', 3650), ('25y', 9125)]
        symbol, timeframes = None, []
        if args:
            for arg in args.split(' '):
                low = arg.lower()
                if low == 'all':
                    continue
                matched = next((w for w in BACKFILL_WINDOWS if w[0] == low), None)
                if matched:
                    timeframes.append(matched)
                else:
                    symbol = arg.upper()
        if not timeframes:
            timeframes = BACKFILL_WINDOWS
        ...
        total_f, total_e = 0, 0
        for wlabel, wdays in timeframes:
            cutoff = date.today() - timedelta(days=wdays)
            print(Fore.CYAN + f"Backfilling {wlabel} ({len(stocks)} stocks, cutoff {cutoff})...")
            for stock in tqdm(stocks, desc=f"Backfill {wlabel}", disable=len(stocks) == 1):
                f, e = stock.backfill_scores(cutoff=cutoff)
                total_f += f; total_e += e
            print(Fore.GREEN + f"  {wlabel}: {total_f} filled, {total_e} errors so far")
        print(Fore.GREEN + f"Done: {total_f} filled, {total_e} errors")
    elif command == 'assess':
        from assess_scores import run as assess_run, compare as assess_compare, meta as assess_meta, DEFAULT_LOOKBACK as ASSESS_LOOKBACK
