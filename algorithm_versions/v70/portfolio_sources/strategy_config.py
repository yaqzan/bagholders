...
OPT_30DTE = OptionStrategyConfig(
    TP_BASE=0.28,   # 0.33->0.28 v69-honest retune 2026-05-31: lower TP = capital
                    # velocity on the thin honest call edge (Stage B: 5y -38%->-1%
                    # at 75+; wider SL HURT). See experiments/v69_portfolio_retune/.
    TP_STRESS=0.42,
    SL_BASE=-0.27,
    SL_STRESS=-0.40,
...
STRATEGY_30DTE = DteStrategyConfig(
    ...
    # v69-honest retune 2026-05-31: down from 14/12/8. On honest (look-ahead-free)
    # v69 scores the old sizing COLLAPSES (5y 100% collapse, 92% DD) because the
    # honest call edge is thin and puts are net-negative. 85+-only, downsized,
    # puts-off (see TIER_ALLOC). N=500: 5y +130%/35%DD, 0 collapse, 7/8 windows +.
    # NOTE (diag2): does NOT beat SPY after realistic option spreads — hygiene
    # config, not a capital-allocation recommendation. experiments/v69_portfolio_retune/.
    MAX_POSITIONS=8,
    MAX_POSITIONS_CALL=7,
    MAX_POSITIONS_PUT=2,
    # Sentinel Stage 3 practical exposure saturation. Promotion candidate:
    ...
    TIER_ALLOC={
        'ultra':    0.13,   # 95+
        'top':      0.0975, # 85-94
        'mid':      0.00,   # 80-84 DROPPED (honest TP ~48% < 49% BE)
        'low':      0.00,   # 75-79 DROPPED (honest TP ~46% < BE)
