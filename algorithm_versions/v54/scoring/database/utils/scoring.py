...
# POET — sector ETF participation echo/thrust wave
POET_WAVE_ENABLED           = _envb('POET_WAVE_ENABLED',      _S.POET_WAVE_ENABLED)
POET_CALL_MIN               = _envi('POET_CALL_MIN',          _S.POET_CALL_MIN)
POET_PUT_MAX                = _envi('POET_PUT_MAX',           _S.POET_PUT_MAX)
POET_CRASH_K                = _envf('POET_CRASH_K',           _S.POET_CRASH_K)
POET_CALL_TARGET            = _envf('POET_CALL_TARGET',       _S.POET_CALL_TARGET)
POET_CRASH_DECAY            = _envf('POET_CRASH_DECAY',       _S.POET_CRASH_DECAY)
POET_REPAIR_K               = _envf('POET_REPAIR_K',          _S.POET_REPAIR_K)
POET_SEED_LEVEL             = _envf('POET_SEED_LEVEL',        _S.POET_SEED_LEVEL)
POET_SEED_VELOCITY          = _envf('POET_SEED_VELOCITY',     _S.POET_SEED_VELOCITY)
POET_SEED_RSI               = _envf('POET_SEED_RSI',          _S.POET_SEED_RSI)
POET_RELIEF_BRD_START       = _envf('POET_RELIEF_BRD_START',  _S.POET_RELIEF_BRD_START)
POET_RELIEF_BRD_FULL        = _envf('POET_RELIEF_BRD_FULL',   _S.POET_RELIEF_BRD_FULL)
POET_RELIEF_AVG5_START      = _envf('POET_RELIEF_AVG5_START', _S.POET_RELIEF_AVG5_START)
POET_RELIEF_AVG5_FULL       = _envf('POET_RELIEF_AVG5_FULL',  _S.POET_RELIEF_AVG5_FULL)
POET_CRASH_POWER            = _envf('POET_CRASH_POWER',       _S.POET_CRASH_POWER)
POET_BULL_RELEASE_K         = _envf('POET_BULL_RELEASE_K',    _S.POET_BULL_RELEASE_K)
POET_PUT_K                  = _envf('POET_PUT_K',             _S.POET_PUT_K)
POET_PUT_TARGET             = _envf('POET_PUT_TARGET',        _S.POET_PUT_TARGET)
POET_BULL_DECAY             = _envf('POET_BULL_DECAY',        _S.POET_BULL_DECAY)
POET_BULL_VELOCITY          = _envf('POET_BULL_VELOCITY',     _S.POET_BULL_VELOCITY)
POET_BULL_LEVEL             = _envf('POET_BULL_LEVEL',        _S.POET_BULL_LEVEL)
POET_BULL_WASH_LEVEL        = _envf('POET_BULL_WASH_LEVEL',   _S.POET_BULL_WASH_LEVEL)
POET_BULL_POWER             = _envf('POET_BULL_POWER',        _S.POET_BULL_POWER)
...
def _poet_params() -> dict:
    return {
        "enabled": POET_WAVE_ENABLED,
        "crash_k": POET_CRASH_K,
        "call_target": POET_CALL_TARGET,
        "crash_decay": POET_CRASH_DECAY,
        "repair_k": POET_REPAIR_K,
        "seed_level": POET_SEED_LEVEL,
        "seed_velocity": POET_SEED_VELOCITY,
        "seed_rsi": POET_SEED_RSI,
        "relief_brd_start": POET_RELIEF_BRD_START,
        "relief_brd_full": POET_RELIEF_BRD_FULL,
        "relief_avg5_start": POET_RELIEF_AVG5_START,
        "relief_avg5_full": POET_RELIEF_AVG5_FULL,
        "crash_power": POET_CRASH_POWER,
        "bull_release_k": POET_BULL_RELEASE_K,
        "put_k": POET_PUT_K,
        "put_target": POET_PUT_TARGET,
        "bull_decay": POET_BULL_DECAY,
        "bull_velocity": POET_BULL_VELOCITY,
        "bull_level": POET_BULL_LEVEL,
        "bull_wash_level": POET_BULL_WASH_LEVEL,
        "bull_power": POET_BULL_POWER,
    }
...
def compute_overall_score(
    ...
    **_extra,
    ...
    # low stochastic phase contradicts call entries, but the penalty decays
    # sharply with score conviction so 80+ tiers are preserved after rounding.
    _scw_dampen = 0.0
    if (SCW_ENABLED
    ...
    # POET — sector ETF Participation Echo/Thrust wave. This is intentionally
    # late-stage and signal-band only, matching the Bayesian replay universe:
    # CALL scores can be dampened below entry during crash echo; PUT scores can
    # be neutralized during bull-repair thrust.
    _poet_meta = None
    if POET_WAVE_ENABLED and score_date is not None:
        _poet_side = None
        if overall >= POET_CALL_MIN:
            _poet_side = "call"
        elif overall <= POET_PUT_MAX:
            _poet_side = "put"
        if _poet_side is not None:
            _poet_before = int(overall)
            _poet_after, _poet_wave = _sector_breadth_adjusted_score(
                _poet_before, _poet_side, score_date, _poet_params()
            )
            if _poet_after != _poet_before:
                overall = _poet_after
                _poet_meta = {
                    "side": _poet_side,
                    "before": _poet_before,
                    "after": _poet_after,
                    "delta": _poet_after - _poet_before,
                    "crash": round(float(_poet_wave.get("crash", 0.0)), 4),
                    "bull": round(float(_poet_wave.get("bull", 0.0)), 4),
                    "pressure": round(float(_poet_wave.get("pressure", 0.0)), 4),
                }
    ...
    if _poet_meta:
        weight_info['poet_wave'] = _poet_meta
    if _pcd_active:
