...
# ── Sector ETF crash-echo breadth ────────────────────────────────────────────

SECTOR_ETF_SYMBOLS = ['XLK', 'XLV', 'XLY', 'XLI', 'XLB', 'XLRE', 'XLF', 'XLE', 'XLP', 'XLU', 'XLC']
SECTOR_ETF_START_DATE = date(2020, 1, 1)

SECTOR_ETF_DUAL_WAVE_PARAMS = {
    # Dashboard echo metrics mirror the shipped POET score dampener.
    'bull_decay': _SCORING.POET_BULL_DECAY,
    'bull_level': _SCORING.POET_BULL_LEVEL,
    'bull_power': _SCORING.POET_BULL_POWER,
    'bull_release_k': _SCORING.POET_BULL_RELEASE_K,
    'bull_velocity': _SCORING.POET_BULL_VELOCITY,
    'bull_wash_level': _SCORING.POET_BULL_WASH_LEVEL,
    'crash_decay': _SCORING.POET_CRASH_DECAY,
    'relief_avg5_full': _SCORING.POET_RELIEF_AVG5_FULL,
    'relief_avg5_start': _SCORING.POET_RELIEF_AVG5_START,
    'relief_brd_full': _SCORING.POET_RELIEF_BRD_FULL,
    'relief_brd_start': _SCORING.POET_RELIEF_BRD_START,
    'repair_k': _SCORING.POET_REPAIR_K,
    'seed_level': _SCORING.POET_SEED_LEVEL,
    'seed_rsi': _SCORING.POET_SEED_RSI,
    'seed_velocity': _SCORING.POET_SEED_VELOCITY,
}
...
def _annotate_sector_etf_breadth_rows(rows: list[dict]) -> None:
    """Add crash echo / bull repair metrics to sector ETF breadth rows."""
    ...
    bull_wave = 0.0
    for row in rows:
        brd = float(row['pct_above_ema50'] if row['pct_above_ema50'] is not None else 50.0)
        rsi = float(row['avg_rsi'] if row['avg_rsi'] is not None else 50.0)
        d5 = float(row.get('breadth_5d_change') or 0.0)
        d10 = float(row.get('breadth_10d_change') or 0.0)
        avg5 = float(row['breadth_avg5'] if row['breadth_avg5'] is not None else 0.0)
        min20 = float(row['min20'] if row['min20'] is not None else brd)
