...
class Stock(BaseModel):
    ...
    def recalculate_scores_batched(self, cutoff, components, cutoff_end=None, force=False,
        ...
        # Continuation echo: load barrier wins + pre-compute prior_signal_map.
        # Use pre_boost when present so recalculations stay no-cascade.
        # Signal map built once before the scoring loop; passed per-date to compute_overall_score.
        _cb_barrier_wins = {}
        _cb_prior_signal_map = {}
        if CONT_BOOST_ENABLED:
            ...
            _score_dates = [s.date for s in scores]
            for _i, _s in enumerate(scores):
                _d = _s.date
                _lo_d = _d - timedelta(days=90)
                _hi_d = _d - timedelta(days=1)
                _i_lo = bisect.bisect_left(_score_dates, _lo_d)
                _i_hi = bisect.bisect_right(_score_dates, _hi_d)
                _prior_pairs = [(scores[_j].date, _cont_preboost_or_overall(scores[_j]))
                                for _j in range(_i_lo, _i_hi)
                                if scores[_j].overall is not None]
                _cb_prior_signal_map[_d] = compute_cont_prior_signal(
                    _d, _prior_pairs, _cb_barrier_wins
                )
            ...
            try:
                ...
                if 'overall' in components or changed:
                    ...
                    overall, weight_info, vol_update = compute_overall_score(
                        ...
                        prior_signal=_cb_prior_signal_map.get(score.date),
