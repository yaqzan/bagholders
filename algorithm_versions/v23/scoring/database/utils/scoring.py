...
def compute_overall_score(
    ...
    *,
    ...
    # ── Floored-regime x_conf gate (v23, Priority #8) ─────────────────────────
    # Counter-trend capture WITHOUT the v22 bull-year regression. Gate only
    # bites when regime is stressed (regime_mult < 0.92) — bull/neutral regimes
    # pass through with d_eff = d, preserving TREND compounding in 2024-type
    # tapes that v22's uniform gate destroyed (−91% on 2024 Realistic).
    #
    # Formula: d_eff = d × max(x_conf^0.50, 0.50)
    #   x_conf = sig_str × agreement²  over 5 base components.
    # Parameters α=0.50, floor=0.50, regime_cutoff=0.92 chosen from Phase I
    # floor sweep + Phase J canonical 3-mode MC. fl050_rc092 cleared both
    # per-trade (CT-PUT WR15 81.3% ≥ 81%) and portfolio gates (DD-C 69.3%/58.5%
    # < 80%, Realistic 2.57×/1.24× v19 baseline on 2023/2025).
    #
    # Put/call direction uses base_sum computed from ungated `d` to mirror the
    # regime-multiplier selection downstream (put_regime_multiplier when bearish).
    _n = 5
    _comps = (trend, bb, rsi, macd, stoch)
    _bull = sum(1 for s in _comps if s >= 50)
    _bear = _n - _bull
    _agreement = max(_bull, _bear) / _n
    _sig_str = min(abs(sum(s - 50 for s in _comps) / _n) / 50.0, 1.0)
    _x_conf = _sig_str * (_agreement ** 2)

    # Naive weighted_sum using ungated d — only used to select put vs call
    # direction for the regime-multiplier gate decision.
    _w_tr0 = 18 + 10 * d
    _w_rs0 = 25 -  9 * d
    _w_md0 = 25 -  6 * d
    _w_ta0 =  9 +  6 * d
    _base_sum = 50 + (
        ...
        (stoch - 50) * 5 +
        (macd  - 50) * _w_md0 +
        (technical_alignment - 50) * _w_ta0
    ) / 100

    _gate_mult = (put_regime_multiplier
                  if (_base_sum < 50 and put_regime_multiplier is not None)
                  else regime_multiplier)
    X_CONF_ALPHA       = 0.50
    X_CONF_FLOOR       = 0.50
    X_CONF_REG_CUTOFF  = 0.92
    if _gate_mult is not None and _gate_mult < X_CONF_REG_CUTOFF:
        d = d * max(_x_conf ** X_CONF_ALPHA, X_CONF_FLOOR)
