...
class Score(BaseModel):
    ...
    def calculate_overall_score(self, regime_multiplier=None):
        ...
        try:
            ...
            from database.utils.scoring import build_kijun_pct_map as _build_kij
            _wph_rows = list(
                _WPH.select(_WPH.date, _WPH.high, _WPH.low)
...
class Stock(BaseModel):
    ...
    def calculate_scores_batched(self, silent=False, regime_multiplier=None, regime_composite=None):
        ...
        # (one-time per symbol; weekly Kijun-sen evolves slowly)
        from database.models.technical import WeeklyPriceHistory as _WPH_ich
        from database.utils.scoring import build_kijun_pct_map as _build_kij_map
        _wph_rows_asc = list(
    ...
    def recalculate_scores_batched(self, cutoff, components, cutoff_end=None, force=False,
        ...
        # ICH: kijun_pct map for Ichimoku Kijun-sen state dampener
        # (one-time per symbol; weekly Kijun-sen evolves slowly so a single
        # bulk computation covers the full scoring window)
        from database.models.technical import WeeklyPriceHistory as _WPH_ich
        from database.utils.scoring import build_kijun_pct_map as _build_kij_map
        _wph_q_ich = _WPH_ich.select(_WPH_ich.date, _WPH_ich.high, _WPH_ich.low).where(
