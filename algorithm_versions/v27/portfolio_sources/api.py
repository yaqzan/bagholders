...
def backtest_temporal():
    """Return pre-computed cascade-backtest temporal stats for the active algorithm version.
    Populated by `trader assess --force` via compute_and_store_temporal()."""
    try:
        ...
        try:
            ...
            return jsonify({
                ...
                'error': 'No temporal stats computed yet. Run `trader assess --force` to generate.',
            }), 404
...
def run_backtest_endpoint():
    ...
    try:
        ...
        tp_pct        = _float('tp_pct',        30.0)
        tp_stress_pct = _float('tp_stress_pct', 35.0)
        sl_pct        = _float('sl_pct',        35.0)
        sl_stress_pct = _float('sl_stress_pct', 40.0)
        put_tp_pct    = _float('put_tp_pct',    30.0)
        ...
        alloc_95plus  = _float('alloc_95plus',  25.0) / 100.0
        alloc_85_94   = _float('alloc_85_94',   15.0) / 100.0
        ...
        alloc_p15     = _float('alloc_p15',     15.0) / 100.0
        ...
        f3f_call_floor  = _float('f3f_call_floor',  0.70)
        f3f_call_low    = _float('f3f_call_low',    20.0)
        f3f_put_thresh  = _float('f3f_put_thresh',  75.0)
        f3f_put_floor   = _float('f3f_put_floor',   0.75)
