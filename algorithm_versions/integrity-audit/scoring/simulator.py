...
class StockContext:
    ...
    def __init__(self, stock, ind_rows, ph_rows_asc, ws_rows, earnings_dates,
                 wph_rows_asc=None, pit_window_lo=None):
        ...
        from database.utils.weekly_pit import build_pit_weekly_map
        self.stock       = stock
        ...
        self.ret10d_sigma_map = build_ret10d_sigma_map(ph_rows_asc)
        # MCD: mcap_b for mcap dampener — point-in-time proxy per date (F4
        # survivorship fix 2026-06-09). The simulator bulk-loads history
        # through the latest date, so the last loaded close IS the anchor
        # matching the current market_cap snapshot.
        self.mcap_b = float(stock.market_cap) / 1e9 if stock.market_cap is not None else None
        _mcap_anchor = None
        for _p in reversed(ph_rows_asc):
            if _p.close is not None:
                _mcap_anchor = float(_p.close)
                break
        self.pit_mcap_map = (build_pit_mcap_map(self.mcap_b, ph_rows_asc, _mcap_anchor)
                             if self.mcap_b is not None else {})
        # ICH: kijun_pct map (weekly Kijun-sen state, one-time per symbol)
        self.kijun_pct_map = (build_kijun_pct_map(wph_rows_asc, ph_rows_asc)
                              if wph_rows_asc else {})
        # WVD-Wave: wv_force1 map (weekly volume force, one-time per symbol)
        self.wv_force1_map = (build_wv_force1_map(wph_rows_asc, ph_rows_asc)
                              if wph_rows_asc else {})
        # Weekly transition blend: point-in-time PARTIAL current-week weekly
        # component scores per date (state 2). {date -> (trend,rsi,macd,t)}.
        # Only compute for the scoring window (pit_window_lo); the deep daily
        # history is loaded purely to warm up weekly EMA200/MACD.
        _pit_targets = None
        if pit_window_lo is not None:
            _pit_targets = [p.date for p in ph_rows_asc if p.date >= pit_window_lo]
        self.pit_weekly_map = build_pit_weekly_map(stock, ph_rows_asc,
                                                   target_dates=_pit_targets)
...
class ScoreSimulator:
    ...
    def __init__(
        ...
        # Opt-in weight_info capture for validation (default off; no behavior
        # change). When `capture_weight_info=True`, `simulate()` records the full
        # weight_info per (symbol, date) into `self.weight_info_by_key`.
        self.capture_weight_info: bool = False
        self.weight_info_by_key: Dict[Tuple[str, date], dict] = {}
        # Opt-in INPUT capture (default off; no behavior change). When
        # `capture_inputs=True`, `simulate()` records the full compute_overall_score
        # kwargs per (symbol, date) with default-overall >= `capture_min_overall`
        # into `self.inputs_by_key`. Lets a reweight sweep re-score by calling
        # compute_overall_score on the cached (weight-invariant) inputs instead of
        # re-running the expensive per-signal component recompute every variant.
        self.capture_inputs: bool = False
        self.capture_min_overall: int = 60
        self.inputs_by_key: Dict[Tuple[str, date], tuple] = {}
        self._load(symbols)
    ...
    def _build_mis_stress_map(self) -> Dict[date, float]:
        ...
        def _spy_wk(d):
            # F2 fix (2026-06-09): last-COMPLETED-week lookup. WeeklyScore rows
            # are Monday-keyed complete weeks; an on_or_before(d) lookup selects
            # the current (future-containing) week for Mon-Thu dates. Lag 7d to
            # land in the prior week — mirrors core._spy_wk_last_completed.
            from datetime import timedelta as _td
            i = bisect.bisect_right(spy_wk_dates, d - _td(days=7)) - 1
    ...
    def _load(self, symbols: Optional[List[str]]):
        ...
        # extra buffer so sliding-window indicators have enough history. The
        # weekly-transition PIT reconstruction needs ~6y of daily bars to warm
        # up weekly EMA200/MACD before the earliest scored date, so the buffer
        # must exceed the daily-indicator 400d need (otherwise early-window
        # dates silently fall back to completed-week-only).
        cutoff = date.today() - timedelta(days=self.lookback_days + 2200)
        ...
        # ICH: weekly OHLC (high/low for Kijun-sen + close optional). Filter
        # by `cutoff - 365d` so we have enough lookback (26 weekly bars = ~6 mo)
        # for the kijun computation at the start of the cutoff window.
        from database.models.technical import WeeklyPriceHistory
        _wph_cutoff = cutoff - timedelta(days=365)
        all_wph = list(
            WeeklyPriceHistory.select(
                WeeklyPriceHistory.symbol, WeeklyPriceHistory.date,
                WeeklyPriceHistory.high, WeeklyPriceHistory.low,
                WeeklyPriceHistory.close, WeeklyPriceHistory.volume,
            )
            .where(
                WeeklyPriceHistory.symbol.in_(sym_list),
                WeeklyPriceHistory.date >= _wph_cutoff,
            )
            .order_by(WeeklyPriceHistory.symbol, WeeklyPriceHistory.date)
            .namedtuples()
        )
        ...
        # ── JA4 put-regime-multiplier map: date → blended put mult ──────────
        # Production paths (core.py) pass put_regime_multiplier into
        # compute_overall_score; the simulator historically did NOT (a known
        # sim/production divergence on the put side). Default {} = legacy
        # behavior (kwarg None). Populate from a harness (e.g. the 2026-06-09
        # integrity-audit A/B) to evaluate JA4 in-simulator.
        self._put_mult_map: Dict[date, float] = {}
        ...
        for r in all_wph:   wph_by_sym[r.symbol].append(r)
        ...
        for sym in sym_list:
            ...
            ctx = StockContext(
                ...
                wph_rows_asc = wph_by_sym[sym],
                # Only reconstruct the PIT partial-week map for the scoring
                # window (+30d slack); deep daily history is warmup-only.
                pit_window_lo = date.today() - timedelta(days=self.lookback_days + 30),
            )
    ...
    def simulate(self, since: Optional[date] = None) -> SimScores:
        ...
        for sym, ctx in self._contexts.items():
            ...
            for ind_row in ctx.ind_rows:
                ...
                # signals.  Now W-1 / W-2, blended with the point-in-time PARTIAL
                # current week (state 2) in the production path below.
                ws      = ctx.ws_map.get(_week_start(d) - timedelta(weeks=1))
                pws     = ctx.ws_map.get(_week_start(d) - timedelta(weeks=2))
                ws_rsi  = ws.rsi  if ws and ws.rsi and ws.macd else None
                ws_macd = ws.macd if ws and ws.rsi and ws.macd else None
                _pit_wk = ctx.pit_weekly_map.get(d)
                ...
                if self.scoring_fn is not None:
                    ...
                    if ind_row.ema_200 and ph and ph.close:
                        _pct_ema200 = (float(ph.close) - float(ind_row.ema_200)) / float(ind_row.ema_200) * 100
                    # Days to next earnings within boost window
                    _d_to_ern = None
                    if ctx.earnings_set:
                        _w = 5  # EARN_BOOST_WINDOW (kept local; scoring.py reads env)
                        for _delta in range(_w + 1):
                            if (d + timedelta(days=_delta)) in ctx.earnings_set:
                                _d_to_ern = _delta; break
                    overall, _wi_prod, _ = compute_overall_score(
                        trend, bb, rsi, macd, stoch, ta,
                        bb_pct=bb_pct,
                        pct_from_ema50=_pct_ema50,
                        pct_from_ema200=_pct_ema200,
                        ws_trend=ws.trend if ws else None,
                        ws_rsi=ws_rsi, ws_macd=ws_macd,
                        prev_ws_trend=pws.trend if pws else None,
                        prev_ws_rsi=pws.rsi   if pws else None,
                        ...
                        macdh_raw=float(ind_row.macd_hist) if ind_row.macd_hist is not None else None,
                        regime_multiplier=_r_mult,
                        put_regime_multiplier=self._put_mult_map.get(d),
                        mis_stress=_mis_stress,
                        days_to_earnings=_d_to_ern,
                        ret_10d_sigma=ctx.ret10d_sigma_map.get(d),
                        mcap_b=ctx.pit_mcap_map.get(d, ctx.mcap_b),
                        kijun_pct=ctx.kijun_pct_map.get(d),
                        wv_force1=ctx.wv_force1_map.get(d),
                        score_date=d,
                    )
                    if self.capture_weight_info and overall is not None:
                        self.weight_info_by_key[(sym, d)] = _wi_prod
                    if (self.capture_inputs and overall is not None
                            and overall >= self.capture_min_overall):
                        # cache weight-INVARIANT compute_overall_score inputs so a reweight
                        # sweep re-scores via compute_overall_score only (no component recompute).
                        self.inputs_by_key[(sym, d)] = (
                            (trend, bb, rsi, macd, stoch, ta),
                            dict(
                                bb_pct=bb_pct, pct_from_ema50=_pct_ema50, pct_from_ema200=_pct_ema200,
                                ws_trend=(ws.trend if ws else None), ws_rsi=ws_rsi, ws_macd=ws_macd,
                                prev_ws_trend=(pws.trend if pws else None),
                                prev_ws_rsi=(pws.rsi if pws else None),
                                prev_ws_macd=(pws.macd if pws else None),
                                pit_ws_trend=_pit_ws_trend, pit_ws_rsi=_pit_ws_rsi,
                                ...
                                mcap_b=ctx.pit_mcap_map.get(d, ctx.mcap_b),
                                kijun_pct=ctx.kijun_pct_map.get(d),
                                wv_force1=ctx.wv_force1_map.get(d), score_date=d,
                            ),
                        )
    ...
    def simulate_full(self, since: Optional[date] = None):
        """Like simulate() but retains auxiliary fields needed by monte_carlo.

        Returns dict mapping (symbol, date) → tuple
            (overall, trend, weight_info_dict, volume_signal_str, regime_mult, symbol_id)

        Used by experiments/sim_mc_bridge.py to feed MC's load_signals /
        load_put_signals from in-memory simulator output instead of DB.

        Slightly slower than simulate() because we capture more state per row;
        same scoring computation otherwise.
        """
        from database.utils.scoring import compute_overall_score
        from volume_amplifier import get_volume_multiplier_from_cache

        cutoff = since or (date.today() - timedelta(days=self.lookback_days))
        rich: Dict[Tuple[str, date], Tuple] = {}

        for sym, ctx in self._contexts.items():
            stock = ctx.stock
            # Stock model uses `symbol` as primary key (CharField). Peewee's
            # `Score.symbol_id` returns the underlying FK value, which is the
            # ticker string itself — NOT an integer. So sym_id == sym.
            sym_id = sym
            prior_vol_signals: Dict[date, Tuple[str, float, int]] = {}
            ...
            for ind_row in ctx.ind_rows:
                ...
                overall, _, weight_info = compute_overall_score(
                    trend, bb, rsi, macd, stoch, ta,
                    bb_pct=bb_pct,
                    pct_from_ema50=_pct_ema50,
                    pct_from_ema200=_pct_ema200,
                ...
                regime_mult = float(_r_mult) if _r_mult is not None else 1.0
                rich[(sym, d)] = (int(overall), int(trend), weight_info or {},
                                  vol_sig, regime_mult, sym_id)
                prior_vol_signals[d] = (vol_sig, vol_mag, vol_raw)

        print(Fore.GREEN + f"[simulator] simulate_full produced {len(rich)} rich rows" + Style.RESET_ALL)
        return rich
