...
#
# REMOVED 2026-06-04 (set to None) per user direction: the 2026-05-15 lock was
# only ~3 weeks old, so its forward window held too few barrier-resolved peaks
# to validate anything yet, while it DID block calibrating on the most recent
# (current-regime) data. With the cutoff None, `experiments._holdout` treats the
# lock as disabled — `assert_no_holdout_leak` / `pre_cutoff_filter` are no-ops —
# so existing sweep call-sites keep working unchanged. To re-instate a holdout,
# set this back to an ISO `yyyy-mm-dd` string.
CALIBRATION_CUTOFF_DATE: str | None = None
...
STRATEGY_30DTE = DteStrategyConfig(
    ...
    TIER_ALLOC={
        ...
        'mid':      0.10,   # 80-84
        'low':      0.10,   # 75-79
        # 70-74 OVERFLOW enabled 2026-06-03: fills the IDLE Apex book (3204->8821 fills)
        # INSIDE the 50% gross cap; never displaces higher-conviction 75+. N=500 ship-grade:
        # +15x 10y, collapse-safe on every window incl 2020-COVID under the LIVE asymmetric
        # cost canon. CAVEAT: the edge is execution-conditional (limit-TP avoids spread);
        # under a conservative symmetric ~3% round-trip it evaporates + the worst-case-crash
        # entry gets fragile -> the regime-knob crash-protection (in flight) is the mitigation.
        'overflow': 0.035,  # 70-74 (return-optimal collapse-safe alloc; cliff at 0.045)
    },
