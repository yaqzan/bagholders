...
def main(command=None, args=None):
    ...
    if command == 'pull' and args:
        ...
        if sub not in ('sync', 'reset', 'status'):
            print(Fore.YELLOW + 'Usage: trader portfolio [sync|reset|status]')
            return
...
def _cmd_intraday_drill(args):
    ...
    Decomposes the intraday swing into components -> weekly -> dampener/volume ->
    boost -> regime and names the dominant cause.
