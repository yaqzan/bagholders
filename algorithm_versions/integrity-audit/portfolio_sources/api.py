...
def _version_payload(version, active_version=None):
    ...
    return {
        ...
        'is_active': is_active,
        'honest': version.id in _honest_version_ids(),
    }
...
def live_allocation_endpoint():
    ...
    try:
        ...
        payload = build_live_allocation_plan(
            ...
            portfolio_profile=request.args.get('profile') or request.args.get('portfolio_profile') or 'apex',
...
def get_intraday_swing(symbol):
    ...
    plus the dominant cause of the day's score swing (weekly / dampener-volume /
    boost / regime). `?date=YYYY-MM-DD` overrides the default (most recent
    ...
    try:
        ...
        for today_d in all_dates:
            ...
            for o in outcomes_by_date.get(today_d, []):
                ...
                premium = TIER_ALLOC[o['tier']] * scale * dd_scale * rxdd_scale * svr_scale * mwdd_scale * tvdd_scale * equity
...
def _resolve_portfolio_profile_arg(default: str = 'apex') -> tuple[str, dict, dict]:
    import portfolio_profiles
