"""
Monte Carlo — Asymmetric Weekly + Put TP/SL Sweep (2022 + 2025)
================================================================

Combines two untested variants that each showed promise:
  1. Asymmetric weekly scaling (calls=1.0x, puts=1.5x)
     - Bear-year win: promotes more borderline put signals past threshold
     - Bull-year concern: generates more puts in regimes where puts drag
  2. Put-specific TP/SL (tighter put SL lowers BE threshold)
     - Current puts use call TP/SL (TP=30/35, SL=35/40) -> BE=56.3%
     - Current <=25 put TP rate ~54% -> below BE
     - Tighter put SL (SL=20-25) lowers BE to 43-49%

Hypothesis: the combination rescues 2025 by (a) filtering put quality up
(asymmetric moves the 24-25 borderline into strong put territory), and
(b) lowering the BE bar so even the filtered puts are profitable.

Pipeline:
  1. Run ScoreSimulator ONCE with put_scale=1.5 applied to calculate_weekly_adjustment
     (asymmetric: call scores untouched, put-direction amplified)
  2. Use simulated scores for both calls and puts (calls identical to DB
     since positive weekly totals pass through unchanged)
  3. Precompute call outcomes and put outcomes (per put_tp/put_sl combo)
  4. Sweep put TP/SL on 2022 + 2025 windows

Call config unchanged: breadth-adaptive TP=30/35 base/stress, SL=35/40.
Put cascade: <=15 -> 15%, 16-20 -> 12%, 21-25 -> 12%.
...
PUT_WEEKLY_SCALE = 1.5   # from weekly_magnitude_sweep findings
...
# ---- MC constants (same as monte_carlo_puts_tpsl_sweep) --------------------
...
# Focused TP/SL sweep — 4 combos span the decision space
PUT_SWEEP = [
    ('TP30_SL35 (mirror)', 0.30, -0.35),
    ('TP30_SL25',          0.30, -0.25),
    ('TP30_SL20',          0.30, -0.20),
    ('TP25_SL20',          0.25, -0.20),
]
...
def load_or_simulate():
    ...
    print(f"Monkey-patched calculate_weekly_adjustment: call x{CALL_WEEKLY_SCALE}, put x{PUT_WEEKLY_SCALE}")

    today = date.today()
    earliest_needed = min(w[1] for w in WINDOWS)
    lookback = (today - earliest_needed).days + 30
    print(f"Simulator lookback: {lookback} days (covers {earliest_needed} -> today)")
    ...
    print(f"Simulated scores: {len(sim_scores)}")
...
def main():
    print("=" * 110)
    print(f"MC - Asymmetric weekly (put_scale={PUT_WEEKLY_SCALE:.2f}x) + Put TP/SL sweep")
    print("=" * 110)

    sim_scores = load_or_simulate()

    # Partition simulated scores into (symbol_str, date, overall) records
    # symbol_str matches PriceHistory.symbol directly (Stock PK is symbol)
    all_sig_records = [(sym, d, overall) for (sym, d), overall in sim_scores.items()]

    # 5. Run sweep per window
    summary = {}
    ...
    print("=" * 110)
    print(f"CROSS-WINDOW SUMMARY (Realistic mode, asymmetric put_scale={PUT_WEEKLY_SCALE}x)")
    print("=" * 110)
    print(f"{'Put variant':<22}  {'2022 MeanRet':>14}  {'2022 WorstDD':>12}  "
          f"{'2025 MeanRet':>14}  {'2025 WorstDD':>12}")
    print('-' * 85)
    for vlabel, _, _ in PUT_SWEEP:
        r22 = summary['2022'][vlabel]['realistic']
        r25 = summary['2025'][vlabel]['realistic']
        print(f"{vlabel:<22}  {r22['mean_ret']:>+13.1f}%  {r22['worst_dd']:>11.1f}%  "
              f"{r25['mean_ret']:>+13.1f}%  {r25['worst_dd']:>11.1f}%")
...
def run_window(label, d_start, d_end, all_sig_records):
    ...
    # Filter & split into calls (>=70) and puts (<=25) within the window
    ...
    print(f"Call signals (sim, >=70): {len(call_sigs)}  |  Put signals (sim, <=25): {len(put_sigs)}")
    ...
    for vlabel, put_tp, put_sl in PUT_SWEEP:
        ...
        print(f"\n  [{vlabel}] BE={be:.1f}%  RawTP={raw_tp_pct:.1f}%  "
              f"RawSL={raw_sl/total*100:.1f}%  N={total}")

        window_results[vlabel] = {}
        for mode in COLLISION_MODES:
            ...
            window_results[vlabel][mode] = dict(
    ...
    print('\n' + '=' * 85)
    print(f"{label} SUMMARY - Realistic mode")
    print('=' * 85)
    baseline = window_results['TP30_SL35 (mirror)']['realistic']['mean_ret']
    print(f"{'Variant':<22}  {'MeanRet':>13}  {'vs mirror':>12}  {'WorstDD':>9}  {'PutTP%':>7}")
    print('-' * 70)
    for vlabel, _, _ in PUT_SWEEP:
        r = window_results[vlabel]['realistic']
        delta = r['mean_ret'] - baseline
        print(f"{vlabel:<22}  {r['mean_ret']:>+12.1f}%  {delta:>+11.1f}pp  "
