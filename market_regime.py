...
def _finite_close_values(series):
    values = []
    for raw in series:
...
def _set_regime_numeric_fields(regime, values):
    for field, value in values.items():
        setattr(regime, field, _finite_float(value))
...
def _fg_series_to_date_map(series):
    ...
    out = {}
    for i, raw in zip(idx, series):
        value = _finite_float(raw)
        if value is not None:
            out[i.date() if hasattr(i, 'date') else i] = value
    return out
...
def _extract_vix_metrics(vix_df):
    ...
    if vix_df is None or len(vix_df) < 11:
        return None, None
    closes = _finite_close_values(vix_df['Close'].tolist())
    if len(closes) < 11:
        return None, None
    vix_close = closes[-1]
    vix_10d_ago = closes[-11]
    if vix_10d_ago == 0:
...
def _extract_spy_metrics(spy_df):
    ...
    closes = _finite_close_values(spy_df['Close'].tolist())
    if len(closes) < 200:
        return None, None, None
    spy_close = closes[-1]
...
def compute_regime(pull_date=None, vix_df=None, spy_df=None, fg_data=None):
    ...
    _set_regime_numeric_fields(regime, {
        'vix_close': vix_close,
        ...
        'hyg_close': fg_raw.get('hyg_close'),
        'lqd_close': fg_raw.get('lqd_close'),
        'tlt_close': fg_raw.get('tlt_close'),
        'skew_close': fg_raw.get('skew_close'),
        'hyg_lqd_ret_diff': fg_raw.get('hyg_lqd_ret_diff'),
        'tlt_spy_ret_diff': fg_raw.get('tlt_spy_ret_diff'),
...
def backfill_regime(days=365):
    ...
    fg_maps['SPY'] = _fg_series_to_date_map(spy_df['Close'])
    ...
    for d in tqdm(ph_dates, desc="Backfilling regime"):
        # VIX metrics
        vix_closes = _finite_close_values(vix_df[vix_df.index.date <= d]['Close'].tolist())
        if len(vix_closes) >= 2:
            vc = vix_closes[-1]
            vix_10d_ago = vix_closes[-11] if len(vix_closes) >= 11 else None
            v_chg = ((vc - vix_10d_ago) / vix_10d_ago * 100
                     if vix_10d_ago else None)
        else:
            vc, v_chg = None, None

        # SPY metrics
        spy_up_to = spy_df[spy_df.index.date <= d]
        closes_list = _finite_close_values(spy_up_to['Close'].tolist())
        ...
        _set_regime_numeric_fields(regime, {
            'vix_close': vc,
            'vix_10d_change': v_chg,
            'spy_close': sc,
            'spy_ema50': se50,
            'spy_ema200': se200,
            'vix_score': vix_sc,
            'market_trend_score': mkt,
            'regime_composite': composite,
            'regime_multiplier': mult,
            'hyg_close': fg_m.get('hyg_close'),
            'lqd_close': fg_m.get('lqd_close'),
            'tlt_close': fg_m.get('tlt_close'),
            'skew_close': fg_m.get('skew_close'),
            'hyg_lqd_ret_diff': fg_m.get('hyg_lqd_ret_diff'),
            'tlt_spy_ret_diff': fg_m.get('tlt_spy_ret_diff'),
            'credit_spread_score': credit_s,
            'haven_score': haven_s,
            'skew_score': skew_s,
            'fg_composite': fg_comp,
            'fg_multiplier': fg_mult,
