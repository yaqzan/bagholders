...
REGISTRY: Tuple[MechanismSpec, ...] = (
    # ── SPREAD_TILT — 75-79 component-spread alloc haircut (Stage-3) ─────
    # Weatherization item #6: down-weight HIGH component-disagreement
    # (sqrt pop-variance of [trend,bb,rsi,macd,stoch]/5) CALLS in the 75-79
    # band ONLY (80-84 inverts sign -> band gate is load-bearing). SHIPPED
    # 30 DTE 2026-06-15 after Stage-3 T1-T7 PASS; 15 DTE not wired.
    MechanismSpec(
        name='SPREAD_TILT',
        config_fields=(
            'SPREAD_TILT_ENABLED',
        ...
        dte_15_reason='30 DTE only; mc15/bc15 not wired (validation scoped to 30 DTE).',
        engine_files_30=('monte_carlo.py', 'backtest_cascade.py'),
        ship_date_30='2026-06-15',
        notes=(
            '75-79-band-only, down-weight-only component-spread haircut. '
            'LO=26.9(p50)/HI=31.3(p75)/DEPTH=0.40 (floor 0.60x). Kill-test SHIP-TO-MC '
            '(in-sample, apex 30dte predictand): low-vs-high spread EV +1.76pp / Welch '
            't+2.17, orthogonal to overall (corr -0.013), sign-stable 4/5y, concentrated '
            'in bear/recovery/choppy 2021-23. Stage-3 N=500x9 paired (#195 baseline vs '
            '#196 arm) PASS T1-T7: 5y WorstDD 53.5%->49.4% (-4.1pp), DD down on 8/9 windows, '
            'return held-or-up, collapse=0 on every window incl COVID. Evidence: '
            'experiments/ensemble_spread/spread_kill_test.py + spread_skill_apex.py.'
        ),
    ),
    ...
    # ── BDIV pre-top breadth-divergence-at-highs call dampener (shipped 2026-06-10, 30 DTE) ─
    MechanismSpec(
        name='BDIV',
        config_fields=(
            'BDIV_ENABLED',
        ...
        dte_15_wiring_mode='not_wired',
        dte_15_reason=(
            'Pre-top breadth-divergence call dampener not validated under bounded-fill '
            'MC for the half-DTE strategy. Not wired into mc15/bc15; '
            'STRATEGY_15DTE.BDIV_ENABLED=False for schema parity.'
        ),
        engine_files_30=('monte_carlo.py', 'backtest_cascade.py'),
        ship_date_30='2026-06-10',
        notes=(
            'Contract CALL alloc when SPY is near its 60d high WHILE internal breadth is '
            'rolling over (the classic pre-top divergence): alloc *= 1 - DEPTH * '
            'prox_ramp(spy_from60h; PROX_CUT->PROX_FULL) * exp(-0.5*((brd_det10-GAP_C)/GAP_W)^2) '
            'where brd_det10 = -(breadth_score 10d change). 5th orthogonal DD lever and the '
            'FIRST leading one (fires PRE-onset at the top, where running dd ~ 0): hence NO '
            'DD-gate and NO VIX-panic knob -- the SPY-near-highs requirement is the structural '
            'crash guard (cannot fire mid-crash; 2022 delta exactly 0.0 by construction). '
            'From the DD-episode-onset mine (fresh v71 tape): the literal Hindenburg-omen '
            'hypothesis is NULL/inverted; the divergence cohort is low-EV (mpnl -0.018 vs '
            '+0.029, z+25, dd_conc 1.82), sign-stable incl 2024/2025, survives the '
            'all-levers-off slice. Gap extremes (>12 = sharp shakeout) are mean-reversion '
            'winners, left alone by the Gaussian. experiments/dd_onset_omens/FINDINGS.md.'
        ),
    ),
    ...
    # ── LIQUIDITY_FLOOR — option-volume admission filter (Stage-3, staged 2026-08-07) ─
    MechanismSpec(
        ...
        dte_15_wiring_mode='not_wired',
        dte_30_reason=(
            "Staged ship-candidate 2026-08-07 (FF3_STAGEB_RESULTS.md): Core floor-150 "
            "beat matched random-drop control at N=500x12; enable gated on P2.B "
            "live-fill confirmation. 0.0 = OFF."
        ),
        dte_15_reason=(
            'Not validated at 15 DTE (every shipped Stage-3 dampener precedent).'
        ),
        engine_files_30=('monte_carlo.py', 'backtest_cascade.py'),
        ship_date_30='2026-08-07',
        notes=(
            'Drops 75+ CALL signals whose per-signal option_volume_30d (nearest-ATM '
            '20-45 DTE call, trailing-30-cal-day avg REAL contract volume where '
            'option_prices covers the date [2025-02-10+], Stock.average_volume*0.005 '
            'fallback otherwise) is below LIQUIDITY_FLOOR contracts/day. Stage B\' '
            'penalized-floor A/B (#305-308, N=500x12 Phase D): Core A4 (floor=150 on '
            'the FF-3\' real-4y map) beats its matched random-drop control decisively '
            '-- 7/8 covered windows net-positive, T4 5y floor-vs-A1 +4.65pp better, '
            'collapse=0 every cell of every arm. Apex FAILS T4 (5y DD +1.47pp worse) '
            '-- Core-evidence-only. Three pre-enable items before any live flip: (1) '
            'P2.B live-fill confirmation N>=30 real fills, (2) LIQ_GAP_INCIDENCE '
            'sensitivity -- DONE 2026-08-07, bit-identical at incidence 0.06/0.24, '
            '(3) this 13-consumer wiring pass. The LIQ_PENALTY_*/LIQ_GAP_*/'
            'LIQUIDITY_RANDOM_DROP env instruments in monte_carlo.py are separate '
            'research-only A/B tooling (Stage B\' penalty-realism + random-drop '
            'control arms) -- NOT part of this registry entry, no strategy_config '
            'fields, intentionally left env-only. '
            'experiments/flatfile_exploitation/FF3_STAGEB_RESULTS.md.'
        ),
    ),
